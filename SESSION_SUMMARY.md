...
## ضمانك 4.1.0+8 — إصلاح أسعار App Store (2026-08-25)

- أصل بقاء «جارٍ جلب أسعار المتجر» في build `7` كان طلب StoreKit 2 غير محدود زمنياً؛ كانت الواجهة تنتظر Future قد لا يكتمل.
- في build `8` صار الاتصال وجلب المنتجات محدودين زمنياً. عند تعليق/فشل/فراغ StoreKit 2، يستعلم iOS عن تفاصيل الأسعار عبر StoreKit 1، ثم يعرض رسالة وزر «إعادة المحاولة» إذا فشل المساران بدلاً من الدوران المستمر.
- الشراء نفسه لم يتحول إلى StoreKit 1؛ بقي عبر StoreKit 2 ويرسل `appAccountToken` الذي يعتمد عليه التحقق الخادمي لربط الدفعة بمالك المتجر.
- نجح `flutter analyze` و23 اختباراً، ومنها محاكاة StoreKit 2 المعلق والتحقق من المسار الاحتياطي ومغادرة حالة التحميل. نجح أيضاً بناء Android debug.
- نجح GitHub run `32836689892` في بناء iOS والتوقيع والتحقق والرفع. build `8` ظاهر في TestFlight بحالة `Ready to Submit` ومضاف إلى `Damanak Internal`.
- IPA المحلية `C:\Users\MOHAMMAD\Downloads\damanak-ios-build8-32836689892\ضمانك.ipa`، حجمها `21,973,039` وبصمتها `31B52DACDA5555F6A9AE9B0A1B7D946A9C9127D84DE1BCFAB25ACA2004F15F55`.
- المتبقي تنزيل build `8` واختبار ظهور أسعار الحساب القطري ثم شراء Sandbox والاستعادة على iPhone حقيقي.

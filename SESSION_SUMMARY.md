...
## ضمانك — إصلاح شامل للاشتراك والاستعادة في Build 26 (2026-09-01)
...
- الاستعادة بعد حذف الحساب أصبحت صريحة ومقيدة: Apple JWS موثق رسمياً، والهوية القديمة يجب أن تكون محذوفة والهدف متجراً وحيداً بلا entitlement. Google Build `23` وlinked/out-of-app يستخدمان RPC ذرية وأقفال lineage. لا تُنقل عملية مرتبطة بحساب أو متجر حي.
- أثبتت تجربة iPhone الحية أن شراء Apple نجح أولاً، ثم حذف المستخدم حساب ضمانك بعد نحو 8 ثوانٍ بينما بقي الاشتراك عند Apple. فشلت الاستعادة إلى الحساب الجديد داخل فحص JWS المتصل قبل إنشاء أي entitlement؛ لذلك لم يكن الحل شراءاً ثانياً ولا تعديل منتج «توسع».
...
- نُشرت migration `20260901012000` ودالة `verify-store-purchase` v26 على `exxayzlklvgeyqhvtzgi` مع `verify_jwt=false` وبصمة حزمة `2e99b9097d3f18014720fd2f0d9b73a1ed2eaa5ae7f39fe6af9745d9917d65e4`. يحاول v26 فحص Apple المتصل أولاً، ثم يعيد الحالات `1/2/6` والمهلة عبر الفاحص الرسمي عند `signedDate`؛ تبقى الحالة `7` وإخفاقات التطبيق/البيئة/السلسلة مرفوضة، وتبقى المطابقة وحواجز orphan والربط الذري كاملة. نجحت اختبارات الإنتاج الخمسة السابقة داخل `BEGIN/ROLLBACK`، والمخطط حجمه `395,324` وبصمته `5A1E083994801006E3C620E6D91C611E2097D3B216FA1D3E8123F97DAD2F8F3C`.
- نجح `flutter analyze` و`181/181` اختبار Flutter و`71/71` اختبار Deno واختبار migration `1/1` و`deno check` وفحص التنسيق و`git diff --check` وبناء web release وAndroid debug. لم يتغير Flutter في الرقعة الخادمية؛ APK Build `26` المحلي بقي حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`.
- بقي Build `23` في App Review وGoogle Alpha بلا تغيير. رفع run [33458183412](https://github.com/m7madv/otlobli/actions/runs/33458183412) Build `4.5.0 (26)` إلى iPhone TestFlight، وصار `VALID` بالمعرّف `10a1e004-c6c1-4b56-9cff-618406c3881b` وداخل `Damanak Internal` ذات المختبر الواحد، من دون إضافته إلى App Review. IPA حجمها `23,942,352` وبصمتها `FE66B7F77FB1892FE8F3D55F0AC85AB6B0A7DF783286C77C2EDA735A82C32A37`. ثبتت الحزمة والشراء على iPhone فعلياً، لكن الاستعادة بعد v26 المنشورة ما تزال بانتظار إعادة المحاولة؛ لم يُنفذ VoiceOver/TalkBack أو قبول جهاز ضعيف.

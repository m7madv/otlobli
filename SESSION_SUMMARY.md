...
- اجتاز المرشح تدقيقًا مستقلاً بلا P0/P1، ونجح format/analyze و55 اختبار Flutter و12 اختبار صور و95 اختبار Deno و5 اختبارات Node وEdge fmt/lint/check وAPK debug. لم يتوفر PostgreSQL/Docker محليًا، ولذلك يبقى اختبار migration حيًا مطلوبًا.
...
- Apple ما زال يراجع build 18، وGoogle ما زال على build 18 في internal/Alpha. لوحة Play تعرض الآن 3 مختبرين من أصل 12، وProduction يبقى مقفلاً حتى 12 مختبرًا لمدة 14 يومًا. المستخدم سيتولى المحاكيات بنفسه، ولم تُلمس في هذه الدفعة.

- ألغى المالك فرنسا لهذا الإصدار؛ أصبح التطبيق متاحًا في `174` بلدًا/منطقة وفرنسا `Not Available`، مع إبقاء البلدان المستقبلية تلقائيًا. رفضت Apple الإرسال الأول آليًا تحت البند `3.1.2` بسبب غياب رابط EULA من وصف المتجر. أُضيف رابط شروط Apple القياسية إلى نهاية الوصف العربي، ثم أُعيد إرسال iOS ‏`0.1.0 (18)` ومجموعة `VoiceBrief Pro` والاشتراكين عند `9:49 PM`. العناصر الأربعة الآن `Waiting for Review` تحت معرّف الإرسال `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b`، والنشر التلقائي بعد الموافقة مفعّل. build 19 لم يُرفع إلى Apple بعد، ولم يصبح التطبيق حيًا.

...
## v86.40 — قراءة اللون والمقاس الحقيقيين من داخل «انقر للشراء»

- تشخيص الجهاز رفض v86.39 وحسم السبب: نافذة SHEIN تضع مجموعتي `بيج/رمادي` و`صغير/كبير` في عنصرين لهما اسم الصنف نفسه تماماً `.SIZE_ITEM_HOOK`. الكود أخذ المجموعة الأولى كمقاس كامل، لذلك اختفت كلمة `كبير`.
- أول ضغطة إضافة صارت تستهدف صف `لون / مقاس` نفسه حتى لو كان يعرض اختياراً قديماً مثل `رمادي / كبير`، فتفتح نافذة الخيارات مرة واحدة ولا تضيف. داخل النافذة تُقرأ كل مجموعة ظاهرة وحدها ثم تُجمع القيمة مثل `بيج / كبير`.
- إذا بقيت أي مجموعة بلا اختيار، تُمنع الإضافة ولا يُستخدم المقاس القديم الموجود خلف النافذة. اختبار مطابق لتشخيص الجهاز أثبت: فتح مرة واحدة بلا إضافة، منع الحالة الناقصة، ثم تشخيص وإضافة `صينية من الخشب الصلب + بيج / كبير + $14.66`. السعر لم يتغير.
- نجح حارس التجمّد والبناء وميزانية الأداء ومزامنة Android/iOS وGradle. APK: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.40-shein-real-drawer-compound-size-debug.apk`، SHA-256 `5A96AAA295BA6B19A1BD55F1A6247DB5EE97689DA9F8370732DA03AD7B5E3D89`.
- نجح GitHub/Xcode run `30697305864` من commit `87c85b1`. IPA غير الموقّعة: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.40-iphone16-unsigned.ipa`، SHA-256 `7076BB13A20F86A8076A3A7BE9A6DA022A30CA1E6CB1D6813B5EC454EA88D189`. الفحص أكد `86.40/900` وعلامات الإصلاح وحراس التجمّد، وعدم وجود توقيع للتطبيق أو provisioning profile.
- v86.40 لم تُقبل على iPhone الحقيقي بعد؛ المطلوب تجربة المنتج نفسه من أول ضغطة، تغيير الخيارات داخل النافذة، ثم التأكد من السلة. مشكلة تبديل منطقة SHEIN منفصلة وما زالت مفتوحة.

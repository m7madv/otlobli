# ملخّص الجلسة — otlobli (2026-08-24)

## VoiceBrief — تطبيق Flutter مستقل جاهز للتهيئة الإنتاجية

- وُضع المشروع باسم واضح داخل `voicebrief_flutter/` من دون استبدال أي مشروع قائم. يعمل فوراً بخدمات تجريبية محلية، ويحتوي تدفقات الدخول والصوت والمعالجة والنتائج والحفظ والسجل والإعدادات والاشتراك والحذف.
- أضيفت بنية Supabase كاملة مع RLS وحصة دقائق ذرية وتخزين خاص، ودوال خادم لـOpenAI وRevenueCat وحذف الحساب. مفاتيح OpenAI لا تدخل تطبيق الهاتف. لم يُنشر شيء إلى سحابة فعلية ولم تُستخدم خدمة مدفوعة.
- نجحت جميع فحوص Flutter المحلية: تحليل بلا ملاحظات، 15 اختبار وحدة/واجهة، 8 اختبارات صور مرجعية، واختبار تكامل على محاكي Android API 35. كما نجح تنسيق Deno وlint وفحص الأنواع للدوال الثلاث.
- روجعت الواجهة فعلياً على المحاكي، ونجحت مشاركة الصوت أثناء تشغيل التطبيق وبعد إيقافه، وظهرت شاشة إعداد ملف 1 ثانية بلا crash. كما صار URI غير المقروء يعرض رسالة آمنة بدل الفشل الصامت.
- ملفات Android موجودة داخل المشروع: `app-debug.apk` ببصمة `BE66658FA6DCB1FB2706AB26E0D36115580E55DBAF9480E56660D2EF0BD65286`، و`app-release.aab` ببصمة `B35074C71240526501DD1C72E603ED888A7BA4E01CC1305AEA1C28D8480F3005`. حزمة AAB في وضع Release لكنها موقعة بمفتاح التجربة، فلا تُرفع إلى المتجر.
- مصدر iOS وShare Extension جاهز في المشروع، لكن Windows لا يملك Xcode؛ لم يُبنَ iOS ولم تُختبر أجهزة أو مشتريات أو OAuth أو Supabase الفعلي. كل خطوات الإعداد والقبول المتبقية موثقة داخل `voicebrief_flutter/docs/`.
- أضيف workflow باسم `voicebrief-ci.yml`، ووسم رجوع `voicebrief-prebuild-20260824`، وApple Design Skill مثبت في `.design-rules/apple-design-skill`. لم أُنشئ commit نهائية حتى لا أخلط VoiceBrief مع تعديلاتك الحالية في `damanak_flutter/`.

## ضمانك 4.0.0 — فرع ومخزون ونقطة بيع في نظام واحد (2026-08-24)

- توسّع `damanak_flutter/` إلى منظومة متجر كاملة: دخول بالبريد وGoogle وApple، فروع مفصلة، مخزون لكل فرع، حركات وتسويات وتحويلات، نقطة بيع، جلسات صندوق، فواتير ومرتجعات، موردون وأوامر شراء، منتجات متسلسلة، ضمانات وصيانة وموظفون وصلاحيات واشتراكات وتقارير.
- صار ماسح الباركود خاصاً بالتطبيق ويُستخدم سياقياً داخل نقطة البيع والمخزون، ويربط دورة المنتج من استلامه في الفرع إلى بيعه وإنشاء ضمانه ثم إرجاعه أو صيانته. يدعم النظام `SAR/AED/KWD/QAR/BHD/OMR/USD/SYP` والضرائب والخصومات والدفع المجزأ والأرقام التسلسلية.
- أضيفت هجرة `Supabase` إنتاجية مع `RLS` ودوال ذرية للمخزون والمبيعات والمرتجعات والصناديق والمشتريات وحذف الحساب. لم تُنشر لأن الحساب الحالي يملك بالفعل الحد الأقصى من المشروعين المجانيين: `talabieh` و`VoiceBrief`، ولم يُمس أي منهما.
- حافظ التصميم على اتجاه `Apple Design`: أخضر واحد `#087F5B` ومحايدات، بلا ازدحام زخرفي، مع واجهات عربية متجاوبة. جرت مراجعة مرئية فعلية لسطح المكتب والجوال وحُفظت الصور محلياً في `damanak_flutter/output/playwright/`.
- نجح `dart format` و`flutter analyze` وجميع الاختبارات الـ14 وبناء الويب. أُنشئ مفتاح رفع Android إنتاجي خارج المستودع، ونجح توقيع وفحص `damanak-4.0.0-signed.apk` و`damanak-4.0.0-signed.aab`. بصمة APK هي `BDB9AF0E69867A93C4D96B61856C77C415D7FAC1DF2F072B758640B9FF15C18E` وبصمة AAB هي `2A88105FC846A304446FC59EA35E690475626B9F53FBEE160641D29C41A5CA0E`.
- رُفع الإصدار في الالتزام `20188f9`، ونجح GitHub/Xcode run `32734178700`. ملف iOS غير الموقع حجمه `8,450,962` وبصمته `6D917F5C4AC9305C9C7B1A795E274D1336D0415CB0B57F23A47AE95C7FAAAC60`؛ فحصه أكد `com.damanak.damanak` و`4.0.0/4` بلا توقيع أو provisioning.
- أضيف مسار GitHub لبناء iOS موقّع، لكنه يحتاج App ID وملف provisioning خاصين بالحزمة `com.damanak.damanak` من حساب Apple Developer. بقي كذلك نشر قاعدة البيانات واختبار القبول على أجهزة Android وiPhone حقيقية؛ لا توجد ادعاءات نشر أو قبول أجهزة غير منفّذة.

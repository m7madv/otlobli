# ملخّص الجلسة — VoiceBrief build 12 (2026-08-30)
...
- أُعد البناء 12 بالتدفق المطلوب: VoiceBrief يبدأ معالجة التسجيل فور مشاركته، يحفظ النتيجة تلقائياً، يغلق نافذة المشاركة، ثم يعرض إشعار «الملخص جاهز». ضغط الإشعار يفتح التطبيق مباشرة على شاشة النتيجة الكاملة دون معالجة أو خصم مكرر. يلزم إبقاء نافذة المشاركة مفتوحة حتى نهاية المعالجة لأن iOS قد ينهي الامتداد إذا أُغلق مبكراً.
...
- أُصلح المثال المرسل: «بكرة الساعة خمسة» و«يوم خمسة تسعة» يبقيان موعدين منفصلين، و«يوم خمسة تسعة» يصبح 5 سبتمبر القادم. عند غياب الوقت يظل التطبيق يطلب التأكيد ويقترح 5:00 مساءً بدل منتصف الليل.
- الإصدار المصدر الحالي `0.1.0+12`. نجح محلياً format وanalyze و32 اختباراً و11 golden وAPK debug؛ بصمة APK هي `37729DAFEECC45CE66DF870AA167D4E660A7F3167C62E3352C003C3818029FE5`. كما نجح CI run `33307726051` بكل وظائفه، بما فيها تجميع Swift/iOS غير الموقّع.
- أصبح البناء `0.1.0 (12)` متاحاً في `VoiceBrief Internal` وحالته `VALID` بحسب run `33307726047`. نجح TestFlight run `33307726042`؛ Delivery UUID/build ID هو `0af3d7d3-a607-4913-9d39-a2429c73b3b0`، وبصمة IPA هي `12CC57885E1C9CEE4827F36D1B3072DA98CAE8DF254C3D714C44A6AC53DD7C8C`، وGitHub artifact رقم `9731160573`. بقي اختبار الإشعار وفتح النتيجة المؤرخة على iPhone الحقيقي.
- نُشر `process-audio` إلى مشروع VoiceBrief الصحيح `jyehqpdbayslhzebdycj` فقط، بالإصدار `9`. الاختبار العربي الحي فصل غداً `2026-08-31` عن `2026-09-05` وطلب التأكيد لكليهما، ثم حذف حساب الاختبار وملفه واستحقاقه المؤقت.
- عولج سبب اختفاء الموعد من تجربة المشاركة: شاشة الامتداد القديمة لم تكن تعرض إلا العنوان والملخص. مسار الإشعار الجديد يفتح `ResultScreen` التي تعرض `actionItems` و`importantDates` وأزرار «إضافة إلى التقويم». اختبار مشاركة مؤرخة يثبت بقاء الموعد وطلب فتح الشاشة، لكن قبول iPhone الحقيقي للبناء 12 لم يُنفذ بعد.

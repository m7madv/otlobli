# ملخّص الجلسة — VoiceBrief build 15 (2026-08-30)
...
- الإصدار المصدر الحالي `0.1.0+15`: على iOS 26.1 فما بعد يطلب إذن Apple الخاص بالمنبّهات والمؤقتات ويجدول منبّهًا نظاميًا فعليًا عبر `AlarmKit`؛ على iOS 14–26.0 يبقى إشعار محلي كحل احتياطي، وعلى Android يبقى منبّه النظام. عبارة مثل «الساعة الخامسة» تفتح تأكيد التاريخ والوقت أولاً.
- لأن منبّه `AlarmKit` لا يظهر داخل قائمة تطبيق «الساعة»، أضيفت صفحة «منبّهات VoiceBrief» داخل التطبيق. تعرض المنبّهات المضبوطة ووقتها ونغمتها، وتسمح بسماع النغمة أو إلغاء المنبّه. يختار المستخدم قبل الضبط بين ثلاث نغمات أصلية: هادئة وواضحة وكلاسيكية، ثم ينتقل مباشرة إلى القائمة.
- نجح محليًا format على 74 ملفًا وanalyze و36 اختبارًا وظيفيًا و11 اختبارًا بصريًا وبناء Android debug، ونجح CI الكامل في run `33316201476`. نجح رفع TestFlight للبناء 15 في run `33315830789` وأصبح `VALID`، لكن أول تقرير لم يجده مرتبطًا بمجموعة؛ أضيفت خطوة توزيع آلية إلى `VoiceBrief Internal` مع تحقق لاحق. تجربة iPhone للرنين وظهور الاسم والنغمات والقائمة والإلغاء ما زالت مطلوبة.

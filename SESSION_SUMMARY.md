...
## ضمانك — متجر بلا حصة مجانية ورفع Build 25 إلى TestFlight (2026-09-01)

- بقي Build `23` داخل App Review وGoogle Alpha، وبقي Build `24` في TestFlight فقط. نجح [33449769807](https://github.com/m7madv/otlobli/actions/runs/33449769807) في توقيع وفحص ورفع iPhone Build `4.5.0 (25)` إلى Apple فقط. أنهت Apple المعالجة؛ البناء `VALID` ومعرّفه `8e864062-28e0-4250-abd2-dac25f8df1b9` وداخل `Damanak Internal` ذات المختبر الواحد. الـIPA `23,930,380` بايت وبصمتها `D2DAF9EF0A97B3C8A6328C32D1B71276352DEAA29A97E144653978983F13A14F`. لم يُرفع إلى Google أو يُضف إلى App Review، وبقي إصدار App Store `WAITING_FOR_REVIEW` على Build `23`.
- إذا استُهلكت التجربة سابقاً صار الحساب يستطيع إنشاء متجره، لكن المتجر يُنشأ بلا فرع أو حصة أو مزايا مجانية ويقف مباشرة عند باقات الدفع أو استعادة اشتراك فعّال. المؤهل الجديد وحده يحصل على تجربة Starter مرة واحدة. أبقيت RPC Build `24` القديمة كما هي حتى لا يدخل العميل القديم حالة غير مدعومة.
- بوابة أول دفع تعرض «غير مشترك» وصفر ضمانات، ولا تعرض باقة حالية أو تجربة وهمية. الحساب المقفول يستطيع إدارة هويته وتسجيل الخروج أو الحذف فقط. لا يفتح التشغيل بعد الشراء أو الاستعادة إلا بعد تحقق الخادم وإكمال المعاملة وتحميل فرع `MAIN`.
- نُشرت migrations `20260901010000` و`20260901011000` على `exxayzlklvgeyqhvtzgi`. تحجب حالة أول دفع الدعوات والمطالبات وروابط الضمان الجديدة، وتؤجل إنشاء الفرع حتى الاستحقاق، وتُقصر RPC الجديدة على المستخدم الموثق. بقيت استمرارية السجلات التاريخية للمشترك السابق المنتهي منفصلة ولم تُكسر.
- نجحت أربعة اختبارات SQL حية داخل `BEGIN/ROLLBACK`، ومنها اختبار Build `24/25` الجديد. نجح `flutter analyze` و`160/160` اختبار Flutter و`56/56` اختبار Deno وفحص التنسيق و`git diff --check` وبناء web release وAndroid debug.
- APK Build `25` المحلي حجمه `180,814,040` بايت وبصمته `C3070BABAA8F452C23B6BBF5D71881C4FD1BF8BDCFAC44A64B39CA723EF1B775`. المخطط المتسق حجمه `391,841` وبصمته `6CE83AD3F374C42C196AB69AA7E117D8A0C3D2D0EA9447E5C64A1D3FE8C33959`؛ أُضيفت الهجرتان إلى لقطة المخطط لأن Docker اللازم لـdump جديد غير متوفر.
- مراجعة Apple Design لبوابة الاشتراك سجلت RTL والتكبير `200%` والتسلسل المحايد والإجراء الواضح. لم يُختبر Build `25` على iPhone حقيقي، ولم يُنفذ شراء/استعادة حقيقيان أو VoiceOver/TalkBack أو جهاز ضعيف.
- التدقيق الأمني ما زال بلا نتائج عالية أو حرجة. حماية الحساب/التثبيت لا تساوي إثبات جهاز؛ App Attest/DeviceCheck وPlay Integrity وقرار الويب باقية، وكذلك قرار مدة روابط الضمان والإبطال.

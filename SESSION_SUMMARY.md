...
## ضمانك 4.3.0+17 — الضمان أولاً وباقات أوضح (2026-08-27)
...
- أُصلح StoreKit 2 على iOS من دون إجبار StoreKit 1. نُشر ترحيلا الحصص والتحصين وتطابقا مع الحالة المحلية، كما نُشرت صفحات `legal` واختُبرت.
- نجح `flutter analyze` و`45` اختباراً و`6/6` لقطات راجعتها العين بصرياً، ونجح بناء Flutter Web وAndroid debug. تعذر release المحلي فقط لأن أسرار التوقيع غير موجودة في البيئة.
- لم تُنفذ بعد تجارب Build `17` على أجهزة حقيقية أو عبر App Store وGoogle Play؛ يبقى قبول الأسعار والشراء والاستعادة والتحقق الخادمي خطوة الإصدار التالية.

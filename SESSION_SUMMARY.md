...
- أُعد App Privacy على `15` نوع بيانات من manifests الحزمة الحقيقية، كلها مرتبطة بالمستخدم ولا تُستخدم للتتبع. حُدثت السياسة المنشورة بإفصاح Google Sign-In وربط RevenueCat؛ النشر الإنتاجي `dpl_EMsF8EH79BfRKPTSxNr8wRDDxRNn` حالته `READY` على `https://voicebrief-legal.vercel.app/privacy`. لم يُضغط Publish في App Store Connect بعد.
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار أن المالك يملك حقوق محتوى الطرف الثالث. جُهزت بيانات المراجع باسم `MOHAMMAD ALZOUABI` وحساب/بريد `mhm1981d@gmail.com` والرقم `70067040`، وسُلّمت صفحة Apple للمالك كي يدخل كلمة المرور مباشرة؛ ما زال يلزم رمز دولة الهاتف. محاولة إضافة إصدار iOS لم تكتمل وبقيت المسودة ثلاثة عناصر فقط. لم يُنشر App Privacy ولم يُرسل التطبيق إلى Apple، وما زال قرار امتثال فرنسا مطلوبًا لأن Flutter/Dart يضم TLS معياريًا عبر BoringSSL وقد يتطلب تصريح تشفير ورمز امتثال وإعادة بناء.

...
## ضمانك — إصلاح شامل للاشتراك والاستعادة في Build 26 والخادم v27 (2026-09-01)
...
- نُشرت migration `20260901012000` سابقاً، ونُشرت `verify-store-purchase` v27 على `exxayzlklvgeyqhvtzgi` مع `verify_jwt=false` وبصمة حزمة `7d219b7941da726d6a10dcb142fa862a21f26a110452be06ab628853ee1498b2`. لا يعمل التوافق إلا بعد `status1/status1`، وتبقى الحالة `7` وإخفاقات التطبيق/البيئة/السلسلة مرفوضة، وتبقى المطابقة وحواجز orphan والربط الذري كاملة. نجحت اختبارات الإنتاج الخمسة السابقة داخل `BEGIN/ROLLBACK`، والمخطط حجمه `395,324` وبصمته `5A1E083994801006E3C620E6D91C611E2097D3B216FA1D3E8123F97DAD2F8F3C`.
- نجح `flutter analyze` و`181/181` اختبار Flutter سابقاً، ونجح الآن `76/76` اختبار Deno واختبار migration `1/1` و`deno check` وفحص تنسيق 27 ملفاً و`git diff --check`. web release وAndroid debug السابقان ناجحان. لم يتغير Flutter في رقعة v27؛ APK Build `26` المحلي بقي حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`، ولم تُنشأ حزمة جديدة.
- بقي Build `23` في App Review وGoogle Alpha بلا تغيير. رفع run [33458183412](https://github.com/m7madv/otlobli/actions/runs/33458183412) Build `4.5.0 (26)` إلى iPhone TestFlight، وصار `VALID` بالمعرّف `10a1e004-c6c1-4b56-9cff-618406c3881b` وداخل `Damanak Internal` ذات المختبر الواحد، من دون إضافته إلى App Review. IPA حجمها `23,942,352` وبصمتها `FE66B7F77FB1892FE8F3D55F0AC85AB6B0A7DF783286C77C2EDA735A82C32A37`. ثبتت الحزمة والشراء على iPhone فعلياً، لكن الاستعادة بعد v27 ما تزال بانتظار محاولة واحدة؛ لم يُنفذ VoiceOver/TalkBack أو قبول جهاز ضعيف.

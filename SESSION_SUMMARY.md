...
## ضمانك — Build 24 على TestFlight وتدقيق الاشتراك الكامل (2026-09-01)

- بقي Build `23` داخل App Review وGoogle Alpha من دون تغيير. المصدر الحالي Build `24` راجع الكتالوج والشراء والاستعادة والترقية والخفض وتغيير الدورة والإلغاء والمصالحة وحذف الحساب، ثم رُفع إلى iPhone TestFlight فقط؛ لا توجد هذه الرقع داخل الحزمة قيد App Review.
- أصلح Apple ربط المعاملة بالمتجر عبر `appAccountToken` والتعامل مع الحدث المبكر أو المتأخر بلا تبني إيصال حساب/متجر آخر. أصلح Google نتيجة BillingClient الكاذبة، وإقرار الخادم، ومصفوفة الاستبدال، وشراء خارج التطبيق عند فتح ضمانك، واختيار المتجر الصحيح في حساب متعدد المتاجر.
- توكن Google legacy غير المربوط لا يُستخدم أبداً لتغيير الخطة قبل إثبات المتجر. الاستعادة وحدها قد ترسل مرشحاً واحداً إلى الخادم؛ التعدد والغموض يفشلان بأمان. شراء دقيق لم يتصالح بعد يوجه إلى الاستعادة بدلاً من فتح شراء ثانٍ.
- نُشرت migrations حتى `20260831180000` على `exxayzlklvgeyqhvtzgi`، وصارت `verify-store-purchase` v23 و`refresh-store-entitlements` v7. نجحت اختبارات دورة الاشتراك وحذف المالك وعزل الضمان على الإنتاج داخل `BEGIN/ROLLBACK`، وآخر تشغيل مجدول مفحوص نجح HTTP `200`.
- المخطط المتزامن حجمه `369,377` وبصمته `EA30919FF7784D1EB6A79BA66C575E4EC10F9EF5B5D0D46DD84DDD46406CCF74`. تدقيق الأمن الأصلي بقي 5 نتائج بلا عالية أو حرجة، مع بقاء توثيق الجهاز للتجربة وسياسة روابط الضمان غير مغلقين.
- نجح `flutter analyze` و`150/150` اختبار Flutter و`56/56` اختبار Deno وفحص التنسيق و`git diff --check`، وبناء web release وAndroid debug. APK Build `24` حجمه `207,919,707` وبصمته `745AB8B44F68C587BBE57294D7D012642C9E059D2C93760C22AF7070AD729B8F`.
- رُفعت صور App Store العشر، ثم رُفع Build `23` الموقّع وبصمة IPA `9C024E7444F339DDEABEB33FA69EF634368B5FAFF820EC9FF5B7D3E57C1E8815`. رفضت Apple الإصدار آلياً لغياب رابط EULA من الوصف ولطلب تأكيد سعري `توسع شهري` (`49.99 USD`) و`توسع سنوي` (`499.99 USD`). أضيف رابط EULA القياسي، وأُرسل تأكيد الأسعار المقصودة، ثم أُعيد submission `69104e50-7a9a-4102-a717-ca341d442897` نفسه عند `2026-08-31 20:34` بتوقيت الرياض. الإصدار ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، كلها الآن `Waiting for Review` على Build `23` نفسه.
- نجح [33443358450](https://github.com/m7madv/otlobli/actions/runs/33443358450) من commit `7fe6bf074e02acfa56b905d8e93431473ef2babd` في بناء وتوقيع وفحص ورفع iOS Build `4.5.0 (24)`. أنهت Apple المعالجة، وأصبح البناء `Ready to Submit` داخل مجموعة `Damanak Internal` ذات المختبر الواحد، بمعرّف `6b986de0-74bb-4649-8619-846aca1eb1b0`. IPA حجمها `23,923,863` وبصمتها `EA55D5F1192886EE9019F0048E3476C1D5416E047B4F1F1823D234FC56BCF768`. لم يُضف Build `24` إلى App Review، ولم يُثبت على iPhone حقيقي أو يُختبر عليه شراء أو استعادة حقيقيان.
- رُفع Google Build `23` إلى `Alpha` وبصمة AAB `5FB3FE00B91FFBC49FE143AF31A77938A208686B410668CCCCA00422DEFCF008`، وهو قيد المراجعة. قائمة `All Apps - Closed Testers` ثابتة ودائمة وتضم بالضبط `12` مختبراً لاستخدامها مع أي تطبيق؛ لم تُنشأ أو تُعدّل قائمة أخرى.
- Note 8 يبقى على Build `22` المثبت من Play حتى قبول Build `23`. لا يوجد Google RTDN، لذلك شراء خارجي جديد يعتمد على فتح التطبيق للمصالحة. ما يزال الشراء والاستعادة الحقيقيان على iPhone وAndroid وقبول VoiceOver/TalkBack والجهاز الضعيف اختبارات جهاز منفصلة، ولا يُدعى نجاحها اعتماداً على البناء وحده.

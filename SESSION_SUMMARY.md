...
- نجح محلياً format وanalyze و42 اختباراً وظيفياً و12 بصرياً و19 اختبار Deno وبناء Android debug. APK حجمه `177,653,885` بايت وبصمته `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`، ونجح CI run `33325718503` بكل وظائفه.
- نُشرت migration الحماية ودوال `create-audio-upload/process-audio/legal/delete-account` إلى مشروع VoiceBrief الصحيح `jyehqpdbayslhzebdycj` فقط؛ لم يُمس Damanak أو `talabieh`. الدوال `ACTIVE` بإصدارات `2/14/8/9`، ونموذج التفريغ الحي هو `gpt-4o-mini-transcribe`. build 17 لم يُرفع إلى TestFlight بعد؛ النشر العام إلى Apple/Google مؤجل حتى قبول المستخدم.
...
- الإصدار المصدر الحالي `0.1.0+16`: على iOS 26.1 فما بعد يطلب إذن Apple الخاص بالمنبّهات والمؤقتات ويجدول منبّهًا نظاميًا فعليًا عبر `AlarmKit`؛ على iOS 14–26.0 يبقى إشعار محلي كحل احتياطي، وعلى Android يبقى منبّه النظام. عبارة مثل «الساعة الخامسة» تفتح تأكيد التاريخ والوقت أولاً.

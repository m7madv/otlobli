...
## ضمانك — انتقالات اشتراك ذكية وTestFlight Build 27 (2026-09-01)

- صار اختيار الباقات يعبّر عن العملية الحقيقية: الأعلى «ترقية» فورية، والأدنى «خفض» عند التجديد، ونفس الباقة بدورة أخرى «تغيير فوترة»، ونفس الباقة والدورة معطلة. يعيد المتحكم التحقق بعد تحديث الخادم كي لا يفتح المتجر على اختيار أصبح حالياً.
- الحصص لا تتكدس: عند الانتقال من «بداية» إلى «نمو» يصبح سقف الشهر 600 مع إبقاء الاستخدام محسوباً. إذا استُخدم 30 يظهر قبل التأكيد أن المتاح 570؛ لا تمنح العملية 600 إضافية فوق رصيد «بداية». الخفض يبقي حدود الخطة الأعلى حتى موعد التجديد، وتغيير الدورة لا يعيد الحصة.
...
- نجحت محاولة المستخدم الحقيقية بعد v28. عند `2026-09-01 09:22:26` بتوقيت الرياض سجل الخادم نجاح توافق Apple في `sandbox` وأعاد HTTP `200`. أصبحت باقة المتجر `scale/active/source=store/provider=app_store/monthly` مع `auto_renews=true` وentitlement حالي غير superseded، ثم أعاد التحديث المجدول التحقق عند `09:25:03` مع HTTP `200`. لم يحصل أي منح يدوي أو شراء ثانٍ.
- مصدر Flutter الآن `4.5.0+27`. نجح `flutter analyze` و`186/186` اختبار Flutter و`77/77` اختبار Deno وفحص تنسيق 27 ملفاً و`git diff --check`، ونجح web release وAndroid debug. APK المحلي حجمه `207,989,448` وبصمته `33295CBE76936B806F7FA21C2FAA663C0B1294159A3DCA03A378FD7ED924DE37`. أعاد مسار iOS الموقّع تحليل Flutter والاختبارات قبل الأرشفة ونجح.
- بقي Build `23` في App Review وGoogle Alpha بلا تغيير. رفع run [33481729437](https://github.com/m7madv/otlobli/actions/runs/33481729437) Build `4.5.0 (27)` إلى iPhone TestFlight، وصار `VALID` بالمعرّف `5b04b5da-22e5-4cc9-affc-691e3b7e840f` وداخل `Damanak Internal` ذات المختبر الواحد، من دون إضافته إلى App Review؛ أكد ذلك فحص القراءة [33482932576](https://github.com/m7madv/otlobli/actions/runs/33482932576). IPA حجمها `23,943,985` وبصمتها `808712BDD6E46942C9CE837B01FC329ED1CE25A3FC47E95F577BEC03790E2547`. ثبتت حزمة Build `26` والشراء واستعادة Apple orphan سابقاً على iPhone؛ لم يُثبت Build `27` أو تُنفذ انتقالاته الجديدة على جهاز حقيقي، ولم يُنفذ قبول Android أو VoiceOver/TalkBack أو جهاز ضعيف.

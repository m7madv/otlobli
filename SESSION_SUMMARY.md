...
## ضمانك — تنظيف الاشتراكات ومنع خفض الباقة في Build 28 (2026-09-01)

- صارت السياسة بسيطة: بدء جديد، ترقية، أو تغيير دورة فوترة. الباقة الأدنى ظاهرة للمقارنة فقط وغير قابلة للشراء أثناء سريان اشتراك أعلى. الحارس موجود في الواجهة و`AppController` بعد تحديث الخادم وخدمة المتجر، وأزيل خفض Google المؤجل.
- يفحص Build `28` اشتراكات StoreKit الحالية قبل كل دفع Apple. إذا بقي اشتراك ضمانك على Apple ID بعد تصفير الخادم، يمنع التطبيق دفعاً مكرراً ويطلب الاستعادة. وإذا كان الاشتراك مرتبطاً بحساب ضمانك، يجب أن يطابق Apple ID المفتوح نفس سلسلة `originalTransactionId` ونفس المنتج؛ اختلاف الحساب أو السلسلة أو المنتج يمنع نافذة الدفع بلا استعادة أو ربط صامت.
...
- مصدر Flutter الآن `4.5.0+28`. نجح `flutter analyze` و`198/198` اختبار Flutter و`77/77` اختبار Deno وفحص تنسيق 27 ملفاً و`git diff --check`، ونجح web release وAndroid debug. APK المحلي حجمه `208,000,201` وبصمته `A3DF9A22739B3E1E7938FA6EC3E9BE8CE21C6B3E0CABECBD22C6513832459BBD`.
- بقي Build `23` في App Review وGoogle Alpha بلا تغيير. نجح فحص macOS الموقّع في run [33520286056](https://github.com/m7madv/otlobli/actions/runs/33520286056)، ثم رُفع Build `4.5.0 (28)` في run [33520953615](https://github.com/m7madv/otlobli/actions/runs/33520953615) وصار `VALID` وداخل `Damanak Internal` بالمعرّف `a9038423-e4fc-4499-8c04-bba4c0f1d317`. أكد inspect [33522527864](https://github.com/m7madv/otlobli/actions/runs/33522527864) أن App Review بقي `WAITING_FOR_REVIEW` بلا mutation. IPA حجمها `23,963,467` وبصمتها `230304290CE79C5E8F47A1D511DC8C845846C88BF529F7D167CA87B07BA65561`. لم يُثبت Build `28` أو يُختبر منع الخفض على جهاز حقيقي أو عبر VoiceOver/TalkBack أو جهاز Android ضعيف.

...
- تحقق Apple حيّاً من bundle ID وميزة `In-App Purchase` وملف التوقيع. المنتجات الستة موجودة ومتاحة في قطر وأسعارها `39.99/399.99` و`79.99/799.99` و`199.99/1999.99 QAR`، لكن حالة الأصل ما تزال `MISSING_METADATA` والإصدارات `PREPARE_FOR_SUBMISSION`.
...
- لم تُرسل مراجعة عامة ولم يُنفذ شراء حقيقي. المطلوب الآن دخول Account Holder إلى `Business` والتأكد أن الاتفاقية المدفوعة والبنك والضرائب كلها `Active`، ثم اختبار Build `19` على iPhone؛ هذه الحالات المالية هي السبب الرسمي الوحيد المتبقي غير المتحقق لعودة كتالوج Apple فارغاً.

...
## ضمانك — إصلاح شامل للاشتراك والاستعادة في Build 26 والخادم v28 (2026-09-01)
...
- نجح `flutter analyze` و`181/181` اختبار Flutter سابقاً؛ نجح Deno الآن `77/77`، واختبار Apple الموجه `6/6` على Deno `2.1.4` و`6/6` على Deno الحديث، واختبار migration `1/1` و`deno check --frozen` وفحص تنسيق 27 ملفاً و`git diff --check`. لم تجد مراجعة أمنية مستقلة مانع نشر. لم يتغير Flutter في رقعة v28؛ لم تُنشأ حزمة جديدة، وتستفيد نسخة TestFlight Build `26` فوراً.

# ملخّص الجلسة — VoiceBrief build 18 (2026-09-01)

## VoiceBrief — المشاركة والسرعة ودقة المواعيد

- جُهزت صفحة Apple للبناء `0.1.0 (18)` عمليًا: تقييم `4+`، تطبيق مجاني، `175` بلدًا/منطقة، أربع صور iPhone وأربع صور iPad، وملاحظات مراجعة عربية تشرح دخول المراجع بحساب Google والتسجيل الصوتي والمشاركة من WhatsApp والمواعيد والمنبهات والاشتراك. جُمعت مجموعة `VoiceBrief Pro` والاشتراكان الشهري والسنوي وإصدار iOS في مسودة مراجعة واحدة، ووُضع الاشتراكان في المستوى نفسه لأن المزايا متساوية.
...
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار أن المالك يملك حقوق محتوى الطرف الثالث. حُفظت بيانات المراجع باسم `MOHAMMAD ALZOUABI` وحساب Google `mhm1981d@gmail.com` والهاتف `+974 70067040` بعد أن أدخل المالك كلمة المرور مباشرة، وصُححت التعليمات لتطلب دخول Google بدل Apple. اختير إبقاء فرنسا، وطلب Apple `French encryption declaration approval form` لأن البناء يضم TLS معياريًا عبر BoringSSL؛ يلزم اعتماد ANSSI ورمز امتثال وبناء جديد. أصبحت مسودة المراجعة `4` عناصر بعد إضافة إصدار iOS، وزر الإرسال فعّال، لكن التطبيق لم يُرسل للمراجعة بسبب مانع الامتثال.
- أُنشئ ملحق تقني فرنسي جاهز للإرفاق بملف ANSSI في `output/pdf/VoiceBrief-ANSSI-technical-annex.pdf`: ثلاث صفحات، `82,233` بايت، SHA-256 ‏`045E6138E5B319D5E66FA606709F14283D159D000827E2B2C18C261873CE4D11`. يوثق التشفير من IPA البناء 18 نفسه وتحقق نصيًا وبصريًا، لكنه ليس اعتمادًا حكوميًا. بقيت بيانات المصرّح القانونية والتوقيع قبل إرسال النموذج الرسمي إلى ANSSI.
- نُزّل نموذج ANSSI الرسمي غير المعدل إلى `output/pdf/ANSSI-crypto-declaration-form-v2.pdf`، حجمه `2,308,710` بايت وSHA-256 ‏`149CF1324CC144C5891B085ED0ACE5E8B90888BBA61DB7A4FAF5822EE7EBF8E7`. النموذج ديناميكي من نوع `XFA` ويتطلب Adobe Reader للحفظ والتوقيع؛ لم تُملأ أي بيانات قانونية بالنيابة عن المالك.
- المرشح الحالي `0.1.0+18`. أضيفت إعادة محاولة آمنة للرفع الموقّع؛ إذا انقطع رد الرفع يتحقق الخادم من وجود الجسم ومطابقة الحجم و`Content-Type` قبل أن يتجاوز الرفع ويبدأ المعالجة، ولا يستبدل ملفاً قد يكون قيد الاستخدام.
...
- نجح CI للبناء 18 في run `33337662952`. نجح TestFlight run `33337662810` ورفع IPA حجمه `34,633,872` وبصمته `2025A7FA77D2C737A6B588982A4C2E053948E6D989B5CC499898351BFD755EA6`، ورقم تسليمه/build ID ‏`59ccf8f9-cbc9-4ac1-9f4c-54492f8ffcaf`. أكد status run `33339093610` أنه `VALID` وموجود في `VoiceBrief Internal`. تغييرات build 18 الخلفية ما زالت غير منشورة، ولم يُرسل التطبيق للنشر العام لدى Apple.
...
- الإصدار المصدر الحالي `0.1.0+18`: على iOS 26.1 فما بعد يطلب إذن Apple الخاص بالمنبّهات والمؤقتات ويجدول منبّهًا نظاميًا فعليًا عبر `AlarmKit`؛ على iOS 14–26.0 يبقى إشعار محلي كحل احتياطي، وعلى Android يبقى منبّه النظام. عبارة مثل «الساعة الخامسة» تفتح تأكيد التاريخ والوقت أولاً.

...
## v86.39 — خيارات النافذة هي المصدر الوحيد عند «انقر للشراء»

- أفضل حل هو الشرط الذي اقترحه المستخدم: إذا ظهر `انقر للشراء` لا نعتمد خيارات الخلفية. أول ضغطة على زر Otlobli تفتح نافذة خيارات SHEIN تلقائياً ولا تضيف المنتج.
- السبب الحقيقي كان على مرحلتين: الكود يختار أول حاوية `size` في DOM حتى لو كانت خلف النافذة، ثم طبقة «جاري الإضافة» تغطي النافذة فتجعل إعادة التحقق ترجع للخلفية القديمة. صار الاختيار من الحاوية المرسومة غير المغطاة، ويُحفظ اختيار النافذة خلال طبقة الإضافة.
- عند فتح النافذة تُمسح ذاكرة خيارات الخلفية؛ لذلك نافذة بلا مقاس تبقى ممنوعة ولا ترث مقاساً قديماً. إذا غيّر الزبون اللون أو المقاس داخل النافذة، فالاختيار الجديد هو الذي يصل للسلة.
- اختبار كامل وضع في الخلفية `أسود + S` وفي النافذة `أزرق + L`: فتحت الضغطة الأولى النافذة مرة واحدة بلا إضافة، مُنعت الإضافة بلا مقاس، ثم وصلت السلة بـ`أزرق + L + $14.43`. بقيت حالات `رمادي / كبير` و`L` و`M / 1PC` سليمة، ولم يتغير السعر.
- نجح حارس التجمّد والبناء وحدود الأداء ومزامنة Android/iOS وGradle. APK: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.39-shein-drawer-options-priority-debug.apk`، SHA-256 `358229CFE3A271C462A4B3C03331A5A6B2FCF9F60A24165D2FB9FDE0F1850953`.
- نجح GitHub/Xcode run `30696575866` من commit `5779f29`. IPA غير الموقّعة: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.39-iphone16-unsigned.ipa`، SHA-256 `C08672FFF95A949F3B1097CB65972705F3E468E2B87521E17A064EB499CD44DC`. الفحص أكد `86.39/899` وعلامات النافذة وإصلاح التجمّد وعدم وجود توقيع أو provisioning profile.
- لم تُجرّب v86.39 على iPhone الحقيقي بعد، ومشكلة تبديل المنطقة المنفصلة ليست محلولة بهذه الجولة.

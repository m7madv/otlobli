# ملخّص الجلسة — VoiceBrief build 17 (2026-08-30)
...
- اكتمل فحص حماية رسمي للنسخة السابقة ووجد 6 نقاط حقيقية. مصدر build 17 يعزل جلسة مشاركة iPhone عند تبديل الحساب، يتحقق من مدة الصوت من الملف نفسه، يمنع الرفع المباشر غير المحدود، يفرض انتهاء النتائج، ويغلق سباق وحدود جسم نموذج الدعم. تقرير الفحص محفوظ في `voicebrief_flutter/docs/security/build17/`.
- نجح محلياً format وanalyze و42 اختباراً وظيفياً و12 بصرياً و19 اختبار Deno وبناء Android debug. APK حجمه `177,653,885` بايت وبصمته `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`، ونجح CI النهائي run `33327898696` بكل وظائفه.
- نُشرت migration الحماية ودوال `create-audio-upload/process-audio/legal/delete-account` إلى مشروع VoiceBrief الصحيح `jyehqpdbayslhzebdycj` فقط؛ لم يُمس Damanak أو `talabieh`. الدوال `ACTIVE` بإصدارات `2/14/8/9`، ونموذج التفريغ الحي هو `gpt-4o-mini-transcribe`.
- نجح TestFlight run `33327898649` ورفع IPA البناء 17 بحجم `34,634,674` وبصمة `5FB74810929DC7D20FF88493F64B7C24F127B9D690CBD2122A68D6C670B388F3` ورقم تسليم `e76f600f-70a6-4ef6-bb6d-0944dd939675`. أكد status run `33327898729` أن البناء `VALID` وموجود في `VoiceBrief Internal`. لم يبدأ النشر العام إلى Apple أو Google؛ الخطوة الحالية اختبار المستخدم على iPhone ثم قرار النشر.
...
- جُهزت أربع صور iPad احترافية من شاشات التطبيق الفعلية فقط، بلا ميزات مخترعة، في `voicebrief_flutter/store_assets/app_store_ipad/` بمقاس `2048x2732` وبدون alpha.
...
- الإصدار المصدر الحالي `0.1.0+17`: على iOS 26.1 فما بعد يطلب إذن Apple الخاص بالمنبّهات والمؤقتات ويجدول منبّهًا نظاميًا فعليًا عبر `AlarmKit`؛ على iOS 14–26.0 يبقى إشعار محلي كحل احتياطي، وعلى Android يبقى منبّه النظام. عبارة مثل «الساعة الخامسة» تفتح تأكيد التاريخ والوقت أولاً.
...
- نجح أول تجميع Xcode للبناء 15؛ تعثر CI بعد التجميع فقط لأن خطوة التحقق القديمة كانت ما تزال تتوقع build 14، وقد صُححت إلى 15 ويجري إعادة الفحص.
...
- نُشر `process-audio` إلى مشروع VoiceBrief الصحيح `jyehqpdbayslhzebdycj` فقط، بالإصدار `11` وبصمة `cd2724f53c9acfe07e007db786a632a9a64ee797e3062345060a5cef17bc1bd8`. الاختبار العربي الحي بعد النشر أعاد HTTP 200 وفصل الغد عن `2026-09-05` خلال `17.153` ثانية، ثم حذف حساب الاختبار وملفه واستحقاقه المؤقت.

...
- الخادم يحتوي إعداد FCM لـAndroid وأسرار APNs الإنتاجية الأربعة لـiPhone. نجح فحص المفتاح من داخل Supabase، وقبل APNs إرسالاً حياً موجهاً لرمز نسخة `86.229` بنتيجة `sent=1/1`؛ بقي تأكيد ظهور التنبيه ومسار الضغط على الجهاز.
...
7. حفظ القيم التالية كأسرار خادم:
   - `APNS_KEY`: محتوى p8 مشفراً Base64 بسطر واحد؛ يفكه الخادم إلى PEM.
   - `APNS_KEY_ID`.
   - `APNS_TEAM_ID`.

...
## حارس التقاط المقاس المركّب — v86.19

- بعض قوالب المنتج تعرض اختياراً مركّباً مثل `M / CP1` بينما يعلن عنصر داخلي جزئياً `1PC`. يجب تفضيل النص المرئي الكامل القريب من عنوان `الكمية / مقاس`; لا تعكس هذا إلى أول aria-label قصير.
- فحص العنوان/الزر يعمل فقط عند الالتقاط، محدود بعدد عناصر، وتُحفظ نتيجته 1.2 ثانية بين محاولات الالتقاط ثم تُبطل مع النقرة التالية. لا تحوله إلى مسح داخل `tick()` أو polling دائم.

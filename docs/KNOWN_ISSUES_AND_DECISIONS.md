...
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  ...
  إذا اجتمع حسابان فلا تُنسخ جلسة Google. ظهر عادل والجديد معًا في `01`، لكن
  إعادة تشغيله ثم الفحص المباشر أعادا `Accounts: 0` من دون تنفيذ حذف يدوي؛
  الجديد غائب وعادل يحتاج إعادة دخول. `01` مفتوح لذلك باسم `RELOGIN` وADB=1
  مؤقتًا، و`10` عاد فارغًا ومغلقًا. بعد دخول عادل فقط يُتحقق بالعنوان الصريح
  `127.0.0.1:5555` ثم يعاد ADB للصفر. لا تستخدم `ldconsole adb --index 0`
  بسبب تصادم جهاز Android Studio، ولا توقف AVD. `02` ما زال ثنائيًا ويمكن
  فصله إلى `11` بتسجيل مستقل لاحقًا.

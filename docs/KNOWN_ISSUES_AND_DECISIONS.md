...
- **Acceptance boundary:** local TypeScript, architecture guard, diagnostic build, hardening, store/Temu/performance gates, and iOS sync pass at `86.188/1050`. Xcode and device acceptance remain pending. Raw/off must pass ten resumes with interaction and Home → list → PDP; full mode must then pass the shopping path and five resumes on both affected iPhones before calling the symptom fixed.

...
## 2026-09-08 — إرسال 1113 للمراجعة بعد قبول WhatsApp

- طلب المالك الإرسال صراحة. استُخدمت مسودة `/releases/2` والحزمة الموجودة
  `86.244 (1113)`، ثم حُفظت وأُكد إرسال 14 تغييرًا من Play Console.
- الحالة المرئية «التغييرات قيد المراجعة»، مع فحوصات أولية جارية؛ لا يُدعى
  نجاح الفحوص أو قبول Google أو بدء عداد المختبرين. الإصدار القديم `1112`
  غير مضمن في الإصدار الجديد، وتصحيح بيان الميزات المالية ضمن المراجعة.
- الإرسال للاختبار المغلق Alpha والسعودية والقائمة المعتمدة فقط. لا
  Production ولا بناء/رفع جديد ولا تغيير لدى Apple أو في الخدمة.
- أعيد تفويض السكربت إلى `operation=inspect` بعد الإرسال. لا تكرر الرفع
  ولا تعدّل المسار الجاري لمجرد أن حقل `releaseStatus=draft` التاريخي بقي
  في إعداد الرفع السابق؛ مصدر حالة المراجعة الحالي هو Play Console.

## 2026-09-07 — MZ3B بدل مرسل WhatsApp المحذوف

- أكد المالك حذف المرسل القديم عمدًا. لا تُعد ربطه ولا تعتبر استعادة جلسة
  Baileys القديمة حلًا. أُجيز ربط مفتاح مشروع «اطلبلي» من `mz3b.com`.
- خادم Otlobli القديم ما زال متاحًا؛ نُشر عليه محوّل MZ3B مع حفظ عنوان
  ومسارات العميل. لا حاجة لبناء جديد لهذا الربط الخادمي. الجاهزية الحية
  ناجحة بعد النشر؛ أكد المالك لاحقًا أن الربط «شتغل» بعد طلب اختبار الرمز
  والدخول. هذا قبول من إفادة المالك دون تحديد الجهاز/المنصة/البناء، وليس
  دليل اختبار مستقل من الوكيل لكل منصة.
- المفتاح خادمي فقط. لا تبديل مفاتيح أو أرقام MZ3B ولا لمس مشروع Mela Wood.
  لا تخزن المفتاح/الرمز في الوثائق. `mz_test_` لا يعني بالضرورة إرسالًا وهميًا.
- التشغيل الحالي عملية واحدة وقرص دائم؛ لا تعدد عمال مع دفتر المحاولات
  المحلي. يحفظ منع التكرار قبل الإرسال ويقبل `approved` فقط، مع ميزانيات
  تراكمية وقفل استهلاك الرمز. لا تغيير لمخطط قاعدة البيانات أو عقد جلساتها.
- الربط لرموز الدخول، وليس لتنبيهات WhatsApp القديمة. لا تستخدم واجهة
  الموظفين لإرسال رسائل العملاء دون تفويض وموافقة مناسبة.
- بقية حالات Google/Apple أدناه لم تتغير؛ لم تُرسل مراجعة في دفعة الربط.

## 2026-09-07 — تصحيح إقرار رصيد الشراء وفق رد Google

- هذا القرار يستبدل القرار المحافظ بتاريخ 30 أغسطس أدناه الذي اختار
  `Mobile payments and digital wallets` لمجرد وجود رصيد قابل للشحن.
- رد Albert على `4-1130000041592` بتاريخ 5 سبتمبر طلب تدقيق الإقرارات،
  ثم زيادة رقم البناء وإعادة المراجعة إذا كان الإقرار لا يطابق الوظائف.
  الرد مشروط، وليس قبولًا أو استثناءً من سياسة الحسابات.
- بعد موافقة المالك وتدقيق واجهة العميل وAPI وترحيلات الدفع: الرصيد مخصص
  لطلبات Otlobli ويُشحن عبر مزود خارجي، ولا مسار تحويل P2P أو سحب نقدي في
  المصدر المفحوص. حُفظ اختيار «لا يوفّر تطبيقي أي ميزات مالية» دون تغيير
  الوظائف؛ يبقى قرار صحة التصنيف النهائي لدى Google. لم تُفحص كل دوال
  قاعدة الإنتاج حيًا ولم تُجرَ معاملة مالية لإثبات ذلك.
- التفويض الجديد Android `86.244 (1113)` للاختبار المغلق `alpha` فقط؛
  لا Production ولا تغيير أسماء/حسابات المختبرين. Apple `1112` منفصل ولا
  يُعاد بناؤه/تقديمه. رفضه الأحدث المعروف `5.2.2` لم يُحل بتصحيح Google.
- نجح البناء الموقّع والتحقق من الهوية والشهادة وبصمات الأصول. تفويض السكربت
  يتطلب 1113 وبصمة AAB الجديدة و`alpha` فقط، ويبدأ بـ`draft` دون إرسال نهائي.
  فحص الخدمة الحي كشف مرسل WhatsApp غير متصل؛ لا يُتجاوز حاجز الجاهزية ولا
  تُعدّل الجلسة المشتركة. Google configured=true بالمُعرّف الفعلي الافتراضي.
  انقطع اتصال Edge بعد نجاح حفظ الإقرار؛ يبقى الإرسال النهائي غير مؤكد.
- Google رفض commit في `34140843006` بلا علم `changesNotSentForReview=true`.
  القرار هو حفظ التغييرات بهذا العلم دائمًا في هذا المسار، ثم إرسالها صراحة
  من Play Console؛ لا يُسمّى نجاح commit قبولًا أو إرسال مراجعة.
- نجح `34141060561` من `68efa11` في حفظ `86.244 (1113)` كـ`draft` على
  `alpha` مع العلم المذكور. لم تُرسل مراجعة ولم يُنشَر إنتاج. أكمل المسودة
  الموجودة لاحقًا ولا ترفع رقم البناء مجددًا بسبب توقف اتصال المتصفح وحده.

## v86.244/1112 — قبول الجهاز وتفويض App Review (2026-08-30)

- **القبول:** أكد المالك نجاح SHEIN كزائر إلى أول منتج على iPhone 16 Pro Max،
  وعدم بقاء صفحة الدخول أو خروج Google/Facebook إلى Safari، وخمس دورات
  background/resume، واختبار force-quit/cold-launch مستقل. أكد أيضًا نجاح
  السلة والدفع حتى رمز ShamCash على iPhone وiPad بلا ضغط «لقد دفعت».
- **قرار الإرسال:** استُبدلت `PREPARE_ONLY` بسجل
  `SUBMISSION_AUTHORIZED.json` مرتبط حصريًا بـ`86.244 (1112)`. الحارس يطلب
  إحدى علامتي التحضير أو التفويض فقط ويتحقق من مصفوفة القبول، وسكربت الإرسال
  يرفض اختلاف الإصدار أو البناء.
- **إعادة إرسال الرفض:** أثبتت محاولة `33304324644` أن إصدار App Store نفسه
  بقي داخل طلب قديم بحالة مشاكل غير محلولة؛ Apple رفضت إضافته لطلب جديد.
  القرار هو العثور على الطلب الذي يملك العنصر، تعيين `resolved=true` للعنصر
  المرفوض، ثم إعادة إرسال الطلب نفسه. لأن platform اختياري في طلبات Apple، لا
  تُصفّى قائمة البحث حسبه؛ ويجوز استرداد معرّف طلب وحيد من associatedErrors مع
  إعادة قراءة العنصر والتحقق من الإصدار. لا حذف للإصدار أو فصل يدوي للبناء.
- **المسار السريع:** لأن البناء `1112` معالج وموزع أصلًا، فإن اختيار
  `distribute-existing` مع App Review يفحص تفويض القبول ثم يشغّل إعادة الإرسال
  مباشرة على Linux، ويتخطى أرشفة iOS والتوقيع ورفع TestFlight. لا يتدخل هذا
  المسار في Oracle أو WhatsApp ولا ينشئ بناء Apple جديدًا؛ حارس المصادقة باقٍ
  لمسارات البناء والرفع الجديدة.
- **الحالة الحية:** أثبت التشغيل `33306098541` أن وصول OTP من التطبيق الآخر
  لا يعني جاهزية عملية Otlobli؛ بقي حقلا الاتصال والمرسل `false` فتوقف قبل
  Apple. صحح المالك الافتراض صراحة: اختبر `86.244 (1112)` نفسها مرارًا ووصل
  OTP ونجح الدخول. القرار المحدّث هو اعتماد قبول الجهاز وعدم ربط إعادة تقديم
  بناء معالج بحالة `/health` لحظية أو التدخل في Oracle؛ حارس البناء الجديد
  يبقى بلا تغيير.
- **ملكية سجل Apple:** كشف `33306887739` أن الطلب القديم يملك عنصر
  appStoreVersion قديمًا غير سجل `86.244` الحالي. لا يجوز تفسير معرّف الطلب
  وحده على أنه يملك السجل الحالي. إذا كان الطلب `UNRESOLVED_ISSUES` وعنصره
  القديم `REJECTED`، يزال عنصر المراجعة القديم فقط بـ`removed=true` ثم تضاف
  النسخة الحالية للمسودة؛ لا حذف لسجل App Store أو بناء، ولا اختيار ضمني عند
  تعدد المعرّفات.
- **الطلب المرحّل:** إذا ظهر الحاجز القديم في `appStoreVersionForReview` بلا
  عنصر حديث، فلا يُلغى الطلب إلا مع `UNRESOLVED_ISSUES` وتطابق معرّف الإصدار
  القديم وعدم وجود أي عنصر نشط آخر واختلافه عن الإصدار الحالي. بعد `COMPLETE`
  فقط يعاد إنشاء عنصر `86.244` في المسودة الحالية.
- **حارس التشخيص:** عدم تطابق علاقة الطلب المرحّل يوقف الإلغاء ويطبع معرّفات
  الموارد وعدد العناصر النشطة فقط؛ لا تُطبع أسرار ولا يحدث تغيير خارجي.
- **الطلب الفعلي:** أعاد `33307207063` علاقة فارغة وعنصرًا نشطًا واحدًا، لذلك
  بقي الإلغاء ممنوعًا. يسمح التشخيص بعرض نوع علاقة العنصر وحالته ومعرّفه فقط
  لتحديد المعالجة من دون تخمين.
- **معرّف العنصر المرحّل:** يسمح اعتبار عنصر بلا relationship إصدارًا قديمًا
  فقط إذا كان base64url قانونيًا يعاد ترميزه حرفيًا إلى
  `<submission>|6|<blockerVersion>` مع تطابق الطلب والحاجز. عندها يزال عنصر
  المراجعة وحده؛ لا يلغى الطلب كله ولا يحذف إصدار أو بناء.
- **النتيجة النهائية:** نجح `33307366619` في إزالة عنصر `890129230` المرفوض
  فقط من الطلب القديم، وإضافة `86.244 (1112)` إلى الطلب
  `2ae4119e-bfbf-4a1a-a813-ddda2bf7e460`. حالة Apple هي
  `WAITING_FOR_REVIEW`؛ لا يُدّعى النشر العام قبل الموافقة.
- **الحد:** إفادة المالك هي دليل الجهاز؛ CI لا يثبتها. قبول Android الضعيف
  للبناء `1112` غير منفذ ولا يمنع إرسال iOS الحالي إلى App Review.
- **Google Play:** التفويض محصور في AAB `86.244 (1112)` ذي البصمة
  `CCFC71D2…BBBE6`. بعد موافقة المالك القانونية الصريحة أُنشئ سجل
  `com.otlobli.app` كعربي/مجاني/تطبيق. أقل صلاحيات الخدمة هي قراءة التطبيق
  والجودة وإصدار جمهور الاختبار فقط. أثبت `33310340151` أن الرصيد صفر حزم وأن
  `alpha` هو closed track المتاح؛ لذلك `publish` مقيد بهذا المسار وحده، بلا
  صلاحية أو تحديث لمرحلة الإنتاج وبلا إعادة بناء Android. التطبيق الجديد لا
  يقبل `completed` بينما سجل أول تطبيق نفسه Draft؛ `33310435203` و
  `33320651915` توقفا بأمان قبل commit. المهام مكتملة والتفويض أصبح
  `releaseStatus=completed` في `5cc4f5d`. عولج حاجز التطبيق الأول لاحقًا من
  Play Console: تمت معاينة الإصدار نفسه وحفظه، ثم أُرسل 14 تغييرًا للمراجعة.
  الحالة الحالية `التغييرات قيد المراجعة` وتشمل `86.244 (1112)` على `alpha`
  فقط؛ لم يُرفع ملف جديد ولم يُرسل Production.
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  واحد في كل مرة ثم إغلاقه. استبعد المالك ثلاثة حسابات مشتركة مع هاتف صديقه.
  القائمة النظيفة اسمها الآن `All Apps - Closed Testers` كي يعاد استخدامها
  لأي تطبيق، وتحتوي 12 حسابًا مختلفًا بالضبط. بطلب المالك حُذفت مثيلات
  `LDPlayer-9` و`z10` و`z11`
  نهائيًا، ثم أضيفت 3 مثيلات جديدة ليصبح العدد 12. تعتمد الأسماء النظام الموحد
  `01 - <email>`؛ استُخدمت علامتا `EMPTY` و`PENDING` خلال الإعداد فقط.
  إذا اجتمع حسابان فلا تُنسخ جلسة Google. ظهر عادل والجديد معًا في `01`، فحذف
  المالك الجديد يدويًا وأكد بصريًا بقاء عادل وحده. AccountManager لم يكن
  موثوقًا أثناء واجهة المصادقة، فلا يُنسب له تحقق الحساب. أكد المالك اكتمال
  حساب مختلف واحد في كل مثيل `01..12`: الحساب الجديد في
  `10` و`djjd19903` في `11` وسمير في `12`، و`02` الآن لـ`chatxhxh` فقط.
  ثُبتت الأسماء، `10` مفتوح والبقية مغلقة، وكل ADB عند الصفر. لا توقف AVD.
- **ما يبدأ عداد 14 يومًا:** لا يكفي البريد في القائمة ولا الحساب في المحاكي
  ولا تثبيت التطبيق وحده. بعد موافقة Google وتفعيل المسار يجب انضمام 12 حسابًا
  عبر رابط opt-in وبقاؤها منضمة باستمرار. Google تطلب في طلب Production وصف الاستخدام
  الفعلي والميزات والملاحظات والتغييرات؛ يلزم اختبار صادق ولا تُختلق مشاركة.
- **مسودة Google Play:** نجح `33310601936` في وضع AAB نفسه `1112` على
  `alpha` كـ`draft`. أثبتت محاولة `33320651915` أن أول تطبيق يحتاج مراجعة
  Google قبل قبول `completed`. بعد ذلك حُفظ الحساب الثاني عشر، وسُمّيت القائمة
  العامة، وحُفظ الإصدار من الواجهة، وأُرسل مع 14 تغييرًا للمراجعة. الاختبار
  لم يبدأ حتى الآن لأن موافقة Google ومرحلة opt-in ما زالتا لاحقتين.
- **المعرّف الإعلاني:** الإقرار الصحيح هو `لا`. merged manifest الفعلي
  للإصدار `86.244 (1112)` لا يحتوي
  `com.google.android.gms.permission.AD_ID`، ولا توجد SDK إعلانية في اعتمادات
  Android/npm. تحذير native debug symbols غير حاجب ولا يبرر تغيير الحزمة.
- **بيانات المراجعة:** حُفظ حساب Google القابل لإعادة الاستخدام والتعليمات؛
  أدخل المالك كلمة المرور بنفسه ولا تُحفظ في المستودع. لا يُقدّم OTP WhatsApp
  المتغير إلى Google.
- **حذف الحساب:** أضيف مسار خارجي يبدأ الطلب عبر البريد ونُشر داخل صفحات
  الخصوصية والدعم على `https://talabieh.vercel.app`.
- **الإقرار المالي:** محفظة Otlobli قابلة للشحن والدفع للطلبات، لذلك القرار
  المحافظ الصحيح هو `Mobile payments and digital wallets`. الدفع لسلع مادية
  لا يستخدم Google Play Billing، ولا يعني ذلك اختيار `no financial features`.
...
- **الفصل:** صفحة `/user/login` الكاملة لا تملك opt-out وتبقى ملك رجوع Native
  من v86.244. النافذة الاختيارية فوق المنتج تملك نص «تسجيل لاحقًا» دقيقًا.
- **سبب الفوات:** الاكتشاف الفوري كان محصورًا في
  `.s_auth__block-login-tip`. تغيير اسم الغلاف الخارجي يستطيع منع المسار
  الحدثي مع بقاء الزر نفسه.
- **القرار:** لا نضيف selector أو observer. تعيد المطابقة استعمال NodeList
  `candidateSelector` الذي تجمعه `scan()` أصلًا، وتفحص الجذر وآخر `16`
  مرشحًا exact فقط. يبقى الغلاف المعروف احتياطًا. fallback القديم صار يفهم
  الصيغ العربية من دون توسيع نطاقه، ويتشارك معه علامتي `action/fired` قبل
  النقرة لمنع التكرار أيًا كان ترتيب التنفيذ.
- **حد الأمان:** challenge يخرج أولًا؛ لا تطابق لـ«تسجيل الدخول» وحدها، ولا
  fields أو OAuth أو صفحة حساب اختارها المستخدم. لا تغيير native lifecycle أو
  المنطقة أو cookies/session أو الدفع أو الطلبات أو المحفظة.
- **Apple:** القراءة الحية تثبت أن `86.230 (1095)` هو المرفوض
  (`REJECTED/UNRESOLVED_ISSUES`). البناء المصحح `86.244 (1110)` صار
  `VALID/IN_BETA_TESTING` في TestFlight فقط وغير مربوط بإصدار عام. لا يجوز
  تشغيل submit العام قبل قراءة رسالة الرفض؛ سببها النصي غير متاح عبر API
  الرسمية. قُرئت لاحقًا من الواجهة: صور App Store لا تعرض إلا Login، والمراجع
  لم يستطع متابعة الدفع بعد السلة على iPhone/iPad. سجل `86.230` المرفوض يسمح
  بتعديل Version، لذلك يُعاد استعماله لـ`86.244/1110` بعد صور استخدام حقيقية
  وقبول دفع نظيف؛ لا يُحذف السجل ولا يُعاد الإرسال قبل ذلك.
- **حد إثبات الدفع:** `1110` يصلح ملكية السلة بين المتجرين ويوضح واجهتها،
  لكنه لا يغير جوهريًا شروط زر المتابعة الموجودة في `1095`، لذلك ليس إصلاحًا
  مثبتًا لـ`2.1(a)`. الاحتمالات هي حد الطلب أو التخصيص/التوفر أو خلط السلة
  القديم، ولا واحد منها مثبت بلا سجل المراجع. القبول المطلوب تثبيت نظيف على
  iPhone/iPad، حساب مكتمل، منتج عادي متوفر لكل متجر فوق `5000 SYP` ومعادل
  `$40`، ثم طلب pending واحد ورمز ShamCash صحيح بلا دفع فعلي. يجب أيضًا حسم
  تعارض انتهاء الطلب: ساعتان في تعريف المستودع مقابل 5 دقائق في الواجهة/الوثائق.

## v86.244 — صفحة دخول SHEIN الكاملة كانت انتقال SPA غير معكوس (2026-08-26)

- **الدليل:** لقطة المستخدم والصفحة الحية تطابقان `/ar/user/login`: حقل
  البريد/الهاتف وGoogle وFacebook ومحدد البلد، ولا يوجد
  `.s_auth__block-login-tip` أو زر «تسجيل لاحقًا».
- **السبب:** التصنيف `blocked-login` صحيح، وقرارات iOS/Android تمنع التنقل
  الشبكي، لكن History API غيّر عنوان الوثيقة نفسها. كان KVO في iOS و
  `doUpdateVisitedHistory` في Android يسجلان العنوان المحجوب من دون عكسه.
- **القرار:** إشعار URL الأصلي ينفذ Back واحدًا. فحص أحادي بعد `200ms` يذهب
  إلى Home فقط إذا لم يغادر Back المسار المحجوب، وغياب سجل الرجوع يذهب إلى
  Home مباشرة. كل popup صادر من صفحة الدخول يُلغى قبل تحميله داخل WebView أو
  تحويله إلى intent خارجي؛ لذلك لا يُصلح التطبيق Google/Facebook ولا يسمح
  لـFacebook بفتح Safari.
- **حد الأداء والأمان:** لا DOM selector أو observer/interval أو مسح أو reload
  أو WebView جديدة. التحقق البشري يبقى مسموحًا وغير مؤتمت، ولا تتغير المنطقة
  أو cookies/session أو الدفع أو الطلبات أو المحفظة أو Temu.
- **التحقق:** حزمة Capgo النظيفة تقبل الرقعة، واختبار `/ar/user/login` وحراس
  المصدر والبناء وAndroid compile/R8 والمزامنة والميزانيات تمر. Note 8 يحمل
  `86.244 (1109)` بلا fatal/ANR/OOM مطابق؛ قبول iPhone الحقيقي ينتظر النسخة
  الموزعة وتجربة المستخدم.

## v86.242 — فراغ منتج Temu على iPhone كان شريط تنقّل أصليًا (2026-08-26)

- **الدليل:** في لقطة `943×2048` انتهى شريط الحالة عند `y=132`، وامتد
  الأبيض الخالص `y=136..295` ثم ظهر فاصل أصلي `y=296..298`. يعادل ذلك نحو
  `75pt`، وهو ارتفاع `UINavigationBar`، بينما Note 8 الحالي بلا الفراغ.
- **السبب:** `ToolBarType.BLANK` يخفي الشريط عند الإنشاء، ثم كان
  `WKWebViewController.setUpState()` يعيد إظهاره دائمًا وبحركة عند
  `viewWillAppear`.
- **القرار:** احترام `blankNavigationTab` في السطر الأصلي نفسه. لا تعديل
  Temu DOM أو safe area، ولا طبقة أو مؤقت أو observer أو مسح جديد. الأنماط
  غير الفارغة تحتفظ بسلوكها السابق.
- **الحماية:** `verify:store-surface` يفحص المصدر المطبق والرقعة الدائمة،
  وحزمة npm النظيفة تقبل الرقعة. SHEIN ودورة التطبيق والمناطق والدفع خارج
  التغيير.

## v86.239 — تعبئة رمز واتساب بأفضل جهد من النظام (2026-08-25)

- **السبب المثبت في العميل:** أول خانة من واجهة الرمز المقسمة كانت
  `maxLength=1`، فتقص اقتراح النظام الكامل قبل أن يصل إلى موزع الخانات.
- **القرار:** الخانة الأولى تقبل ستة محارف وتحمل `one-time-code` مع
  `id/name` ثابتين؛ البقية تبقى محرفًا واحدًا. اللصق الصريح متزامن مع
  `preventDefault` لمنع طلب تحقق مزدوج.
- **قرار الرسالة:** الرمز ستة أرقام، يظهر مرة واحدة وفي السطر الأول، ولا يوجد
  تسلسل رقمي ثان أو رابط. يحسن ذلك تعرّف الإشعار من دون خدمة أو صلاحية جديدة.
- **حد المنصة:** لا يمكن ضمان اقتراح رمز رسالة WhatsApp عادية على Android أو
  iOS. المسار الحالي Baileys هو best-effort؛ قوالب Meta الرسمية هي المسار
  الموثق إذا أصبح الضمان شرطًا. لا نضيف قارئ إشعارات أو حافظة خلفية.
- **الحالة الحية:** تنسيق الرسالة منشور على Oracle والخدمة online. أُعيد وصل
  الجلسة `0` المحفوظة بلا QR أو مسح، والصحة تعيد `whatsappConnected=true` و
  `whatsappSenderReady=true`. TestFlight `86.239 (1104)` في حالة
  `IN_BETA_TESTING`؛ القبول الحقيقي لوصول الرمز واقتراحه ينتظر تجربة iPhone
  وهاتف Android قديم.

## v86.238 — Android 15/16 وإرشاد المنتقي (2026-08-25)

- **التشخيص:** ازدحام صفوف Temu أصلي، لكن A53/API 36 كشف inset حالة مكررًا:
  المحتوى بدأ `y=87` وWebView `y=175`. السبب AppBar فارغ احتفظ بهامش الحالة
  رغم أن المضيف سبق أن حجزه.
- **القرار:** تصفير الهامش فقط لـTemu + الشريط الأصلي + `blank`. لا تعميم على
  SHEIN أو متصفح آخر. الحجب/الموافقة في Temu document-start محدودان ولا
  يستخدمان observer/interval/DOM scan واسعًا.
- **قرار الإرشاد:** Temu وSHEIN يعرضان تلميح دخول أصليًا؛ نقرتان خلال `320ms`
  تفتحان المنتقي فقط. لا تبديل مباشر ولا نص دائم ولا شريط داخل الصفحة.
- **التحقق:** challenge يلغي التلميح ويملك اللمس. حارس fallback علم خفيف
  ومسارات URL دقيقة كي لا يمنع DCE أو يحجب بحثًا عاديًا.
- **الحدود:** APK النهائي قُبل تثبيتًا وإقلاعًا على محاكي Android 15 فقط.
  Note 8 انفصل قبل تثبيته، وA52/Android 16 النهائي وملف cookies نظيف وiPhone
  الحقيقي لم تُقبل بعد.

## v86.237 — تبويب السلة ليس هوية جلسة المتجر (2026-08-25)

- **التشخيص المثبت:** أعاد Note 8 المسار Temu ← Cart ← تبويب SHEIN ← Home ←
  chooser ← SHEIN إلى Temu. كان `selectedStoreRef` يملك تبويب السلة وجلسة
  WebView معًا، فيسمح fast path بإظهار جلسة Temu المركونة ويصنف رسالة إضافة
  متأخرة حسب التبويب الحالي.
- **القرار:** للجلسة القياسية مالك صريح `{store, sessionId, id}` مستقل عن
  التبويب. لا يُقبل الحدث إلا من المالك الحالي، ويحدد نطاق رابط المنتج سلته،
  ويستهدف ACK معرّف المصدر. تبديل تبويب السلة لا يغلق أو يعيد فتح المتجر.
- **ترحيل البيانات:** يصلح mount واحد العناصر التي يثبت `temu.com` أو
  `shein.com` أنها في السلة الأخرى؛ الروابط المجهولة تبقى مكانها. على الجهاز
  أصبحت العدادات `Temu=2` و`SHEIN=0` من دون إضافة منتج جديد.
- **قرار الزر:** موضع زر Temu الثابت `bottom:24px` بدل `16px`؛ المقاس
  `128×48px` واليمين `14px`. SHEIN لا يتغير. لا dynamic inset ولا timer أو
  observer أو DOM scan أو شريط صفحة جديد.
- **الدليل:** اجتاز Note 8 السيناريو الأصلي وعكسه على APK النهائي. زر Temu
  `[706,1666][1044,1795]` يبعد `63px` فعلية عن WebView، أي `24 CSS px`.
  سجل القبول بلا fatal/ANR/OOM/native crash.
- **قرار تأخير العرض:** لقطة «جاري تجهيز المتجر» في الاتجاه العكسي سبقت
  إرفاق Temu بـ`19ms`، وليست hang. لا نغير `isPresentAfterPageLoad` أو
  `preShowScript` ولا نضيف غطاء أو مؤقتًا بناء عليها.
- **الحدود:** اختبار العرض يمر عند `320–430 CSS px`، لكن لا يوجد A52 حقيقي؛
  محاكيه التقريبي كان عند auth ولم يُتجاوز. SHEIN region/human verification
  والدفع والطلبات والمحفظة وiPhone lifecycle خارج نطاق التغيير.

## v86.236 — طيّ الغلاف وتصحيح مرجع Temu اللاصق كوحدة واحدة (2026-08-25)

- **التشخيص المثبت:** كثافة صفوف Home من Temu نفسها، لكن التداخل بعد دورة
  التمرير لم يكن أصليًا. طي Otlobli لغلاف التنزيل أزال `0.66rem = 66 CSS px`،
  في حين أبقت Temu `[js-selector="bg-cui-top-sticky"]` على
  `translate(-50%, 0.66rem)`. قفزة Note 8 كانت `173px / 2.625 = 65.9 CSS px`.
- **القرار:** لا نخفي بانرًا أو صفًا آخر ولا نثبت حقل البحث. نصحح transform
  عنصر العرض الدلالي نفسه فقط، مع الحفاظ على تمركز X، وبعد إثبات نجاح طي
  الغلاف عبر علامة على `html`. إذا تغيّر DOM وفشل الكاشف لا تعمل القاعدة.
- **حد الأداء:** لا `top` قسري، ولا scroll listener، ولا timer/observer/scan
  جديد. مزامنة العلامة انتقالية كي لا تصنع طفرتي DOM كل `650ms`.
- **الدليل:** v86.235 نقل شعار Temu من `y=95..138` إلى `268..311` بعد
  التمرير. v86.236 أبقاه `95..138` بعد 10 دورات، مع تطابق بكسلي لقصّ الرأس،
  ونجاح البحث والمنتجين والخلفية/الاستئناف. Chrome عرض تحقق Temu ولم يُتجاوز؛
  لذلك لا يُدّعى خط أساس تمرير خام للمتصفح.
- **الحدود الباقية:** لم تُغيّر SHEIN أو المنطقة أو الجلسات أو التحقق أو
  الدفع أو الطلبات أو المحفظة. iPhone 16 وAndroid حديث بإيماءات النظام،
  وقبول VoiceOver/TalkBack، ما زالت اختبارات منفصلة.

## v86.235 — قياس واحد للشريط وطيّ فراغ Temu المحدد (2026-08-25)

- **قرار الشريط:** واجهة التطبيق وTemu وSHEIN تشترك في مجال بصري واحد. لا
  تُحسب مساحة أزرار Android الثلاثة كمساحة إيماءة داخل الشريط؛ تُوضع خارج
  السطح، بينما تُضاف مساحة الإيماءة الآمنة فقط عند وجودها. تبقى العناصر بعرض
  أقصى `440dp/pt` ومحاذاة وسطية، وتحجز الصفحة كامل ارتفاع الشريط بدل المرور
  خلفه. أكد Note 8 بداية موحّدة عند `y=1858` للسطوح الثلاثة.
- **قرار الإرشاد:** أول نقرة على Home داخل Temu لا تخرج المستخدم بصمت؛ تعرض
  `انقر مرة ثانية لفتح قائمة المتاجر` لمدة `1.8s`. النقرة الثانية خلال
  `320ms` تفتح `store-select` وتحافظ على الجلسة. لا يوضع نص دائم داخل الشريط،
  ويفتح تفعيل TalkBack/VoiceOver المنتقي مباشرة.
- **سبب فراغ Temu:** إخفاء عنصر التنزيل الداخلي وحده لا يغيّر ارتفاع الأغلفة
  المجهولة التي تحيط به، لذلك بقيت مساحة بيضاء. الحل يحدد أول غلاف علوي متعدد
  الأبناء يحيط بعنصر التنزيل، ثم يطوي ذلك الغلاف وأبناءه المباشرين فقط.
- **حدود الأمان:** البحث لا يتجاوز ثمانية آباء، ويشترط موضعًا علويًا وعرضًا
  وارتفاعًا محدودين، ويرفض `body`/الجذر وأي غلاف يحوي input أو searchbox أو
  search bar. يعمل فقط على Android Home وبوجود علامة المنصة الأصلية؛ iOS
  وGecko والمنتج والحساب والتحقق خارج النطاق. لا يوجد مؤقت أو مراقب أو مسح DOM
  واسع جديد.
- **الدليل والحد المتبقي:** نجحت اللقطة الحية وNote 8 في إزالة الفراغ، وإظهار
  رسالة النقرة الأولى، وفتح المنتقي بالنقرتين، ومطابقة شريط SHEIN، بلا انهيار
  أو ANR. ما زال قبول iPhone 16 وAndroid حديث بإيماءات النظام وVoiceOver/
  TalkBack اختبارًا منفصلًا لا يُستنتج من البناء أو Note 8.

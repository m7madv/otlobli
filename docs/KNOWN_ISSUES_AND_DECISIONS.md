...
- **Google Play:** التفويض محصور في AAB `86.244 (1112)` ذي البصمة
  ...
  `releaseStatus=completed` في `5cc4f5d`، لكن يجب إرسال التطبيق أولًا إلى
  مراجعة Google من Play Console، ثم إعادة المسار مع versionCode `1112` والبصمة
  نفسها.
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  ...
  قائمة `Otlobli Closed Testers` المستقلة تحتوي 11 حسابًا نظيفًا؛ يلزم حساب
  مختلف واحد قبل النشر. بطلب المالك حُذفت مثيلات `LDPlayer-9` و`z10` و`z11`
...
- **ما يبدأ عداد 14 يومًا:** لا يكفي البريد في القائمة ولا الحساب في المحاكي
  ولا تثبيت التطبيق وحده. يجب نشر مسار الاختبار ثم انضمام 12 حسابًا عبر رابط
  opt-in وبقاؤها منضمة باستمرار. Google تطلب في طلب Production وصف الاستخدام
...
- **مسودة Google Play:** نجح `33310601936` في وضع AAB نفسه `1112` على
  `alpha` كـ`draft`. وافق المالك على إضافة البريد الثاني عشر، لكن الواجهة لم
  تحفظه وبقيت القائمة 11؛ النافذة عند الإدخال اليدوي. محاولة `33320651915`
  أثبتت أن أول تطبيق يحتاج مراجعة Google قبل قبول `completed`. لم يبدأ الاختبار.

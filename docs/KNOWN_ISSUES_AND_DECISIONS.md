...
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  واحد في كل مرة ثم إغلاقه. الجرد أثبت 12 حسابًا فقط عبر 12 مثيلًا: `0` فارغ،
  `1` بحسابين، و`2..11` بحساب واحد لكل منها. لم يظهر تثبيت آخر أو مثيلان
  إضافيان. العدد يطابق حد Google الرسمي الأدنى 12/14 يومًا لكنه بلا هامش.
  أُعيد ADB المغلق بعد الجرد، ولا يُستخدم اللمس اليدوي ولا يُوقف AVD المنفصل.
- **مسودة Google Play:** نجح `33310601936` في وضع AAB نفسه `1112` على
  ...
  المتجر وتحويل الحالة إلى `completed`.
- **بيانات المراجعة:** لا يُقدّم OTP WhatsApp المتغير إلى Google؛ يلزم حساب
  Google تجريبي مخصص، قابل لإعادة الاستخدام، وبلا 2FA. البريد العام وبيانات
  المراجعة لا تُستنتج من حسابات المشروع التاريخية.
- **حذف الحساب:** الحذف النهائي داخل التطبيق موجود، لكن Google تطلب أيضًا
  موردًا خارج التطبيق يبدأ طلب الحذف. لا تُسجل صفحة الإرشادات الحالية وحدها
  كأنها تحقق الشرط؛ أضف وسيلة بريد/نموذج بعد موافقة المالك على بريد الدعم.

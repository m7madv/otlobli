...
- **Validated artifact:** GitHub run `31851860388` built commit `4dcd222`. The inspected unsigned IPA is `86.195/1057`, arm64, iOS 15+, device families `[1,2]`, 6,582,113 bytes, SHA-256 `EB53DFE1EA8FDB0C93B71C4DB5B78E3E6A8E8C32632AFB8D3D58641EB30722AA`, with the diagnostic marker present and no signature, provisioning profile, or source maps.
...
- **Acceptance boundary:** local build/guards/sync passing is not a fix. Xcode run `31849799496` compiles the Swift logger and packages the inspected `86.194/1056` IPA at SHA-256 `448DBAB412F0B7F08DF5ECA535593FBCBC3F7EF07C2D9B225E5DBD9FE722257F`. One clean-install first-entry/exit/second-entry sequence must still correlate the three layers. Android and iPhone 6 are outside this diagnostic run.
- **Inherited-frozen result:** the user updated v86.194 over the already-frozen v86.193 container. The symptom is not absolute loss of touch: rapid/random taps may eventually route to a section, but the destination is also inert or only partly populated. Native hit testing and trusted DOM touch/click capture+bubble were intact, with no preventDefault, fixed top overlay, detached surface, or WebContent termination. One anchor routed on attempt 32; the new document kept the same WKWebView, rendered header/filters with a blank product body, and logged JavaScript error hash `3ce0b2a11d15fff2`. This rejects another touch-overlay/recompose fix and makes page runtime/hydration or inherited WebKit data the active lane. Counts in that inherited document were cookies 5, localStorage 38, sessionStorage 1, CacheStorage 0, and IndexedDB 6; they are fingerprints, not proof that any one store is causal.
- **Next decision gate:** delete the app and reinstall the exact same v86.194, then capture from before the first SHEIN entry through internal exit and second entry. Compare persistent fingerprints, navigation, and JS failure chronology. Do not clear all website data as a proposed customer fix before the causal component is identified.

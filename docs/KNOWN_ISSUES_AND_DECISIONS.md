...
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  ...
  إذا اجتمع حسابان فلا تُنسخ جلسة Google ولا يُحذف أحدهما قبل تسجيله مستقلًا
  في مثيل فارغ. الحسابان عادل والجديد في `01` قيد الفصل: `10` مفتوح لتسجيل
  الجديد، وبعد نجاحه يزال الجديد من `01` ويبقى عادل. `02` ما زال ثنائيًا ويمكن
  فصله إلى `11` بالطريقة نفسها. بعد الإضافة يعاد ADB إلى الصفر. لا يُوقف AVD.

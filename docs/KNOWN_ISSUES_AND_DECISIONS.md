...
- **Google Play:** التفويض محصور في AAB `86.244 (1112)` ذي البصمة
  `CCFC71D2…BBBE6`. يبدأ المسار بـ`inspect` من دون نشر لتحديد closed track
  الفعلي، ثم يتطلب `publish` مع اسم المسار الصريح قبل رفع الملف وتحديث ذلك
  المسار وحده. لا يعيد المسار بناء Android ولا يطبع حساب الخدمة أو حسابات
  المختبرين.
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  واحد في كل مرة ثم إغلاقه. لا يُستخدم اللمس اليدوي ولا يُوقف AVD المنفصل.

...
- **Google Play:** التفويض محصور في AAB `86.244 (1112)` ذي البصمة
  ...
  يقبل `completed` قبل إعداد المتجر؛ `33310435203` حذف edit المرفوض. القرار هو
  `releaseStatus=draft` أولًا ثم تغيير الحقل نفسه إلى `completed` بعد المهام،
  مع إعادة استعمال versionCode `1112` والبصمة نفسها.
- **محاكيات الاختبار:** التحكم المقبول هو `ldconsole` وADB عبر الطرفية، مثيل
  ...
  `01 - <email>`؛ الفارغ موسوم `EMPTY` والحساب غير المكتمل موسوم `PENDING`.
  ...
  موثوقًا أثناء واجهة المصادقة، فلا يُنسب له تحقق الحساب. الاسم النهائي هو
  أكد المالك اكتمال حساب مختلف واحد في كل مثيل `01..12`: الحساب الجديد في
...
- **مسودة Google Play:** نجح `33310601936` في وضع AAB نفسه `1112` على
  `alpha` كـ`draft`. هذا لا يبدأ الاختبار ولا يبرر enrollment قبل إكمال مهام
  المتجر وتحويل الحالة إلى `completed`. اكتملت مهام المتجر، لكن القرار هو إبقاء
  المسودة حتى يضاف الحساب النظيف الثاني عشر إلى القائمة المستقلة.

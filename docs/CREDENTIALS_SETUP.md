...
> `com.otlobli.app`، والأسرار الأربعة موجودة في Supabase. نجح فحص Apple
> من داخل Supabase، وقبل APNs رسالة حية موجهة للآيفون بنتيجة
> `sent=1/1`. يبقى تأكيد المالك لظهورها ومسار الضغط فقط.

- ✅ أنشئ مفتاح Apple Push Notifications service المخصص واحتُفظ بملف `.p8` خارج المشروع.
- ✅ ضُبط المسار المباشر المستخدم حالياً في `send-push`: `APNS_KEY` مخزّن
  كـBase64 بسطر واحد ويفكه الخادم إلى PEM دون تعريض الأسطر للتشويه، ثم
...
- ✅ قبل APNs الاختبار الموجه لهاتف المالك بنتيجة `sent=1/1`؛ بقي تأكيد الظهور ومسار الضغط على الجهاز.

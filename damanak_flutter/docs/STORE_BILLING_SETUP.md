...
إصدار المصدر الحالي: `4.5.0+22` — معرّف التطبيق: `com.damanak.damanak`

حالة المتاجر قبل Build `22`: iOS Build `20` مرفوع و`VALID` داخل مجموعة TestFlight الداخلية، وAndroid `versionCode 20` منشور بحالة `completed` على مسار الاختبار الداخلي. Build `22` هو الإصدار التالي بعد نشر الخادم والذكاء الاصطناعي والتكاملات. هذا يثبت التسليم فقط؛ يلزم اختبار شراء واستعادة فعليان على الجهاز.
...
تطبيق Google Play ظاهر في الحساب باسم `relzeo` وحزمة `com.damanak.damanak` ورقمه الداخلي `4973700586650447558`. أول إصدار داخلي `4.1.0 (7)` مرفوع، وBuild `20` منشور على مسار الاختبار الداخلي. المنتجات المطلوبة هي:
...
أُضيف حساب الخدمة `damanak-play-verifier@damanak-production.iam.gserviceaccount.com` في Play Console بتاريخ 30 أغسطس 2026 لتطبيق ضمانك فقط، مع أذونات البيانات المالية والطلبات والاشتراكات وإدارة التوفر ومنتجات المتجر وإصدارات الاختبار، ومن دون نشر الإنتاج. فُعّلت `Google Play Android Developer API`، وحُفظ كائن الحساب في Supabase وGitHub كسِر:

```text
GOOGLE_PLAY_SERVICE_ACCOUNT_JSON
```

محاولة التفعيل [33322857483](https://github.com/m7madv/otlobli/actions/runs/33322857483) أُجريت مباشرة بعد منح الصلاحيات وما زالت أعادت `The caller does not have permission`. لوحة Play تعرض سبباً أعلى من صلاحيات حساب الخدمة: حساب المطور الشخصي رقم `7616754513484681287` محظور من النشر لأن Google لم تثبت الهوية ولم يُؤكد هاتف التواصل؛ طلب إعادة النظر في الهوية أُرسل بالفعل. لا يمكن تفعيل منتجات الاشتراك أو نشر التطبيق العام قبل إغلاق مشكلتي الهوية والهاتف من حساب المالك.
...
مشروع Supabase المستقل لضمانك مرتبط بالمرجع `exxayzlklvgeyqhvtzgi`. نُشرت جميع migrations حتى `20260831020000` بتاريخ 30 أغسطس 2026، كما أن دوال `verify-store-purchase` و`legal` و`warranty-card` و`import-products-ai` و`analyze-claim-ai` و`damanak-api` و`dispatch-webhooks` نشطة. لإعادة النشر في بيئة أخرى:
...
ملف `20260830130000_damanak_starter_trial_consistency.sql` يجعل تجربة المتجر الجديدة لمدة 14 يوماً على باقة `starter` بحد 100 ضمان وحسابين للفريق. نُشر واختُبر فعلياً بحساب جديد. أضيف بعده حارسان: إنشاء الفرع الرئيسي مؤجل إلى نهاية المعاملة كي يرى الاشتراك، وحذف المتجر المتسلسل لا ينشئ سجل تدقيق يتيم.

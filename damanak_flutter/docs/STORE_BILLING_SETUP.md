...
Buildات `24` حتى `29` موجودة في iPhone TestFlight، وأحدثها Build `4.5.1 (29)` بحالة `VALID` وداخل `Damanak Internal`. Build `23` بقي على Google Alpha. إصدار Apple `4.5.0` الذي كان يضم Build `23` والاشتراكات أصبح `READY_FOR_SALE`، ولا توجد الآن مسودة مراجعة نشطة. مصدر `4.5.1+29` يعيد تنظيم Flutter حول آلة حالة وسياسة انتقال مركزيتين ويبقي فحص ملكية Apple ومنع الخفض من داخل التطبيق. لا تنسب تغييرات Build `29` إلى بناء أقدم، ولا تنشئ إصدار App Store أو تغيّر Google ضمن اختبار TestFlight.
...
## اختبار القبول الإلزامي لـBuild 29 من TestFlight

Build `4.5.1 (29)` هو مرشح قبول iPhone للبنية الجديدة بعد رفعه إلى TestFlight وصيرورته `VALID`، من دون إنشاء أو تغيير إصدار App Store. Android Build `29` غير مرفوع؛ عند قرار إصداره استخدم مسار Play الاختباري ولا تعتمد على APK الجانبي وحده لقبول فوترة المتجر:

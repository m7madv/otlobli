...
Build `23` هو حزمة App Review وGoogle Alpha، وBuildات `24` و`25` و`26` موجودة في iPhone TestFlight فقط. Build `26` ينظم الاستعادة بعد حذف حساب ضمانك وحالات الفوترة العالقة، وv28 إصلاح خادمي يستفيد منه Build `26` فوراً. لا تنسب تغييراته إلى بناء أقدم، ولا تضفه إلى App Review أو Google من دون قرار منفصل.
...
10. **ناجح في الحالة المبلغ عنها:** اشتراك Sandbox على حساب ضمانك محذوف، ثم حساب/متجر نهائي واحد، وضغطة «استعادة المشتريات» واحدة بعد v28؛ أعاد التحقق HTTP `200` وظهرت «توسع» الشهرية فعالة مع entitlement وربط App Store بلا دفع مكرر.

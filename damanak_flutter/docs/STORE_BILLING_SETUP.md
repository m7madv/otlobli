...
- الوصول المجاني ليس تجربة ولا اشتراك متجر: 20 ضماناً تتجدد في كل شهر تقويمي، مع فرع وعضو واحدين، لحساب واحد على تثبيت محمي واحد. لا يعرض دورة فوترة أو سعراً أو مزوداً، ولا يدخل في كتالوج الشراء. إذا سبق استخدامه للحساب أو التثبيت، يُنشأ المتجر مقفلاً حتى شراء مدفوع متحقق أو استعادة استحقاق فعّال.
...
قبل كل شراء Apple، يستعلم Build `30` محلياً عن `Transaction.currentEntitlements` ويرجع فقط `productId` و`originalTransactionId` و`appAccountToken` إن وجد؛ لا يعيد JWS ولا يربط اشتراكاً. بلا اشتراك خادمي، وجود أي entitlement حالي لضمانك يمنع الشراء ويطلب استعادة صريحة. مع اشتراك خادمي، يجب أن يملك Apple ID الحالي سلسلة `originalTransactionId` نفسها والمنتج الحالي نفسه قبل فتح StoreKit. وإذا ظهرت الخطة المجانية مع lineage مدفوع مخفي، يجبر preflight تحديث المزود حتى لو كان آخر تحقق حديثاً؛ غياب الاستحقاق أو اختلاف الحساب/السلسلة/المنتج أو فشل الاستعلام يمنع الدفع، ولا يوجد recovery صامت.
...
9. حساب/تثبيت جديدان يحصلان على 20 ضماناً فقط، ويعاد حد الاستخدام مع الشهر التالي. حساب أو تثبيت سبق أن أخذ المجاني ينشئ المتجر مقفلاً ولا يستطيع إصدار ضمان قبل شراء أو استعادة صحيحة.

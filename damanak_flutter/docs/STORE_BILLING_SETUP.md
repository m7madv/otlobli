...
الإصدار المستهدف التالي: `4.3.0+17` — معرّف التطبيق: `com.damanak.damanak`
...
اكتملت الترجمة العربية، والتوافر في دول الخليج الست، وصور المراجعة، واتفاقية التطبيقات المدفوعة والحساب البنكي والضرائب وDSA. أسعار واجهة قطر الحالية هي: بداية `39.99/399.99 QAR`، نمو `79.99/799.99 QAR`، وتوسع `199.99/1999.99 QAR`. المجموعة والمنتجات الستة في حالة `Prepare for Submission`؛ هذه حالة مناسبة لاختبار Sandbox وTestFlight، أما أول اشتراك عام فيجب إرفاقه بإصدار تطبيق عند الإرسال إلى مراجعة Apple.

Build `15` هو آخر إصدار كان متاحاً في `TestFlight` قبل دفعة `4.3.0+17`. كان يجبر مكتبة الشراء كلها على `StoreKit 1` ولم يُرجع الكتالوج على جهاز الاختبار. الإصدار `17` يعيد `StoreKit 2` كمسار أساسي مع طلب `StoreKit 1` مستقل كمسار احتياطي محدود. لا يُعتبر نجاح الرفع أو ظهور المنتجات في `App Store Connect` بديلاً عن اختبار ظهور الأسعار والاستعادة على iPhone حقيقي.
...
الأسعار التي أعادها مسار الاختبار الداخلي قبل دفعة `4.3.0+17` كانت `33/330` و`84/835` و`170/1700 QAR`. الهدف الموحد الذي يطبقه سكربت الإعداد لهذه الدفعة هو: بداية `39.99/399.99 QAR`، نمو `79.99/799.99 QAR`، وتوسع `199.99/1999.99 QAR`. لا يوجد عرض تمهيدي مطلوب؛ التطبيق يختار العرض الأساسي المباشر الذي لا يحمل `offerId`. لا تعتبر الأسعار الجديدة مؤكدة حتى يُفحص تقرير `activate` ويُختبر Build `17` من مسار Google Play الداخلي.
...
الفحص للقراءة فقط. `--apply` ينشئ المنتجات والخطط المسعرة لدول `SA/AE/BH/KW/OM/QA` كمسودات ويحدّث الوصف العربي، و`--activate` ينشئ الناقص ثم يفعّل الخطط الأساسية. المنتجات الثلاثة وخططها الشهرية والسنوية مفعّلة حالياً، وأعاد تطبيق مثبت من مسار الاختبار الداخلي أسعار قطر بنجاح. يلزم إعادة اختبار الإصدار `17` بعد رفعه؛ التثبيت الجانبي لا يكفي لاختبار `Google Play Billing`.

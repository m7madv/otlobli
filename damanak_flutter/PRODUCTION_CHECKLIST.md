...
آخر تحديث: 2026-09-02 — المصدر المحلي `4.5.1+30`، وiPhone Buildات `24` حتى `30` في TestFlight، وأحدثها `4.5.1 (30)` بحالة `VALID`، وGoogle Alpha على Build `23`، وإصدار Apple `4.5.0` منشور
...
- [x] migrations حتى `20260901200000` منشورة على `exxayzlklvgeyqhvtzgi`، واختبارات SQL الحية السابقة واختبار الخطة المجانية الجديد نجحت داخل `BEGIN/ROLLBACK`.
- [x] إنشاء المتجر لا يعود يتوقف: الحساب والتثبيت المؤهلان يحصلان على 20 ضماناً شهرياً وفرع وعضو واحدين؛ وإذا سبق استخدام المجاني للحساب أو التثبيت يُنشأ المتجر مقفلاً حتى اشتراك أو استعادة متحققين.
...
- [x] نُشرت migration `20260901200000` و`verify-store-purchase` v29 و`refresh-store-entitlements` v8 بعد إتاحة Build `30`، وبقيت بيانات الإنتاج وGoogle وعدادات الحماية خارج نطاق التنظيف.
...
- [x] أُغلق Build `29` كمرشح قديم لصالح Build `30` من دون نسبة قبول جهاز أو شراء حقيقي إليه.
...
- [x] رُفع Build `30` في [33559362847](https://github.com/m7madv/otlobli/actions/runs/33559362847)، وأكد inspect [33560754821](https://github.com/m7madv/otlobli/actions/runs/33560754821) أنه `VALID` وداخل `Damanak Internal` بالمعرّف `47da8d54-cadc-4f96-82cb-9e81a01c3664`، مع بقاء `4.5.0` `READY_FOR_SALE` و`mutationRequested=false`.
- [x] نُشرت migration و`verify-store-purchase` v29 و`refresh-store-entitlements` v8، وبقيت بصمة عدادات الشراء الأربعة قبل/بعد مطابقة. نجح `free_access_billing_repair_live.sql` بعد النشر، وحُدث `supabase/schema.sql` إلى `475,132` بايت وبصمة `97A391B3FC3F5119940233960DBDF54E62DE977D1D58DBBACFE742E80FD7EB12`.
...
- [ ] تثبيت Build `30` من TestFlight على iPhone حقيقي والتأكد أن الباقة الحالية والأدنى تبقيان معلوماتيتين ولا تفتحان الدفع.
- [ ] اختبار حساب/تثبيت جديدين: يحصلان على 20 ضماناً للشهر، ثم اختبار حساب أو تثبيت سبق أن أخذ المجاني: ينشئ المتجر مقفلاً حتى شراء أو استعادة متحققة.
...
- [ ] شراء Sandbox/اختبار حقيقي واستعادة وتجديد وترقية وتبديل دورة على iPhone، والتأكد أن الباقة الأدنى لا تفتح الدفع داخل Build `30`.

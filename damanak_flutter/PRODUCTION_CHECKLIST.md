...
- [x] رُفع رقم المصدر إلى `4.5.0+26`، ونجح `flutter analyze` و`181/181` اختبار Flutter و`76/76` اختبار Deno واختبار migration `1/1` وفحص تنسيق 27 ملفاً و`git diff --check`.
...
- [ ] بعد نشر `verify-store-purchase` v27: إعادة الحالة المبلغ عنها بحذف حساب ضمانك المرتبط باشتراك Sandbox، وإنشاء حساب/متجر جديدين، ثم استعادة واحدة تنقل الاستحقاق للمتجر الوحيد وتسمح بالترقية إلى «توسع» بلا دفع مكرر.

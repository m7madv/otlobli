...
- [x] تثبيت Build `26` من TestFlight ونجاح شراء Apple حقيقي قبل حذف حساب ضمانك؛ حذف الحساب لا يلغي الاشتراك ولا يثبت الاستعادة اللاحقة.
- [ ] اختبار حساب/تثبيت غير مؤهل للتجربة: ينشئ المتجر بحصة صفر، ويبقى مقفلاً حتى شراء أو استعادة متحققة.
- [ ] بعد نشر `verify-store-purchase` v28: من حساب/متجر نهائي واحد، اضغط «استعادة المشتريات» مرة واحدة فقط؛ لا تسجل النجاح قبل HTTP `200` وظهور `source=store/provider=app_store` وentitlement إنتاجي، ثم تحقق من إمكان الترقية إلى «توسع» بلا دفع مكرر.

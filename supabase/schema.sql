...
create table if not exists public.cart_group_members (
  ...
  customer_id uuid not null references public.customers(id) on delete cascade,
  member_key text not null default gen_random_uuid()::text,
  phone text not null,
...
alter table public.cart_group_members add column if not exists member_key text not null default gen_random_uuid()::text;
...
on public.cart_group_members (group_id)
where role = 'member';
alter table public.cart_group_members alter column member_key set default gen_random_uuid()::text;
...
as $$
begin
  raise exception 'retired_group_rpc';
end;
$$;

revoke all on function public.replace_cart_group_items(uuid, uuid, jsonb) from public, anon, authenticated, service_role;
...
revoke all on function public.cart_group_snapshot(uuid) from public, anon, authenticated, service_role;
...
revoke all on function public.create_cart_group(text, text, text, jsonb) from public, anon, authenticated, service_role;
...
revoke all on function public.join_cart_group(text, text, text, jsonb) from public, anon, authenticated, service_role;
...
revoke all on function public.sync_cart_group_items(text, uuid, jsonb) from public, anon, authenticated, service_role;
...
create table if not exists public.app_issue_reports (
  ...
  report_kind text not null default 'visual' check (report_kind in ('visual', 'diagnostic')),

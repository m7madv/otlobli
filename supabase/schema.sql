...
-- ============================================================================
-- أكواد الخصم — يديرها المشرف بإدراج صفوف في public.coupons مباشرةً (بلا لوحة
-- إدارة مطلوبة). خصائص كل كود:
--   kind='percent' + value=10   → خصم 10% من مجموع الطلب
--   kind='fixed'   + value=5    → خصم 5$ (يُحوَّل لليرة بسعر الصرف الرسمي)
--   applies_to='all'|'shein'|'temu' → على أي متجر أو متجر محدّد
--   max_uses (إجمالي) ، min_subtotal_syp ، starts_at ، expires_at (كلها اختيارية)
-- الاستخدام مرّة واحدة لكل رقم هاتف ولكل جهاز (منع الاحتيال) — مفروض ذرّياً
-- بفهرسين فريدين، فلا يُعاد استخدامه بتغيير الحساب ولا بتغيير الجهاز على نفس
-- الرقم. مثال إنشاء كود:
--   insert into public.coupons (code, kind, value, applies_to) values ('WELCOME10','percent',10,'all');
-- ============================================================================
...
-- استخدام واحد لكل هاتف ولكل جهاز لكل كود
...
create unique index if not exists coupon_redemptions_device_uidx
  on public.coupon_redemptions (coupon_id, device_id) where device_id <> '';

alter table public.coupons enable row level security;
alter table public.coupon_redemptions enable row level security;
-- بلا policies عامة — الوصول فقط عبر redeem_coupon (security definer)

-- يتحقق من الكود ويحسب الخصم ويستهلكه ذرّياً في عملية واحدة. يُعيد
-- {valid, discountSyp, code, reason}. reason عند الرفض: not_found/inactive/
-- expired/not_started/wrong_store/below_min/exhausted/already_used/no_phone.
...
begin
  ...
  if c.applies_to <> 'all' and c.applies_to <> lower(coalesce(p_store, '')) then
    return jsonb_build_object('valid', false, 'reason', 'wrong_store');
  end if;
  if coalesce(p_subtotal_syp, 0) < c.min_subtotal_syp then
  ...
  if c.kind = 'percent' then
    discount_syp := floor(coalesce(p_subtotal_syp, 0) * least(greatest(c.value, 0), 100) / 100.0);
  else
    select value::numeric into rate from public.app_settings where key = 'usd_to_syp_rate';
    rate := coalesce(p_usd_rate, rate, 13000);
    discount_syp := floor(c.value * rate);
  end if;
  if discount_syp < 0 then discount_syp := 0; end if;
  if discount_syp > coalesce(p_subtotal_syp, 0) then discount_syp := coalesce(p_subtotal_syp, 0); end if;

  begin
    insert into public.coupon_redemptions (coupon_id, phone, device_id, discount_syp)
    values (c.id, trim(p_phone), coalesce(nullif(trim(p_device_id), ''), ''), discount_syp);
  exception when unique_violation then
  ...
  return jsonb_build_object('valid', true, 'discountSyp', discount_syp, 'code', c.code, 'kind', c.kind);
end;
$$;

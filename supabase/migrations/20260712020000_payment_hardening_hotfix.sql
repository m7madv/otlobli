-- Forward hotfix for environments that already recorded the original
-- 20260711 payment-hardening migration before its final definitions existed.
-- This file is intentionally self-contained and idempotent so every deployed
-- database receives the same secured RPC surface and atomic payment flow.
...
begin
  ...
  if found_event.status in ('matched', 'unmatched', 'ambiguous', 'rejected', 'duplicate') then
    return coalesce(found_event.result, '{}'::jsonb);
  end if;
  if found_event.status not in ('received', 'error') then

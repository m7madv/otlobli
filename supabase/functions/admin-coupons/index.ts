...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    return json({ coupons: (data || []).map((row) => mapCoupon(row as Record<string, unknown>)) })
  }
  ...
  if (req.method === 'POST') {
    ...
    if (error) {
      ...
      return json({ error: duplicate ? 'code_exists' : error.message }, duplicate ? 409 : 500)
    }
    return json({ coupon: mapCoupon(data as Record<string, unknown>) })
  }

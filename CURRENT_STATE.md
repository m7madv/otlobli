...
## v86.41 SHEIN shipping-add guard and drawer reopen (2026-08-01)

- Current marker is `2026.08.01-v86.41-shein-shipping-add-guard-drawer-reopen`; Android/iOS are `901/86.41`. The user confirmed v86.40 now captures color and size correctly; that compound capture and all price logic are intentionally preserved.
- New real-device boundary: tapping Otlobli add while SHEIN's shipping/location drawer is open could continue through product capture and mix unrelated visible UI into the product. `sheinOpenSkuDrawer()` now forces a fresh strict shipping-root check on the user tap, clears the path SKU memo/drawer flag, shows `أغلق قائمة الشحن أولاً`, and returns handled without opening options or posting an add.
- The remaining first-tap failure had a separate deterministic cause: SHEIN keeps its closed `.SIZE_ITEM_HOOK` option groups rendered but translated outside the viewport. v86.40 treated positive dimensions as visible, so it believed the drawer was still open and skipped `انقر للشراء`. Compound drawer groups must now intersect the viewport; an offscreen retained drawer is ignored. The exact `.goods-detail__top-other` row itself is clicked because SHEIN can delegate the opener on the row rather than the inner value text.
- Device-shaped full-script Playwright passes: shipping drawer gives `opens=0/added=false`; after closing it, first tap gives `opens=1/added=false`; an offscreen retained options drawer reopens with `opens=2/added=false`; selecting `بيج + كبير` sends `صينية من الخشب الصلب | بيج / كبير | $14.66`. v86.40/v86.39/v86.38 regressions still pass, including incomplete-option blocking, `أزرق/L`, `رمادي / كبير`, `L`, and `M / 1PC`.
- Freeze guard, production/performance build, Android/iOS sync, Gradle debug, APK metadata, GitHub/Xcode build, and IPA inspection pass. Budgets: JS raw `1,199,514/1,200,000`, JS gzip `358,266/370,000`, CSS `63,029/70,000`, fonts `81,364/100,000`, SHEIN source `549,675/550,000`.
- Android APK: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.41-shein-shipping-add-guard-drawer-reopen-debug.apk`; SHA-256 `B973FEB36D1E9B93FBF27DAE42D8CA19DC3E1CEFDBAE0DD3E10E75916766583F`; size `11,124,934` bytes; metadata confirms `com.otlobli.app`, `86.41/901`, release marker, shipping guard, and compound-size marker.
- Code commit is `760d62f` on `claude/ios6-cover-fix`; GitHub/Xcode run `30698158082` passed. Unsigned IPA: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.41-iphone16-unsigned.ipa`; SHA-256 `1DEE3626DF5CFA575310FE8877975EE39A6BEC8AB805228031884F9143D75379`; size `7,069,211` bytes. Inspection confirms `com.otlobli.app`, `86.41/901`, iOS 15 minimum, only `otlobli`, shipping/drawer/compound-size and all native-recompose markers, with no app-root signature or provisioning profile.
- Real-iPhone acceptance is not yet performed for v86.41. Test add while the shipping drawer is open (must not add), close it and tap add (must open product options), close/reopen options, change both groups, and inspect the cart. Then run five background/resume cycles and a cold launch. The separate automatic store-region switching defect remains open.

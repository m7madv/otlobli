...
## مشروع ضمانك المستقل — Build 25 على TestFlight وBuild 23 قيد المراجعة (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+25`. Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير، وBuild `24` بقي داخل TestFlight فقط. لا تنسب رقعة Build `25` إلى أي حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
- أصبح إنشاء المتجر متاحاً حتى لو استُهلكت التجربة سابقاً: Build `25` يستدعي `create_store_with_subscription`. المؤهل يحصل على تجربة Starter مرة واحدة، وغير المؤهل يحصل على متجر وصف اشتراك غير فعّال بلا فرع أو حصة أو مزايا مجانية، ثم يرى بوابة الاشتراك المدفوع أو الاستعادة فقط. RPC القديمة `create_store_with_trial` لم تتغير، لذلك Build `24` لا يدخل حالة لا يفهمها.
- أول شراء أو استعادة لا يفتح واجهة التشغيل بمجرد نجاح نافذة المتجر؛ يلزم تحقق الخادم، إكمال معاملة المتجر، إعادة تحميل مساحة العمل، ووجود فرع `MAIN` فعّال. صفحة الانتظار تعرض «غير مشترك» وحصة صفر، وتسمح بإدارة الهوية وتسجيل الخروج أو حذف الحساب فقط.
- نُشرت migrations حتى `20260901011000` على Supabase الصحيح `exxayzlklvgeyqhvtzgi`. migration `20260901010000` تضيف الإنشاء المقفول، التهيئة المؤجلة للفرع، وحواجز الدعوات والمطالبات وروابط الضمان للمتجر الذي لم يُفعّل قط. migration `20260901011000` تجعل RPC الجديدة متاحة لـ`authenticated` فقط. دوال Edge المنشورة لم تتغير عن `verify-store-purchase` v23 و`refresh-store-entitlements` v7 وبقية الإصدارات الموثقة سابقاً.
- نجحت اختبارات الإنتاج `store_creation_without_repeat_trial_live.sql` و`subscription_lifecycle_live.sql` و`account_deletion_successor_live.sql` و`warranty_tenant_integrity_live.sql` داخل `BEGIN/ROLLBACK`. أثبت الاختبار الجديد توافق Build `24`، عدم إنشاء متجر مكرر، غياب الحصة والفرع قبل الدفع، رفض عمليات التشغيل، وإنشاء `MAIN` مرة واحدة بعد أول تفعيل صالح.
- ملف `supabase/schema.sql` متسق مع الإنتاج، حجمه `391,841` بايت وبصمته `6CE83AD3F374C42C196AB69AA7E117D8A0C3D2D0EA9447E5C64A1D3FE8C33959`. تعذر إنشاء dump جديد لأن Docker غير متوفر، لذلك أُعيدت لقطة Build `24` النظيفة ثم أُلحقت بها تعريفات الهجرتين المنشورتين القابلة لإعادة التشغيل، وسُجل هذا القيد بدلاً من الادعاء بأنها dump جديدة.
- نجح `flutter analyze` و`160/160` اختبار Flutter و`56/56` اختبار Deno، وفحص تنسيق 23 ملفاً و`git diff --check` وبناء web release وAndroid debug. APK Build `25` المحلي `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `180,814,040` بايت وبصمته `C3070BABAA8F452C23B6BBF5D71881C4FD1BF8BDCFAC44A64B39CA723EF1B775`. لم يُرفع Build `25` إلى Google.
- راجعت واجهة بوابة أول دفع وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: RTL وتسلسل بصري محايد وإجراء واضح وتكبير `200%` ناجحة آلياً. لم يُنفذ قبول VoiceOver أو جهاز iPhone حقيقي، فلا يُدعى نجاحهما.
- بقي التدقيق الأمني الأصلي 5 نتائج، بلا عالية أو حرجة. تضييق إعادة التجربة على الحساب/التثبيت تحسن، لكنه لا يغلق نتيجة إثبات الجهاز؛ ما يزال App Attest/DeviceCheck وPlay Integrity وقرار الويب مطلوباً. كذلك لم تُحسم مدة روابط الضمان وسياسة v1 والإبطال، ولم تُكسر الروابط التاريخية.
- App Store Connect: submission `69104e50-7a9a-4102-a717-ca341d442897` ما يزال يضم Build `23` ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، وحالته `Waiting for Review`. لا تلغِه أو تستبدل بناءه أثناء المراجعة. شغّل commit `163d34562697256bd85cdc1a6bf1651e9cf9b547` سير العمل [33449769807](https://github.com/m7madv/otlobli/actions/runs/33449769807)، فنجح التوقيع والتحقق والرفع لـBuild `25`. أنهت Apple المعالجة؛ Build `4.5.0 (25)` معرّفه `8e864062-28e0-4250-abd2-dac25f8df1b9` وحالته `VALID` وداخل `Damanak Internal` ذات المختبر الواحد. IPA حجمها `23,930,380` بايت وبصمتها `D2DAF9EF0A97B3C8A6328C32D1B71276352DEAA29A97E144653978983F13A14F`. لم يُضف Build `25` إلى App Review، وبقي Build `24` TestFlight بالمعرّف `6b986de0-74bb-4649-8619-846aca1eb1b0`.
...
- لم يُنفذ شراء أو استعادة حقيقيان على iPhone أو Android، ولم يُثبت Build `25` على جهاز حقيقي، ولم تُنفذ اختبارات VoiceOver/TalkBack أو جهاز ضعيف. لا تستنتج قبولها من البناء أو الاختبارات الآلية.

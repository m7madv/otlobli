...
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` توجه كل مداخل إنشاء المتجر القديمة والجديدة إلى القاعدة نفسها، وتبقي مرآة توافق فقط لـBuildات `23–29` بينما تكون الصلاحية الفعلية في جداول `private`. تنظف تجارب legacy وحالات Apple Sandbox فقط إلى tombstones، وتبقي إنتاج Apple وGoogle وسجلات التدقيق وعدادات الحماية بلا مساس. لم تُنشر بعد؛ يجب إتاحة Build `30` في TestFlight أولاً ثم نشرها مع `verify-store-purchase` والتحقق من بصمة الإنتاج قبل/بعد.

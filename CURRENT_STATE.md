...
إضافة البريد إلى القائمة وحدها لا تبدأ المدة. بقي الإصدار `draft` والمسار غير
نشط حتى توفير الحساب الثاني عشر وتحويل نفس الإصدار إلى `completed`. بطلب صريح
...
`01 - <email>` حتى `12 - <email>`؛ المثيل ذو الحسابين يذكرهما معًا، والمثيلات
الجديدة `10..12` تحمل `EMPTY - ADD GOOGLE ACCOUNT` ولا تدعي وجود حساب. ضُبطت
الجديدة على `900x1600@320` و4 CPU و4096MB.

الجرد الطرفي أثبت أن كل حسابات Google المسجلة في المثيلات `02..09` مستخدمة
أصلًا في قائمة Play. ظهر حساب عادل والحساب الجديد معًا في المثيل `01`، ثم حذف
المالك الحساب الجديد يدويًا وأكد بصريًا أن عادل هو الحساب الوحيد الظاهر. فحص
AccountManager أثناء واجهة مصادقة Google لم يكن مصدرًا موثوقًا لهذه الحالة؛
لذلك لا تدّع تحقق ADB للحساب واعتمد تأكيد المالك المسجل. ثُبت الاسم النهائي
`01 - adelalzoubi2022@gmail.com`، وأُغلق المثيل وأعيد `adbDebug` إلى `0` في كل
مثيلات LDPlayer. عاد `10` إلى `10 - EMPTY - ADD GOOGLE ACCOUNT` وهو مغلق.
أكد المالك أنه سجّل `djjd19903@gmail.com` مستقلًا في المثيل `11`. ثُبت اسمه
`11 - djjd19903@gmail.com` ثم أُغلق. المثيل `02` هو المفتوح الوحيد الآن على
تطبيق الإعدادات، وما زال اسمه الانتقالي يذكر `chatxhxh@gmail.com` و
`djjd19903@gmail.com` إلى أن يحذف المالك `djjd19903@gmail.com` منه يدويًا
ويبقي `chatxhxh@gmail.com` فقط. بعد تأكيد الحذف يجب تثبيت اسم `02` على
`chatxhxh@gmail.com` وحده ثم إغلاقه. بقي `10` و`12` فارغين، وكل إعدادات ADB عند `0`،
وبقي AVD Android Studio المنفصل دون تغيير.

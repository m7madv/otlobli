...
This is intentionally not a rebuild/reload workaround. The exact v86.193 lifecycle, one WKWebView, `WKWebsiteDataStore.default()`, cookies, cache, IndexedDB, Service Workers, region/session rules, VPN behavior, scripts, product capture, navigation, and text-selection behavior are unchanged. Version/build is `86.197/1059`; marker is `2026.08.20-v86.197-inactive-scheduling-throttle`. Physical iPhone acceptance is pending and must cover first entry, five background/resume cycles with a list and PDP tap after every return, one App Switcher force-quit/cold launch, and SHEIN → Temu → SHEIN. Do not call the root cause device-proven until those pass.

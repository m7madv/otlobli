...
- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.1+29`. Build `4.5.1 (29)` هو الأحدث في iPhone TestFlight وحالته `VALID` وداخل `Damanak Internal`. إصدار Apple العام `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة، بينما بقي Google Alpha على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً ضمن اختبار Build `29`.
...
- التحقق النهائي المحلي للمصدر `4.5.1+29`: `flutter analyze --no-pub` نظيف، وFlutter `211/211`، واختبارات الاشتراك المركزة السابقة `69/69` بمراجعة خصومية مستقلة، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ أكد `aapt` أن APK يحمل `versionName=4.5.1` و`versionCode=29`. حجمه `180,868,636` بايت وSHA-256 `C00835D114330136AC2A7A2411C2283C8A564EFC3AAF0DD08388081011E115C1`.

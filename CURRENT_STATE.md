# الحالة الحالية — إرسال Android 1113 إلى مراجعة Google (2026-09-08)

بعد تأكيد المالك نجاح ربط WhatsApp، طلب صراحة إرسال النسخة الموجودة للمراجع.
اكتمل الإرسال من Play Console نحو `00:41` بتوقيت الرياض في 8 سبتمبر
(`2026-09-07 21:41 UTC`). صفحة النشر تعرض **«التغييرات قيد المراجعة»**
وتدرج `Otlobli 86.244 (1113)` على `الاختبار المغلق - Alpha` مع بدء الطرح
الكامل داخل المسار المغلق فقط، والسعودية، وقائمة `All Apps - Closed Testers`.
أُرسل 14 تغييرًا؛ يدرج قسم التغييرات المأخوذة بعين الاعتبار تصحيح بيان
الميزات المالية وبيانات الوصول للمراجع والإقرارات القائمة.

قبل الإرسال أُكملت المسودة الموجودة `/releases/2` باستخدام حزمة `1113`
المرفوعة سابقًا، مع استبعاد `1112` من الإصدار الجديد. أضيفت ملاحظات عربية
عن تحديث الاختبار المغلق وإعادة تفعيل رمز WhatsApp وصفحات الخصوصية والدعم.
التحذير الوحيد هو غياب native debug symbols، وهو غير حاجب. لا رفع AAB أو
بناء أو تغيير رقم جديد. حُفظ الإصدار، ثم ضُغط إرسال 14 تغييرًا وأُكد الحوار.
تظهر الفحوصات الآلية الأولية جارية، مع إشعار بأنها تسبق إحالة التغييرات
للمراجعة فور نجاحها؛ لا يُدعى اكتمال الفحص أو قبول التطبيق أو بدء الاختبار.

لا Production ولا تغيير للمختبرين/المحاكيات أو كود التطبيق/الخادم.
Apple `86.244 (1112)` لم تُمس، ورفض التفويض `5.2.2` لم يُحل بهذا الإرسال.
حوّل سجل الرفع `CLOSED_TEST_AUTHORIZED.json` إلى `operation=inspect`
لمنع إعادة نشر مسودة بالخطأ؛ لا تشغّل الرفع مجددًا ولا تُزل التغييرات الحالية.
المتبقي انتظار نتيجة Google، ثم انضمام المختبرين والتجربة الفعلية بعد إتاحة
المسار؛ عداد 14 يومًا لا يُعد بدأ بمجرد إرسال المراجعة.

## تحديث سابق — ربط OTP بخدمة MZ3B من الخادم فقط (2026-09-07)

أكد المالك حذف مرسل WhatsApp القديم عمدًا، واختار `https://mz3b.com` وقدّم
مفتاح مشروع «اطلبلي». ليس عطل جلسة مطلوبًا إيقاظها. فحص ملخص المشروع نجح
بـ`ready=true` دون إرسال أو خصم. لم نستخدم حساب `Mela Wood` الذي كان مفتوحًا
في المتصفح، ولم نغيّر أي رقم أو مشروع داخل خدمة MZ3B.

نُشر الربط على خدمة Otlobli القائمة `otlobli-wa` فقط، مع حفظ عنوان التطبيق
`https://84-8-100-128.sslip.io` ومساري `/api/auth/whatsapp/start` و`verify`.
حُفظ المفتاح في `.env` الخادم بصلاحيات `600`، لا في المستودع أو العميل.
فحص HTTPS بعد إعادة التشغيل نجح: `whatsappProvider=mz3b` و
`whatsappConnected=true/whatsappSenderReady=true/sessionStoreReady=true`،
و`authContract=customer-session-v1/otpSecurityReady=true`. العملية online
بنمط fork واحد، ولا تُحمّل جلسات Baileys القديمة أو تُستعاد أرشيفاتها.

التغيير محوّل خادمي يحفظ معرّف محاولة المزوّد ومنع التكرار وميزانيات الإرسال
والتحقق على القرص الخاص؛ لا يحفظ أرقامًا خامًا أو رموز OTP أو جلسات العميل.
لا يصدر جلسة إلا بعد `approved` للمحاولة نفسها، ويمنع إعادة استعمال الرمز.
لم يتغير مخطط Supabase أو RPC الجلسات، ولا الدفع أو المحفظة أو المتاجر.
نجحت اختبارات المحوّل الوهمية وإعادة التشغيل والتزامن وحدود المحاولات،
واختبار HTTP محلي للمسارين مع RPC وهمي، وحواجز المصادقة وإدارة WhatsApp
وخدمات الإصدار والأسرار وSHEIN. نجح رفض مدخلات غير صالحة حيًا دون أي إرسال.

لم ترسل الاختبارات الآلية OTP حقيقيًا ولم تنشئ حسابًا أو جلسة عميل إنتاجية.
بعد طلب اختبار الرمز والدخول من النسخة الموجودة، أكد المالك نجاح الربط
بقوله «شتغل». هذا قبول من إفادة المالك، لا اختبار جهاز نفذه الوكيل؛ لم
يحدد المالك نوع الهاتف أو نظامه أو رقم البناء الذي اختبره. لا يُعمم ذلك
كقبول مستقل لكل منصة. هذه الدفعة لرموز الدخول/توثيق الهاتف، وليست لتنبيهات
WhatsApp القديمة؛ الأخيرة تحتاج ربطًا مستقلاً وموافقة مستلمين محددين.

يبقى Android `86.244 (1113)` مسودة `alpha`، وiOS `86.244 (1112)` دون تغيير.
لا بناء/مزامنة/رفع جديد ولا إرسال مراجعة أو Production في هذه الدفعة.
النسخة الاحتياطية الخاصة قبل النشر:
`/home/ubuntu/otlobli-server/.mz3b-backup-XjWXmE`؛ لا تحتوي المستندات على
المفتاح. التفاصيل وحدود تشغيل العامل الواحد في `docs/MZ3B_OTP_INTEGRATION.md`.

## تحديث سابق اليوم — تصحيح إقرار Google وبناء Android 1113

هذا القسم أحدث من السجل التاريخي أدناه. المجلد والفرع لم يتغيرا.
رفض Google البناء `86.244 (1112)` في 1 سبتمبر بسبب متطلبات حساب المؤسسة.
رد الدعم Albert في 5 سبتمبر على الطلب `4-1130000041592` بأن إقرار الميزات
المالية قد يسبب الشرط، وطلب تدقيقه ثم زيادة رقم البناء وإعادة التقديم؛ الرد
مشروط وليس موافقة على التطبيق أو إعفاءً صريحًا.

فوّض المالك في 7 سبتمبر تدقيق الإقرار وتصحيحه وبناء Android جديد وإرساله إلى
الاختبار المغلق فقط. تدقيق `src/App.tsx` و`src/services/supabaseAppApi.ts`
ومخطط/ترحيلات الدفع يبين رصيد شراء داخليًا يُشحن بدفع خارجي عبر ShamCash
ويُحجز/يُخصم لطلبات Otlobli. لم يُعثر على وظيفة تحويل بين العملاء أو سحب نقدي
في المصدر المفحوص؛ هذا ليس تدقيقًا حيًا لكل دوال قاعدة الإنتاج. حُفظ في Play
Console اختيار «لا يوفّر تطبيقي أي ميزات مالية» بدل تصنيف المحافظ الرقمية،
بناءً على نطاق رصيد الشراء الداخلي، مع إبقاء كل الوظائف دون إخفاء أو تعديل.
ظهرت رسالة نجاح الحفظ وعدم الحاجة إلى مستندات مالية إضافية حاليًا. لم يُرسل
التصحيح للمراجعة بعد؛ حُفظ البناء الجديد كمسودة `alpha` بنجاح.

Android صار `86.244 (1113)` بتغيير `versionCode` وحده. نجحت اختبارات الإصدار
والمصادقة والأمان وSHEIN/Temu وبناء الويب وميزانيات الأداء بالسقوف السابقة.
نجحت مزامنة Android والبناء الموقّع `bundleRelease/assembleRelease` مع R8.
تحققت هوية الحزمة و`86.244 (1113)` والتوقيع v2/v3 بنفس شهادة البناء 1112
ذات SHA-256 `E0B0F44CC677888F9535C01C9125077E09B014BDB9096DC2813E3BD06F17F784`.
نجح `jarsigner` مع التحذيرات المعتادة للشهادة الذاتية وترتيب manifest في AAB.
الأصول تحت `artifacts/release-86.244/`:

- AAB `Otlobli-86.244-1113-release.aab`: `5,775,711` بايت، SHA-256
  `5E27349B35D66C0F627DCD888FAFABD3520ADA4A578DD4721800E698BA5A0352`.
- APK `Otlobli-86.244-1113-release.apk`: `4,114,340` بايت، SHA-256
  `9A6067409F91BF12F7044340ECCC8D7451FC56D0936FCA30DAD995F6879C2636`.

مقارنة 25 أصلًا داخل AAB السابق تؤكد تطابق JavaScript/CSS التطبيق وملفات
المتاجر؛ الاختلاف في صفحات `privacy.html/support.html/legal.css` المتعقبة
سابقًا لإضافة طلب حذف الحساب. لا تعديل مصدر لهذه الصفحات في الدفعة الحالية.
تفويض الرفع وسكربته مربوطان الآن بـ1113 والبصمة الجديدة و`alpha` كـ`draft`.
المحاولة `34140843006` من `a80061c` رفعت الملف داخل edit ثم رفض Google
commit مطالبًا بـ`changesNotSentForReview=true`؛ حذف السكربت المعاملة غير
الملتزمة. لم يُحفظ إصدار جديد أو تُرسل مراجعة. أضيف العلم إلى commit صراحة
ليحفظ المسودة فقط ويترك الإرسال النهائي للواجهة. نجحت إعادة المحاولة
`34141060561` من `68efa11`: رفعت AAB نفسه وتحقق commit بنجاح، وأكد السجل
`Saved 86.244 (1113) ... alpha; status=draft; review not submitted`.
Production بقي بلا إصدارات، ولا تغيير لقائمة المختبرين. AAB متعقب ومدفوع
مع المصدر؛ APK محلي حسب `.gitignore`. لا تُعد تشغيل الرفع أو تُنشئ بناءً
آخر؛ المتبقي مراجعة المسودة الحالية وحفظ/إرسال تغييرات التطبيق من الواجهة.
لا تتجاوز المسودة إلى مراجعة قبل معالجة حاجزي المتابعة:
انقطع اتصال متصفح Edge بعد حفظ الإقرار، وفحص `/health` أعاد HTTP 200 لكن
`whatsappConnected=false/whatsappSenderReady=false`. فحص Google بالمُعرّف
الفعلي الافتراضي نجح HTTP 200 و`configured=true`. محاولتا فحص Google/Apple
الأوليان أعادتا 400 لأن متغيراتهما غير موجودة في `.env`، لا دليل فشل Google؛
المصدر يستخدم مُعرّف Google الافتراضي، ودخول Apple على Android غير مضبوط
في هذا البناء كما في أصول 1112. لا تضف إعدادات أو تغيّر الجلسة ضمن هذا الطلب.
لا جهاز Android متصل وفق ADB. لم يُبنَ iOS لأن التغيير الجديد Android فقط.
لا تغيير في المنطق أو الاعتمادات أو الخادم أو WhatsApp أو المحاكيات أو المختبرين.
لم يُجرَ اختبار جهاز فعلي جديد. Production غير نشط وعدد المنضمين الحي 0.

Apple يبقى `86.244 (1112)`؛ آخر حالة معروفة في المحادثة رفض `5.2.2` لغياب
تفويض SHEIN/Temu، وليست حالة الانتظار التاريخية أدناه. فحص بريد 5 سبتمبر
لم يجد تفويضًا: رد SHEIN آلي فقط ولا رد Temu. لم تُفحص Apple أو تُغيّر في
هذه الدفعة، ولم يُبنَ أو يُرفع IPA جديد.

## سجل 2026-08-30 التاريخي — ليس الحالة الحالية

# v86.244/1112 — أُرسل إلى App Review وحالته WAITING_FOR_REVIEW (2026-08-30)
...
الموجود إلى App Review، ونفذ الإرسال لاحقًا بنجاح بحالة `WAITING_FOR_REVIEW`.
...
نجح التشغيل النهائي `33307366619` من الالتزام `8f53ba0`: تحقق من البناء
`86.244 (1112)` والصور الست، وأزال عنصر المراجعة القديم المرفوض
`890129230` من الطلب `e5e27b8a-b628-4116-b135-361b91266929` من دون حذف أي
إصدار أو بناء، ثم أضاف `86.244` إلى الطلب
`2ae4119e-bfbf-4a1a-a813-ddda2bf7e460` وأرسله. حالة Apple المؤكدة هي
`WAITING_FOR_REVIEW`. لم يُبنَ أو يُرفع IPA/TestFlight جديد، ولم يُفحص أو
يُغيّر Oracle أو WhatsApp في هذا التشغيل. التطبيق لم يصبح عامًا بعد؛ ينتظر
قرار Apple.

## Google Play والاختبار المغلق — أُرسل إلى المراجعة (2026-08-30)

حُفظت قائمة المختبرين المشتركة باسم `All Apps - Closed Testers` لتكون قابلة
لإعادة الاستخدام مع تطبيقات الحساب، وتأكدت Play Console من وجود 12 حسابًا
بالضبط فيها، ومنها `bbhzhxjd@gmail.com`. بقيت القائمة القديمة
`Closed Testers 12` خارج النطاق لأنها تتضمن الحسابات الثلاثة التي استبعدها
المالك. تمت معاينة مسودة `alpha` نفسها وحفظها من Play Console، ثم أُرسلت
14 تغييرًا إلى Google. تعرض صفحة النشر الآن `التغييرات قيد المراجعة` وتشمل
`Otlobli 86.244 (1112)` مع `بدء الطرح الكامل` لمسار الاختبار المغلق، ولا
تشمل أي إصدار Production. لا يُدّعى أن المسار أصبح نشطًا أو أن عداد 14 يومًا
بدأ قبل موافقة Google وظهور رابط الانضمام الفعلي.

أكمل Google فحص المتطلبات بعد إضافة بيان المعرّف الإعلاني بإجابة `لا`.
الدليل هو merged manifest الفعلي للبناء `86.244 (1112)`، الذي لا يحتوي
`com.google.android.gms.permission.AD_ID`، وعدم وجود SDK إعلاني في اعتمادات
Android/npm. تحذير غياب native debug symbols غير حاجب ولا يغيّر الحزمة.

فوّض المالك رفع AAB نفسه `86.244 (1112)` إلى الاختبار المغلق ثم تشغيل حسابات
الاختبار من محاكيات LDPlayer بالتتابع عبر الطرفية فقط. ملف AAB متعقب داخل
المستودع، حجمه `5,775,124` وبصمته SHA-256
`CCFC71D2EE3A4E40C86DEF9E102C4C5846ED5F0D9E68DEEB4D67CCC1742BBBE6`.
أضيف سجل تفويض exact-build ومسار GitHub يستخدم حساب خدمة Google المحمي ولا يعيد
بناء Android. التشغيل الأول `33308484614` نجح في المصادقة ثم توقف بأمان عند
`404 Package not found: com.otlobli.app` قبل رفع أي ملف. وافق المالك صراحة على
إقراري سياسات المطورين وقوانين التصدير الأمريكية، فأنشئ سجل عربي مجاني باسم
`Otlobli` للحزمة نفسها؛ معرّف Play Console هو `4974026156685692649`. أضيف
التطبيق إلى حساب الخدمة بثلاث صلاحيات فقط: قراءة التطبيق، قراءة الجودة التابعة
لها، وإصدار تطبيقات لجمهور الاختبار؛ لا صلاحية إنتاج أو مالية. نجح فحص
`33310340151`: التطبيق يملك `0` حزم، والمسارات الفارغة هي `production` و`beta`
و`alpha` و`internal`. حُدد `alpha` بوصفه مسار الاختبار المغلق، وتحوّل سجل
التفويض إلى `publish` مرتبط به وبالبصمة نفسها. التشغيل `33310435203` رفع AAB
داخل edit غير ملتزم ثم رفض Google إنشاء حالة `completed` لأن التطبيق ما زال
مسودة: `Only releases with status draft may be created on draft app`. حذف
السكربت الـedit غير الملتزم، فلم يبدأ اختبار أو مسار منشور. أضيف حقل
`releaseStatus` مقيد بـ`draft|completed`؛ المرحلة الحالية `draft` لحفظ الحزمة
نفسها في `alpha`، وبعد إكمال مهام إعداد التطبيق يتحول الإصدار نفسه إلى
`completed` من دون بناء أو versionCode جديد.

نجح التشغيل `33310601936` في رفع AAB نفسه وتثبيت `86.244 (1112)` على
`alpha` بحالة `draft`؛ لم يُعد بناء Android ولم يبدأ الاختبار بعد. اكتملت
مهام إعداد Play Console الإحدى عشرة: بيانات المتجر وأصوله، سياسة الخصوصية،
الإعلانات، الحكومة، الصحة، الميزات المالية، الفئة، `IARC`، أمان البيانات،
الجمهور المستهدف، وبيانات دخول المراجع. أدخل المالك كلمة مرور حساب المراجعة
بنفسه وتأكد ظهور حقول اسم المستخدم وكلمة المرور والتعليمات؛ لا توجد كلمة مرور
في المستودع أو ملخصات الجلسة. أصول Google Play المتعقبة داخل
`store-assets/google-play/` هي رسم مميز `1024x500` ببصمة
`D1A07F89…6F6F86`، ولقطتا هاتف `1512x2688` ببصمتي
`EC961F64…C8279` و`96DBDD0C…DEDFA`. اللقطتان مشتقتان من صور المتجر والسلة
المتعقبة مع حواف 9:16 فقط؛ لا تحتويان بيانات دخول أو رقم هاتف، وقد رُفعت وحُفظت
مع وصف المتجر وإقرار المحتوى المنشأ بالذكاء الاصطناعي.

بعد تأكيد المالك النشر تغير سجل التفويض في الالتزام `5cc4f5d` إلى
`releaseStatus=completed` و12 حسابًا متوقعًا، ثم شُغّل المسار `33320651915`.
أعاد استخدام bundle `1112` نفسه وتوقف بأمان عند commit برسالة Google:
`Only releases with status draft may be created on draft app`. كان ذلك حاجز
التطبيق الأول قبل الإرسال اليدوي. عولج الحاجز لاحقًا من Play Console بإدراج
الإصدار نفسه ضمن التغييرات المرسلة للمراجعة؛ لم يُرفع AAB آخر ولم يُعد البناء
ولم يتغير `versionCode`.

الإقرارات المحفوظة: التطبيق بلا إعلانات، وليس حكوميًا ولا صحيًا، والميزة المالية
هي `Mobile payments and digital wallets`. فئة المتجر `Shopping`، والبلد المحدد
للمسار المغلق هو المملكة العربية السعودية. وافق المالك صراحة على شروط `IARC`
والعرض العام لبريد الدعم، واكتمل التصنيف. اكتمل نموذج أمان البيانات بأربعة عشر
نوع بيانات والنقل المشفر، واكتمل الجمهور المستهدف للأعمار 13 فما فوق. أضيف
مسار حذف خارجي عبر البريد في صفحتي الخصوصية والدعم ونُشر على
`https://talabieh.vercel.app`، مع الرابط
`https://talabieh.vercel.app/privacy.html#request-account-deletion`.

أداة LDPlayer الفعلية هي `C:\LDPlayer\LDPlayer9\ldconsole.exe`. أصبحت قائمة
Play النظيفة باسم `All Apps - Closed Testers` وربطت بمسار `alpha` مع بريد
الملاحظات المعتمد. تحتوي القائمة 12 حسابًا بالضبط. استبعد المالك صراحة ثلاثة حسابات
مشتركة مع هاتف صديقه: كودكس، درة القرآن، ومحمد الزعبي؛ لا تستخدم القائمة
القديمة `Closed Testers 12` ولا هذه الحسابات الثلاثة. Google تطلب 12 مختبرًا
منضمين باستمرار 14 يومًا؛ وجود البريد في القائمة وحده لا يبدأ المدة. بطلب صريح
من المالك حُذفت مثيلات `LDPlayer-9` و`z10` و`z11` نهائيًا لأنها كانت تحمل
الحسابات الثلاثة المستبعدة. بعد ذلك أنشئت ثلاثة مثيلات جديدة، فأصبح `list2`
يحتوي 12 مثيلًا من الفهرس `0` إلى `11`. أعيدت تسميتها بنظام موحد
`01 - <email>` حتى `12 - <email>`. أكد المالك أن كل مثيل يحتوي الآن حسابًا
مختلفًا واحدًا. التوزيع النهائي هو: عادل، Chat1xhxh، `mhm1981x`، `mhm1981dx`،
`mhm1981d`، `mhmaadd.974`، `dhhddhhhd96`، `workmoh7`، `ryanalzby524`،
`bbhzhxjd`، `djjd19903`، ثم سمير على الترتيب `01..12`. ثُبتت الأسماء وفق هذه
البريدات. المحاكي `10` هو المفتوح حاليًا، والبقية مغلقة، وكل إعدادات ADB عند
`0`. لا تدّع تحقق ADB لمحتوى الحسابات؛ التوزيع يعتمد تأكيد المالك البصري.
بقي AVD Android Studio المنفصل دون تغيير.

وجود الحساب في المحاكي أو في قائمة Play لا يبدأ عداد الاختبار. بعد موافقة
Google وظهور مسار `alpha` نشطًا، يجب أن يفتح كل حساب رابط الانضمام ويختار
الانضمام ويبقى منضمًا 14 يومًا متصلة. التثبيت والفتح ليسا النص الحرفي لعداد
الانضمام، لكن Google
تطلب عند طلب Production وصف الاستخدام الفعلي والميزات المختبرة والملاحظات
والتغييرات الناتجة؛ لذلك يجب تثبيت نسخة Play واختبارها فعليًا وجمع ملاحظات
صادقة، ولا يكفي إنشاء 12 حسابًا أو فتح التطبيق شكليًا.

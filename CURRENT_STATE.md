...
- المرشح المحلي الحالي هو `0.1.0+17` على الفرع `codex/voicebrief-ios`. يصلح ثبات اختيار المظهر بعد إغلاق التطبيق، ويعيد آخر رصيد مؤكد من الخادم عند التشغيل بدلاً من الرجوع بصرياً إلى 300 دقيقة؛ الاختبار يغطي بقاء `299/300` بعد إنشاء نسخة تفضيلات جديدة. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
- اكتمل فحص أمني قياسي للنسخة السابقة `9f228c1` برقم `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` ووثائقه في `voicebrief_flutter/docs/security/build17/`. ثبّت 6 نقاط: عزل جلسة/نتيجة App Group عند تبديل الحساب، عدم الثقة بمدة الصوت من العميل، منع الرفع المباشر غير المحدود، إنفاذ انتهاء النتائج، جعل حد نموذج الدعم ذرياً، وقياس جسم الطلب الفعلي. نُفذت المعالجات، ونُشر الخادم، ونجح تجميع Swift على CI؛ يبقى قبول iPhone والتحقق التشغيلي الطويل للتنظيف المجدول.
...
- التحقق المحلي الحالي ناجح: format على 78 ملفاً، analyze بلا مشكلات، 42 اختباراً غير golden، 12 golden، 3 اختبارات JSON محدودة، 13 اختبار تاريخ/مدة صوت بعد منع تكرار الموعد، 3 اختبارات form محدودة، وكل `deno lint/check`، وAndroid debug. APK حجمه `177,653,885` بايت وبصمته SHA-256 `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`. نجح CI النهائي للبناء 17 في run `33327898696` بكل وظائفه.
- نجح TestFlight run `33327898649` في الأرشفة والتوقيع وفحص التطبيق وShare Extension والملفات والاستحقاقات والتحقق والرفع. IPA `VoiceBrief-0.1.0-build17-AppStore.ipa` حجمه `34,634,674` بايت وبصمته SHA-256 `5FB74810929DC7D20FF88493F64B7C24F127B9D690CBD2122A68D6C670B388F3`؛ Delivery UUID/build ID هو `e76f600f-70a6-4ef6-bb6d-0944dd939675`. GitHub artifact رقم `9736983367` وحجمه `34,385,468` بايت وبصمة ZIP `5C0119A69D8EE919B7CF9D6D051E07656A71A0FAA3EADA479748351AE9FCC847`.
...
- أُنشئت أربع صور iPad احترافية من شاشات Android الفعلية فقط في `voicebrief_flutter/store_assets/app_store_ipad/`، وكلها PNG بلا alpha وبمقاس Apple المقبول `2048x2732`: الرئيسية، مراجعة الصوت، المعالجة، والنتيجة. لم تُرفع إلى App Store Connect بعد.

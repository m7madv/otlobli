...
## Active candidate — v86.76 iPhone resume recompose (2026-08-08)

The iPhone 16 / iOS 27 freeze returned in v86.75 after background/resume. Git history comparison isolated a lifecycle regression: the device-proven v85.8.92 patch used one detach/reattach at `appDidBecomeActive + 0.25s`, but v86.5.1 added `willEnterForeground` plus four forced reattachments at `0.12/0.5/1.2/2.2s`. Both lifecycle notifications could fire on one resume, causing up to eight forced detach/reattach operations while WebKit rebuilt its remote layer tree. v86.76 restores the single delayed call and the normal one-second cooldown; it retains the actual fix (remove/re-add the `WKWebView`, restore constraints and scroll position), the injected recompose fallback, the Android host-resume defense, and the store-region `JSON.stringify` rebuild guard.

Marker: `2026.08.08-v86.76-iphone-resume-single-recompose`; native version `86.76 / 936`. `npm run build`, the updated iPhone freeze guard, and the low-end budget all pass: largest JS `1,198,131 / 1,200,000`, gzip `354,562 / 370,000`, SHEIN source `544,453 / 550,000`; Android and iOS are synchronized. GitHub/Xcode [run `31277870400`](https://github.com/m7madv/otlobli/actions/runs/31277870400) passed from `8845a91`; its unsigned IPA is `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-ios-v86.76\otlobli-ios-v86.76-iphone16\otlobli-v86.76-iphone16-unsigned.ipa` (7,064,759 bytes, SHA-256 `84B080C0893BF3BABA854A9866C91FF53F2D81630D7B412CF318B4BCFEA12B2B`) and the archive confirms `86.76 / 936`. This is a release candidate only until a physical iPhone 16 completes five background/resume cycles and a force-quit/cold-launch check without a frozen frame. The v86.75 quick-add safe-space fix remains in this candidate; the later toast/size-detection report remains pending verification.

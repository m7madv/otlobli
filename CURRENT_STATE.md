...
- المرشح الحالي هو `0.1.0+11` على الفرع `codex/voicebrief-ios`. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
...
- البناء 11 لا يدّعي فتح Runner. عند اختيار VoiceBrief من أي تطبيق يقدّم ملفاً صوتياً، ينسخ `Share Extension` الملف، يستخدم جلسة Supabase التي زامنها التطبيق داخل App Group المحمي، يرفع الصوت إلى مساره الخاص، يستدعي `process-audio`، ثم يعرض العنوان والملخص داخل نافذة المشاركة نفسها. يبقى المستخدم في التدفق ولا يحتاج إلى فتح التطبيق يدويًا للحصول على النتيجة.
- بعد اكتمال المعالجة يختار المستخدم `حفظ في VoiceBrief`؛ عند فتح التطبيق لاحقاً تُستهلك نتيجة App Group المعالجة وتُحفظ محلياً من دون رفع الصوت أو خصم الدقائق مرة ثانية. يجدد الامتداد الجلسة عند الحاجة، ويعيد Runner جلسة App Group المحدثة إلى Supabase عند تشغيله. عند غياب جلسة أو شبكة أو رصيد يبقى الملف محفوظاً لمسار الاستيراد القديم وتظهر رسالة صريحة.

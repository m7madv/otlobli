# v86.197 — preserve SHEIN scheduling across iOS inactivity (2026-08-20)

The old-version comparison separates two incidents. v86.125's current product spinner is not evidence for the lifecycle freeze: that build predates multiple live SHEIN PDP/risk-markup compatibility changes. The stable lifecycle signature is v86.65 working on first entry, freezing after app inactivity, and recovering only after Temu forces a fresh SHEIN runtime. v86.187 also froze with every normal Otlobli script group disabled, while the v86.193 USB trace showed a live WebContent process, successful loads/navigations, and no process termination after foregrounding.

The native cause now targeted is WebKit's SDK-aligned inactive scheduling contract. A WKWebView defaults to `WKInactiveSchedulingPolicySuspend`; when built against a modern SDK, inactive/background content is suspended promptly. WebKit can still temporarily resume it for page loads or `evaluateJavaScript`, explaining the misleading state where network, passive probes, and trusted DOM touches work while the SHEIN SPA scheduler remains non-interactive. The app-owned SHEIN configuration now sets `configuration.preferences.inactiveSchedulingPolicy = .throttle` on iOS 17+, preserving schedulability without the battery cost of `.none`.

This is intentionally not a rebuild/reload workaround. The exact v86.193 lifecycle, one WKWebView, `WKWebsiteDataStore.default()`, cookies, cache, IndexedDB, Service Workers, region/session rules, VPN behavior, scripts, product capture, navigation, and text-selection behavior are unchanged. Version/build is `86.197/1059`; marker is `2026.08.20-v86.197-inactive-scheduling-throttle`. Local TypeScript/build, release hardening, freeze, Temu, surface, and performance gates pass. GitHub/Xcode run `32382985235` from code commit `deb5325` passes; artifact ID `9411857885` is ARM64, iOS 15+, iPhone/iPad `[1,2]`, `6,556,233` bytes, SHA-256 `27BE184E0409E71EAC3B8EF08ECE25D0B78CF6F3A1BFE4D13C917289F2A603E2`, unsigned/unprovisioned, without source maps or relay placeholder. Binary inspection confirms the `setInactiveSchedulingPolicy:` selector. Physical iPhone acceptance is pending and must cover first entry, five background/resume cycles with a list and PDP tap after every return, one App Switcher force-quit/cold launch, and SHEIN → Temu → SHEIN. Do not call the root cause device-proven until those pass.

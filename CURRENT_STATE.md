...
## مشروع ضمانك المستقل — مصدر Build 27 وانتقالات الاشتراك الذكية (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+27`. Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير، وBuildات `24` و`25` و`26` و`27` داخل iPhone TestFlight فقط؛ لم يُرفع Android Build `27` ولم يُضف Build `27` إلى App Review. لا تنسب تغييرات Build `27` إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
- Build `27` يصنف اختيار الباقة مركزياً إلى اشتراك جديد أو باقة حالية أو ترقية أو خفض عند التجديد أو تغيير دورة فوترة. لم يعد اختيار «نمو» من «توسع» يظهر كاشتراك ثانٍ؛ يظهر خفضاً مجدولاً، بينما «بداية» إلى «نمو» يظهر ترقية فورية. يعيد `AppController` التصنيف بعد فحص الخادم ويمنع فتح المتجر إذا أصبحت الخطة والدورة المختارتان حاليتين.
- الحصص الشهرية حدود وليست أرصدة متراكمة: عند الترقية من `100` إلى `600` يبقى استخدام الشهر محسوباً ويصبح المتبقي `600 - usedWarranties`. مثال: بعد استخدام 30 من «بداية» يصبح المتاح 570 في «نمو»، لا 670 أو 700. الخفض يبقي حدود الخطة الأعلى حتى يؤكده المتجر عند التجديد، وتغيير الشهري/السنوي لا يعيد الحصة. تعرض الشاشة هذا الحساب والتوقيت قبل المتابعة.
...
- قبول الحالة المبلغ عنها نجح فعلياً بعد v28: عند `2026-09-01 09:22:26` بتوقيت الرياض سجل الخادم `APPLE_RECOVERY_RUNTIME_COMPATIBILITY_SUCCEEDED { environment: "sandbox" }`، وأعاد طلب `verify-store-purchase` HTTP `200`. تحولت قاعدة بيانات الإنتاج من التجربة الملغاة بلا entitlement إلى `scale/active/source=store/provider=app_store/monthly/sandbox` مع `auto_renews=true` وentitlement حالي غير superseded ونهاية فترة `2026-09-02 09:22:26.976016+03:00`. عند `09:25:03` أعاد التحديث المجدول التحقق منها، وأعاد `refresh-store-entitlements` HTTP `200`. هذا يثبت استعادة اشتراك Apple orphan إلى المتجر النهائي في الحالة المبلغ عنها، لا بقية سيناريوهات التجديد أو الترقية أو Android.
...
- التحقق النهائي للمصدر `4.5.0+27`: `flutter analyze` نظيف، وFlutter `186/186`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK Build `27` المحلي في `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `207,989,448` بايت وSHA-256 `33295CBE76936B806F7FA21C2FAA663C0B1294159A3DCA03A378FD7ED924DE37`. أعاد مسار iOS الموقّع تشغيل تحليل Flutter والاختبارات قبل الأرشفة ونجح.
- راجعت واجهة الاشتراك وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: RTL، إجراء أساسي واضح للترقية، إجراء ثانوي للخفض/تغيير الدورة، أسطح محايدة، أهداف لمس، و`320×568` مع تكبير `200%` ناجحة آلياً. لم يُنفذ قبول VoiceOver أو iPhone حقيقي أو جهاز ضعيف للواجهة الجديدة.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، ثم نجحت الاستعادة الحقيقية بعد حذف الحساب على v28 مع HTTP `200` واشتراك «توسع» شهري فعّال وentitlement مؤكد في قاعدة بيانات الإنتاج. لا تعمم هذا القبول على تجديد/ترقية/خفض/استرداد Apple أو الشراء/الاستعادة على Android أو VoiceOver/TalkBack أو الجهاز الضعيف؛ ما تزال بلا قبول نهائي.

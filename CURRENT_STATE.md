...
## مشروع ضمانك المستقل — Build 24 على TestFlight وBuild 23 قيد المراجعة (2026-09-01)

- Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير. بناء iPhone منفصل من المصدر `4.5.0+24` رُفع إلى TestFlight فقط بعد قرار المستخدم؛ رقع Flutter الجديدة أصبحت داخل Build `24` التجريبي، لكنها ليست داخل Build `23` قيد المراجعة. بقي `scripts/app_store_setup.mjs` على Build `23` ليعبر عن submission الجاري، ولم يُرفع Build `24` إلى Google.
- عالج مصدر Build `24` كتالوج الأسعار، أحداث المتجر المبكرة والمتأخرة، مهلات الشراء والاستعادة، المصالحة عند الفتح والاستئناف، منع المزود المزدوج، وعرض الباقة والدورة والمزود ونهاية الفترة. Apple يربط `appAccountToken` بالمتجر؛ Google يفحص نتيجة BillingClient الحقيقية، ويستخدم توكن المتجر المطابق فقط، ويرفض legacy المبهم قبل فتح الدفع. الاستعادة تختار المتجر المفتوح حتى في حساب متعدد المتاجر.
- نُشرت migrations حتى `20260831180000` على Supabase الصحيح `exxayzlklvgeyqhvtzgi`. دوال الإنتاج النشطة: `verify-store-purchase` v23، `refresh-store-entitlements` v7، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v16، و`warranty-card` v14. migration الأخيرة تفرض predecessor/current receipt لسلسلة Google بنمط CAS وتمنع sibling/replay، وإقرار Google يتم خادمياً بعد تطبيق الاستحقاق.
- تعمل جدولة `damanak-entitlement-refresh` كل 5 دقائق؛ آخر تشغيل مفحوص عند `2026-08-31 20:35 UTC` نجح وأعاد HTTP `200` مع `processed=0/refreshed=0/failed=0`. نجحت اختبارات `subscription_lifecycle_live.sql` و`account_deletion_successor_live.sql` و`warranty_tenant_integrity_live.sql` على الإنتاج داخل `BEGIN/ROLLBACK`.
- ملف `supabase/schema.sql` متزامن، حجمه `369,377` بايت وبصمته `EA30919FF7784D1EB6A79BA66C575E4EC10F9EF5B5D0D46DD84DDD46406CCF74`. قيود المتاجر المركبة وحارس حذف المالك وسلاسل إيصالات Google موجودة في المخطط المنشور.
- بقيت نتيجة التدقيق الأمني الأصلية 5 نتائج: 3 متوسطة و2 منخفضة، بلا عالية أو حرجة. أُغلقت سلامة علاقات المتاجر وCSV وحدود جسم المطالبة. بقي توثيق الجهاز الحقيقي للتجربة عبر App Attest/DeviceCheck وPlay Integrity وقرار الويب، وبقيت مدة روابط الضمان وإبطالها قرار منتج.
- نجح `flutter analyze` و`150/150` اختبار Flutter و`56/56` اختبار Deno مع فحص التنسيق و`git diff --check` وبناء web release وAndroid debug. APK Build `24` المحلي `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `207,919,707` بايت وبصمته `745AB8B44F68C587BBE57294D7D012642C9E059D2C93760C22AF7070AD729B8F`، ويثبت `versionCode 24` و`versionName 4.5.0`.
- App Store Connect: رُفعت صور iPhone الخمس وصور iPad الخمس في run `33341350096`. وقّع run `33341350366` Build `23` ورفعه؛ بصمة IPA هي `9C024E7444F339DDEABEB33FA69EF634368B5FAFF820EC9FF5B7D3E57C1E8815`. رفضت Apple الإصدار أولاً آلياً تحت `3.0.0` و`3.1.2` لغياب رابط EULA من وصف المتجر ولطلب تأكيد سعري `توسع شهري` (`49.99 USD`) و`توسع سنوي` (`499.99 USD`). أضيف رابط EULA القياسي إلى الوصف، وأُرسل رد يؤكد أن السعرين مقصودان وأن أسعار قطر المقابلة `199.99/1999.99 QAR` تأتي من App Store، ثم أُعيد submission `69104e50-7a9a-4102-a717-ca341d442897` نفسه عند `2026-08-31 20:34` بتوقيت الرياض. الإصدار ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، كلها الآن `Waiting for Review` على Build `23` نفسه؛ لا تغيّر أو تلغِ الطلب أثناء المراجعة.
- شغّل commit `7fe6bf074e02acfa56b905d8e93431473ef2babd` سير عمل iOS الموقّع [33443358450](https://github.com/m7madv/otlobli/actions/runs/33443358450). نجحت الأرشفة والتوقيع والتحقق والرفع، ثم أنهت Apple المعالجة. TestFlight Build `4.5.0 (24)` معرّفه `6b986de0-74bb-4649-8619-846aca1eb1b0` وحالته `Ready to Submit` ومربوط تلقائياً بمجموعة `Damanak Internal` الداخلية ذات المختبر الواحد. IPA حجمها `23,923,863` بايت وSHA-256 `EA55D5F1192886EE9019F0048E3476C1D5416E047B4F1F1823D234FC56BCF768`؛ لم يُضف البناء إلى App Review، ولم يُثبت بعد على iPhone حقيقي أو يُنفذ عليه شراء أو استعادة حقيقيان.
- Google Play: رفع run `33341350386` Build `23` إلى `Alpha`؛ بصمة AAB هي `5FB3FE00B91FFBC49FE143AF31A77938A208686B410668CCCCA00422DEFCF008` والإصدار قيد مراجعة Google. المسار `4699097266255892286` مرتبط بقائمة **دائمة وقابلة لإعادة الاستخدام لأي تطبيق** باسم `All Apps - Closed Testers` وتضم بالضبط `12` مختبراً؛ لا تُنشئ قائمة جديدة ولا تعدّل هذه القائمة. لا يبدأ عداد `14` يوماً إلا بعد قبول Google وانضمام المختبرين من `https://play.google.com/apps/testing/com.damanak.damanak`.
- حساب مراجعة Google داخل متجر `Damanak Note 8 Test` يملك وصول `scale` حتى `2036-08-31` بلا شراء أو تجربة. الهاتف يبقى على Build `22` المثبت من Play إلى أن تقبل Google Build `23`؛ لا تستبدله بملف جانبي مختلف التوقيع. لا يوجد Google RTDN، لذا شراء خارجي جديد يُكتشف عند فتح التطبيق فقط. ما تزال اختبارات الشراء والاستعادة الحقيقيين وVoiceOver/TalkBack والجهاز الضعيف منفصلة؛ لا يُستنتج قبولها من البناء أو API.

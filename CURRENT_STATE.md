...
TypeScript, release hardening, SHEIN freeze/architecture guard, Temu guard, store-surface guard, production build, performance budgets, and iOS/Android Capacitor sync pass. Final budgets: startup JS `639,165/720,000`, total JS gzip `266,126/370,000`, CSS `69,819/70,000`, fonts `81,364/100,000`, shipped store scripts `238,685/470,000`, and store source `557,374/600,000` bytes. GitHub/macOS Xcode build evidence and artifact inspection must be recorded when complete; no physical-device run has been performed. The exact two-process capture protocol and decoder are in `docs/SHEIN_V86_196_RUNTIME_DIAGNOSTIC.md`; do not call a cause proven without the first-success/kill/second-freeze evidence set.

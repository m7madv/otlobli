...
- أُعد إفصاح App Privacy من فحص manifests داخل IPA البناء 18: `15` نوع بيانات، كلها مرتبطة بالمستخدم حسب تكاملات التطبيق، ولا يُستخدم أي منها للتتبع. زر Publish جاهز لكنه لم يُضغط لأنه نشر خارجي نهائي. حُدثت سياسة الخصوصية ثنائية اللغة لتشرح Google Sign-In وRevenueCat ونُشرت إنتاجيًا في deployment ‏`dpl_EMsF8EH79BfRKPTSxNr8wRDDxRNn` بحالة `READY` على `https://voicebrief-legal.vercel.app/privacy`.
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار Content Rights على أن التطبيق يصل إلى محتوى طرف ثالث ولدى المالك الحقوق اللازمة. أُعدت حقول مراجعة Apple باسم `MOHAMMAD ALZOUABI` وحساب/بريد `mhm1981d@gmail.com` والرقم الخام `70067040`، لكن لا يمكن تثبيتها من دون كلمة مرور حساب المراجع؛ سُلّمت الصفحة للمالك ليدخلها بنفسه. يحتاج الرقم إلى تأكيد رمز الدولة. محاولة إضافة إصدار التطبيق إلى المسودة لم تكتمل؛ بقيت المسودة `3` عناصر فقط وتعرض أن إصدار iOS مطلوب. يبقى كذلك حسم امتثال التشفير لفرنسا ونشر App Privacy ثم الإرسال النهائي. IPA البناء 18 يحتوي `ITSAppUsesNonExemptEncryption=false` لكنه يضم TLS معياريًا من Flutter/Dart عبر BoringSSL؛ لا يُدّعى أن النسخة أُرسلت أو نُشرت لدى Apple.

# الليرة السورية الجديدة + إصلاح تصادم مبالغ الدفع (2026-08-11) — القاعدة **منشورة**

فرع `claude/syp-currency-v86133` متفرّع من `claude/v86.122-vpn-conceal-fix` عند `b5d078e` / **v86.133**.

## 🔴 حالة النشر — القاعدة نُشرت، التطبيق والسيرفر لأ

**نُشرت الهجرتان على الإنتاج (`dcicqdprtyhwmhegabay`) بتاريخ 2026-08-11** بإذن صريح من المستخدم، ومسجَّلتان في `supabase_migrations.schema_migrations`، ومنسوختان إلى `supabase/migrations/` في المستودع الرئيسي (غير متتبّعتين بعد).

الحالة بعد النشر (مُتحقَّقة): `usd_to_syp_rate = 131.70`، `syp_denomination = new`، أجور الشحن `500`، الطلبات مقسومة (744,127 → 7,441؛ وطلب الدولار بقي `95.93`)، رصيد المحفظة 110 → 1، ومشغّل الحارس مُركَّب. **جُرّب الحارس فعلياً**: محاولة كتابة `13170` رُفضت برسالة صريحة والسعر بقي `131.70`.

**نسخة احتياطية قبل التحويل:** `C:\Users\MOHAMMAD\Desktop\otlobli-backup-before-redenomination-20260811.json` (كل المبالغ الأصلية: 5 طلبات، 10 بنود، 3 حركات محفظة، 15 شحنة، 47 بند سلة، والإعدادات).

### سيرفر أوراكل — **منشور ومُتحقَّق** (2026-08-11)

`server/src/routes.js` مرفوع ومُعاد تشغيله. النتيجة الحيّة:

```
GET /api/exchange-rate → {"rate":131.7,"buy":131.2,"sell":131.7,"source":"sp-today.com"}
سجلّ السيرفر            → 💱 Exchange rate updated and persisted: 131.2/131.7 SYP/USD
/health                 → {"status":"ok"}
```

**⚠️ لم تُرفع نسخة المستودع كما هي.** الملف الحيّ كان متأخّراً 27 سطراً عن المستودع، ونسخة المستودع تستورد `releaseOtpVerification` من `otpStore.js` — **وهذه الدالة غير موجودة في `otpStore.js` الحيّ**، فرفعها كما هي كان سينهي العملية وقت الإقلاع ويُسقط تسجيل الدخول بالكامل. لذلك طُبّق تعديل السعر وحده فوق الملف الحيّ (الفرق محصور بدوال السعر). نسخة احتياطية: `~/otlobli-server/src/routes.js.bak-precurrency-20260810-214714`.

**بقي متأخّراً عن المستودع (قرار مستقل):** معالجة `recipient_not_on_whatsapp`، و`releaseOtpVerification` عند فشل إنشاء الجلسة، و«لا توجد جلسة». نشرها يتطلّب رفع `otpStore.js` معه.

**لم يُتحقَّق:** إرسال رمز واتساب حقيقي بعد إعادة التشغيل. الجلسة سليمة (`wa-sessions/0/creds.json` موجود، لا QR) و`whatsappConnected:false` طبيعي لأن الاتصال عند الطلب — لكن التأكيد يحتاج طلب رمز دخول فعلي.

### أسعار السلّة المحفوظة — عطل كشفه اختبار الجهاز بعد النشر

بعد اكتمال النشر أظهر الجهاز أن السلّة المحفوظة في `localStorage` تحمل أسعاراً بالليرة القديمة ($10.96 → 144,453) لأن `App.tsx` يستعمل `priceSyp` المخزَّن إذا كان موجباً ولا يعيد اشتقاقه. أُضيف `useEffect` يصحّح مرة واحدة أي سعر يتجاوز `priceUsd × exchangeRate` بعشرة أضعاف. **مُتحقَّق على الجهاز:** $10.96 → 1,443 و$8 → 1,054، مطابقان للمتوقَّع تماماً.

### ⚠️ خطر ما زال قائماً

**أي نسخة تطبيق قديمة مثبَّتة تُسعّر بمئة ضعف.** حارسها القديم `>= 1000` يرفض `131.7` فتُبقي `13170` المخزَّن. **لا يطلب أحد من نسخة قديمة حتى تُحدَّث.** لا توجد آلية تحديث إجباري. نوت 8 عليه النسخة الجديدة؛ الآيفون لا.

### النسخ المبنيّة — v86.134 / 994

- **أندرويد:** `app-debug.apk` مبنيّ ومثبَّت على نوت 8 ومُتحقَّق عليه.
- **آيفون:** تشغيل `31435825374` **نجح**. الحزمة `otlobli-v86.134-iphone16-unsigned.ipa` في `C:\Users\MOHAMMAD\Desktop\otlobli-ios-v86.134\`؛ SHA-256 `C3F3625CCEE03282D0D31730C700F42FCD533DA2FA14C2AF094C65DCAC103D7D`. حارس التجمّد وميزانية الأداء يمرّان في CI (`1,075,564 / 1,200,000`).
- **⚠️ التثبيت على iPhone يجب أن يكون حذفاً ثم تثبيتاً نظيفاً** — التثبيت فوق القديم يُبقي حالة WebView قديمة، وهذا كلّف جلسة سابقة تشخيصاً خاطئاً. ولم يُتحقَّق من شيء على iPhone بعد.
- الفرع مدفوع: `claude/syp-currency-v86133` عند `773ed13`.

### الباقي

1. **لوحة الإدارة (Vercel):** غير عاجلة — تقرأ القيم من القاعدة وهي صحيحة أصلاً؛ التعديل مسّ القيم الافتراضية التي لا تُستعمل إلا لو غاب الصف. نشرها يحتاج دمجاً إلى `main` (وهو عند v86.67) — قرار مستقل.
3. **أسرار GitHub Actions** (`SUPABASE_URL` و`SUPABASE_SERVICE_ROLE_KEY`): يضبطها المستخدم بنفسه — مهمة السعر ستبقى فاشلة حتى ذلك. السيرفر يغطّي التحديث حالياً.

## ما تأكّد بالقراءة من قاعدة الإنتاج (لا تخمين)

- **الحساب الداخلي بالليرة القديمة** — قاطع: طلب حقيقي `total_syp = 1,271,120` مقابل `payment_amount = 95.93 USD` → 13,251. وsp-today ينشر الاثنين: `SYP (new) Sell 131.70` مقابل `13,170 old`. النسبة 100 بالضبط.
- **القاعدة شبه فارغة**: 5 طلبات، 10 بنود، 3 حركات محفظة (الرصيد الكلي **110 ل.س قديمة**)، 15 شحنة، 47 بند سلة جماعية، 0 كوبونات/استخدامات/تحقّقات، 29 زبوناً. ولا نيّة SYP معلّقة واحدة وقت الفحص. فهجرة البيانات بلا مخاطرة عملياً.
- **`enforce_exact_payment_intent` الحيّة تطابق `schema.sql` حرفياً** — المساواة التامة مؤكَّدة على الإنتاج، وهي سبب «payment amount collision».
- **نوافذ الإزاحة الحيّة 120 خطوة لا 40** في مسارات الجلسة الفعّالة (`create_pending_order` و`create_wallet_topup` ذات `p_session_token`). النسخ القديمة 40.
- **دوال متراكبة في الإنتاج**: 3 نسخ من `create_wallet_topup`، ونسختان من `create_pending_order` و`create_order_issue_payment`. التطبيق ينادي `create_pending_order_v2` الذي يفوّض للنسخة ذات 4 وسائط.
- **تلف ترميز عربي حيّ** في `create_pending_order` (4 وسائط): الحالات مخزَّنة `ظپط´ظ„ ط§ظ„ظ…ط·ط§ط¨ظ‚ط©` (49 بايت/23 محرفاً) بدل `فشل المطابقة`. هجرة `20260729223000` سترته بمشغّل `orders_aa_normalize_payment_status` ولم تُصلحه. **مهمة مستقلة مفتوحة.**
- **مهمة تحديث السعر على GitHub Actions فاشلة في آخر 100 تشغيل** — `مفقود: SUPABASE_URL أو SUPABASE_SERVICE_ROLE_KEY`. **يحتاج تدخّل المستخدم.**
- **سيرفر أوراكل هو الكاتب الفعلي للسعر**: التطبيق ينادي `/api/exchange-rate` فيسحب السيرفر من sp-today ويكتب في Supabase. رُصد حيّاً وهو يغيّر القيمة من `13180` إلى `13170` أثناء الجلسة.

## ما أُنجز (كله في المستودع، **لم يُنشر شيء على القاعدة الحيّة**)

1. `supabase/migrations/20260810160000_payment_intent_nudge_window.sql` — المشغّل يقبل نافذة الإزاحة (120 خطوة، 0.01$ / 1 ل.س) بدل المساواة التامة، ورسالة الخطأ تذكر المبلغ والنافذة.
2. `supabase/migrations/20260810170000_syp_redenomination.sql` — idempotent (حارس `app_settings.syp_denomination`): يُلغي نيّات SYP المعلّقة، يقسم كل عمود ليرة مخزَّن على 100، يضبط السعر 131.70 وأجور الشحن 500، ويضيف `guard_usd_rate_setting` الذي **يرفض أي سعر ≥ 1000 بعد اعتماد الجديدة** — يحوّل إفساد السيرفر الصامت إلى خطأ صريح، ويجعل احتياطيات `13000` الإحدى عشرة في الدوال غير قابلة للوصول.
3. العميل: `OLD_LIRA_RATE_FLOOR = 1000` على **كاتبَي السعر** في `src/App.tsx` وعلى `DEFAULT_EXCHANGE_RATE` و`storedRate` المحفوظ، و`parseInt`→`parseFloat`، `MIN_ORDER_SYP` 500000→5000، `FIXED_SHIPPING_SYP` 90000→900، و`DISPLAY_USD_RATE` في `supabaseAppApi.ts` (يُمرَّر كـ`p_usd_rate` لـ`redeem_coupon`).
4. `server/src/routes.js` و`.github/scripts/update-exchange-rate.mjs`: يقرآن `Sell 131.70` الجديدة ويشتقّانها من القديمة ÷100 عند الغياب؛ لا تدوير؛ رفض ما ≥ 1000. **مُختبَر على صفحة sp-today الحقيقية → 131.7.**
5. الافتراضات في `app-settings`/`migrate-v2`/`AdminApp`/`schema.sql`/`.env.example`.

## التحقّق

- بناء العميل والإدارة والأندرويد ناجح؛ **حارس تجمّد SHEIN وميزانية الأداء يمرّان** (`1,075,194 / 1,200,000`).
- **الحزمة المبنيّة مفحوصة**: تحوي `131.7`، ولا أثر لحارس `> 1000` القديم، والظهوران المتبقيان لـ`13000` هما قيمة `.env` المُضمَّنة وكلاهما داخل دالّة محروسة تسقط على 131.7.
- **تحقّق على الجهاز (Galaxy Note 8، CDP) على حزمة v86.133 هذه نفسها** (`index-DWAxZ3BI.js`): ضُبط `talabieh.exchangeRate = 7` ثم أُعيد تحميل الصفحة، فبقيت `7` بينما `‎/api/exchange-rate` يقول `13170` والقاعدة تقول `13170` → التطبيق **يرفض الليرة القديمة من كلا المصدرين** فعلياً. أُعيدت حالة الجهاز بعد الاختبار.
- **الدرس:** الاختبار نفسه على الأساس السابق كشف خطأً حقيقياً — كاتب سعر ثانٍ في `App.tsx` (نتيجة `/api/exchange-rate`) فاتني في التعديل الأول، **والبناء كان يمرّ رغمه**. لا تكتفِ بنجاح البناء في مسار النقود.
- **لم يُتحقَّق:** عرض سعر حقيقي بعد الهجرة (يحتاج نشر القاعدة)، ولا مطابقة شام كاش الفعلية، ولا أي شيء على iPhone.

## الترتيب الإلزامي للنشر (ثلاث أرجل)

1. هجرتا Supabase.
2. `server/src/routes.js` إلى أوراكل يدوياً (scp + `pm2 restart otlobli-wa`) — وإلا رفض الحارس كتابته وبقي السعر بلا تحديث.
3. بناء التطبيق + لوحة الإدارة على Vercel.

النافذة بين (1) و(3): النسخ المثبَّتة القديمة تحمل حارساً `>= 1000` فترفض السعر الجديد وتُبقي القديم → أسعار مئة ضعف. لا توجد آلية تحديث إجباري في التطبيق.

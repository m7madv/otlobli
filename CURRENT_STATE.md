...
- No APK or IPA was produced in this batch: this worktree has no native toolchain and no real `VITE_*` secrets, so a local `cap sync` would have propagated a placeholder build. The artifacts must come from the workflows on this branch (`ios-unsigned-build.yml`, `android-debug-build.yml`).

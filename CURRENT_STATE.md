...
- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+27`. Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير، وBuildات `24` و`25` و`26` داخل TestFlight فقط؛ Build `27` مصدر محلي ولم تُنشأ أو تُرفع له IPA/AAB. لا تنسب تغييرات Build `27` إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
...
- التحقق المحلي النهائي للمصدر `4.5.0+27`: `flutter analyze` نظيف، وFlutter `186/186`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK Build `27` المحلي في `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `207,989,448` بايت وSHA-256 `33295CBE76936B806F7FA21C2FAA663C0B1294159A3DCA03A378FD7ED924DE37`. لم تُنشأ حزمة iPhone موقعة ولم يُرفع Build `27` إلى أي متجر.
...
- App Store Connect: submission `69104e50-7a9a-4102-a717-ca341d442897` ما يزال يضم Build `23` ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، وحالته `Waiting for Review`. رفع run [33458183412](https://github.com/m7madv/otlobli/actions/runs/33458183412) الالتزام `4a5e96bdd1eefd3f27ff89fccf47fe18012ef4f7` بوصفه iPhone Build `4.5.0 (26)`؛ قبلت Apple الرفع وأنهت المعالجة، والبناء `VALID` بالمعرّف/Delivery UUID `10a1e004-c6c1-4b56-9cff-618406c3881b` وداخل `Damanak Internal` ذات المختبر الواحد. الـIPA حجمها `23,942,352` بايت وSHA-256 `FE66B7F77FB1892FE8F3D55F0AC85AB6B0A7DF783286C77C2EDA735A82C32A37`. لم يُضف Build `26` إلى App Review.

...
أصلًا في قائمة Play. بعد دخول المالك تبيّن أن المثيل `01` يحتوي حساب عادل
والحساب الجديد معًا. طلب المالك توزيع الحسابات بحيث يكون حساب واحد في كل
مثيل. لم يُنسخ قرص أو رمز جلسة ولم يُحذف أي حساب قبل تأمين تسجيل مستقل.
أعيدت تسمية `01` مؤقتًا لتذكر الحسابين و`SPLIT PENDING`، ثم أُغلق وأعيد ADB
فيه إلى `0`. أعيد تخصيص المثيل الفارغ `10` للحساب الجديد باسم `PENDING`، وهو
المثيل الوحيد المفتوح الآن على شاشة تسجيل Google مع `adbDebug=1` وAccountManager
فارغ. بعد أن يكمل المالك الدخول في `10`: تحقق من الحساب، ثم احذف الحساب الجديد
فقط من `01` وأبق عادل، وأزل علامات الانتظار من الاسمين وأعد ADB إلى `0`.
المثيل `02` ما زال يحتوي حسابين أيضًا ويمكن فصله بالطريقة نفسها إلى `11` بعد
توفير تسجيل أحدهما. بقي AVD Android Studio المنفصل دون تغيير.

...
أصلًا في قائمة Play. بعد دخول المالك ظهر حساب عادل والحساب الجديد معًا في
المثيل `01`، وطلب المالك إزالة الجديد منه. عند إعادة تشغيل `01` وفحصه عبر ADB
المباشر `127.0.0.1:5555` ظهر `Accounts: 0`: الحساب الجديد غير موجود، لكن حساب
عادل لم يبقَ أيضًا، ولم تُنفذ عملية حذف يدوي لأي حساب. أعيدت تسمية المثيل
بصدق إلى `01 - adelalzoubi2022@gmail.com - RELOGIN`، وهو المثيل الوحيد المفتوح
على شاشة إضافة حساب Google كي يدخل المالك حساب عادل فقط. `adbDebug=1` مؤقتًا
للتحقق بعد الدخول؛ بعدها يجب إثبات وجود عادل وحده، إزالة `RELOGIN` من الاسم،
إغلاق المثيل، وإعادة ADB إلى `0`. عاد `10` إلى
`10 - EMPTY - ADD GOOGLE ACCOUNT` وهو مغلق. المثيل `02` ما زال يحتوي حسابين
ويمكن فصله إلى `11` بعد توفير تسجيل مستقل. بقي AVD Android Studio دون تغيير؛
لا تستخدم `ldconsole adb --index 0` لأن رقمه يتصادم معه، واستخدم العنوان الصريح.

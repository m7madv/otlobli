...
- المرشح الحالي هو `0.1.0+12` على الفرع `codex/voicebrief-ios`. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
...
- البناء 12 ينفذ التدفق الجديد المدعوم: عند اختيار VoiceBrief ينسخ `Share Extension` الملف ويبدأ الرفع والنسخ والتلخيص فوراً عبر جلسة Supabase المزامنة، مع بقاء نافذة المشاركة مفتوحة حتى تكتمل المهمة. عند النجاح يحفظ نتيجة App Group تلقائياً، يحذف النسخة الصوتية المحلية، يجدول إشعاراً محلياً «الملخص جاهز»، ثم يغلق الامتداد؛ لم يعد هناك زر `حفظ في VoiceBrief`.
- يطلب Runner إذن التنبيه/الصوت/الشارة بعد تسجيل الدخول. عند ضغط إشعار الجاهزية، يلتقط AppDelegate علامة `sharedResult`، يستورد Flutter النتيجة ويحفظها محلياً من دون رفع أو خصم ثانٍ، ثم ينتقل مباشرة إلى `/result`. شاشة النتيجة الكاملة تعرض `actionItems` و`importantDates` وأزرار الإضافة إلى التقويم؛ نافذة المشاركة القديمة كانت تعرض العنوان والملخص فقط، وهذا هو سبب عدم ظهور الموعد للمستخدم رغم وجوده في النتيجة.

...
## Google Play والاختبار المغلق — قيد التنفيذ (2026-08-30)
...
`Only releases with status draft may be created on draft app`. بقي الإصدار
الفعلي في Play بحالة `draft` ولم يبدأ الاختبار. السبب أن سجل التطبيق الأول
نفسه ما زال Draft ويجب إرساله أولًا من Play Console إلى مراجعة Google؛ بعدها
يعاد تشغيل المسار نفسه، بلا رفع أو بناء أو versionCode جديد.
...
أداة LDPlayer الفعلية هي `C:\LDPlayer\LDPlayer9\ldconsole.exe`. أنشئت قائمة
Play مستقلة باسم `Otlobli Closed Testers` وربطت بمسار `alpha` مع بريد الملاحظات
المعتمد. تحتوي القائمة 11 حسابًا نظيفًا فقط. استبعد المالك صراحة ثلاثة حسابات
مشتركة مع هاتف صديقه: كودكس، درة القرآن، ومحمد الزعبي؛ لا تستخدم القائمة
القديمة `Closed Testers 12` ولا هذه الحسابات الثلاثة. يلزم حساب Google نظيف
ومختلف واحد للوصول إلى 12. Google تطلب 12 مختبرًا منضمين باستمرار 14 يومًا؛
إضافة البريد إلى القائمة وحدها لا تبدأ المدة. وافق المالك على إرسال
`bbhzhxjd@gmail.com` إلى Google، لكن إدخال المتصفح الآلي لم يُحوّل النص إلى
عضو ولم يفعّل الحفظ؛ لم يُغيّر شيء في القائمة وما زالت 11. تُركت نافذة تعديل
`Otlobli Closed Testers` عند خطوة الإدخال ليعتمد المالك البريد ويحفظه يدويًا.
بقي الإصدار `draft` والمسار غير نشط حتى حفظ الحساب الثاني عشر وإرسال التطبيق
الأول إلى مراجعة Google ثم نجاح `completed`. بطلب صريح
...
وجود الحساب في المحاكي أو في قائمة Play لا يبدأ عداد الاختبار. بعد إضافة
`bbhzhxjd@gmail.com` بصفته الحساب الثاني عشر إلى القائمة المستقلة ونشر نفس
إصدار `alpha`، يجب أن يفتح كل حساب رابط الانضمام ويختار الانضمام ويبقى منضمًا
14 يومًا متصلة. التثبيت والفتح ليسا النص الحرفي لعداد الانضمام، لكن Google

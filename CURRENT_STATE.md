...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901012000`، و`verify-store-purchase` v25 نشطة مع `verify_jwt=false` والتحقق اليدوي من الجلسة، و`refresh-store-entitlements` v7 وبقية الدوال بالإصدارات السابقة. migration الجديدة توسع RPC الذرية بمعامل recovery افتراضيه `false` وأقفال مرتبة على hashes، ولا تسمح بمطالبة إيصال ذي هوية قديمة حية أو ربط قائم.
...
- التحقق المحلي النهائي: `flutter analyze` نظيف، Flutter `181/181`، Deno `67/67`، واختبار migration `1/1`، و`deno fmt --check` على 23 ملفاً و`git diff --check` ناجحان. نجح web release وAndroid debug؛ APK Build `26` المحلي حجمه `207,970,698` بايت وSHA-256 `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لم يُرفع Build `26` إلى Google.
...
- لم يُنفذ شراء أو استعادة حقيقيان على iPhone أو Android، ولم يُثبت Build `26` على جهاز حقيقي، ولم تُنفذ اختبارات VoiceOver/TalkBack أو جهاز ضعيف. نجاح الخادم والبناء والاختبارات لا يثبت قبول الجهاز.

...
- المرشح الحالي هو `0.1.0+13` على الفرع `codex/voicebrief-ios`. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
...
- يطلب Runner إذن التنبيه/الصوت/الشارة بعد تسجيل الدخول. عند ضغط إشعار الجاهزية، يلتقط AppDelegate علامة `sharedResult`، يستورد Flutter النتيجة ويحفظها محلياً من دون رفع أو خصم ثانٍ، ثم ينتقل مباشرة إلى `/result`. البناء 13 يضيف لكل موعد زر «ضبط منبّه» إلى جانب التقويم: iOS يجدول إشعاراً محلياً في الوقت المختار، وAndroid ينشئ منبّه النظام. العبارة المبهمة مثل «الساعة الخامسة» تفرض تأكيد التاريخ والوقت ولا تخمّن صباحاً أو مساءً.

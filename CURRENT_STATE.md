...
Local TypeScript, production and diagnostic web builds, release hardening, SHEIN/Temu/surface guards, low-end budgets, and Capacitor iOS sync pass. The diagnostic build is enabled only by `VITE_SHEIN_IOS_ROOT_CAUSE_DIAGNOSTICS=true`; the workflow exposes a matching manual input. Xcode/IPA compilation and the clean-install device sequence are still pending, so no root cause or fix is claimed. Do not modify Android or the iPhone 6 during this classification run.

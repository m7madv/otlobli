...
## مشروع ضمانك المستقل — مصدر Build 26 وإصلاح الاستعادة (2026-09-01)
...
- الاستعادة الصريحة وحدها ترسل `recoveryRequested=true`. اشتراك Apple القديم بعد حذف حساب ضمانك لا ينتقل إلا بعد تحقق JWS بمكتبة Apple الرسمية والجذور المثبتة، والتأكد أن الحساب/المتجر القديمين محذوفان وأن الهدف متجر وحيد بلا entitlement؛ ثم يُحدث `appAccountToken` بعد commit. Google يدعم ربط Build `23` الحسابي وسلسلة predecessor/out-of-app ذرياً بالشروط نفسها. المسارات الخلفية تبقى صارمة ولا تنقل legacy تلقائياً.
- أثبتت تجربة TestFlight الحية شراء Apple ناجحاً على Build `26` عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حذف المستخدم حساب ضمانك بعد نحو 8 ثوانٍ مع بقاء الاشتراك لدى Apple. كانت استعادته إلى الحساب الجديد تصل إلى App Store Server API ثم تفشل داخل فحص JWS المتصل بـ`APPLE_RECOVERY_PROOF_INVALID`؛ لم تكن المشكلة غياب دفع أو منتج «توسع»، ولم يُكتب استحقاق للحساب الجديد قبل الإصلاح.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901012000`، و`verify-store-purchase` v26 نشطة مع `verify_jwt=false` وSHA-256 للحزمة `2e99b9097d3f18014720fd2f0d9b73a1ed2eaa5ae7f39fe6af9745d9917d65e4`، و`refresh-store-entitlements` v7 وبقية الدوال بالإصدارات السابقة. v26 تبقي فاحص Apple المتصل أولاً، ثم تستخدم فاحص Apple الرسمي عند `signedDate` فقط للمهلة والحالات `1/2/6`؛ تبقى `FAILURE=7` وإخفاقات التطبيق/البيئة/طول السلسلة مرفوضة، وبعد النجاح تبقى المطابقة الدقيقة وحواجز الحساب المحذوف والربط الذري كما هي.
...
- التحقق المحلي النهائي: `flutter analyze` نظيف، Flutter `181/181`، Deno `71/71`، واختبار migration `1/1`، و`deno check` للدالة و`deno fmt --check` على 23 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK Build `26` المحلي حجمه `207,970,698` بايت وSHA-256 `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لم يتغير Flutter في إصلاح v26 الخادمي ولم يُرفع Build `26` إلى Google.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، لكن حذف الحساب مباشرةً أنشأ حالة orphan وكشف فشل الاستعادة السابق. ما تزال إعادة الاستعادة بعد نشر v26، والشراء/الاستعادة على Android، وVoiceOver/TalkBack، والجهاز الضعيف بلا قبول نهائي؛ لا يُستنتج نجاحها من الاختبارات الآلية.

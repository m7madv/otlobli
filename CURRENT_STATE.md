...
- اكتملت تهيئة App Store Connect للبناء `0.1.0 (18)` من دون إرسال نهائي: حُفظ تقييم العمر العالمي `4+`، وضُبط التطبيق مجانيًا ومتوافرًا في `175` بلدًا/منطقة مع البلدان المستقبلية، وحُفظت ملاحظات المراجعة ومسار اختبار التسجيل والمشاركة والمواعيد والاشتراك. مسودة المراجعة تحتوي الآن مجموعة `VoiceBrief Pro` والاشتراكين `voicebrief_pro_monthly` و`voicebrief_pro_annual` بحالة Ready for Review؛ كلا الاشتراكين في المستوى `1` لأن المزايا نفسها والاختلاف في المدة فقط. إصدار التطبيق نفسه لم يُضف بعد إلى المسودة، وزر الإرسال ما زال معطّلًا.
...
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار Content Rights على أن التطبيق يصل إلى محتوى طرف ثالث ولدى المالك الحقوق اللازمة. أُعدت حقول App Review باسم `MOHAMMAD ALZOUABI` وحساب Google للمراجع `mhm1981d@gmail.com` والهاتف `+974 70067040`، لكنها ما زالت تحتاج إدخال كلمة مرور الحساب وحفظ النموذج؛ لا تُقرأ أو تُطبع كلمة المرور. أبقى المالك فرنسا، واختير في استبيان Apple التشفير المعياري غير الخاص؛ ظهر طلب إلزامي لرفع `French encryption declaration approval form`. البناء 18 يعلن `ITSAppUsesNonExemptEncryption=false` رغم TLS المعياري المضمّن عبر Flutter/Dart BoringSSL، لذلك يلزم اعتماد ANSSI ورمز Apple وبناء بديل قبل الإرسال النهائي. بقيت مسودة المراجعة `3` عناصر ولم يُضف إصدار iOS ولم يُرسل التطبيق للمراجعة.

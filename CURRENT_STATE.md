...
## مشروع ضمانك المستقل 2.0.0 (2026-08-24)

- توسّع التطبيق المستقل `damanak_flutter/` إلى نظام متاجر كامل بمعرّف `com.damanak.damanak`: حساب بالبريد وكلمة المرور، إنشاء متجر أو الانضمام برمز، حساب منفصل لكل موظف، أدوار `owner/manager/staff`، كتالوج منتجات، ماسح باركود داخل التطبيق، ضمانات، صيانة، اشتراكات، رموز تفعيل وسجل تدقيق. لم يتغير مصدر Otlobli أو دفعه أو محفظته.
- البنية تفصل الواجهة عن `DamanakRepository`. التشغيل بلا مفاتيح يفتح عرضاً تشغيلياً واضحاً، والتشغيل السحابي يستخدم `Supabase` عبر `DAMANAK_SUPABASE_URL` و`DAMANAK_SUPABASE_PUBLISHABLE_KEY`. أضيف مخطط مستقل في `damanak_flutter/supabase/migrations/20260824090000_damanak_core.sql` مع `RLS` وعزل المتاجر وفرض حدود الاشتراك والمقاعد. لم يُنشأ أو يُربط مشروع `Supabase` بعيد، ولم يُطبّق المخطط على قاعدة Otlobli.
- الماسح يستخدم `mobile_scanner` داخل واجهة «ضمانك»، مع إذن كاميرا، إضاءة، تبديل كاميرا، إدخال يدوي، مطابقة الكتالوج وإكمال إصدار الضمان. أُصلح استئناف الكاميرا بعد الرجوع من وثيقة الضمان، واختُبر مسار الباركود `6281000000142` كاملاً في المتصفح بلا خطأ إضافات أو كاميرا.
- أُعيد التصميم وفق مهارة `Apple Design` الخاصة بالمشروع: لون علامة واحد `#087F5B` وحياديات دلالية، حالات لا تعتمد على اللون وحده، مظهر فاتح/داكن تابع للنظام، أهداف لمس `48dp`، احترام تقليل الحركة، وختم باركود أحادي اللون. التقرير في `docs/DESIGN_AUDIT.md`.
- نجح `flutter analyze` بلا ملاحظات، ونجحت 8 اختبارات، ونجح `flutter build web --release --no-wasm-dry-run`. راجعت الواجهة بمتصفح حقيقي في `320×568` و`393×852` و`1280×800`، فاتحاً وداكناً، وشملت الرئيسية والماسح والنموذج والوثيقة والفريق والدعوة والاشتراك. أدلة الصور في `output/playwright/damanak-apple/`، وسجل المتصفح بلا أخطاء في المسارات المختبرة.
- Android release-test APK: `C:\Users\MOHAMMAD\Projects\SHEIN IN SIRYA\output\damanak\damanak-2.0.0-release-test.apk`; الحجم `72,563,673` بايت؛ SHA-256 `97237091826F2021C7E54E8E614C94246856C7B7D7D1402C5ED8F514949F66E3`. هو ملف تجربة محلي وليس موقّعاً بمفتاح متجر إنتاج.
- نجح workflow `damanak-ios-unsigned.yml` في GitHub Actions ضمن التشغيل `32715444002`. ملف iOS غير الموقّع: `C:\Users\MOHAMMAD\Projects\SHEIN IN SIRYA\output\damanak\github-ios-32715444002\damanak-2.0.0-ios-unsigned.ipa`؛ الحجم `8,173,743` بايت؛ SHA-256 `68D976EA9727E8C0F512CB9C23421FA686CB7492871EF26EB083468F1C89E867`. فحص الحزمة أكد `com.damanak.damanak` و`2.0.0/2`، وعدم وجود توقيع لجذر التطبيق أو `embedded.mobileprovision`؛ توقيعات أطر Flutter المضمّنة فقط موجودة.
- لم تُجرَ تجربة على Android ضعيف أو iPhone حقيقي. يلزم مشروع `Supabase` مستقل لتفعيل الدخول والمزامنة بين الأجهزة، وتوقيع Apple لتثبيت IPA، ومفتاح Android إنتاجي قبل النشر في المتجر.

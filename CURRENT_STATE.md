...
`alpha` بحالة `draft`؛ لم يُعد بناء Android ولم يبدأ الاختبار بعد. مهام Play
الإحدى عشرة ما زالت تمنع تحويله إلى `completed`. جُهزت أصول Google Play
المتعقبة داخل `store-assets/google-play/`: رسم مميز `1024x500` ببصمة
`D1A07F89…6F6F86`، ولقطتا هاتف `1512x2688` ببصمتي
`EC961F64…C8279` و`96DBDD0C…DEDFA`. اللقطتان مشتقتان من صور المتجر والسلة
المتعقبة مع حواف 9:16 فقط؛ لا تحتويان بيانات دخول أو رقم هاتف. لم تُرفع هذه
الأصول ولم يُحفظ أي بيان سياسة في Play Console بعد.

الحقائق المحسومة للإقرارات: التطبيق بلا إعلانات، وليس حكوميًا ولا صحيًا،
ويجمع بيانات الحساب والتسليم والطلبات/الدفع والمحفظة والمرفقات الاختيارية
ورمز الإشعارات والدعم كما في سياسة الخصوصية؛ النقل مشفر. لأنه يسمح بشحن
رصيد المحفظة والدفع منه، يجب إعلان `Mobile payments and digital wallets` لا
`no financial features`. يوجد حذف نهائي داخل التطبيق، لكن صفحات الويب الحالية
تعيد المستخدم إلى التطبيق فقط؛ سياسة Google تتطلب مسار ويب خارجيًا يبدأ طلب
الحذف. قبل الإكمال يلزم من المالك: بريد دعم يوافق على عرضه علنًا ومشاركته مع
`IARC`، موافقة صريحة على شروط `IARC`، وحساب Google تجريبي مخصص قابل لإعادة
الاستخدام بلا OTP أو 2FA لأن المراجع لا يقبل رمز WhatsApp المتغير. لا تُخمن
هذه القيم ولا تستخدم بريدًا شخصيًا من تاريخ المشروع.

أداة LDPlayer الفعلية هي `C:\LDPlayer\LDPlayer9\ldconsole.exe`. أعادت 12
مثيلًا بالأرقام `0..11`؛ تثبيت LDPlayer4 القديم لم يُعد أي مثيل. الجرد الطرفي
الفعلي أثبت 12 حساب Google فريدًا فقط: `z12` بلا حساب، `LDPlayer-1` فيه حسابان،
وكل مثيل من `LDPlayer-2` حتى `z11` فيه حساب واحد. لم تُطبع عناوين أو كلمات
مرور؛ استُخدمت بصمات قصيرة داخليًا لمنع العد المكرر. فُتح ADB مؤقتًا لكل مثيل
ثم أعيد `basicSettings.adbDebug=0` للجميع، وأُغلقت كل مثيلات LDPlayer. بقي AVD
Android Studio `Diagnostic_API_36`/`emulator-5554` متصلًا ولم يُوقف. Google
تطلب رسميًا 12 مختبرًا منضمين باستمرار 14 يومًا؛ العدد الحالي يحقق الحد لكنه
لا يطابق قول المالك بوجود 14، ولم يُعثر على مثيلين إضافيين في تثبيتي LDPlayer.

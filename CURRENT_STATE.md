...
Marker: `2026.08.08-v86.76-iphone-resume-single-recompose`; native version `86.76 / 936`. `npm run build`, the updated iPhone freeze guard, and the low-end budget all pass: largest JS `1,198,131 / 1,200,000`, gzip `354,562 / 370,000`, SHEIN source `544,453 / 550,000`; Android and iOS are synchronized. This is a release candidate only until a physical iPhone 16 completes five background/resume cycles and a force-quit/cold-launch check without a frozen frame. The v86.75 quick-add safe-space fix remains in this candidate; the later toast/size-detection report remains pending verification.

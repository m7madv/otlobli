...
- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+28`. Build `28` مصدر محلي غير مرفوع؛ Build `27` هو الأحدث في iPhone TestFlight، وBuild `23` بقي حزمة App Review وGoogle Alpha من دون تغيير. لا تنسب منع الخفض إلى Build `27` أو `23`، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، ثم نجحت الاستعادة بعد حذف الحساب قبل reset الاختبار. Build `28` لم يُرفع أو يُثبت بعد، وسجل الشراء `sandbox` يبقى لدى Apple حتى يمسحه المستخدم من حساب الاختبار؛ لذلك قد تعيد الاستعادة تفعيله. لا يوجد قبول جهاز لمنع الخفض أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو الجهاز الضعيف.

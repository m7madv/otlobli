...
- المرشح الحالي هو `0.1.0+10` على الفرع `codex/voicebrief-ios`. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
- رفض اختبار iPhone الحقيقي للبناء 9 الانتقال التلقائي وزر `فتح VoiceBrief` بعد نجاح حفظ الملف. هذا يطابق توثيق Apple: Share Extension لا يدعم `NSExtensionContext.open`؛ الدعم على iOS محصور بامتدادي Today وiMessage. أزيل الطلب والزر غير المدعومان، ولم تُضف حيلة `UIApplication.shared` أو responder-chain.
- البناء 10 يسجّل Runner كعارض لـ`public.audio` وأنواع MP3/MPEG-4/WAV الصريحة عبر `CFBundleDocumentTypes`. اختيار VoiceBrief من خيار فتح الملف يجعل iOS يشغّل التطبيق نفسه ويسلمه الملف؛ `SceneDelegate` و`AppDelegate` ينسخان الملف بأمان عند التشغيل البارد أو الدافئ ثم يمررانه إلى Flutter. بقي Share Extension احتياطاً باسم `VoiceBrief Save` ليستقبل من أي تطبيق يقدّم مرفقاً صوتياً/ملفاً عبر Share Sheet، ويحفظه في App Group ويعرض `تم` فقط.
- لا يستطيع التطبيق فرض الانتقال المباشر من كل تطبيق مصدر: iOS والتطبيق المصدر يقرران هل يقدمان الملف كوجهة فتح مباشرة أم كـShare Extension. المساران معاً يوسّعان الاستقبال، لكن امتداد المشاركة نفسه لا يملك API عامة لتشغيل Runner.
...
- البناء 10 متاح الآن في TestFlight. تحذير الرفع الوحيد هو أن Apple ستطلب iOS 15 للبناءات المرفوعة بدءاً من ربيع 2027. يجب التحقق على iPhone من ظهور خيار VoiceBrief الذي يفتح الملف مباشرة، ومن التشغيل البارد والدافئ والاستيراد من WhatsApp وتطبيقات أخرى، ولا يجوز ادعاء قبوله قبل ذلك.

...
## Active candidate — v86.77 iPhone freeze diagnostics (2026-08-09)

This is a diagnostic IPA, not another guess at a fix. iOS SHEIN opens with `SHEIN_IOS_FREEZE_DIAGNOSTICS=true`: the automatic resume `otlobliForceRecompose`, a recompose request from injected JS, and the native loading/privacy covers are deliberately skipped for this one session. Region rules, cart, navigation, the `JSON.stringify` store-region guard, and Android behavior remain unchanged. This isolates the interventions that could have hidden or altered the freeze without turning the whole store into a different app.

The iOS patch persists the last 180 lifecycle records in `UserDefaults`: app resign/active, WebView attachment/window/visibility/bounds/scroll/progress/loading state, page navigation failures, WebContent-process termination, and the document-start page signals (`visibilitychange`, `pageshow`, `pagehide`, `freeze`, `resume`, focus/blur and JS errors). It sends no diagnostic data over the network. A native `LOG` control remains above the WKWebView even if its remote layer is frozen; it copies the complete report to the clipboard and survives a force-quit so it can be pasted in the next chat message.

Marker: `2026.08.09-v86.77-shein-ios-freeze-diagnostics`; native version `86.77 / 937`. `npm run build`, the expanded freeze guard, patch reversibility, and the low-end budget pass: largest JS `1,197,993 / 1,200,000`, gzip `354,513 / 370,000`, SHEIN source `543,347 / 550,000`; Android/iOS are synchronized. GitHub/Xcode [run `31279023108`](https://github.com/m7madv/otlobli/actions/runs/31279023108) passed from `a26817d`. Artifact: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-ios-v86.77-diagnostics\otlobli-ios-v86.77-iphone16\otlobli-v86.77-iphone16-unsigned.ipa` (7,070,394 bytes, SHA-256 `2B2468B46CF50E74FD19B259D8EAAC2A7DF5462CD5040A4D9BEE5932D8860609`); archive inspection confirms `86.77 / 937`. This flag must be turned off before any normal release; no iPhone acceptance is claimed. When a freeze occurs, tap `LOG` before switching stores, restarting, or applying any recovery, then paste the copied report.

...
نشط حتى توفير الحساب الثاني عشر وتحويل نفس الإصدار إلى `completed`. `z12` لم
يحتو حسابًا نظاميًا. بطلب صريح من المالك حُذفت مثيلات `LDPlayer-9` و`z10`
و`z11` نهائيًا لأنها كانت تحمل الحسابات الثلاثة المستبعدة؛ تحقق `list2` أن
المتبقي هو `z12` و`LDPlayer-1..8` فقط. `z12` مفتوح وحده الآن على شاشة تسجيل
دخول Google ليُدخل المالك الحساب النظيف الثاني عشر بنفسه، وADB مفعّل مؤقتًا
فيه فقط (`basicSettings.adbDebug=1`). لا تُدخل كلمة المرور أو تعرضها؛ بعد نجاح
الإضافة تحقق طرفيًا من وجود حساب واحد، ثم أعد ADB إلى `0` وأغلق المثيل. بقي
AVD Android Studio المنفصل دون تغيير.

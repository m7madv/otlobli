...
- التطبيق يشمل الدخول، التسجيل والاستيراد، ومسار مشاركة واتساب واضح من ثلاث خطوات. ملفات `.opus` وأسماء واتساب غير القياسية تُطبّع إلى OGG، ويصل الملف المشترك إلى شاشة «جاهز» ذات إجراء رئيسي واحد. النتائج تعرض الملخص والنقاط والمهام والمواعيد؛ كل موعد يُراجع ثم يفتح محرر تقويم النظام، ولا توجد كتابة صامتة مبنية على تخمين AI.
- مشروع Supabase المستقل الحي هو `jyehqpdbayslhzebdycj`. نُشرت هجرتا `20260824010000_voicebrief_initial.sql` و`20260824020000_lock_server_rpc_permissions.sql`، ودوال `process-audio` و`delete-account` و`revenuecat-webhook`. حصة الخدمة العامة ذرية ومحدودة بـ500 دقيقة صوت مقربة شهرياً، وفشل اختبار anon عند صلاحيات RPC كما يجب. نُشرت نسخة `process-audio` الجديدة التي تستخدم فرق توقيت المستخدم لفهم «غداً/الخميس».
- رصيد OpenAI الحي `$10.00` وإعادة الشحن التلقائي متوقفة. النموذجان الافتراضيان هما `gpt-transcribe` و`gpt-5.6-luna`. نافذة إنشاء مفتاح service account باسم `VoiceBrief Supabase Production` جاهزة ولم يُضغط الإنشاء بعد؛ يلزم تأكيد المستخدم الفوري ثم حفظ المفتاح في Supabase وإجراء smoke test. لم يُستخدم Gemini المجاني لأن شروط الخدمة غير المدفوعة تسمح باستخدام المحتوى لتحسين منتجات Google/ML.
- نجح `flutter analyze` بلا ملاحظات، و16 اختبار وحدة/واجهة، و8 golden مراجعة بصرياً، واختبار التكامل على API 35، وفحوص Deno. اختبار Android instrumentation بملف Ogg/Opus حقيقي وURI من `ContentProvider` نجح في التشغيل البارد والدافئ على محاكي API 35 وعلى هاتف `SM_N950F` بنظام Android 9؛ أزيلت حزمة مزود الاختبار من الجهازين بعد النجاح.
- Live-config Android APK: `voicebrief_flutter/build/app/outputs/flutter-apk/app-debug.apk`؛ الحجم `213,017,571` بايت؛ SHA-256 `4780E074EA8EFD1636BD1078DC7655E6AA208B239D5F70874414E30C5FBD327D`. Release-mode AAB: `voicebrief_flutter/build/app/outputs/bundle/release/app-release.aab`؛ الحجم `56,293,386` بايت؛ SHA-256 `3F7C2246791B85584F4E416003CDE77F2D78BE1B3C5A905FB1E96E19FDD4AA14`. كلاهما يستخدم مفتاح Android التجريبي وليس صالحاً للرفع إلى المتجر.

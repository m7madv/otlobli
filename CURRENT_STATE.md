...
## VoiceBrief — توسيع اللغات وصور المتجر، 2026-09-06

- المصدر الحالي `0.1.2+21` على `codex/voicebrief-ios` في `C:\Users\MOHAMMAD\Projects\VoiceBriefAuthRepair`. هذه متابعة جديدة للغات، ولا تعيد نشر البناء 20 أو تمس المحاكيات أو Otlobli.
- الواجهة أصبحت 11 لغة: العربية والإنجليزية والصينية المبسطة والهندية والإسبانية والفرنسية والبنغالية والبرتغالية والروسية والأردية والإندونيسية. توجد 266 رسالة لكل لغة، واختيار محفوظ في الإعدادات مع اتباع ترتيب لغات النظام عند عدم الاختيار. العربية والأردية من اليمين إلى اليسار. تمت إضافة نصوص المشاركة وأذونات iOS وبيانات اللغات الأصلية للمنصتين.
- أُزيل افتراض لغة التسجيل من لغة الجهاز في العميل وامتداد المشاركة؛ نص الخادم يدعم حفظ لغة الصوت وتلخيصه بها، مع إبقاء خيار الترجمة الإنجليزية وقواعد التواريخ العربية. نشر هذا التعديل على `jyehqpdbayslhzebdycj` قيد التحقق؛ لا تغييرات على الأسرار أو قاعدة البيانات.
- جاهزة 88 صورة متجر: أربع صور فعلية من عناصر واجهة Flutter لكل لغة ولكل جهاز، بمحتوى توضيحي غير شخصي. iPhone: `1284×2778`، iPad: `2064×2752`، تحت `voicebrief_flutter/store_assets/localized/`. لم تُلتقط على أجهزة حقيقية ولم يُخترع فيها تصميم بواسطة الصور المولدة. `manifest.json` يسجل المسارات والبصمات والترجمات ورموز أبل بما فيها `bn-BD` و`ur-PK`.
- نجحت اختبارات اللغات الجديدة (15) وتوليد الصور (88) وفحوص Deno الخاصة بالصوت والتواريخ (13). الفحص الكامل وبناء Android وCI/Xcode وربط المسودة قيد التنفيذ؛ لا تدّعِ قبول جهاز فعلي أو مراجعة لغوية بشرية لكل لغة.
- مسار `voicebrief-localize-store.yml` يستعلم فقط عند الدفع؛ وضع `prepare` الصريح يعد مسودة `0.1.2` ويحفظ الصور والترجمات دون اختيار بناء أو إرسال مراجعة أو نشر أو تغيير البلدان. الربط الفعلي بأبل لم يؤكد بعد. تفاصيل التجديد في `voicebrief_flutter/docs/LOCALIZATION.md`.
- أكد استعلام أبل `34036838798` أن 0.1.1 حالته `READY_FOR_SALE`؛ معلومات انتظار البناء 20 أدناه تاريخية. التحديث 0.1.2 (21) لم ينشر أو يرفع إلى TestFlight.
- نشر process-audio نجح: الإصدار17 ACTIVE وبصمة الحزمة `446356916b1076bcdf2d9985c2d05030d033c3c0b7b70594e82bfdb1e8f25910`. عُزل ملف CLI `profile` مؤقتًا وأُعيد كما كان بعد استدعاء supabase-go دون --profile؛ لم تتغير أسرار أو حسابات أو مشاريع أخرى.
- نجاح محلي: analyze و70 اختبارًا و12 golden و88 صورة وبناء APK debug (177862565 بايت، SHA256 `DD13296F11026A7E72E36D08320264806781D536F0BC2D0AFC57287FC8A6118A`). هذا APK قبل تعديل احتواء اسم التطبيق في شاشة الدخول.
- الالتزام `dad1bb2` دُفع؛ CI34036838778 كشف overflow قدره1.5px في اسم التطبيق بشاشة دخول عربية مكبّرة على Linux، وعولج بإضافة Flexible دون تخفيض اختبار التكبير. فحص Xcode قيد التنفيذ.
- أنشأ تشغيل34036907851 مسودة0.1.2 وقبل صور العربية الثماني بحالة COMPLETE. توقف عند إنشاء الإنجليزية بـ`STATE_ERROR.DUPLICATE_NAME.DIFFERENT_ACCOUNT`: اسم VoiceBrief غير متاح لهذه الترجمة. طُلب من المالك اعتماد اسم وصفي محلي للمتجر؛ لا تغيير للاسم أو رفع بقية اللغات قبل جوابه. الصور العامة للنسخة السابقة لم تُمس.
- تجميع Xcode نجح في34036838778؛ فشل خطوة التغليف كان بسبب تحقق قديم يطلب0.1.1(20). حُدثت توقعات CI إلى0.1.2(21) وأُضيف تحقق وجود وصحة ملفات اللغات الإحدى عشرة في التطبيق وامتداد المشاركة. إعادة CI قيد التنفيذ. مسارا الرفع الموقّع إلى TestFlight وGoogle ما زالا مثبتين على20 عمدًا ولم يشغّلا؛ يجب تحديثهما عند طلب رفع21 دون إطلاق تشغيل دفع تلقائي غير مقصود.
- أكد تشغيل34037395642 نجاح iOS كاملًا، بما فيه ملفات اللغات والتغليف؛ كشفت بقية الفحوص ازدحام زر Pro بالنص البديل على Linux واختلاف صور مرجعية بعد ضبط الاحتواء. حُدّ عرض زر Pro بنصف عرض شريط البداية مع التفاف النص، وراجعت صور الدخول والبداية ثم حُدثت مراجعها الثلاثة فقط. نجح اختبار إعادة إنتاج العربية بخط Ahem، و15 اختبار لغات موسعًا يشمل الاشتراك والمنبهات والسجل، و12 golden وanalyze. الجولة السحابية النهائية قيد الإرسال.

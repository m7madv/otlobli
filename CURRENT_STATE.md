...
- اكتمل فحص أمني قياسي للنسخة السابقة `9f228c1` برقم `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` ووثائقه في `voicebrief_flutter/docs/security/build17/`. ثبّت 6 نقاط: عزل جلسة/نتيجة App Group عند تبديل الحساب، عدم الثقة بمدة الصوت من العميل، منع الرفع المباشر غير المحدود، إنفاذ انتهاء النتائج، جعل حد نموذج الدعم ذرياً، وقياس جسم الطلب الفعلي. نُفذت المعالجات في مصدر build 17؛ يبقى تجميع Swift على CI وقبول iPhone والتحقق التشغيلي من التنظيف المجدول.
- خادم build 17 يضيف حجز رفع ذرياً محدوداً ومسار `create-audio-upload` بتوقيع دقيق، ويستخرج مدة الصوت من الملف على الخادم قبل حجز الرصيد، ويحمي أجسام JSON/form المرسلة فعلياً، ويعزل نتائج Share Extension بالـuser والجلسة generation. migration `20260830190000_security_hardening.sql` ودوال `create-audio-upload` و`process-audio` و`legal` و`delete-account` ما زالت محلية ولم تُنشر؛ جلسة Supabase الحالية ترى Damanak فقط. يجب النشر حصراً إلى `jyehqpdbayslhzebdycj` بعد دخول الحساب الصحيح، ولا يجوز رفع TestFlight 17 قبله.
- التحقق المحلي الحالي ناجح: format على 78 ملفاً، analyze بلا مشكلات، 42 اختباراً غير golden، 12 golden، 3 اختبارات JSON محدودة، 10 اختبارات تواريخ/مدة صوت، 3 اختبارات form محدودة، وكل `deno lint/check`، وAndroid debug. APK حجمه `177,653,885` بايت وبصمته SHA-256 `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`. لم يُرفع build 17 بعد.
...
- مشروع Supabase الصحيح حصراً `jyehqpdbayslhzebdycj`. نُشر `process-audio` بحالة `ACTIVE`، الإصدار `11`، المعرّف `e4a26c2e-b9f2-43d3-9a6b-61467b8c8747`، وبصمة الحزمة `cd2724f53c9acfe07e007db786a632a9a64ee797e3062345060a5cef17bc1bd8`. السر `OPENAI_TRANSCRIPTION_MODEL` مضبوط على `gpt-4o-mini-transcribe` مع الحفاظ على بقية الأسرار. لا تنشر إلى Damanak أو `talabieh`. ملف اختيار Supabase القديم حُفظ قابلاً للاسترجاع في `C:\Users\MOHAMMAD\.supabase\profile.voicebrief-cli-2.107.backup` لأن CLI `2.107.0` يتعطل عند وجوده.

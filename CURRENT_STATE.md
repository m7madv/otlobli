...
`CCFC71D2EE3A4E40C86DEF9E102C4C5846ED5F0D9E68DEEB4D67CCC1742BBBE6`.
أضيف سجل تفويض exact-build بوضع `inspect` ومسار GitHub يستخدم حساب خدمة Google
المحمي لقراءة أسماء المسارات والإصدارات وأعداد المختبرين من دون نشر، ولا يعيد
بناء Android. التشغيل `33308484614` نجح في مصادقة حساب الخدمة ثم أعاد Google
عند أول edit الخطأ `404 Package not found: com.otlobli.app`؛ لم يُرفع AAB ولم
يتغير مسار أو إصدار. فحص Play Console أكد أن الحساب يملك أربعة تطبيقات فقط ولا
يحتوي سجل Otlobli. اسم الحزمة `com.otlobli.app` متاح، ونموذج إنشاء تطبيق عربي
مجاني باسم `Otlobli` مُعبأ لكنه لم يُرسل: يلزم أن يوافق المالك صراحة على إقرار
سياسات المطورين وإقرار قوانين التصدير الأمريكية الظاهرين في النموذج. حساب
الخدمة موجود لكنه مقيّد حاليًا بتطبيقي Damanak وVoiceBrief؛ بعد إنشاء السجل يجب
إضافة Otlobli له بصلاحيتي القراءة وإصدار تطبيقات لجمهور الاختبار، ثم إعادة
`inspect` وتثبيت اسم closed track قبل التحويل إلى `publish`.

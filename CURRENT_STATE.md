...
- المرشح الحالي هو `0.1.0+9` على الفرع `codex/voicebrief-ios`. التعديل محصور في `voicebrief_flutter/` وملفات GitHub الخاصة به؛ لم تتغير مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
...
- نجح التحقق المحلي الحالي: `dart format --set-exit-if-changed .` على 70 ملفاً، و`flutter analyze --no-pub` بلا مشكلات، و30 اختباراً غير golden، و11 golden، وDeno format، وخمسة اختبارات تاريخ، وDeno check. نجح CI النهائي run `33280858741` من commit `05d9c3e` بجميع وظائف Android APK وiOS غير الموقع وgolden وDeno؛ CI البناء 9 السابق run `33031413075` ناجح أيضاً.
...
- تحذير الرفع الوحيد هو أن Apple ستطلب iOS 15 للبناءات المرفوعة بدءاً من ربيع 2027. لم يظهر iPhone في `flutter devices`؛ اختبار مشاركة WhatsApp والفتح المباشر على iPhone حقيقي ما زال مطلوباً ولا يجوز ادعاء قبوله.

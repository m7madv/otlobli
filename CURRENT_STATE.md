...
## مشروع ضمانك المستقل 4.2.0+16 (2026-08-27)

- كشف الفحص الحي [33025708724](https://github.com/m7madv/otlobli/actions/runs/33025708724) أن تقرير Apple السابق لم يكن يقرأ الأسعار الفعلية، بل كان يعرض القيم المعتمدة في السكربت فقط. كما توقفت محاولة التطبيق [33025892341](https://github.com/m7madv/otlobli/actions/runs/33025892341) قبل الاشتراكات بسبب تعارض اسم ملفات provisioning قديمة. عُدّل `app_store_setup.mjs` ليقرأ حالة مراجعة كل اشتراك وأسعار الدول الست الفعلية وسعر قطر، ويفصل تنظيف ملفات التوقيع عن إعداد الكتالوج حتى لا يمنع تفعيل الأسعار. لم يُثبت ظهور كتالوج iPhone بعد؛ يلزم تشغيل الأداة المصححة ثم بناء TestFlight جديد واختباره.
- `damanak_flutter/` مصدره الآن `4.2.0+16` بمعرّف `com.damanak.damanak`. استُبدل مسار OAuth الذي كان يفتح المتصفح على iOS بتسجيل Apple الأصلي ونافذة اختيار حساب Google الأصلية، ثم تُرسل رموز الهوية مباشرة إلى Supabase مع nonce آمن لـApple. يبقى مسار المتصفح للويب فقط، وعلى Android يبقى Apple عبر الويب بينما Google أصلي.
- أصلح Build `16` كتالوج اشتراكات Android من دون الرجوع إلى Billing 7: يستعلم التطبيق عن `ProductType.subs` مباشرة عبر عميل Billing 8 المتصل، فلا تُسقط نتيجة اشتراكات صحيحة بسبب فشل استعلام منتجات `inapp` غير المستخدم. أضيف اختبار تحويل تفاصيل اشتراك Google، واجتاز `flutter analyze` وكل الاختبارات وعددها `37`. المصدر مرفوع في `72d6324` وتصحيح فحص توقيع CI في `63026e6`.
- نجح GitHub run [33016989476](https://github.com/m7madv/otlobli/actions/runs/33016989476) وبنى ووقّع ونشر الإصدار الداخلي `Damanak build 16` بحالة `completed`. ملف AAB هو `damanak_flutter/output/github-run-33016989476/app/outputs/bundle/release/app-release.aab`، حجمه `55,921,147` بايت وبصمته `6454EF23BD9CC5B7AF5C1EA18C2B65016554D395D0261BDB2BB28008CE03032F`.
- حدّث Note 8 التطبيق من Google Play نفسه إلى `4.2.0 (16)` مع بقاء المثبّت `com.android.vending`. ظهر اتصال Google Play والأسعار القطرية الستة فعلياً: بداية `33/330 QAR`، نمو `84/835 QAR`، وتوسع `170/1700 QAR` شهرياً/سنوياً. فتحت نافذة Google Play الأصلية لخطة بداية الشهرية بسعر `33.00 QAR/month` وزر الاشتراك، ثم أُغلقت بلا تأكيد أو خصم. كما نجح استعلام استعادة المشتريات للمنتجات والاشتراكات بنتيجة سليمة ولا توجد عملية شراء سابقة. هذا قبول للكتالوج وبدء الشراء والاستعادة، وليس قبولاً للدفع الفعلي أو تفعيل الاستحقاق الخادمي.
- قائمة الاختبار الداخلي تضم حالياً مختبراً واحداً هو حساب Note 8، وقد قُبل رابط الانضمام. أُعيد إعداد شبكة الجهاز إلى `DHCP` بعد الاختبار وحُذف العنوان الثابت المؤقت؛ الموجّه لم يمنح الجهاز عنواناً تلقائياً، وهو عطل الشبكة السابق لا إعداداً باقياً على الهاتف.
- أنشئ Google OAuth client لـiOS ومعرّفه العام `521030062021-lupo3mel3si0nmu9tijcblo13d4sqg4d.apps.googleusercontent.com`، وعميل Android باسم `Damanak Android Play` مربوط بالحزمة وبصمة شهادة Play الفعلية `97:C9:F4:40:66:30:32:DD:EC:89:FC:E9:CA:E7:42:FD:F6:F3:46:76`. اكتملت صفحات Branding، وانتقل Google OAuth من `Testing` إلى `In production` للمستخدمين كافة.
- نُشرت دالة Supabase العامة `legal` بلا JWT على المشروع `exxayzlklvgeyqhvtzgi`، وتعيد صفحات الدعم والخصوصية والشروط الثلاث بحالة HTTP `200`. رُبطت بها شاشة موافقة Google. نجح `flutter analyze` و35 اختباراً وweb release وAndroid debug؛ APK حجمه `178,201,600` وبصمته `C072A9EEFDFFD9E893DBF7257E51948DCFAD8603FC926A8706EF776E7B9E0ED2`. قبول Apple وGoogle الأصليين على جهاز حقيقي ما يزال مطلوباً.
- رُفع مصدر الدخول الأصلي في commit `eb20232`، ثم صُحح تحقق الحزمة في commit `bfac976`. نجح iOS signed/upload run `32934507382` في التحليل والاختبارات والبناء والتوقيع والتحقق من `GoogleSignIn.framework` و`sign_in_with_apple.framework` ورفع الحزمة إلى Apple بلا أخطاء. أنهت Apple معالجة Build `14` بحالة `Complete`، وهو ظاهر في TestFlight بحالة `Ready to Submit` ومضاف تلقائياً إلى `Damanak Internal`. IPA المحلية `damanak_flutter/output/github-run-32934507382/ضمانك.ipa` حجمها `23,118,604` وبصمتها `373FDCC76FB92EC4E50341DC974B4314074A6D38A79F00185EAED63382231AD8`. يبقى قبول نافذتي Apple وGoogle والأسعار والشراء على الجهاز الحقيقي.
- أكد المستخدم نجاح تسجيل Apple الأصلي فعلياً على iPhone في Build `14`، بينما فشل Google بعد اختيار الحساب برسالة عامة. أصلح Build `15` المسار بإرسال `accessToken` المطلوب، وnonce عشوائي مشفر إلى Google مع قيمته الأصلية إلى Supabase، من دون تعطيل فحص nonce. نجح التحليل و36 اختباراً وweb release وAndroid debug؛ APK حجمه `178,202,056` وبصمته `9F87177312A3EB7B9B054C6327AD1CB1AA81FC071EA76CFA1CC96024DAB98C44`.
- رُفع commit `15c8044`. نجح iOS signed/upload run `32949319708` في التوقيع وفحص الحزمة والرفع. Apple أنهت معالجة Build `15` بحالة `VALID` ومعرّفه `73ee565c-4191-495f-9684-3629fbb77c61`، وهو داخل `Damanak Internal` ذات 11 بناءً ومختبراً واحداً. IPA المحلية `damanak_flutter/output/github-run-32949319708/ضمانك.ipa` حجمها `23,122,882` وبصمتها `B5EF04F43C64AA1F6D81AD8A710F4CD08128C821309AC4B6993C79C3C8A621DC`. قبول Google على الجهاز ما يزال مطلوباً.
- أصلح commit `15c8044` أيضاً فصل تطبيق الأسعار عن وضع إنشاء الموارد. نجح run `32949324011` في تثبيت توفر المنتجات الستة صراحة في `SAU/ARE/BHR/KWT/OMN/QAT`، وأكد وجود السعر والمراجعة لكل منتج من دون استبدال الأسعار الحالية. أسعار قطر المؤكدة: `39.99/399.99/79.99/799.99/199.99/1999.99 QAR`. إذا بقي `APPLE-CATALOG-0-QAT` بعد انتشار هذا التحديث، تحقّق من أن Paid Apps Agreement حالته `Active` وأن البنك والضرائب مكتملان، ثم صعّد إلى Apple وفق TN3186.
- أثبتت لقطة المستخدم من Build `13` الرمز `APPLE-CATALOG-0-QAT`: Storefront قطري لكن Apple أعادت صفرًا من المنتجات الستة. فحص App Store Connect أكد أن المنتج الشهري الأساسي معروض للبيع، ومترجم، ومتاح في قطر بسعر `39.99 QAR`؛ لذلك لم يعد تغيير دولة الحساب أو تكرار التحميل حلاً مثبتاً، ويبقى تصعيد كتالوج Sandbox إلى Apple إذا استمر بعد Build `14`.
- أكد App Store Connect أن المستخدم ثبّت Build `12` فعلياً على `iPhone 16 Pro Max / iOS 27.0`، ثم أكد المستخدم أن الاشتراك بقي غير متاح؛ لذلك لا يُعد Build `12` قبولاً لجلب الأسعار أو الشراء.
- يقرأ Build `13` دولة Storefront الفعلية بالتوازي مع طلب المنتجات، ويعيد الطلب الفارغ مرة واحدة على دفعتين من ثلاثة معرّفات بمهلة محدودة ومن دون polling. عند استمرار الصفر يعرض `APPLE-STOREFRONT-...` إذا كان الحساب خارج الخليج أو `APPLE-CATALOG-0-...` إذا كان المتجر خليجياً، مع خطوة تعافٍ واضحة وزر إعادة المحاولة. اجتاز `flutter analyze` و32 اختباراً وweb release وAndroid debug؛ APK حجمه `177,327,571` وبصمته `12EAB7EF3FA282B54E62CDFA3163CDC0ACA0C1883D43E7035F39F61C33573E37`. قبول الأسعار والشراء ما يزال مطلوباً على الجهاز الحقيقي.
- commit `d38da06` مرفوع. نجح iOS signed/upload run `32931311918` في البناء والتوقيع والتحقق والرفع. IPA المحلية `damanak_flutter/output/github-run-32931311918/damanak-ios-4.2.0-signed/ضمانك.ipa` حجمها `21,868,751` وبصمتها `245F7A206A108262C51A90B2F869647E959585591F2C66698408D535A51D55E6`. فحص Apple `32931995628` أكد Build `13` بحالة `VALID` ومعرف `aa95a332-aea8-4160-88b2-e92f555c018e` داخل `Damanak Internal` التي تضم 9 أبنية ومختبراً واحداً و`hasAccessToAllBuilds=true`.
- أصبح iOS يختار `StoreKit 1` قبل إنشاء `InAppPurchase.instance`، فتستخدم قراءة المنتجات والشراء والاستعادة والإكمال مساراً واحداً بدلاً من تسجيل `StoreKit 2` ثم محاولة استعلام جانبي. يبقى معرّف Supabase UUID في `applicationUsername`، وتؤكد Apple أنه يعود في `appAccountToken`، لذلك لم يُضعف تحقق الخادم. كما يعاد جلب المنتجات تلقائياً بعد إنشاء المتجر أو الانضمام إليه. نجح `flutter analyze` و30 اختباراً وبناء الويب وAndroid debug؛ APK التحقق حجمه `177,322,947` وبصمته `E4570C6CFE291A96B204ADF63310573B010F188098BA2187A88CDFB7380D0419`. لم يُقبل ظهور الأسعار أو شراء Sandbox بعد على جهاز حقيقي.
- رُفع مصدر الإصلاح في commit `e9ba841`. نجح iOS signed/upload run `32927589588` في التحليل والاختبارات والبناء والتوقيع والتحقق والرفع إلى Apple. IPA المحلية `damanak_flutter/output/github-run-32927589588/ضمانك.ipa` حجمها `21,866,548` وبصمتها `DFAB58195A165B9C6F8C4ECEAAC14F77D2056F1C1339954B0CC5A425049502BC`. أكد فحص Apple النهائي `32928313864` أن Build `12` حالته `VALID` ومعرفه `70c61531-6595-4878-a363-4564cca62bf2`، وأن `Damanak Internal` تضم 8 أبنية ومختبراً واحداً مع `hasAccessToAllBuilds=true`.
- أزيل انتظار Supabase واستعادة الجلسة قبل `runApp`: يرسم التطبيق أول إطار فوراً، ثم يهيئ الخدمات بعده ويعرض حالة تحميل فعلية أو رسالة تعافٍ وزر «إعادة المحاولة». توحدت خلفية الإقلاع الأصلية وواجهة Flutter على أخضر `#087F5B`، فلا تبقى شاشة بيضاء أثناء انتظار الجلسة.
- استُبدلت أيقونة Flutter الافتراضية بأيقونة ضمانك من لونين فقط: باركود يتحول إلى درع وعلامة تحقق. الأصل `1024×1024` في `damanak_flutter/assets/branding/damanak_app_icon_master.png`، ومقاسات iOS وAndroid وشارة الإقلاع مولدة عبر `tool/generate_brand_assets.dart`. أعيد رسم `BrandMark` نفسه، وهو العنصر الذي يظهر أعلى اليمين في الواجهات الصغيرة.
- نجح `flutter analyze` و28 اختباراً وبناء الويب وAndroid debug. روجع الشعار العلوي بصرياً عند `390×844`، وAPK التحقق build `11` حجمه `177,357,603` بايت وبصمته `7185A48466F4A285BD096DFDE3A32A7722B4749CBF2CF50C82CAC948D7EC33A8`. الأصل البصري النهائي بصمته `C93D50BF9C96242A53D28F1385207F9B1FAF9CD123CAE088B2925C00EC04A185`.
- رُفع commit `1a2e9e0` إلى `codex/damanak-simple-pos`. نجح iOS signed/upload run `32913232810`: التوقيع والتحقق والرفع بلا أخطاء، وIPA المحلية `damanak_flutter/output/github-run-32913232810/ضمانك.ipa` حجمها `21,836,890` وبصمتها `D08EB42EC3B15CC4A0F6952EE643FFB11D860AA66C8E55693A16EAA4751ED2B5`. أكد فحص Apple run `32914401544` أن build `11` رقم `be65a9ad-0381-4892-ad10-fb200ad4489d` حالته `VALID` وموجود ضمن `Damanak Internal` ذات 7 أبنية ومختبر واحد.
- عند إضافة منتج متسلسل من شاشة البيع تظهر ورقة قصيرة تجعل «مسح الرقم التسلسلي» الإجراء الأول، وتبقي الكتابة اليدوية تحته. المسح الناجح يعيد القطعة مباشرة إلى السلة، ويُرفض تكرار الرقم نفسه داخل السلة دون اعتبار لحالة الأحرف. لا تُفتح لوحة المفاتيح تلقائياً ولا يُطلب إذن الكاميرا قبل ضغط زر المسح.
- نجح `flutter analyze` و26 اختباراً وبناء الويب وAndroid debug. يشمل ذلك إدخال رقم يدوي، منع التكرار، وتخطيط `320×568` مع تكبير نص `200%`. روجعت الورقة بصرياً عند `390×844` و`320×568` بلا قص. APK التحقق build `10` حجمه `203,229,255` بايت وبصمته `2D3689801912E9AD7D8085D625BCA2647150C27242755D3641ACE7D5723C5766`؛ يبقى مسح ملصق حقيقي وقبول قارئ الشاشة والجهاز مطلوباً.
- رُفع commit `d9ff4c2` إلى `codex/damanak-simple-pos`. نجح iOS signed/upload run `32908893461` في التحليل والاختبارات والبناء والتوقيع وفحص الملف المضمّن والتحقق والرفع. IPA المحلية `damanak_flutter/output/github-run-32908893461/ضمانك.ipa` حجمها `21,797,715` وبصمتها `4E6EB098B5D8F6C4F536E7BF23A792B8EBD694604E42E6D007323EBFFB787ACA`. أكد فحص Apple run `32909673563` أن build `10` رقم `3a3390ce-5002-4e25-aa57-9c40ed071323` حالته `VALID` وموجود في `Damanak Internal` ذات 6 أبنية ومختبر واحد.
- أُعيد بناء المسار التشغيلي ليبدأ مباشرة من «البيع» ضمن أربع وجهات فقط: البيع، المنتجات، الضمانات، والمزيد. إضافة السلة بلمسة، والعميل لا يظهر إلا بطلب البائع أو عند وجود ضمان، والخصم وتقسيم الدفع والملاحظات مطوية. نموذج المنتج يبدأ بالاسم والسعر والكمية وباركود اختياري، وباقي الحقول متقدمة ومطوية.
- أزيلت الضريبة من شاشة البيع والإيصال والمنتج والمورد والإعدادات والتقارير وملف CSV. العمليات الجديدة تُنشأ بضريبة صفر، مع إبقاء أعمدة السجلات التاريخية للتوافق وعدم إعادة كتابة مبيعات قديمة. نُشر الترحيل `20260825190000_damanak_simple_receipts.sql` على مشروع الإنتاج `exxayzlklvgeyqhvtzgi`، فأصبحت إعدادات المتاجر الحالية والجديدة صفراً مع قيد قاعدة بيانات. النسخة الحالية إيصال بيع داخلي وليست نظام فاتورة ضريبية خليجياً؛ يلزم امتثال مستقل لكل دولة قبل تسويقها كبديل للفوترة الرسمية.
- نجح `flutter analyze` و24 اختباراً وبناء الويب وAndroid debug. روجعت الواجهة بصرياً عند `390×844`: شاشة البيع والسلة والدفع والمنتجات وإضافة المنتج بلا قص، باتجاه عربي ولون تفاعل أخضر واحد. APK التحقق `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` بحجم `177,288,591` بايت وبصمة SHA-256 `67419811746E9881ABA7E8D521B777869DCD9D3074125A1ED317E45F5FDA9F7A`، وفحصه يؤكد `com.damanak.damanak` و`4.2.0 (9)`. هذا Debug بلا مفاتيح الإنتاج؛ لا يوجد بعد قبول قارئ شاشة أو هاتف حقيقي.
- بُني Android Release محلياً بإعدادات Supabase الحية ومفتاح الرفع الحالي ذي بصمة الشهادة `BF0B373699911FDAB8BD46DE90664D0915CED35E2ED046A6AD4E4B030F814574`. APK `build/app/outputs/flutter-apk/app-release.apk` حجمه `74,240,126` وبصمته `3F935F4C3A4FB1782BC1F4E564E4D0B19876FD35EDCFE1015AE2CBE5DE7C6097`، وAAB `build/app/outputs/bundle/release/app-release.aab` حجمه `55,400,119` وبصمته `C10030D4FEE1ABB9E2CB3CB7A4DBD5A0A36FD720E6C0E1CDCFA1A9800985F9C6`. فحص APK يؤكد v2 و`4.2.0 (9)`، وفحص JAR لـAAB نجح. عُدّل `build_signed_android.ps1` حتى يستخدم مفتاح v2 الحالي لا المفتاح القديم.
- نجح GitHub iOS signed run `32884908928` عند commit `7b9c90a`: تحليل و24 اختباراً، بناء `4.2.0 (9)`، ملف `Damanak App Store 2026`، معرّف `com.damanak.damanak` واستحقاق Apple Sign-In. IPA المحلية `damanak_flutter/output/github-run-32884908928/ضمانك.ipa` حجمها `21,792,793` وبصمتها `0359FC51C40C432457C9D419EC4540179C40F3C3DCCC7DC1597D32D446BAAE12`. كان هذا التشغيل build-only؛ الرفع الفعلي اللاحق نجح عبر run `32886635323` عند commit `a7c6799`.
...
- الواجهة تحافظ على أخضر واحد ومحايدات. روجعت شاشة الدخول في الوضعين الفاتح والداكن وورقة دعوة الموظف عند `393×852`، وشاشة الاشتراك عند `1280×720` و`390×844`، بلا تجاوزات. حالة جلب الأسعار لا تبقى معلقة: بعد مهلة محددة يستخدم iOS استعلام StoreKit 1 لأسعار المنتجات إذا لم يُجب StoreKit 2، ثم يعرض خطأ نصياً وزر «إعادة المحاولة» إذا فشل المساران. نتيجة Apple HIG والإتاحة موثقة في `docs/DESIGN_AUDIT.md`.
...
- نجح `flutter analyze` و23 اختباراً. بُنيت AAB إنتاجية build `7` بالإعدادات الحية والتوقيع المحلي: `damanak_flutter/build/app/outputs/bundle/release/app-release.aab`، `55,775,502` بايت، SHA-256 `88073698D408A62D5D6C548C0B2BFD0320D4B5CCB2EA281381853917CC5C1F72`. شهادة رفع Android SHA-256 `BF0B373699911FDAB8BD46DE90664D0915CED35E2ED046A6AD4E4B030F814574`. المفتاح المحلي وكلمة مروره المشفرة بـDPAPI ما زالا خارج Git، ونسخته المشفرة وكلمة المرور والاسم المستعار محفوظة الآن في GitHub Secrets بالأسماء `DAMANAK_ANDROID_UPLOAD_KEYSTORE_BASE64` و`DAMANAK_ANDROID_UPLOAD_KEY_PASSWORD` و`DAMANAK_ANDROID_UPLOAD_KEY_ALIAS`.
...
- البناءان `5` و`6` في TestFlight لا يحتويان إعدادات Supabase ولا يجوز استخدامهما. كشف الاختبار الحقيقي أن build `7` قد يبقى في «جارٍ جلب أسعار المتجر» لأن طلب StoreKit 2 بلا مهلة، وعولج ذلك في build `8`. النسخة الحالية هي `4.2.0 (9)`؛ رفعها run `32886635323` بنجاح. أكد فحص App Store Connect المباشر عبر run `32887730020` أن build ID `c1eb37ba-508a-40e4-9d0e-3ab00b35da5a` حالته `VALID` وموجود ضمن مجموعة `Damanak Internal` التي تملك `hasAccessToAllBuilds=true` وتضم مختبراً واحداً. يبقى تنزيل build `9` واختبار الأسعار وشراء Sandbox والاستعادة على iPhone حقيقي مطلوباً.
- Google Play: سجل التطبيق `Damanak - ضمانك` رقمه `4975804725938350009`. فُعّلت Android Publisher API، وأضيف حساب الخدمة `damanak-play-verifier@damanak-production.iam.gserviceaccount.com` بأذونات التطبيق المحدودة، وحُفظ JSON في Supabase وGitHub. حُذف المفتاح اليتيم `d86a3e51984e7ba7b942f00595a37c268b367316` نهائياً بعد موافقة المستخدم، وبقي المفتاح العامل `426377cca3335db2db9a17611153fb1b37e92a1d` وحده نشطاً. الإصدار الداخلي الحالي `4.2.0 (16)` منشور، وأنشئت وربطت قائمة `Damanak Internal` التي تضم حساب Note 8 فقط. رابط الانضمام `https://play.google.com/apps/internaltest/4701722430109624070`.
- أُنشئت منتجات Google Play الثلاثة نهائياً عبر Play Console: `com.damanak.subscription.starter` و`com.damanak.subscription.growth` و`com.damanak.subscription.scale`. في 2026-08-26 أُنشئت يدوياً وفعّلت خطتا `monthly` و`yearly` لكل منتج، مع حصر التوفر في الإمارات والبحرين والكويت والسعودية وعُمان وقطر. الأسعار السعودية النهائية التي قرّبها Google إلى نقاطه الجذابة هي: بداية `38.99/389.99 SAR`، نمو `98.99/989.99 SAR`، وتوسع `199.99/1999.99 SAR` شهرياً/سنوياً. أكدت صفحة الاشتراكات الحية أن المنتجات الثلاثة تعرض `2` تحت «الخطط الأساسية المفعّلة»، أي ست خطط مفعّلة إجمالاً. لم تُستخدم محاولة GitHub العالقة `32984022063` كمصدر نجاح، وما يزال يلزم تثبيت النسخة من مسار Google Play الداخلي بالحساب المختبِر ثم تنفيذ شراء واستعادة فعليين؛ تفعيل الكتالوج لا يثبت قبول الشراء على جهاز حقيقي.

...
## v86.40 SHEIN real drawer compound size (2026-08-01)

- Current marker is `2026.08.01-v86.40-shein-real-drawer-compound-size`; Android/iOS are `900/86.40`. The user confirmed price is fixed, so selected-price/range logic is intentionally unchanged.
- Real-iPhone evidence rejects v86.39: the first add did not reliably open the options drawer, and inside the drawer both attribute groups use the identical class `.SIZE_ITEM_HOOK goods-size__sizes choose-size`. The old code took the first group (`بيج/رمادي`) as the entire size and never read the second group (`صغير/كبير`).
- The first add now targets the exact visible `لون / مقاس` summary row even when its value is already `رمادي / كبير`, not only when it says `انقر للشراء`. Once the drawer is open, up to six painted/uncovered `.SIZE_ITEM_HOOK` groups are read separately and their actual selections are joined in DOM order, producing `بيج / كبير`; if any detected group has no selection, add remains fail-closed instead of inheriting the background summary.
- Device-shaped full-script Playwright passes: first tap opens exactly once without adding; a drawer with the size group unselected stays blocked; selecting `بيج + كبير` yields diagnostic and payload `صينية من الخشب الصلب | بيج / كبير | $14.66`. v86.39 drawer priority and v86.38 combined-size tests also remain passing, including `أزرق/L`, `رمادي / كبير`, empty-size blocking, normal `L`, and legacy `M / 1PC`.
- Freeze guard, production/performance build, Android/iOS sync, Gradle debug, APK metadata, GitHub/Xcode build, and IPA inspection pass. Budgets: JS raw `1,199,520/1,200,000`, JS gzip `358,357/370,000`, CSS `63,029/70,000`, fonts `81,364/100,000`, SHEIN source `549,688/550,000`.
- Android APK: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.40-shein-real-drawer-compound-size-debug.apk`; SHA-256 `5A96AAA295BA6B19A1BD55F1A6247DB5EE97689DA9F8370732DA03AD7B5E3D89`; size `11,125,054` bytes; metadata confirms `com.otlobli.app`, `86.40/900`, and the release/compound-size markers.
- Code commit is `87c85b1` on `claude/ios6-cover-fix`; GitHub/Xcode run `30697305864` passed. Unsigned IPA: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-v86.40-iphone16-unsigned.ipa`; SHA-256 `7076BB13A20F86A8076A3A7BE9A6DA022A30CA1E6CB1D6813B5EC454EA88D189`; size `7,069,298` bytes. Inspection confirms `com.otlobli.app`, `86.40/900`, iOS 15 minimum, only `otlobli`, the real-drawer/compound-size markers, and all native-recompose markers, with no app-root signature or provisioning profile.
- Real-iPhone acceptance is not yet performed for v86.40. Test the exact product from a clean entry: first tap must open the drawer; no size must block; changing the model/color and `صغير/كبير` inside the drawer must update diagnostic and cart; then run five background/resume cycles and a cold launch. The separate store-region switching defect remains open and is not claimed fixed by this release.

...
فحص المحاكيات بطلب يدوي جديد من المالك في7سبتمبر: سمح بمراجعة LDPlayer والمختبرين، ثم اشترط الطرفية فقط دون التحكم بالشاشة. عُثر على12نسخة في `C:\LDPlayer\LDPlayer9`؛11تسمية بريد وواحدة بعنوان EMPTY، وليست التسميات دليل تسجيل دخول. شُغّل index0 ثم أُغلق لأن ADB غير متاح؛ شُغّل index2 منفردًا، وأكد Android اكتمال الإقلاع وحساب Google واحدًا مطابقًا للتسمية وعدم وجود `app.voicebrief.mobile`. محاولة فتح رابط الاختبار وقراءة الواجهة عبر ADB حُجبت قبل التنفيذ؛ لا تحاول الالتفاف بأداة أو صيغة أخرى. أُغلق index2 وأكد list2 أن الجميع متوقف كما كان. لم تُغيّر إعدادات ADB أو root أو الأجهزة، ولم يحدث دخول جديد أو انضمام أو تثبيت أو اختبار فعلي. الطلب غير مكتمل؛ يلزم إتمام خطوات Google التفاعلية بواسطة المالك مع قيد الطرفية الحالي. لا يوسّع هذا الطلب اليدوي صلاحيات heartbeat ليعمل على حسابات المختبرين.
...
Apple 0.1.2 (21) submitted successfully: run `34042091900` confirms version and review submission `df53c6ca-4690-4ccc-ad52-2e25af964208` are `WAITING_FOR_REVIEW`, with `AFTER_APPROVAL`. Build `cee042ab-1a98-44a3-9dd2-fc6ac6ce3477` is VALID and selected; 0.1.1 remains public. Report: `output/voicebrief-release-34042091900/report.json`; artifact `9991984966`, 1504 bytes, ZIP SHA256 `263129e189cf00b5085905fd13aa80cc7be1cc29c2401890efd2d7355635becf`. No new binary was required or uploaded for the metadata correction.

Owner reported an Apple warning email. App Store Connect and the upload log show only warning `90068`: the current minimum is iOS 14; iOS 15 will be required starting Spring 2027. Upload completed with zero errors and one warning. This is not a current rejection. We inspected the actual Apple warning, not the email inbox; request the message if its code differs.
...
- متابعة 2026-09-06 الساعة17:09 UTC: فحص أبل `34047690129` ناجح وكل الشروط مطابقة؛ الإصدار والإرسال ما زالا WAITING_FOR_REVIEW والبناء21 المحدد VALID وفرنسا مستثناة. التقرير `output/voicebrief-release-34047690129/report.json`؛ artifact9993597335، 1453بايت، SHA256 `9392b1b2e8a8b5dc2528379e488107b5e40ad8919b779e0d62eca214fd6c2b6a`. لم يُعَد الرفع أو التقديم.
- نجح CI `34041819329` كاملًا للالتزام `f36ce0b`: Android وiOS غير الموقّع وEdge وgolden. نجحت اختبارات بيانات المتجر الثلاثة كذلك؛ تحديث التوثيق اللاحق لا يغيّر التطبيق ولا يحتاج بناء أصلي جديد.
- أُصلح نقص الكلمات المفتاحية للغات الجديدة في `f36ce0b` ونُشرت البيانات بنجاح في `34041819225`؛ لم تُستبدل أي صورة في هذه المحاولة. نجح الفحص المستقل `34042050190` لكل البيانات والصور88 والأسماء11 والكلمات المفتاحية وبيانات المراجع واستثناء فرنسا. استأنف التقديم الناجح `34042091900` المسودة المعروفة التي تركتها المحاولة الأولى `34041578197`، دون إنشاء إرسال مكرر. تقرير البيانات في `output/voicebrief-localization-34041819225/`، artifact `9991907554`، 32683 بايت، SHA256 `8801d1c18b8addc4cf78d5dcb22d67b61ef70e84a1673fc63b82f516e52847a4`.
...
- نجح رفع iOS21 في34040157335 من7d06685؛ Delivery/build ID `cee042ab-1a98-44a3-9dd2-fc6ac6ce3477`. IPA محلي `output/voicebrief-ios-34040157335/ios/signed/VoiceBrief-0.1.2-build21-AppStore.ipa` حجمه34911006 وSHA256 `E0A2669E1B4142C236D87B31B92F9F5169B9554E0148B520BCAF28F7A1E0279E`؛ artifact9991614076 حجمه34631239 وبصمته `54e983775139ff5a86ff17f960c88532709057a10aea3b210109690d8ed38322`. اجتاز التوقيع والملفات الأصلية11للتطبيق/الامتداد والتحقق والرفع، وأصبح VALID واختير وقُدّم للمراجعة. تحذير الانتقال من الحد الأدنى iOS14 إلى15 في ربيع2027 غير مانع الآن.
- Google Play: اكتملت مراجعة البناء21 وأصبح Alpha release2 باسم `VoiceBrief 0.1.2 build 21` «متوفرًا للمختبرين على Google Play» بالطرح الكامل، في177من177بلدًا. صفحة أحدث الإصدارات تعرض آخر تعديل6سبتمبر2026 الساعة6:36م، وأكدتها متابعة17:12UTC. رابط الدليل https://play.google.com/console/u/0/developers/8441225038702199576/app/4973918177043935523/tracks/4699063416342020275/releases/2/details . ما زالت لوحة الإنتاج تعرض4مختبرين وزر الطلب معطّل حتى12مختبرًا/14يومًا. الداخلي18 كما هو. لا نشر عام Google ولا تعديل للبلدان أو المختبرين أو المحاكيات في المتابعة.
- Android الإنتاجي الموقّع في `voicebrief_flutter/output/voicebrief/`: AAB `VoiceBrief-0.1.2-build21-production-signed.aab` حجمه59585541 وSHA256 `5F213F2CDC46D6BA4E030416FB2BEEE394480FE22E636F34FDA005DBEE7BA855`؛ APK المقابل70364850 وSHA256 `F93894B519E5BFD89172A8BC04725F37B3BDA17B6035E369215456E121C9E8FE`. تحقق توقيعv2 والشهادة81AA4D…EE8FD9 وحزمةapp.voicebrief.mobile و0.1.2(21) وSDK24/36 وثلاثABI. البناء يستخدم Supabase الصحيح وخدمات فعلية ومفاتيح SDK عامة مضبوطة.
- المصدر `0.1.2+21`، آخر تعديل برمجي `1cec214` على `codex/voicebrief-ios` في `C:\Users\MOHAMMAD\Projects\VoiceBriefAuthRepair`. لم تُمس المحاكيات أو Otlobli أو ملفات المستخدم غير المتتبعة.
- الواجهة 11 لغة و266 رسالة لكل لغة: ar/en/zh المبسطة/hi/es/fr/bn/pt/ru/ur/id. الاختيار محفوظ في الإعدادات ومستقل عن الحساب والمظهر والحصة؛ وضع النظام يتبع ترتيب لغات الجهاز، مع RTL للعربية والأردية. نصوص المشاركة وأذونات iOS وبيانات اللغات الأصلية مضافة للمنصتين. راجع `voicebrief_flutter/docs/LOCALIZATION.md`.
- لم تعد لغة الجهاز تفرض لغة التسجيل. نُشر process-audio v17 ACTIVE حصريًا على `jyehqpdbayslhzebdycj`، بصمة الحزمة `446356916b1076bcdf2d9985c2d05030d033c3c0b7b70594e82bfdb1e8f25910`. لم تتغير الأسرار أو قاعدة البيانات. عُزل ملف CLI profile مؤقتًا ثم أعيد كما كان؛ التشغيل الناجح كان supabase-go دون --profile.
- جاهزة 88 صورة من واجهات Flutter الفعلية بمحتوى توضيحي غير شخصي: أربع لكل لغة ولكل iPhone/iPad، بمقاسي1284×2778 و2064×2752. المسارات والبصمات في `voicebrief_flutter/store_assets/localized/manifest.json`. هذه لقطات برمجية وليست قبولًا أو تصويرًا على أجهزة حقيقية.
- تحقق محلي: format/analyze، 70 اختبارًا وظيفيًا، 12 golden، 15 اختبار لغات يشمل سبع شاشات، 88 اختبار توليد صور، واختبار إضافي للعربية بخط fallback. عولج احتواء اسم التطبيق وزر Pro عند تكبير النص، مع مراجعة الصور المرجعية. APK debug النهائي `voicebrief_flutter/build/app/outputs/flutter-apk/app-debug.apk`، 177862565 بايت، SHA256 `4C04E8EF957DB2EAEACED170768E0898C647F7DF3BF4759ED8176814D86BDDB5`.
- CI النهائي `34038008172` نجح كاملًا للالتزام `1cec214`: Android وiOS غير الموقع وEdge وgolden. فحص Xcode تحقق من ملفات اللغات11 في التطبيق والامتداد. IPA `VoiceBrief-0.1.2-build21-live-unsigned.ipa`، SHA256 `41b3c7936730ca1445a57afcce7015ee0181ba56c94bd63dfcdca0f40b5ce800`، artifact9990882682 (17318964 بايت، ZIP SHA256 `768fc57054706036bc4655597078556761c06f22f3e9465357f2758db1890400`). Android artifact9990896248، ZIP SHA256 `53e0c700fc7862dfbda67b7846137bb567a282a3b9dfde45b56ba92c7f1112c2`. لا مراجعة لغوية بشرية لكل لغة ولا قبول iPhone/iPad أو هاتف ضعيف فعلي.
- أبل أكدت0.1.1 بحالة READY_FOR_SALE في الاستعلام34036838798. أنشأ تشغيل34036907851 مسودة0.1.2 (`4ad274c0-fb1f-4f61-947a-ce57c461554c`) واستبدل صورها العربية الثماني وتحقق من COMPLETE؛ صور النسخة المنشورة لم تُمس ويمكن الرجوع إليها. نسخ قبل التغيير في artifact التشغيل ومحليًا `output/voicebrief-localization-20260906/`.
- وافق المالك على وصف عادي يضاف للاسم الإنجليزي وعلى متابعة النشر الكامل. الأسماء المحددة في `scripts/store_names.mjs` تبقي VoiceBrief أساسًا، والإنجليزية `VoiceBrief: Audio Summaries` والعربية VoiceBrief دون تغيير. عُدّل السكربت لإنشاء AppInfoLocalization قبل VersionLocalization لتجاوز تعارض الاسم السابق؛ يستهدف AppInfo المسودة `176e4637-e8be-499e-a3f2-93acf9dfeb92` حصريًا ويحمي الاسم العربي والمنشور.
- 0.1.2(21) مرفوع للمتجرين لكنه ليس منشورًا عامًا بعد. TestFlight21 يعمل بتشغيل صريح فقط. `voicebrief-appstore-release.yml` يفحص أو يقدم البناء21 حصريًا بتأكيد6805194629:0.1.2:21:submit؛ يفحص كل البيانات وVALID ثم يضبطAFTER_APPROVAL دون تغيير البلدان أو بيانات المراجع. Google internal/status workflow القديمان20 لا يُستخدمان لهذا التحديث؛ Alpha21 رُفع من لوحة Google. لغة المتجر الأساسية ومعرفات التطبيق وملكيته والاشتراكات واستثناء فرنسا لا تتغير؛ تعديل الاسم ليس ضمانًا لحقوق علامة تجارية.

## آخر متابعة نشر VoiceBrief — 2026-09-05

### الحالة الأحدث المعتمدة (تتجاوز ملاحظات الانتظار أدناه)

- في 2026-09-05 طلب المالك صراحة نشر التحديث بعد بيان أن اختبار الاستعادة على جهاز فعلي غير منفّذ. رُبط build20 بإصدار0.1.1 وحُفظ ثم أُرسل للمراجعة؛ أكدت أبل `1 Item Submitted` برقم `a5d94fa0-6a28-4e3f-b049-d151a2108cc8`. النشر التلقائي بعد الموافقة مفعّل؛ هذا إرسال للمراجعة وليس نشرًا عامًا مكتملًا. ظهر البناء أيضًا ضمن VoiceBrief Internal بعد انتهاء Processing.
- status run33925529206 انتهى قبل ظهور البناء بمهلة not found؛ ليس فشل بناء أو رفضًا من أبل. تحقق المتصفح اللاحق أكد Complete للبناء20.
...
- دخل المالك RevenueCat وتأكد مشروع VoiceBrief `59213dad` وتطبيقا Apple/Google. لا يوجد Secret API key؛ جُهز نموذج `VoiceBrief Supabase server` إصدار V1 ولم يُضغط Generate بعد. إنشاء المفتاح يحتاج تأكيدًا عند التنفيذ وفق سياسة أداة المتصفح. الحساب يعرض أن البريد لم يؤكد بعد.

- وافقت Apple على الإرسال `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b` للإصدار `0.1.0 (18)`، وتأكد ظهور صفحة المتجر العامة في متابعة 2 سبتمبر: https://apps.apple.com/qa/app/voicebrief/id6805194629 . أوصاف Waiting for Review أدناه تاريخية.
- البناء `0.1.0 (19)` صالح في TestFlight فقط. يُجهز الآن تحديث `0.1.1+20` بنفس إصلاحات الحصة؛ لا يمكن استبدال إصدار 0.1.0 المنشور ببناء آخر للإصدار نفسه. فحوص وبناء التحديث قيد التنفيذ ولم يُرفع بعد.
- اكتمل دخول Supabase الصحيح في المتابعة السابقة بملف CLI باسم `voicebrief`، وتحقق الربط بالمشروع `jyehqpdbayslhzebdycj` وdry-run للمهاجرات الأربع فقط. يلزم إعادة التحقق من الحساب قبل أي نشر. مفتاح RevenueCat الخادمي وVault والنشر والاختبار الحي ما زالت معلقة؛ صفحة RevenueCat تتطلب دخول المالك مجددًا.
- لم تُمس المحاكيات أو ملفات المستخدم غير المتتبعة. تجهيز Google build 20 لم يكتمل، ولا يجوز تشغيل الناشر قبل تثبيت حجم وبصمة الحزمة النهائية ونشر الخادم.

## مشروع VoiceBrief المستقل (2026-09-02)

- المرشح المحلي الحالي هو `0.1.0+19` على الفرع `codex/voicebrief-ios`. يعالج سبب رجوع رصيد Pro إلى `300` بعد «استعادة المشتريات»: أصبحت الحصة `300` دقيقة لكل نافذة شهرية مثبتة من الخادم، والسنوي 12 نافذة فقط، ولا يستطيع طلب من التطبيق إنشاء جيل Pro أو تعبئة رصيد؛ إنشاء الجيل محصور بWebhook RevenueCat الموقّع أو `TRANSFER`. الاستعادة لنفس الجيل تحفظ الاستهلاك، ونقل A→B→A يجمع الاستهلاك بلا refill، ووقت الخادم هو المرجع بدل ساعة الهاتف.
- التدقيق المستقل النهائي لم يجد P0/P1 متبقيًا. نجح `dart format` على 80 ملفًا بلا تغيير، و`flutter analyze --no-pub`، و55 اختبار Flutter غير golden، و12 golden، و95 اختبار Deno، و5 اختبارات Node، وDeno fmt/lint/check، و`git diff --check`، وبناء APK debug. كما نجح CI السحابي كاملًا في run `33637316774` للالتزام `ba5ddab`: Android وiOS غير الموقّع وEdge وgolden. لا يتوفر PostgreSQL/Docker محليًا، لذلك لم تُطبّق migrations الجديدة على قاعدة محلية حقيقية؛ يلزم smoke حي قبل التوزيع.
- بُني Android build 19 النهائي محليًا وموقّعًا: AAB ‏`voicebrief_flutter/output/voicebrief/VoiceBrief-0.1.0-build19-production-signed.aab` حجمه `59,192,552` وبصمته SHA-256 ‏`6999644E2D065EED9D74CBB0DD901480E0EC9CD2C3AA76457E7DC773781C7183`، وAPK ‏`VoiceBrief-0.1.0-build19-production-signed.apk` حجمه `69,446,598` وبصمته `038BFAD20D043CEA87D034F50864CF3666647AADB9B0E1BE7CA2880BC80BA4BE`. تحقق `app.voicebrief.mobile` و`versionCode 19` وtarget SDK 36 وتوقيع APK v2 وشهادة الرفع `81AA4D...EE8FD9`. لم تُرفع هذه الحزمة إلى Google بعد.
- Backend build 19 غير منشور بعد. يجب دخول حساب Supabase الذي يرى حصراً المشروع الصحيح `jyehqpdbayslhzebdycj`، وضبط `REVENUECAT_SECRET_API_KEY` من مشروع VoiceBrief في RevenueCat من دون كشفه، ثم نشر migrations/functions المتطابقة واختبار purchase/restore/transfer حيًا قبل توزيع العميل. جلسة Supabase CLI الحالية لحساب آخر؛ لا تنشر بها ولا تمس Damanak `exxayzlklvgeyqhvtzgi` أو `talabieh`.
- نجح TestFlight run `33637316975` ورفع build 19 إلى App Store Connect. IPA ‏`VoiceBrief-0.1.0-build19-AppStore.ipa` حجمه `34,638,044` بايت وبصمته SHA-256 ‏`BFE400EC4D21037E331110F763DF93D48254D82E54DF1BAA49A1ED88946E2773`، وDelivery UUID/App Store build ID ‏`b08f526a-eec2-4a66-b707-919e7f57eeca`. GitHub artifact رقم `9849894624` واسمه `voicebrief-ios-0.1.0-build19-appstore`، وحجمه المضغوط `34,388,723` وبصمته `sha256:3ed158c97659d720648722aae26e081afe42083e541a86a74939456851ef02d9`. أكد status run `33637317007` أن البناء `VALID` ومضاف إلى `VoiceBrief Internal`؛ رُفع في `2026-09-02T06:57:29-07:00` وينتهي في `2026-12-01T05:57:29-08:00`. status artifact رقم `9850157535` وبصمته `sha256:55072db4d5297b8910370348aeafb38c648fa15b3bcfdcb125dbc1bc614ef12c`. التحذير الوحيد غير المانع هو أن Apple ستطلب iOS 15 بدل 14 ابتداءً من ربيع 2027.
- Apple ما زال يعرض إرسال build 18 والعناصر الأربعة بحالة `Waiting for Review` تحت `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b`؛ build 19 أصبح `VALID` في TestFlight لكنه لا يُستبدل في الإرسال قبل جاهزية Backend. Google Production غير متاح: لوحة Play تعرض الآن `3` مختبرين من أصل `12` مطلوبين، ثم يلزم 14 يومًا متتالية؛ المستخدم سيتولى المحاكيات وحسابات المختبرين بنفسه، وممنوع على AI تشغيلها أو تعديلها أو حذفها. build 18 وحده موجود حاليًا في internal/Alpha، وbuild 19 ينتظر Backend ثم الرفع إلى Google.

- ألغى المالك فرنسا لهذا الإصدار؛ أصبحت متاحة في `174` بلدًا/منطقة مع إبقاء إضافة البلدان المستقبلية تلقائيًا، وتظهر فرنسا `Not Available` في App Store Connect. رفضت Apple الإرسال الأول آليًا في `2026-09-01 6:49 PM` تحت البند `3.1.2` لغياب رابط شروط الاستخدام من وصف صفحة المتجر. أُضيف إلى نهاية الوصف العربي رابط Apple القياسي `https://www.apple.com/legal/internet-services/itunes/dev/stdeula/`، وحُفظ، ثم أُعيد إرسال العناصر الأربعة نفسها عند `9:49 PM`: إصدار iOS ‏`0.1.0 (18)`، ومجموعة `VoiceBrief Pro`، والاشتراكان `voicebrief_pro_monthly` و`voicebrief_pro_annual`. تعرض العناصر الأربعة الآن `Waiting for Review` تحت معرّف الإرسال نفسه `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b`. build 19 أصبح `VALID` ومتاحًا لمجموعة TestFlight الداخلية، لكنه لم يحل محل build 18 في الإرسال بعد؛ النشر التلقائي بعد موافقة Apple ما زال مفعّلًا، وهذا إرسال للمراجعة وليس نشرًا حيًا بعد.
- نُشر إفصاح App Privacy في App Store Connect بعد ضبط `15` نوع بيانات من manifests البناء 18؛ كلها مرتبطة بالمستخدم حسب تكاملات التطبيق ولا يُستخدم أي منها للتتبع. حُدثت سياسة الخصوصية ثنائية اللغة لتشرح Google Sign-In وRevenueCat ونُشرت إنتاجيًا في deployment ‏`dpl_EMsF8EH79BfRKPTSxNr8wRDDxRNn` بحالة `READY` على `https://voicebrief-legal.vercel.app/privacy`.
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار Content Rights على أن التطبيق يصل إلى محتوى طرف ثالث ولدى المالك الحقوق اللازمة. حُفظت حقول App Review باسم `MOHAMMAD ALZOUABI` وحساب Google للمراجع `mhm1981d@gmail.com` والهاتف `+974 70067040`، وصُححت التعليمات لتطلب زر «المتابعة باستخدام Google» بدل تسجيل Apple؛ أدخل المالك كلمة المرور مباشرة ولم تُقرأ أو تُطبع. بعد استبعاد فرنسا لم يعد نموذجها المحلي مانعًا لمسار هذا الإصدار، وقبل App Store Connect إرسال البناء 18 بالعناصر الأربعة. لا تُعد فرنسا ولا تصريح `ANSSI` إلى هذا الإصدار ما لم يقرر المالك لاحقًا إعادة فرنسا صراحةً.
- بقيت ملفات التحضير الفرنسي محليًا فقط ولم تُرسل إلى `ANSSI` أو Apple: الملحق التقني `output/pdf/VoiceBrief-ANSSI-technical-annex.pdf`، والنموذج الرسمي `output/pdf/ANSSI-crypto-declaration-form-v2.pdf`، والنسخة المعبأة غير الموقعة `output/pdf/VoiceBrief-ANSSI-declaration-filled-unsigned.pdf`، ودليل المنتج `output/pdf/VoiceBrief-ANSSI-product-and-user-guide.pdf`. هي مراجع غير مستخدمة وليست تصريحًا أو اعتمادًا، ولا يلزم توقيعها أو إرسالها للإصدار الحالي بعد استبعاد فرنسا. لا تحذفها لأنها ضمن مخرجات محلية غير متتبعة، ولا تستأنف مسارها إلا إذا طلب المالك إعادة فرنسا مستقبلًا.
- المرشح السابق كان `0.1.0+18` على الفرع `codex/voicebrief-ios`. فوق إصلاح ثبات المظهر والرصيد في build 17، أضاف إعادة محاولة آمنة للرفع: إذا وصل الملف وانقطع الرد، يتحقق `create-audio-upload` من المسار والحجم و`Content-Type` ثم يعيد `uploadedAlready:true` ويكمل Flutter/Share Extension إلى المعالجة من دون استبدال جسم قد يكون قيد الاستخدام. طلب Responses يرسل `store:false`، وWebhook RevenueCat يقرأ JSON بحد فعلي لا يعتمد على `Content-Length`.
- اكتمل فحص فرق أمني لـbuild 18 برقم `a6a51d74-8bdb-4d43-8c39-a4634125cc8d`: لم يجد خطورة حرجة أو عالية أو متوسطة، ووجد تجديدًا منزلقًا واحدًا منخفض الخطورة لحجز رفع المستخدم نفسه. أُغلق في المصدر بنافذة إعادة محاولة لا تتجاوز ساعتين من إنشاء الحجز، وسقف قاعدة بيانات ثابت `created_at + 4 hours` لا يمكن دفعه إلى الأمام. بقي احتمال نظري منخفض الثقة لرفع `PUT` يستمر أكثر من مرورَي التنظيف مؤجلًا لاختبار Supabase حي؛ لا يُدّعى إغلاقه من المصدر وحده.
- أصبح حذف الحساب محليًا يفشل مغلقًا حتى يحذف عميل RevenueCat أولًا عبر مفتاح خادم `REVENUECAT_SECRET_API_KEY`؛ `200/404` نجاحان idempotent، وأحداث webhook للحساب المحذوف تُسجل بلا UUID مع بقاء `event_id` لمنع الإعادة. migration `20260830234000_revenuecat_account_deletion.sql` والدوال المعدلة محلية وغير منشورة، والمفتاح السري غير منشأ بعد لأن جلسة RevenueCat تحتاج دخول المالك.
- تنظيف الصوت المستقل المحلي في `20260830230000_independent_audio_cleanup.sql` صار على مرحلتين: مطالبة ذرية بدفعة 100 بعد انتهاء الرابط ومهلة 15 دقيقة، إزالة Storage أولى تبقي tombstone، ثم مطالبة لاحقة بعد 15 دقيقة على الأقل تكرر الإزالة قبل حذف المؤشر، مع lease قابل للاسترداد بعد 30 دقيقة وجدولة كل 15 دقيقة عبر `pg_cron/pg_net/Vault`. إعادة إصدار رابط الرفع محصورة بأول ساعتين، والحجز لا يتجاوز أربع ساعات منذ إنشائه. لا توجد كتابة SQL إلى `storage.objects`. هذا المصدر وVault والدالة غير منشورة؛ الاختبار الحي يبقى حصراً على `jyehqpdbayslhzebdycj`.
- اكتمل فحص أمني قياسي للنسخة السابقة `9f228c1` برقم `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` ووثائقه في `voicebrief_flutter/docs/security/build17/`. ثبّت 6 نقاط: عزل جلسة/نتيجة App Group عند تبديل الحساب، عدم الثقة بمدة الصوت من العميل، منع الرفع المباشر غير المحدود، إنفاذ انتهاء النتائج، جعل حد نموذج الدعم ذرياً، وقياس جسم الطلب الفعلي. نُفذت معالجات build 17 ونُشر خادمها ونجح تجميع Swift على CI؛ أما تنظيف build 18 ثنائي المرور وحذف RevenueCat فما زالا محليين ولم يبدأ التحقق التشغيلي الحي لهما.
- خادم build 17 يضيف حجز رفع ذرياً محدوداً ومسار `create-audio-upload` بتوقيع دقيق، ويستخرج مدة الصوت من الملف على الخادم قبل حجز الرصيد، ويحمي أجسام JSON/form المرسلة فعلياً، ويعزل نتائج Share Extension بالـuser والجلسة generation. نُشرت migration `20260830190000_security_hardening.sql` حصراً إلى `jyehqpdbayslhzebdycj` وتطابق سجلها المحلي والبعيد. الدوال المنشورة `ACTIVE`: `create-audio-upload` الإصدار 2، و`process-audio` الإصدار 14، و`legal` الإصدار 8، و`delete-account` الإصدار 9. لم يُمس Damanak أو `talabieh`.
- تحقق build 18 المحلي ناجح بعد معالجة الفحص الأمني: `dart format` على 80 ملفًا بلا تغيير، و`flutter analyze --no-pub`، و45 اختبار Flutter غير golden، و12 golden، و42 اختبار Deno، و`deno fmt/lint/check`، و`git diff --check`. صُححت كذلك فحوص CI وTestFlight التي كانت ما تزال تتوقع build 17 فأصبحت تطابق 18. بُنيت حزمة Android موقعة بالإصدار `0.1.0 (18)` وtarget SDK 36: AAB في `voicebrief_flutter/output/voicebrief/VoiceBrief-0.1.0-build18-production-signed.aab` حجمه `59,195,892` وبصمته `AEC8B94AA105FA083372FC7B65419A602AC2A3452DA8F98161CE59E53E262AAD`، وAPK في `voicebrief_flutter/output/voicebrief/VoiceBrief-0.1.0-build18-production-signed.apk` حجمه `69,446,598` وبصمته `BA2971E1C656EF7BB5D7E340F25A5455C9B313D4D35CD6A6DBE588858B9A06B0`. APK Signature Scheme v2 يطابق شهادة الرفع SHA-256 `81AA4DA6E425794E880C8DDE0A7568F68CEE774A3C1889170221AD0446EE8FD9`.
- نجح CI للبناء 18 في run `33337662952`. نجح TestFlight run `33337662810` في الأرشفة والتوقيع والفحص والرفع؛ IPA `VoiceBrief-0.1.0-build18-AppStore.ipa` حجمه `34,633,872` بايت وبصمته SHA-256 `2025A7FA77D2C737A6B588982A4C2E053948E6D989B5CC499898351BFD755EA6`، وDelivery UUID/build ID هو `59ccf8f9-cbc9-4ac1-9f4c-54492f8ffcaf`. GitHub artifact رقم `9739706290` واسمه `voicebrief-ios-0.1.0-build18-appstore` وبصمته API ‏`sha256:dcc508aa07823e9f8adbb477f3dd332e835a34cc58750450591b4d710627a739`.
- أكد App Store status run `33339093610` أن `0.1.0 (18)` حالته `VALID` وغير منتهٍ وموجود في `VoiceBrief Internal`، وتاريخ انتهائه `2026-11-28T14:08:06-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9739954624` وبصمته API ‏`sha256:d5935e52f41efa97d6fe7237b57078b35d89b2dc051c7540609f9018ce895bd1`؛ ملف التقرير `413` بايت وبصمته SHA-256 `CF5F77E72588F207CCA7C1AB9763635B7AE248C9EFF6F1CE5B19037682B3181`. أُرسل هذا البناء لاحقًا إلى App Review ضمن الإرسال `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b` وحالته الآن `Waiting for Review`، ويبقى قبول iPhone الحقيقي للمظهر والحصة والمنبّه والصوت المخصص مطلوبًا.
- نُشر AAB البناء 18 إلى مسار Google Play ‏`internal` فقط في run `33341090128` من الالتزام `26b99e8e8e6132e14f7b26a61e5361178667289b`. أثبت تقرير الناشر أن الحزمة والبصمة وشهادة الرفع صحيحة، وأن المسار انتقل من draft للبناء 7 إلى `completed` للبناء 18 مع `committed:true`. تقرير GitHub artifact رقم `9740576360` وبصمته API ‏`sha256:b98c07031726bcb38cf88b000dc7a7d01ab3a2874102788a706ab39f32f87647`؛ ملف التقرير `838` بايت وبصمته SHA-256 `6E42CB83EBBBA56450A800647544F5D823F83A7BAB22EAB3E56A95F58D324E36`. أكد Play Console بصريًا `VoiceBrief 0.1.0 build 18` و«متاح للمختبِرين الداخليين».
- حُفظت بيانات القائمة العربية في Google Play مع الأيقونة وFeature Graphic وأربع صور هاتف. بعد تأكيد المستخدم الإجرائي، رُبطت قائمة `All Apps - Closed Testers` ذات 12 عضوًا بمسار الاختبار المغلق `Alpha`، واختيرت جميع المناطق المتاحة: `176` بلدًا/منطقة إضافة إلى «بقية بلدان العالم». نُشر بريد التواصل العام `mhm1981x@gmail.com` والموقع `https://voicebrief-legal.vercel.app`، وحُفظ IARC بفئة «كل أنواع التطبيق الأخرى» مع المحتوى المولّد عبر الإنترنت والمشتريات الرقمية؛ نتائجه `Everyone` و`PEGI 3` و`IARC 3+` مع `ClassInd 14+` للبرازيل ووسم المشتريات داخل التطبيق. حُفظ الجمهور `18+`، وأُضيف build 18 وحده إلى إصدار `VoiceBrief 0.1.0 build 18` في Alpha، ثم أُرسلت 14 تغيّرًا إلى Google؛ تعرض نظرة النشر الآن «التغييرات قيد المراجعة». هذا ليس Production ولا يعني أن Alpha أصبح متاحًا بعد؛ عند الإرسال كان عدد المنضمين الفعلي 0 ولم تبدأ مدة 14 يومًا.
- استُخرجت شهادة `Google Play App Signing` العامة من Play Console: SHA-1 ‏`05:E4:8C:69:53:7A:BB:EF:D2:CE:01:CB:C5:C4:59:68:07:19:1A:A5` وSHA-256 ‏`32:C1:07:D0:43:9F:B5:83:C0:E1:DB:12:8A:A0:81:5C:1B:98:E8:25:AB:48:D3:BB:4B:9D:92:8B:5C:29:88:35`. أُنشئ لها في مشروع Google Cloud الصحيح `voicebrief-prod-mhm1981x` عميل Android باسم `VoiceBrief Android Play` ومعرّفه `872187920899-krms6k2de8ad87isa4aba7828a4u1u6n.apps.googleusercontent.com`. ما زال يلزم اختبار تسجيل Google من تثبيت Play فعلي بعد انتشار الإعداد.
- رُبطت مجموعة `All Apps - Closed Testers` نفسها، وعددها 12، أيضًا بمسار Google Play الداخلي الذي يحتوي build 18؛ تحولت حالة المسار إلى ملفات نشطة. قبل الحساب `mhm1981d@gmail.com` دعوة الاختبار الداخلي عبر الرابط `https://play.google.com/apps/internaltest/4700504097313794694`. توقف الإرسال البعيد إلى Note 8 عند إعادة تحقق Google (كلمة مرور أو passkey) ولم تُدخل أي بيانات اعتماد. الجهاز يحمل حاليًا build 1 الموقّع بمفتاح الرفع؛ قد يلزم إزالة النسخة القديمة قبل تثبيت Play الموقّع، ولا تحذف بياناتها بلا موافقة المستخدم. هذا الانضمام داخلي ولا يُحسب ضمن 12 مختبرًا/14 يومًا لمسار Alpha المغلق.
- نُشرت سياسة build 18 وصفحة حذف الحساب/البيانات الجزئي باللغتين على مشروع Vercel الصحيح `voicebrief-legal`؛ النشر الإنتاجي النهائي `dpl_3apBv66qn7cKF2RsqUfPnC1y1yDg` حالته `READY`، وروابط `privacy` و`delete-account` العربية والإنجليزية تعيد HTTP 200. لم تُنشر دوال أو migrations من build 18 ولم يُمس إلا Supabase الصحيح في نموذج الدعم المنشور مسبقًا. حُفظ Data Safety نهائيًا وأُرسل للمراجعة: جمع فقط بلا مشاركة، تشفير أثناء النقل، OAuth، حذف الحساب والبيانات عبر `/delete-account`، والأنواع هي البريد/معرّف المستخدم/الاسم/صورة المزود/سجل الشراء/الصوت/الملفات/المحتوى المولّد/إجراءات الاستخدام/التشخيص/اللغة والمنطقة؛ كلها غير مؤقتة ومطلوبة للأغراض الموثقة. حُفظ سجل تفاصيل دخول باسم `Google reviewer account` للحساب `mhm1981d@gmail.com` من دون قراءة كلمة المرور أو إخراجها؛ بقاء الحساب بلا 2FA وظهور Pro/300 دقيقة لم يُتحققا مستقلًا بعد، وقد يؤثران في مراجعة Google.
- رفض iPhone الحقيقي البناءين 9 و10 لمسار فتح التطبيق بعد مشاركة تسجيل WhatsApp. البناء 9 أثبت أن `NSExtensionContext.open` والزر المرئي يفشلان، والبناء 10 أثبت أن WhatsApp لا يعرض Runner كوجهة document-open ويقدّم `Share Extension` فقط. هذا يطابق توثيق Apple؛ لا تُعد `UIApplication.shared` أو responder-chain أو أي رابط فتح من الامتداد.
- أثبت اختبار المستخدم للبناء 12 أن المعالجة داخل `Share Extension`، الإغلاق اليدوي، إشعار «الملخص جاهز»، وفتح النتيجة من الإشعار تعمل على iPhone الحقيقي. البناء 13 يوضح أثناء المعالجة: «يمكنك إغلاق هذه النافذة الآن؛ سيصلك إشعار عندما يصبح الملخص جاهزًا».
- يطلب Runner إذن التنبيه/الصوت/الشارة بعد تسجيل الدخول. عند ضغط إشعار الجاهزية، يلتقط AppDelegate علامة `sharedResult`، يستورد Flutter النتيجة ويحفظها محلياً من دون رفع أو خصم ثانٍ، ثم ينتقل مباشرة إلى `/result`. البناء 14 يبقي زر «ضبط منبّه» إلى جانب التقويم، لكنه يستبدل إشعار iPhone المحلي بمنبّه نظامي حقيقي عبر `AlarmKit` على iOS 26.1 فما بعد بعد إذن المستخدم؛ iOS 14–26.0 يبقى على إشعار محلي، وAndroid يستخدم `AlarmClock.ACTION_SET_ALARM`. العبارة المبهمة مثل «الساعة الخامسة» تفرض تأكيد التاريخ والوقت ولا تخمّن صباحاً أو مساءً.
- إذا كانت الإشعارات معطلة تُحفظ النتيجة مع رسالة صريحة لتفعيلها، لكن لا يمكن فتح Runner تلقائياً. يجدد الامتداد الجلسة عند الحاجة، ويعيد Runner جلسة App Group المحدثة إلى Supabase عند تشغيله. عند غياب جلسة أو شبكة أو رصيد يبقى الملف محفوظاً لمسار الاستيراد القديم وتظهر رسالة صريحة.
- يبقى Runner مسجلاً لـ`public.audio` وMP3/MPEG-4/WAV كي يعمل الفتح المباشر من التطبيقات التي تعرض document-open فعلاً، لكن WhatsApp على جهاز الاختبار لم يفعل ذلك. امتداد المشاركة لا يملك API عامة لتشغيل Runner.
...
- أضيف تصحيح حتمي للتواريخ العربية: `يوم خمسة تسعة` و`بتاريخ ٥/٩` يعنيان 5 سبتمبر القادم، ولا يُخلطان مع `الساعة خمسة`. البناء 13 يستعيد كذلك `اليوم` و`غدًا` و`بكرة` و`بعد غد` مباشرة من النص إذا أغفلها التوليد المنظم. أصل اختفاء الموعد في الصورة كان أن مسار الخطة المجانية يمسح `importantDates` عمداً؛ أُبقيت التواريخ الآن لكل الخطط، مع استمرار إخفاء النقاط والمهام المدفوعة فقط.
- نجح اختبار build 17 الحي عبر مسار التذكرة والرفع الموقّع بعينة تكرر الموعد؛ أزال الإصدار 14 التكرار وأبقى حدثين فقط. ثم نجح النص العربي الذي قدّمه المستخدم نفسه بعد تحويله إلى عينة `29.208s`: HTTP 200 خلال `16,141ms`، حدث مستقل للغد `2026-08-31` عند الخامسة مع طلب تأكيد، وحدث مستقل `2026-09-05` بلا وقت مخترع ومع طلب تأكيد. نُظف مستخدم Auth المؤقت وملف Storage واستحقاق الاختبار في كل تشغيل.
- مشروع Supabase الصحيح حصراً `jyehqpdbayslhzebdycj`. `process-audio` منشور بحالة `ACTIVE`، الإصدار `14`، والمعرّف `e4a26c2e-b9f2-43d3-9a6b-61467b8c8747`. السر `OPENAI_TRANSCRIPTION_MODEL` مضبوط على `gpt-4o-mini-transcribe` ببصمة `b0f412bcb089db8e74db4057e4dd6ad5af7536c3a11d56011fb159efc3f51e5a` مع الحفاظ على بقية الأسرار. ملف اختيار CLI الذي يتعطل معه الإصدار `2.107.0` حُفظ قابلاً للاسترجاع في `C:\Users\MOHAMMAD\.supabase\profile.voicebrief-cli-2.107.backup-20260830-2059`، واستخدام اعتماد Windows بقي داخل الذاكرة فقط.
- نجح التحقق المحلي للبناء 10: `dart format --set-exit-if-changed .` على 70 ملفاً، و`flutter analyze --no-pub` بلا مشكلات، و30 اختباراً غير golden، و11 golden، و`flutter build apk --debug`. APK في `voicebrief_flutter/build/app/outputs/flutter-apk/app-debug.apk`، حجمه `177,592,197` بايت وبصمته SHA-256 `3C9B96B65281D140CAF3FEA05DAEBFB6CD636A7E78BD1ABB5803E7A548F97C15`. نجح CI للبناء 10 في run `33283233763`: Deno وAndroid و11 golden وتجميع iOS غير الموقّع والتحقق من التطبيق والامتداد كلها ناجحة.
...
- تحقق workflow القراءة فقط `voicebrief-appstore-status.yml` في run `33281554578` من App Store Connect API: البناء `0.1.0 (9)` على iOS حالته `VALID`، غير منتهٍ، ومرتبط بمجموعة `VoiceBrief Internal`، ومعرّف البناء `d3788d62-0ca7-423f-b234-425a325012f7` يطابق Delivery UUID. تاريخ الرفع `2026-08-26T19:04:29-07:00` والانتهاء `2026-11-24T18:04:29-08:00` والحد الأدنى iOS 14. تقرير GitHub artifact رقم `9723131680`، حجمه `416` بايت، وبصمته SHA-256 `15DF660E61DAA9F05FC42A3696CB071A4F7FEF5ED772892131A8717C0EB71C1F`.
- نجح TestFlight run `33283233779` للبناء `0.1.0 (10)`: الأرشفة والتوقيع وفحص التطبيق والامتداد والملفات والاستحقاقات والتحقق والرفع كلها ناجحة. Delivery UUID وApp Store build ID كلاهما `d8a16f4a-74ad-4d7f-8368-220ca5ab9152`. IPA `VoiceBrief-0.1.0-build10-AppStore.ipa` حجمه `34,432,164` بايت وبصمته SHA-256 `81763F19AEE9476F255CA6D117459CD99B29DB07D289E9257D567836FB5FF1FA`. GitHub artifact رقم `9723733688` واسمه `voicebrief-ios-0.1.0-build10-appstore`، وبصمة ZIP المرفوع `A80931E213B74DE63EBAC49C411AB94DF70E0D333DCE617D9D716550F4EE49E1`.
- أكد App Store Connect status run `33283233781` أن البناء 10 حالته `VALID` وغير منتهٍ ومرتبط بمجموعة `VoiceBrief Internal`. تاريخ الرفع `2026-08-29T17:36:37-07:00` والانتهاء `2026-11-27T16:36:37-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9723780941` وبصمة ZIP `7225944E1434DA90BDCDBFFBA3E93230F877996937C09A197252B98D3188B73B`.
- البناء 10 متاح في TestFlight لكنه مرفوض جهازياً للفتح المباشر من WhatsApp. نجح التحقق المحلي للبناء 11: format على 71 ملفاً، analyze بلا مشكلات، 31 اختباراً غير golden، 11 golden، وAPK debug حجمه `177,596,157` وبصمته SHA-256 `CF8F63D8CBC22A9AC681A2F51FAED8F7C86CB0B6DA88D6856A84B24657D84E30`.
- نجح CI للبناء 11 في run `33304112117`: Deno وAndroid و11 golden وتجميع iOS غير الموقّع كلها ناجحة. ونجح TestFlight run `33304112113` في الأرشفة والتوقيع وفحص التطبيق والامتداد والملفات والاستحقاقات والتحقق والرفع؛ Delivery UUID هو `4fdd86e6-da1e-41d6-820e-48290839548a`. IPA `VoiceBrief-0.1.0-build11-AppStore.ipa` حجمه `34,462,676` بايت وبصمته SHA-256 `AACAFA2BADBCA2026B1AF8080B66D0F455E79724E825BF15A8F0F352F04C577D`. GitHub artifact رقم `9730071886`، وبصمة ZIP `9F3724194F38BD15F3B612F20B035D2D99E3CBBA608B546893D6601C9F9647BF`.
- أكد App Store Connect status run `33304112131` أن البناء `0.1.0 (11)` حالته `VALID` وغير منتهٍ ومرتبط بمجموعة `VoiceBrief Internal`. App Store build ID يطابق Delivery UUID، وتاريخ الرفع `2026-08-30T02:43:00-07:00` والانتهاء `2026-11-28T01:43:00-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9730121859` وبصمة ZIP `66FAF74DDB4D12EA5B9359B00D917498AC1D9DB00D89BF54E93A4BEDE9100660`. بقي فقط قبول iPhone الحقيقي لمسار المعالجة داخل نافذة المشاركة، ولا يوجد ادعاء نجاح جهاز قبل تجربة المستخدم. Apple ستطلب iOS 15 للرفع بدءاً من ربيع 2027.
- التحقق المحلي للبناء 12 نجح: format على 71 ملفاً، analyze بلا مشكلات، 32 اختباراً غير golden تشمل فتح النتيجة المؤرخة من الإشعار، 11 golden، وAPK debug حجمه `177,599,813` بايت وبصمته SHA-256 `37729DAFEECC45CE66DF870AA167D4E660A7F3167C62E3352C003C3818029FE5`.
- نجح CI للبناء 12 في run `33307726051`: Deno وAndroid و11 golden وتجميع Swift/iOS غير الموقّع كلها ناجحة. ونجح TestFlight run `33307726042` في الأرشفة والتوقيع وفحص التطبيق والامتداد والملفات والاستحقاقات والتحقق والرفع؛ Delivery UUID هو `0af3d7d3-a607-4913-9d39-a2429c73b3b0`. IPA `VoiceBrief-0.1.0-build12-AppStore.ipa` حجمه `34,484,969` بايت وبصمته SHA-256 `12CC57885E1C9CEE4827F36D1B3072DA98CAE8DF254C3D714C44A6AC53DD7C8C`. GitHub artifact رقم `9731160573`، وبصمة ZIP `23A977FF2BECD533E5A2CF608A9E5472045DB8674817E60FCC2E4B5C20B114BE`.
- أكد App Store Connect status run `33307726047` أن البناء `0.1.0 (12)` حالته `VALID` وغير منتهٍ ومرتبط بمجموعة `VoiceBrief Internal`. App Store build ID يطابق Delivery UUID، وتاريخ الرفع `2026-08-30T04:11:28-07:00` والانتهاء `2026-11-28T03:11:28-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9731212465` وبصمة ZIP `767594E102E4DB43B67B599F7E12517C0C68289BED8DE49442D05CA88D9B7416`. بقي قبول iPhone الحقيقي لمسار الإشعار وفتح الملخص المؤرخ؛ لا يوجد ادعاء نجاح جهاز قبل تجربة المستخدم.
- البناء 13 هو إصلاح الموعد والمنبّه بعد قبول المستخدم لتدفق الإشعار في البناء 12. نجح محلياً format على 73 ملفاً، وanalyze بلا مشكلات، و34 اختباراً غير golden تشمل الجسر الأصلي وفرض التأكيد للساعة العربية المبهمة، و11 golden. نجحت 7 اختبارات Deno للتواريخ و`deno check`. APK debug حجمه `177,609,821` بايت وبصمته SHA-256 `86EA9BF33CA6FC39C337A83EBE7879781EE21AE1080AC74CEBFAFDF85DA4ECE7`. الاختبار الحي بعد نشر الإصدار 11 أعاد HTTP 200 خلال `17,153ms`، وفصل الغد عن `2026-09-05`، ثم حذف مستخدم Auth وملف Storage واستحقاق الاختبار.
- نجح CI run `33310667058` للبناء 13 في Deno وAndroid و34 اختباراً و11 golden وتجميع iOS غير الموقّع. نجح TestFlight run `33310667057` في الأرشفة والتوقيع وفحص التطبيق والامتداد والملفات والاستحقاقات والتحقق والرفع. IPA `VoiceBrief-0.1.0-build13-AppStore.ipa` حجمه `34,495,012` بايت وبصمته SHA-256 `7AF8B0C14248BB40B1D5566362B56C271254385F383C20C0E116153970B6C4DF`؛ Delivery UUID هو `0625345c-86d1-495b-adba-430f117da4f7`. GitHub artifact رقم `9732022681` وحجمه `34,247,503` وبصمة ZIP `DBC688437838411FFCC323F4E316CB2330A51AD049B778C9A7335882523BEDA7`.
- أكد App Store Connect status run `33310667046` أن `0.1.0 (13)` على iOS حالته `VALID` وغير منتهٍ ومرتبط بمجموعة `VoiceBrief Internal`. build ID يطابق Delivery UUID، وتاريخ الرفع `2026-08-30T05:21:36-07:00` والانتهاء `2026-11-28T04:21:36-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9732090968`، حجمه `419` وبصمة ZIP `760F65D86C11F37CEC16DFFB3E443E88914085FC6839DDC66D5AB57C81963A82`. بقي قبول iPhone الحقيقي لظهور بطاقة الموعد، تأكيد الساعة، وزر المنبّه في البناء 13.
- أثبت اختبار iPhone الحقيقي للبناء 13 أن بطاقة الموعد وإجراء التقويم يعملان، وأن إجراء «ضبط منبّه» السابق لم يكن منبّهًا بل إشعارًا محليًا ولذلك رفضه المستخدم وظيفيًا. البناء 14 يضيف `NSAlarmKitUsageDescription` ويجدول `Alarm.Schedule.fixed` عبر `AlarmManager` على iOS 26.1 فما بعد. نجح محليًا format على 73 ملفًا وanalyze و34 اختبارًا غير golden و11 golden وبناء APK debug حجمه `177,608,929` بايت وبصمته SHA-256 `55E9D3A19A34167B37C49B2D84B485FC21C66B86A787E0379DDF289A87B51986`.
- نجح CI للبناء 14 في run `33312938537` بكل وظائفه، ونجح TestFlight run `33312941887` في تجميع Swift بـXcode 26.6، والأرشفة، والتوقيع، وفحص التطبيق والامتداد وملفات التوزيع والاستحقاقات، والتحقق والرفع. IPA `VoiceBrief-0.1.0-build14-AppStore.ipa` حجمه `34,505,689` بايت وبصمته SHA-256 `DD407238013187CDC315FAA0627F73EAA674467B94D55FD06B9B78C1E4887AF6`؛ Delivery UUID هو `b5d2a430-b878-42e6-9436-2cedabb0647f`. GitHub artifact رقم `9732779976` وحجمه `34,258,854` بايت وبصمة ZIP `8252ABBA01FCE551BC0E6145E32528AF04BF0628129F020D49F8F1BC57FDF049`.
- أكد App Store status run `33313777125` أن `0.1.0 (14)` على iOS حالته `VALID`، غير منتهٍ، ومضاف إلى `VoiceBrief Internal`. build ID يطابق Delivery UUID، وتاريخ الرفع `2026-08-30T06:17:21-07:00` والانتهاء `2026-11-28T05:17:21-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9732841813` وحجمه `419` بايت وبصمة ZIP `01D9F57C262508F111A1987921AA1D246BD9D952EDEF525404948C6AE70D4A4D`. بقي قبول المنبّه الفعلي على iPhone: الإذن، واجهة الرنين، الصوت مع الصامت/التركيز، والإيقاف؛ لا يوجد ادعاء قبول جهاز بعد.
- أوضح اختبار المستخدم للبناء 14 أن `AlarmKit` يطلب الإذن لكن منبّه التطبيق لا يظهر داخل قائمة تطبيق «الساعة»، وهو سلوك Apple المتوقع للمنبّهات المملوكة للتطبيق. البناء 15 يضيف إدارة صريحة داخل VoiceBrief: ثلاث نغمات أصلية مضمّنة مع معاينة قبل الضبط، وحفظ المعرّف/العنوان/الوقت/النغمة محليًا، وصفحة «منبّهات VoiceBrief» تعرض المنبّهات الفعلية الموجودة لدى `AlarmManager` وتسمح بإلغائها. بعد الضبط ينتقل iPhone مباشرة إلى القائمة. المنبّهات المنفردة التي رنّت وتوقفت تُنظف عند القراءة وفق حالة `AlarmKit`.
- تحقق البناء 15 محليًا عبر format على 74 ملفًا، وanalyze بلا مشكلات، و36 اختبارًا غير golden، و11 golden، وبناء Android debug. APK حجمه `177,626,113` بايت وبصمته SHA-256 `8E0764D5E64C7C98282AF8ECF24E6258E63A3B8E40495F08AF8E18FE24118A9E`. نجح CI الكامل في run `33316201476` بعد تصحيح تحقق رقم البناء: Deno وAndroid وgolden وتجميع iOS غير الموقّع كلها ناجحة.
- نجح TestFlight run `33315830789` في الأرشفة والتوقيع وفحص التطبيق والامتداد والاستحقاقات والتحقق والرفع. IPA `VoiceBrief-0.1.0-build15-AppStore.ipa` بصمته SHA-256 `44502A8D9754F5B71927866689DB08E18A549A23CE66E44FE66859F945160174`؛ Delivery UUID هو `08fc7b55-add6-4223-8540-39a7919a733b`. GitHub artifact رقم `9733609871` وحجمه `34,765,290` بايت وبصمة ZIP `8C02A04728443AC8D4A0E94798032F106869D91CC7E7C396C13557F973D63991`.
- أكد App Store Connect status run `33317195493` أن `0.1.0 (15)` حالته `VALID` وغير منتهٍ ومرتبط فعليًا بمجموعة `VoiceBrief Internal`، لذلك أصبح متاحًا للمختبرين في TestFlight. build ID يطابق Delivery UUID `08fc7b55-add6-4223-8540-39a7919a733b`؛ تاريخ الرفع `2026-08-30T07:20:07-07:00` والانتهاء `2026-11-28T06:20:07-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9733809725` وحجمه `442` بايت وبصمة ZIP `7AF3A431885AC3C5F55B61543A7562C896AB7236B55FB047201BFF09032F6DF1`. تبقى تجربة iPhone الحقيقية لظهور اسم الموعد في واجهة Apple، والنغمات، والقائمة، والإلغاء مطلوبة ولا يوجد ادعاء قبول جهاز بعد.
- البناء 16 يستجيب لرفض المستخدم لنغمات البناء 15: حُذفت ملفات WAV الثلاثة ومراجع Xcode، وأصبح الاختيار الأساسي صوت Apple الافتراضي الحقيقي عبر `AlarmKit .default`. يمكن اختيار ملف صوتي أو فيديو؛ يستخرج iOS أول 29 ثانية، يحولها محليًا على طابور خلفي إلى CAF أحادي PCM داخل `Library/Sounds`، ويستخدم الاسم المعروض مع المنبّه. لا يمكن لتطبيق طرف ثالث عرض مكتبة نغمات تطبيق «الساعة» نفسه. أُعيد تصميم `/alarms` بعنوان عربي خالص وبطاقة صوت وحالة فراغ متوسطة وبطاقات مواعيد قابلة للتكبير، مع اختبار بصري عربي جديد.
- تحقق البناء 16 محليًا عبر format على 75 ملفًا، وanalyze بلا مشكلات، و37 اختبارًا غير golden، و12 golden، وبناء Android debug. APK حجمه `177,642,309` بايت وبصمته SHA-256 `5C285160AB09649C7409DF25AA28E2923EF768A8F822A28BF12E709D71FB50F0`. نجح CI الكامل في run `33319637732`، بما فيه تجميع Xcode 26.6 غير الموقّع.
- نجح TestFlight run `33319637699` في الأرشفة والتوقيع وفحص التطبيق والامتداد والاستحقاقات والتحقق والرفع. IPA `VoiceBrief-0.1.0-build16-AppStore.ipa` حجمه `34,612,519` بايت وبصمته SHA-256 `C63AE9AD859F24A325095A954411D72A4501A96B44155098647AB432AD0BE32E`؛ Delivery UUID/build ID هو `1d2cc0f0-f375-4665-bd20-8b88f89dd431`. GitHub artifact رقم `9734711162` وحجمه `34,363,709` بايت وبصمة ZIP `1F09A8271A994C569D806A78831D9DAD2B792F59A94E7C19424EBEF2C0ECABA5`.
- أكد App Store status run `33320659130` أن `0.1.0 (16)` حالته `VALID`، غير منتهٍ، وموجود في `VoiceBrief Internal`؛ تقرير الحالة artifact رقم `9734784217` وحجمه `442` بايت وبصمة ZIP `D139033ED636C29593C225FD8C4AF8FC81DB2D6A6BE17489974E01B4C97713F1`. تجربة استيراد الصوت/الفيديو ورنين الصوت الافتراضي/المخصص على iPhone الحقيقي ما زالت مطلوبة ولا يوجد ادعاء قبول جهاز.

...
## مشروع ضمانك المستقل — Build 30 والخطة المجانية الشهرية منشوران للاختبار (2026-09-02)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.1+30`. Build `4.5.1 (30)` هو الأحدث في iPhone TestFlight وحالته `VALID` وداخل `Damanak Internal` ذات المختبر الواحد. إصدار Apple العام `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة، بينما بقي Google Alpha على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً ضمن اختبار Build `30`.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901200000`. `verify-store-purchase` v29 `ACTIVE` مع `verify_jwt=false` وSHA-256 للحزمة `b53fcf3aa2fc04bc03621b40f8bf7acc4f6fe39a96618e368622c9f8dab0568e`، و`refresh-store-entitlements` v8 `ACTIVE` مع `verify_jwt=false` وبصمة `9720254cbbb3a7d9ca344551b38515347797c1cb2e0de64a23b2deead598aabc`. أعاد مسارا runtime بلا جلسة/سر `401`، وبقيت جدولة `damanak-entitlement-refresh` نشطة كل 5 دقائق. بقي الفاحص المتصل أولاً ثم فاحص Apple الرسمي عند `signedDate`؛ فقط عند `status1/status1` يستخدم فاحص التوافق fail-closed، وتبقى `FAILURE=7` وإخفاقات التطبيق/البيئة/السلسلة مرفوضة مع حواجز الحساب المحذوف والربط الذري.
...
- App Store Connect: أكد الفحص القرائي [33560754821](https://github.com/m7madv/otlobli/actions/runs/33560754821) أن Build `4.5.1 (30)` `VALID` بالمعرّف `47da8d54-cadc-4f96-82cb-9e81a01c3664` وداخل `Damanak Internal` ذات المختبر الواحد. بقي إصدار `4.5.0` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707` `READY_FOR_SALE`، ولا توجد مسودة مراجعة أو إصدار iOS قابل للتعديل، و`mutationRequested=false`.

...
## مشروع ضمانك المستقل — مرشح Build 30 والخطة المجانية الشهرية (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.1+30`. Build `4.5.1 (29)` يبقى الأحدث في iPhone TestFlight إلى أن يكتمل رفع Build `30` المنفصل. إصدار Apple العام `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة، بينما بقي Google Alpha على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً ضمن اختبار Build `30`.
...
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` توجه كل مداخل إنشاء المتجر القديمة والجديدة إلى القاعدة نفسها، وتبقي مرآة توافق فقط لـBuildات `23–29` بينما تكون الصلاحية الفعلية في جداول `private`. تنظف تجارب legacy وحالات Apple Sandbox فقط إلى tombstones، وتحصر RPC الطرفي في `app_store`، وتفشل المعاملة بدلاً من تعديل صف manual/activation_code غير متسق. أثبت snapshot داخل `BEGIN/ROLLBACK` قبل تطبيق migration وبعدها بقاء إنتاج Apple وكل Google وسلالته وإيصالاته وعدادات الحماية بلا تغيير، كما نجحت مصالحة `scale→growth` عبر RPC المزود الفعلي. لم تُنشر بعد؛ يجب إتاحة Build `30` في TestFlight أولاً ثم نشرها مع دالتي التحقق والتحديث والتحقق من بصمة الإنتاج قبل/بعد.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901012000`، و`verify-store-purchase` v28 نشطة مع `verify_jwt=false` وSHA-256 للحزمة `ecee6ad20c0efd25c452d581552aafe903be8316cc43c767a0b551bd83dd7262`، و`refresh-store-entitlements` v7 وبقية الدوال بالإصدارات السابقة. v28 تبقي الفاحص المتصل أولاً ثم فاحص Apple الرسمي عند `signedDate` للحالات `1/2/6` والمهلة؛ فقط عند `status1/status1` تستخدم فاحص التوافق fail-closed. تبقى `FAILURE=7` وإخفاقات التطبيق/البيئة/طول السلسلة مرفوضة، ثم تلزم المطابقة الدقيقة وحواجز الحساب المحذوف والربط الذري. حالة الدالة `ACTIVE` واستجاب runtime الحي لطلب بلا جلسة بـ`401`.
- نجحت استعادة Apple orphan فعلياً بعد v28 عند `2026-09-01 09:22:26` بتوقيت الرياض وأعادت HTTP `200`، ثم أعاد التحديث المجدول التحقق عند `09:25:03`. بناءً على طلب المستخدم نُفذ لاحقاً reset ذري للاشتراك الاختباري الوحيد: لا يوجد الآن أي `source=store` أو entitlement حالي أو refresh candidate، وبقي tombstone واحد `app_store/sandbox/scale/monthly/revoked` لحماية lineage، وحُذفت grant المرتبطة من دون مس نافذة مراجعة Apple أو عدادها. أكد جرد قراءة فقط بعد دورة الجدولة أن القيم بقيت صفراً ولم يُعد التفعيل. لا توجد أي اشتراكات أو استحقاقات `production`.
- نجحت اختبارات الإنتاج الخمسة داخل `BEGIN/ROLLBACK`: `orphan_store_lineage_recovery_live.sql` و`store_creation_without_repeat_trial_live.sql` و`subscription_lifecycle_live.sql` و`account_deletion_successor_live.sql` و`warranty_tenant_integrity_live.sql`. ملف `supabase/schema.sql` حجمه `395,324` بايت وSHA-256 `5A1E083994801006E3C620E6D91C611E2097D3B216FA1D3E8123F97DAD2F8F3C`، واختبار PGlite يثبت تطابق تعريف RPC مع migration.
- التحقق المحلي لمرشح `4.5.1+30`: `flutter analyze --no-pub` نظيف، وFlutter `225/225`، وDeno `83/83`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ أكد `aapt` أن APK يحمل `versionName=4.5.1` و`versionCode=30`. حجمه `180,876,532` بايت وSHA-256 `C4CA7B5F6EABF80C74FE5121067E4D0AC11B85809F0FDC36BCA9FD4526D99479`.
...
- App Store Connect: أكد الفحص القرائي [33533818718](https://github.com/m7madv/otlobli/actions/runs/33533818718) أن الإصدار `4.5.0` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707` أصبح `READY_FOR_SALE`، ولا توجد مسودة مراجعة نشطة أو إصدار iOS قابل للتعديل. الاشتراكات الستة ومجموعة الاشتراك `22332400` معتمدة. أكد الفحص اللاحق [33537059054](https://github.com/m7madv/otlobli/actions/runs/33537059054) أن Build `4.5.1 (29)` صار الأحدث، وحالته `VALID` ومعرّفه `e029d611-6d1a-44b7-a93b-be8478d6e7d0` وداخل `Damanak Internal` ذات المختبر الواحد، مع `mutationRequested=false`.
- نجح فحص Build `4.5.1 (29)` الموقّع بلا رفع في [33534828721](https://github.com/m7madv/otlobli/actions/runs/33534828721)، ثم نجح التحقق والرفع في [33535634162](https://github.com/m7madv/otlobli/actions/runs/33535634162) من الالتزام `316e31771974e5b069a3f5e98b0e2df58ee302cf`. Delivery UUID هو `e029d611-6d1a-44b7-a93b-be8478d6e7d0`. الـIPA المحلية `damanak_flutter/output/github-run-33535634162/damanak-ios-4.5.1-build-29-signed/ضمانك.ipa` حجمها `23,972,034` بايت وSHA-256 `297C151419D1CC4AA09BE776C1673303CF1A00782F9B840555CEA34F2BD88EA6`. سبقت ذلك محاولة `4.5.0 (29)` في [33532659092](https://github.com/m7madv/otlobli/actions/runs/33532659092) رفضتها Apple بالرمزين `90062/90186` لأن قطار `4.5.0` مغلق؛ لذلك نُقل رقم التسويق فقط إلى `4.5.1` مع إبقاء رقم البناء `29`.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، ثم نجحت الاستعادة بعد حذف الحساب قبل reset الاختبار. Build `4.5.1 (29)` مرفوع و`VALID` لكنه لم يُثبت أو يُقبل بعد على جهاز حقيقي. سجل الشراء `sandbox` يبقى لدى Apple حتى يمسحه المستخدم من حساب الاختبار؛ لذلك قد تعيد الاستعادة تفعيله. لا يوجد قبول جهاز للبنية الجديدة أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو الجهاز الضعيف.

...
أصلًا في قائمة Play. الحساب الجديد الوحيد غير المستخدم هو حساب المالك المخصص
للمثيل `01`، لكنه لم يثبت في Android AccountManager بعد؛ لذلك اسم المثيل يحمل
`PENDING`. المثيل `01` مفتوح وحده الآن على شاشة تسجيل Google، وADB مفعّل
مؤقتًا فيه فقط (`basicSettings.adbDebug=1`) بينما أعيد إلى `0` في `02..12`.
لا تُدخل كلمة المرور أو تعرضها؛ بعد نجاح الإضافة تحقق طرفيًا من وجود حساب واحد،
أزل `PENDING` من الاسم، ثم أعد ADB إلى `0` وأغلق المثيل. بقي AVD Android Studio
المنفصل دون تغيير.

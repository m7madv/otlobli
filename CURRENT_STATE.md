...
## مشروع ضمانك المستقل — مصدر Build 26 وإصلاح الاستعادة الخادمي v28 (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+26`. Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير، وBuildات `24` و`25` و`26` داخل TestFlight فقط. لا تنسب رقعة Build `26` إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
...
- التحقق المحلي النهائي: `flutter analyze` نظيف وFlutter `181/181` سابقاً؛ Deno الآن `77/77`، واختبار التوافق الموجه `6/6` على Deno `2.1.4` نفسه و`6/6` على Deno الحديث، واختبار migration `1/1`، و`deno check --frozen` للدالة و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. لم تجد المراجعة الأمنية المستقلة مسار تجاوز أو مانع نشر. نجح web release وAndroid debug سابقاً؛ APK Build `26` المحلي حجمه `207,970,698` بايت وSHA-256 `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لم يتغير Flutter في إصلاح v28 الخادمي ولم يُنشأ Build جديد أو يُرفع Build `26` إلى Google.
- راجعت واجهة الاشتراك وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: RTL، حالة تقدم واحدة، رسائل قابلة للإجراء، أهداف لمس، و`320×568` مع تكبير `200%` ناجحة آلياً. لم يُنفذ قبول VoiceOver أو iPhone حقيقي أو جهاز ضعيف.

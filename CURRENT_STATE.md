...
- المشروع المستقل داخل `voicebrief_flutter/` باسم **VoiceBrief** ومعرّف مرشح الإنتاج `app.voicebrief.mobile` وإصدار `0.1.0+1`. مصدره مرفوع على الفرع `codex/voicebrief-ios` دون تضمين تعديلات المستخدم المرحّلة في الجذر، ولم تُعدّل مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة.
...
- رُفع المشروع إلى GitHub على الفرع `codex/voicebrief-ios`. التشغيل الأخضر `32829955468` عند `6279ad4` نجح في Linux/Windows/macOS: تحليل و25 اختباراً وبناء Android debug، الصور الذهبية الثماني، و`pod install` وبناء iOS release بلا توقيع. أصلح هذا البناء رفض تثبيت `VoiceBriefShare.appex` الناتج عن `bundleVersion must be set` بتعيين `0.1.0 (1)` صراحةً للإضافة، ثم تحقق من تطابق `CFBundleShortVersionString` و`CFBundleVersion` بين التطبيق والإضافة. IPA المحلية `voicebrief_flutter/output/github-run-32829955468/VoiceBrief-0.1.0-build1-unsigned.ipa`، الحجم `16,807,035`، SHA-256 `0BBD51929715DB597F13A8ADB92F4D9E73FC3A492451F841217BC329A1A020B8`. الملف بلا provisioning وتوقيع، لذلك يجب أن توقّع أداة التثبيت التطبيق والإضافة معاً؛ لا يزال Apple signing الرسمي واختبار iPhone الحقيقي مطلوبين قبل المتجر. نقطة الرجوع السابقة `voicebrief-prebuild-20260824`، وApple Design Skill مثبت عند `d0bac1e765a27a696839e62962e36330ce72f0b7`.

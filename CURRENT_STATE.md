...
## مشروع ضمانك المستقل — مصدر Build 29 وإعادة بناء الاشتراكات (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+29`. Build `28` هو الأحدث في iPhone TestFlight إلى أن يكتمل رفع Build `29`، وBuild `23` بقي حزمة App Review وGoogle Alpha من دون تغيير. لا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ولا تمس submission الجاري.
...
- التحقق النهائي المحلي للمصدر `4.5.0+29`: `flutter analyze` نظيف، وFlutter `211/211`، واختبارات الاشتراك المركزة `69/69` بمراجعة خصومية مستقلة، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK Build `29` المحلي في `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `208,066,330` بايت وSHA-256 `08D0DDEC1E1BA1C734DA36C1F4CDCA5F2B77543D8044A8D523435E034E14A0AE`.
- راجعت واجهة الاشتراك وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: ملخص حالة واحد، ثلاث باقات مضغوطة، إجراء أساسي سفلي واحد، والاستعادة والإدارة كإجراءين ثانويين. أزيلت بطاقة الاتصال والرقم صفر والتنبيهات المتكررة ومعرّفات المنتجات. نجح RTL واختبار `320×568` مع تكبير `200%` والتمرير آلياً؛ لم يُنفذ قبول VoiceOver أو iPhone حقيقي أو جهاز ضعيف للواجهة الجديدة.
...
- App Store Connect: submission `69104e50-7a9a-4102-a717-ca341d442897` ما يزال يضم Build `23` ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، وحالته `Waiting for Review`. رفع run [33481729437](https://github.com/m7madv/otlobli/actions/runs/33481729437) الالتزام `e073cd5f7fad489f094337a142c46786eda2d57f` بوصفه iPhone Build `4.5.0 (27)`؛ نجح التوقيع والتحقق والرفع، ثم أكد فحص القراءة [33482932576](https://github.com/m7madv/otlobli/actions/runs/33482932576) أن البناء `VALID` بالمعرّف/Delivery UUID `5b04b5da-22e5-4cc9-affc-691e3b7e840f` وداخل `Damanak Internal` ذات المختبر الواحد، مع `mutationRequested=false`. الـIPA في `damanak_flutter/output/github-run-33481729437/ضمانك.ipa` حجمها `23,943,985` بايت وSHA-256 `808712BDD6E46942C9CE837B01FC329ED1CE25A3FC47E95F577BEC03790E2547`. لم يُضف Build `27` إلى App Review.
- فحص macOS الموقّع بلا رفع نجح في run [33520286056](https://github.com/m7madv/otlobli/actions/runs/33520286056)، ثم رفع run [33520953615](https://github.com/m7madv/otlobli/actions/runs/33520953615) الالتزام `baaa3e151036d280d8c2ca17f5b069f3aa9e44e8` بوصفه iPhone Build `4.5.0 (28)`. نجح `validate-app` و`upload-app`، وأكد فحص القراءة [33522527864](https://github.com/m7madv/otlobli/actions/runs/33522527864) أن البناء `VALID` بالمعرّف/Delivery UUID `a9038423-e4fc-4499-8c04-bba4c0f1d317` وداخل `Damanak Internal` ذات المختبر الواحد، مع بقاء App Review `WAITING_FOR_REVIEW` و`mutationRequested=false`. الـIPA في `damanak_flutter/output/github-run-33520953615/ضمانك.ipa` حجمها `23,963,467` بايت وSHA-256 `230304290CE79C5E8F47A1D511DC8C845846C88BF529F7D167CA87B07BA65561`.

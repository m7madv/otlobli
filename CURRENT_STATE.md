...
- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+26`. Build `23` بقي حزمة App Review وGoogle Alpha من دون تغيير، وBuildا `24` و`25` بقيا داخل TestFlight فقط. لا تنسب رقعة Build `26` إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
...
- App Store Connect: submission `69104e50-7a9a-4102-a717-ca341d442897` ما يزال يضم Build `23` ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر، وحالته `Waiting for Review`. Build `25` ما يزال `VALID` بالمعرّف `8e864062-28e0-4250-abd2-dac25f8df1b9` وداخل `Damanak Internal` ذات المختبر الواحد. Build `26` لم يُرفع بعد في هذه النقطة، ولن يُضاف إلى App Review.

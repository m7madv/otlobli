...
## مشروع ضمانك المستقل 4.5.0+23 قيد الإصدار (2026-08-31)

- المصدر وإجراءات GitHub موحدة على `4.5.0+23`. يعالج Build `23` فشل تعرف التطبيق على شراء Apple بعد نجاح الدفع، ويربط إيصال المتجر بالمتجر ذرياً، ويجدد الاستحقاقات دورياً، ويمنع منح تجربة جديدة بتبديل الحساب على التثبيت نفسه. لا يُعامل نجاح الدفع محلياً كاستحقاق قبل تحقق الخادم. تتضمن الشجرة المحلية إصلاحات أمنية لاحقة؛ الأجزاء الخادمية منشورة، أما تغييرات Flutter فتحتاج رقم بناء تالياً ولا تُنسب إلى الحزمة المرسلة فعلاً.
- نُشرت migrations من `20260831030000` حتى `20260831150000` على Supabase الصحيح `exxayzlklvgeyqhvtzgi`. دوال الإنتاج النشطة: `verify-store-purchase` v20، `refresh-store-entitlements` v4، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v16، و`warranty-card` v14. تعمل جدولة `damanak-entitlement-refresh` كل 5 دقائق. قيود قاعدة البيانات المركبة تمنع ربط ضمان أو مطالبة بصف من متجر آخر، كما يفلتر رابط الضمان العام المطالبات بـ`warranty_id` و`store_id` معاً.
- ملف `supabase/schema.sql` متزامن مع ترحيلات سلامة المستأجر، حجمه `336,290` بايت وبصمته `48EF5289755DF7698F92384B2930AB1CAEA20BDF5DCBA4F2BF97546A4EF946D2`. نجح اختبار قاعدة بيانات حي بمتجرين ورفض المراجع العابرة ثم نفذ `ROLLBACK`. سياسة الخصوصية المنشورة تفصح عن الذكاء الاصطناعي والمرفقات ومعرّف التثبيت والشراء والتكاملات والاحتفاظ بعد الحذف.
- أظهر التدقيق الأمني الشامل 5 نتائج: 3 متوسطة و2 منخفضة، من دون عالية أو حرجة. أُغلقت سلامة علاقات المتاجر، وحقن صيغ CSV في المصدر، وحدود جسم المطالبة في الإنتاج. بقي إثبات الجهاز الحقيقي للتجربة مرتبطاً بـApp Attest/DeviceCheck وPlay Integrity وقرار الويب، وبقيت مدة روابط الضمان وإبطالها قرار منتج حتى لا تُكسر روابط العملاء الحالية بصمت.
- نجح `flutter analyze` و`93/93` اختبار Flutter و`23/23` اختبار Deno مع `--allow-env` وفحص التنسيق و`git diff --check` وبناء web release وAndroid debug. ملف APK المحلي `build/app/outputs/flutter-apk/app-debug.apk` حجمه `180,737,784` بايت وبصمته `4E842A3676EA4C7DB01C45BEAF0CC7443ED44F3C6CC54AFB8AB552EED3269BC1`.
- App Store Connect: رُفعت صور iPhone الخمس وصور iPad الخمس في run `33341350096`. وقّع run `33341350366` Build `23` ورفعه؛ بصمة IPA هي `9C024E7444F339DDEABEB33FA69EF634368B5FAFF820EC9FF5B7D3E57C1E8815`. الإصدار `4.5.0` مع مجموعة الاشتراك ومنتجاتها الستة ضمن submission `69104e50-7a9a-4102-a717-ca341d442897` المكون من 8 عناصر، وكلها `Waiting for Review`. لا تغيّر أو تلغِ هذه المسودة أثناء المراجعة.
...
- حساب مراجعة Google داخل متجر `Damanak Note 8 Test` يملك وصول `scale` حتى `2036-08-31` بلا شراء أو تجربة. الهاتف يبقى على Build `22` المثبت من Play إلى أن تقبل Google Build `23`؛ لا تستبدله بملف جانبي مختلف التوقيع. ما يزال اختبار شراء/استعادة حقيقي على iPhone وAndroid وقبول VoiceOver/TalkBack والجهاز الضعيف منفصلاً؛ لا يُستنتج من نجاح البناء أو API.

...
## مشروع ضمانك المستقل — مصدر Build 28 ومنع خفض الباقة (2026-09-01)

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.0+28`. Build `28` هو الأحدث في iPhone TestFlight، وBuild `23` بقي حزمة App Review وGoogle Alpha من دون تغيير. لا تنسب منع الخفض إلى Build `27` أو `23`، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` ما دام submission الجاري يستخدمه.
- عقد الانتقال المركزي أصبح `start/current/upgrade/blockedDowngrade/billingCycleChange`. الباقة الأدنى تبقى ظاهرة للمقارنة فقط بزر معطل ورسالة واضحة، ويعيد `AppController` التصنيف بعد تحديث الخادم ثم يرفضها، كما تعيد خدمة المتجر الحراسة وترفض Google بدلاً من `ReplacementMode.deferred`. الاختبارات تثبت أن محاولة خفض Apple أو Google لا تصل إلى نافذة الدفع.
- قبل كل شراء Apple يفحص Build `28` استحقاقات StoreKit الحالية محلياً. إذا لم يوجد ربط خادمي لكن Apple ID يملك منتج ضمانك، يمنع شراءً جديداً ويطلب الاستعادة الصريحة. وإذا وجد ربط خادمي، فلا يفتح StoreKit إلا عند تطابق `originalTransactionId` والمنتج الحاليين مع Apple ID المفتوح؛ غياب الاستحقاق أو اختلاف الحساب/السلسلة/المنتج يفشل مغلقاً بلا JWS أو استعادة أو نقل ربط صامت.
...
- ثُبت أن منتجي «توسع» الشهري والسنوي موجودان وصحيحان في App Store Connect وبالمستوى الأعلى داخل مجموعة الاشتراك؛ كان التعطيل من حالة معاملة/استعادة سابقة لا من معرّفات الخطة أو سعر محلي. تعرض Flutter الكتالوج الجزئي القابل للاستخدام، وتسمي الخطة/الدورة/المعرّف المفقود، وتتيح إعادة المحاولة من دون تعطيل منتج «توسع» المتاح.
...
- التحقق النهائي للمصدر `4.5.0+28`: `flutter analyze` نظيف، وFlutter `198/198`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK Build `28` المحلي في `damanak_flutter/build/app/outputs/flutter-apk/app-debug.apk` حجمه `208,000,201` بايت وSHA-256 `A3DF9A22739B3E1E7938FA6EC3E9BE8CE21C6B3E0CABECBD22C6513832459BBD`.
- راجعت واجهة الاشتراك وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: RTL، ترقية بإجراء أساسي، باقة أدنى معلوماتية مقفلة بلا إجراء شراء، تغيير دورة بإجراء ثانوي، أسطح محايدة، وأهداف لمس. نجح اختبار `320×568` مع تكبير `200%` آلياً؛ لم يُنفذ قبول VoiceOver أو iPhone حقيقي أو جهاز ضعيف للواجهة الجديدة.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، ثم نجحت الاستعادة بعد حذف الحساب قبل reset الاختبار. Build `28` مرفوع و`VALID` لكنه لم يُثبت أو يُقبل على جهاز بعد، وسجل الشراء `sandbox` يبقى لدى Apple حتى يمسحه المستخدم من حساب الاختبار؛ لذلك قد تعيد الاستعادة تفعيله. لا يوجد قبول جهاز لمنع الخفض أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو الجهاز الضعيف.

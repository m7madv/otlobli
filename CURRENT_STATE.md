...
## مشروع ضمانك المستقل — مصدر Build 26 وإصلاح الاستعادة الخادمي v27 (2026-09-01)
...
- الاستعادة الصريحة وحدها ترسل `recoveryRequested=true`. اشتراك Apple القديم بعد حذف حساب ضمانك لا ينتقل إلا بعد تحقق JWS والجذور المثبتة، والتأكد أن الحساب/المتجر القديمين محذوفان وأن الهدف متجر وحيد بلا entitlement؛ ثم يُحدث `appAccountToken` بعد commit. يبدأ v27 بفاحصي Apple الرسميين المتصل وعند `signedDate`، ولا يشغّل فاحص التوافق الصارم إلا إذا أعاد كلاهما الحالة `1`. Google يدعم ربط Build `23` الحسابي وسلسلة predecessor/out-of-app ذرياً بالشروط نفسها. المسارات الخلفية تبقى صارمة ولا تنقل legacy تلقائياً.
- أثبتت تجربة TestFlight الحية شراء Apple ناجحاً على Build `26` عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حذف المستخدم حساب ضمانك بعد نحو 8 ثوانٍ مع بقاء الاشتراك لدى Apple. وصلت إعادة المحاولة بعد v26 إلى App Store Server API، لكن الفاحصين الرسميين المتصل وعند `signedDate` أعادا `VERIFICATION_FAILURE=1` قبل أي كتابة؛ محاولة «الشراء» اللاحقة لم تصل إلى الخادم لأن Apple تعتبر الاشتراك مملوكاً. لم تكن المشكلة غياب دفع أو منتج «توسع»، وبقي الحساب الجديد بلا entitlement قبل نشر v27.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901012000`، و`verify-store-purchase` v27 نشطة مع `verify_jwt=false` وSHA-256 للحزمة `7d219b7941da726d6a10dcb142fa862a21f26a110452be06ab628853ee1498b2`، و`refresh-store-entitlements` v7 وبقية الدوال بالإصدارات السابقة. v27 تبقي الفاحص المتصل أولاً ثم فاحص Apple الرسمي عند `signedDate` للحالات `1/2/6` والمهلة؛ فقط عند `status1/status1` يستخدم فاحصاً محلياً fail-closed يتحقق من DER وES256 وسلسلة Apple وOID والتواريخ والجذر المثبت. تبقى `FAILURE=7` وإخفاقات التطبيق/البيئة/طول السلسلة مرفوضة، ثم تلزم المطابقة الدقيقة وحواجز الحساب المحذوف والربط الذري.
...
- التحقق المحلي النهائي: `flutter analyze` نظيف، Flutter `181/181`، Deno `76/76`، واختبار migration `1/1`، و`deno check` للدالة و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug سابقاً؛ APK Build `26` المحلي حجمه `207,970,698` بايت وSHA-256 `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لم يتغير Flutter في إصلاح v27 الخادمي ولم يُنشأ Build جديد أو يُرفع Build `26` إلى Google.
...
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، لكن حذف الحساب مباشرةً أنشأ حالة orphan وكشف فشل الفاحص الرسمي في Edge. ما تزال إعادة الاستعادة بعد نشر v27، والشراء/الاستعادة على Android، وVoiceOver/TalkBack، والجهاز الضعيف بلا قبول نهائي؛ لا يُستنتج نجاحها من الاختبارات الآلية.

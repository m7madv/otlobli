...
## مشروع ضمانك المستقل 4.3.0+18 (2026-08-27)

- `damanak_flutter/` هو المصدر الحالي للإصدار `4.3.0+18`، وتجربته التشغيلية «الضمان أولاً»: الوصول إلى إصدار الضمان والبحث والمشاركة والمتابعة هو المسار الأبرز، مع بقاء البيع والمخزون والإدارة متاحة.
- حصص الباقات الحالية هي: بداية `100` ضمان شهرياً وعضوان، نمو `600` و`5` أعضاء، وتوسع `3000` و`15` عضواً. تعرض شاشة الاشتراك مزايا منفذة فقط، وتوصي بخطة نمو، وتوضح الاستعادة وإدارة الاشتراك وبقاء الضمانات السابقة للعرض والمتابعة.
- السعر والعملة يأتيان حصراً من Apple أو Google؛ لم تتغير معرّفات منتجات المتجر ولا توجد أسعار محلية بديلة.
- سبب فشل Build `17` في إظهار اشتراكات iPhone كان اجتماع إعداد توفر Apple القديم مع مسار كتالوج يعيد الفشل عند أول نتيجة ناقصة. Build `18` يجمع النتائج الجزئية، يستخدم مهلة كلية محدودة، ثم ينتقل من StoreKit 2 إلى StoreKit 1 والطلبات المجمعة من دون تعليق الشاشة. أضيفت كذلك مهل للشراء والاستعادة، وبقي السعر الحقيقي من المتجر فقط.
- فحص App Store Connect الأخير [33074176969](https://github.com/m7madv/otlobli/actions/runs/33074176969) ناجح: Build `18` حالته `VALID` وداخل مجموعة `Damanak Internal`، وبيانات إصدارات المجموعة والمنتجات `6/6` مكتملة، وتوفر خطط `UPFRONT` صحيح `6/6` في دول الخليج المحددة، وقطر موجودة لكل المنتجات مع أسعار `39.99/399.99` و`79.99/799.99` و`199.99/1999.99 QAR`. حقل الحالة القديم على أصل المنتج يعيد `MISSING_METADATA` لكنه غير مرجع البيانات بعد انتقال Apple إلى نموذج الإصدارات `4.4.1`؛ تحقق التقرير من كل إصدار وترجمته مباشرة.
- محاولة آمنة لإضافة إصدارات الاشتراكات إلى مسودة المراجعة رفضتها Apple بـ`STATE_ERROR.ENTITY_STATE_INVALID` وأضافت `0` عناصر؛ بقيت المسودة بعنصر مجموعة واحد ولم يُرسل التطبيق أو أي إصدار للمراجعة العامة. هذا لا يمنع كتالوج Sandbox/TestFlight، ولا يجوز إرسال أول اشتراك عاماً قبل تجهيز إصدار App Store العام.
- نُشر ترحيلا Supabase `20260827120000_damanak_plan_value_quotas.sql` و`20260827150000_damanak_database_hardening.sql`، وتطابقت قائمة الترحيلات البعيدة مع المحلية. كما نُشرت صفحات `legal` واختُبرت.
- التحقق الحالي: `flutter analyze` بلا مشاكل، و`52` اختباراً ناجحاً، و`6/6` لقطات ناجحة ومراجعة بصرياً، وبناء Flutter Web وAndroid debug ناجحان.
- وقّع التشغيل [33071848783](https://github.com/m7madv/otlobli/actions/runs/33071848783) Build `18` ورفعه إلى TestFlight. IPA المحلية `damanak_flutter/output/github-run-33071848783/ضمانك.ipa`، حجمها `23,303,261` بايت وبصمتها SHA-256 `E697D870320F768458DF626C06F3EB6545A4488D6D1CD851B13CB7D5C0AED4E8`.
- نشر التشغيل [33071309057](https://github.com/m7madv/otlobli/actions/runs/33071309057) Android Build `18` إلى الاختبار الداخلي. AAB المحلية `damanak_flutter/output/github-run-33071309057/app/outputs/bundle/release/app-release.aab`، حجمها `56,313,036` بايت وبصمتها SHA-256 `8FDC30EE19EE850649171E90A619BF40ACEC2D9E028B32D296A48D71607A7E08`. إعادة تسعير Google ما تزال مرفوضة بصلاحية حساب الخدمة، لذلك قد تبقى الأسعار القديمة. جهاز Note 8 ما يزال يحمل نسخة Play رقم `16` ولا يجوز تثبيت Build `18` جانبياً لاختبار الفوترة.
- لم يُنفذ شراء حقيقي، ولم يؤكد جهاز iPhone بعد ظهور الأسعار `6/6` أو الاستعادة على Build `18`؛ لا يوجد ادعاء قبول جهاز قبل هذه التجربة.

...
## مشروع VoiceBrief المستقل (2026-08-24)

- أُنشئ تطبيق Flutter إنتاجي مستقل وواضح داخل `voicebrief_flutter/` باسم **VoiceBrief**؛ لم تُعدّل مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة. معرّف العمل المؤقت `app.voicebrief.mobile` والإصدار `0.1.0+1`، ويلزم استبداله قبل تسجيل المتاجر.
- التطبيق يشمل onboarding، دخول البريد/Google/Apple، استيراد/تسجيل/مشاركة الصوت، شاشة إعداد وتشغيل، معالجة صادقة قابلة لإعادة المحاولة، نتائج منظمة، حفظ نصي محلي صريح ومعزول بالحساب، سجل، إعدادات، حذف حساب، واشتراك RevenueCat. التشغيل الافتراضي يستخدم mocks بلا مفاتيح أو خدمات مدفوعة.
- الخادم موجود داخل المشروع كمخطط Supabase متطابق في `supabase/migrations/20260824010000_voicebrief_initial.sql` و`supabase/schema.sql`، مع RLS وحجز دقائق ذري وتخزين صوت خاص ودوال Edge لـOpenAI وRevenueCat وحذف الحساب. لم يُنشر شيء إلى مشروع Supabase المرتبط بالجذر. دوال Deno الثلاث وتنسيقها وlint نجحت، وثُبّت `@supabase/supabase-js` على `2.112.4` في إعداد مستقل لكل دالة.
- نجح `flutter analyze` بلا ملاحظات، ونجح 15 اختبار وحدة/واجهة، وثمانية golden، واختبار تكامل مثبت على محاكي API 35. فُحصت onboarding/home/audio preparation بصرياً؛ ونجحت مشاركة Android الدافئة والباردة بملف صوت 1 ثانية، وظهر خطأ آمن لـURI غير المقروء، ولم يظهر `FATAL EXCEPTION` أو `E/flutter` في المسارات المراجعة.
- Android debug APK: `voicebrief_flutter/build/app/outputs/flutter-apk/app-debug.apk`؛ الحجم `213,017,571` بايت؛ SHA-256 `BE66658FA6DCB1FB2706AB26E0D36115580E55DBAF9480E56660D2EF0BD65286`. Release-mode AAB: `voicebrief_flutter/build/app/outputs/bundle/release/app-release.aab`؛ الحجم `55,446,125` بايت؛ SHA-256 `B35074C71240526501DD1C72E603ED888A7BA4E01CC1305AEA1C28D8480F3005`. كلاهما يستخدم مفتاح Android التجريبي وليس صالحاً للرفع إلى المتجر.
- مصدر iOS يتضمن هدف `VoiceBriefShare` وApp Group والتقويم، وworkflow مستقل في `.github/workflows/voicebrief-ci.yml`، لكنه لم يُبنَ على Windows ولم يُشغّل workflow. يلزم macOS/Xcode والتوقيع وجهاز حقيقي، كما يلزم فحص `content://` ناجح من تطبيق مُرسِل حقيقي وعمليات الشراء/OAuth/Supabase الفعلية قبل النشر.
...
## مشروع ضمانك المستقل 3.0.0 (2026-08-24)

- أصبح `damanak_flutter/` نظام تشغيل متكامل لبيع المنتجات وضماناتها وخدمة ما بعد البيع بمعرّف `com.damanak.damanak` وإصدار `3.0.0+3`. لم يتغير مصدر Otlobli أو دفعه أو محفظته.
- أضيفت العملات `SAR/AED/KWD/QAR/BHD/OMR/USD/SYP` بدقة كل عملة، والضريبة الشاملة أو غير الشاملة، والرقم الضريبي والسجل التجاري والعنوان وبادئة الفواتير ومدة الضمان الافتراضية. إصدار الضمان يحفظ سعر البيع والخصم والضريبة والإجمالي وطريقة الدفع ورقم الفاتورة.
- أضيف دليل العملاء والفروع ونقاط البيع وربطهما بالفاتورة والضمان، وتعديل المنتجات وتصنيفها وأرشفتها، وتقارير المبيعات والشهر والضريبة ومتوسط الفاتورة وطرق الدفع، وتصدير `CSV` وسجل التدقيق، واستعادة كلمة المرور.
- التنفيذ المحلي و`Supabase` متطابقان عبر `DamanakRepository`. مخطط الأساس هو `20260824090000_damanak_core.sql`، ومخطط العمليات الجديد `20260824130000_damanak_business_operations.sql` يضيف الجداول والقيود و`RLS` والتدقيق وتوليد الفاتورة. لم يُنشأ مشروع `Supabase` بعيد ولم يُطبق شيء على قاعدة Otlobli.

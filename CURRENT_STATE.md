...
## ضمانك — اللغات وصور المتجر 2026-09-06

الفحص `34040072308` أكد Build32 VALID بالمعرّف `7848f16a-97f9-4315-93bd-daea14024fc2` ووجوده ضمن Damanak Internal؛ لا تغيير للمختبرين. اكتملت أيضاً الإنجليزية البريطانية en-GB المناسبة للغة متجر الخليج الافتراضية: metadata run `34040227259` والصور `34040273515`. الإجمالي 11 محلية متجر و110 صور COMPLETE وبصماتها مطابقة، مع بقاء 10 لغات داخل التطبيق. لا تغيير للحزمة.

المصدر الصحيح في `C:\Users\MOHAMMAD\Projects\SHEIN IN SIRYA` على `codex/damanak-simple-pos`، وليس worktree `12a5`. مصدر التطبيق `8f1327b` للإصدار `4.6.0+32` مرفوع؛ تعديلات التشغيل اللاحقة لا تغيّر التطبيق. اللغات العشر مكتملة بنيوياً بـ1205 مفتاح لكل لغة، مع تفضيل الجهاز/اختيار يدوي وRTL/LTR وتنسيق التواريخ وموارد iOS. بيانات العملاء والأسعار لا تُترجم.

التحليل نظيف، Flutter `299/299` منها 53 لغة/تخطيط بعرض320 وتكبير150%، وبناء web release ناجح بلا نشر. مراجعة Apple Design موثقة؛ التصحيحات تشمل العناوين وقائمة الدعوة وبطاقات المطالبات والنصوص السياقية. لا اعتماد لغوي شامل أو قبول جهاز حقيقي/قارئ شاشة/شراء.

Apple: أُرسل `4.6.0 (32)` بطلب المستخدم الصريح يوم2026-09-06 الساعة18:13 بتوقيت الرياض، وحالته المؤكدة في App Store Connect هي Waiting for Review. طلب المراجعة `e80028ff-2ee6-48d6-b414-6172d31b2c3a` يضم عنصر إصدار واحد؛ معرّف الإصدار `af5f3581-d237-476d-b8c4-a42c73f7cc19`. رُبط Build32 وفُعّل النشر التلقائي بعد الموافقة لجميع المستخدمين مع حفظ التقييمات. حُدّثت ملاحظات المراجعة للغات العشر والحصة المجانية20 ضماناً شهرياً بدل وصف التجربة القديم. المنشور يبقى4.5.0 حتى الموافقة والنشر؛ لا تلغِ الطلب أو تعِد رفع الحزمة. IPA السابق حجمه24965730 وبصمته `E4728DFE2289CEC91FD9449AB057A514593F682BE1815B543F2F2DEF6CBBA1A7`. لم يُحل قيد Sandbox ولم يُختبر شراء جديد؛ يلزم وصول مالك Supabase للتحقق منه مستقلاً. التقرير `damanak_flutter/docs/LOCALIZATION_RELEASE_2026-09-06.md` يحفظ التفاصيل.

لم تتغير بلدان التوفر الستة أو منتجات الاشتراك أو الاستحقاقات، ولم تُحذف حسابات. عائق Sandbox/Supabase السابق لم يُحل. سير التشغيل المسجّل هو `damanak-apple-setup.yml` مع `asset_kind=metadata|screenshots` وإصدار صريح؛ لا تستخدم setup القديم المثبت على4.5.0/Build23 للنشر الجديد. بقي AGENTS.md والمشاريع الأخرى خارج الالتزامات.

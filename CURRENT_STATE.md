...
## ضمانك — اللغات والصور 2026-09-06 (قيد التنفيذ، غير منشور)

Apple metadata run `34039292196` أنشأ فعلاً مسودة `4.6.0` بمعرّف `af5f3581-d237-476d-b8c4-a42c73f7cc19` وحالة `PREPARE_FOR_SUBMISSION`، وحدثت العربية وأُنشئت بيانات إنجليزية جزئية. توقف عند تكرار App Info locale الذي تنشئه Apple تلقائياً مع لغة الإصدار؛ صُحح السكربت ليعيد قراءة App Info بعد إنشاء كل لغة ويحدث الموجود. لا حذف مسودة/لغة ولا تغيير النسخة الحية. iOS البديل `34039342937` جارٍ ولم تُرفع حزمة بعد.

run iOS `34039168233` فشل قبل البناء لأن حزمة أدوات تأليف الترجمة المستقلة لم تُثبت تبعياتها على CI؛ ليست أخطاء ترجمة واجهة أو توقيع. أضيف تثبيت/تحليل هذه الحزمة في خطوة CI مستقلة؛ لا تبعيات جديدة داخل التطبيق. لم تُرفع حزمة32. إعداد metadata جارٍ في run `34039292196` عبر سير Apple المسجّل.

تصحيح مسار التشغيل: قائمة GitHub workflows أكدت أن سير الصور أيضاً غير مسجّل؛ المسار القابل للاستدعاء هو `damanak-apple-setup.yml` رقم `341412875`. أضيفت إليه فروع معزولة `asset_kind=metadata|screenshots` تتجاوز خطوات الإعداد/الأسعار القديمة وتستهدف إصداراً محدداً. لا تغييرات Apple خارجية ناجحة حتى الآن؛ run التوقيع ما يزال جارياً.

رُفع المصدر `8f1327b`، وبدأ توقيع/رفع Build32 في GitHub run `34039168233`؛ لا نتيجة Apple بعد. اكتمل web release النهائي (75.1s). سير metadata الجديد رفضه GitHub بـ404 لأنه غير مسجّل في الفرع الافتراضي؛ أضيف خيار `asset_kind=metadata` إلى سير الصور الموجود ليشغّل السكربت المقيد بالإصدار نفسه، دون تعديل الفرع الافتراضي أو استعمال إعداد Apple القديم. إعداد المسودة الخارجية لم ينجح بعد.

التحقق النهائي للمصدر: التحليل نظيف، Flutter `299/299` منها `53/53` للغات والتخطيط، و100 صورة أُعيد توليدها ونجحت بوابتا المفاتيح والأبعاد. بطاقات المطالبات أصبحت تعرض العدد والأيقونة فوق النص لمنع تجزئة الكلمات الطويلة. بناء web release السابق ناجح وتُعاد نسخته بعد آخر تحرير. سيُحفظ مرشح المصدر وتُشغّل فحوص iOS الموقّعة؛ لا تعتبره مرفوعاً قبل نتيجة Apple.

المصدر الصحيح في `C:\Users\MOHAMMAD\Projects\SHEIN IN SIRYA` على `codex/damanak-simple-pos`؛ لا تستخدم worktree `12a5` القديم. مرشح الإصدار `4.6.0+32` محلي ولم يُرفع بعد. فحص Apple القرائي `34037302990` أكد المنشور `4.5.0 READY_FOR_SALE` وBuild `31 VALID`. طلب المستخدم إصداراً جديداً باللغات وصور المتجر؛ التفويض يخص هذا الإصدار، ولا يعني حذف الحسابات أو تغيير منتجات الاشتراك أو التوفر الجغرافي.

اكتملت 1205 مفاتيح لكل من ar/en/es/fr/de/pt/zh/hi/ja/ru، مع اختيار لغة الجهاز أو تفضيل يدوي وRTL/LTR وتنسيق التواريخ. الأسعار تبقى من المتجر ولا تُترجم بيانات العملاء. أضيفت موارد اسم/إذن كاميرا iOS للغات العشر وربطها بـXcode. توجد 10 أوصاف متجر و100 صورة فعلية، ويجري إعادة توليدها بعد التصحيحات التحريرية الأخيرة. الترجمة المولدة روجعت جزئياً وصُححت مصطلحات حرجة؛ اكتمال المفاتيح لا يساوي اعتماداً لغوياً شاملاً.

آخر مجموعة كاملة ناجحة 289/289، منها 43 اختبار لغة/اتجاه/عرض 320 وتكبير 150%. نجح التحليل وبناء الويب قبل آخر تحرير؛ تجري إعادة الفحوص النهائية. صُححت قيود العناوين وقائمة الدعوة وظلال مولّد الصور وصيغة رمز الدعوة التجريبية إلى 10 hexadecimal. الصور في `damanak_flutter/app_store_assets/ios/localized/` دون أسعار اشتراك مصطنعة. أدوات التأليف ونموذج الترجمة ليست اعتماداً للتطبيق؛ `.venv` و`model-cache` والكاش مستثناة من Git. لم تُرفع حزمة أو صور ولم يُرسل طلب مراجعة. عائق Supabase/Sandbox السابق لم يُحل، ولا قبول شراء حقيقي أو قارئ شاشة أو جهاز ضعيف. سير إعداد Apple القديم المثبت على 4.5.0/Build23 ليس أداة نشر 4.6.0.

Last updated: 2026-09-02
متابعة ضمانك المحلية: 2026-09-05 — نجح بناء web release؛ لا نشر أو حزمة هاتف جديدة.

## مشروع VoiceBrief المستقل (2026-08-25)

- اكتمل إعداد RevenueCat الإنتاجي الأساسي لمشروع `VoiceBrief` رقم `59213dad`: ربط Apple `app85cd86a950` وGoogle Play `appa60d40b8c1`، وإنشاء المنتجين الشهري والسنوي لكل متجر، وربط المنتجات الأربعة بالاستحقاق `pro` وبحزمتي `$rc_monthly` و`$rc_annual` داخل العرض `default`. حزمة lifetime بقيت Test Store-only وغير مربوطة بأي متجر إنتاجي.
- رُفع ملف Google القياسي إلى RevenueCat من `C:\Users\MOHAMMAD\.voicebrief\google\revenuecat-play-service-account.json` وحُفظ الاتصال بنجاح. يبقى الملف خارج Git وبصمته `60BF02E2D1AD0C9AE2B2321A4EC207B7EAD0134C38D52AC5D467E15D56497107` وصلاحياته محصورة بالمستخدم الحالي وSYSTEM. مفتاح Apple P8 يبقى خارج Git أيضاً.
- حُفظ مفتاحا SDK العامان لـRevenueCat في GitHub Secrets باسم `VOICEBRIEF_REVENUECAT_IOS_PUBLIC_SDK_KEY` و`VOICEBRIEF_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`. الإصدار `0.1.0+8` هو أول مصدر مُجهز لاستهلاكهما. أُنشئ سر webhook عشوائي 32 بايت وحُفظ فقط في Supabase باسم `REVENUECAT_WEBHOOK_SECRET`، ثم حُفظت قيمة `Bearer` المطابقة في RevenueCat webhook `whintgr41c91a4676` لكل أحداث الإنتاج وsandbox. تطابقت البصمة ورفضت الدالة الطلب غير المفوض بـ`401`؛ بقي حدث RevenueCat التجريبي وتأكيد البريد واختبار شراء/استعادة sandbox. خطتا Google الأساسيتان ما تزالان غير محفوظتين بسبب عائق Google الموثق، لذلك شراء Android ليس جاهزاً بعد.
- المشروع المستقل داخل `voicebrief_flutter/` باسم **VoiceBrief** ومعرّف الإنتاج `app.voicebrief.mobile` وإصدار المصدر `0.1.0+8`. مصدره على الفرع `codex/voicebrief-ios` دون تضمين تعديلات المستخدم المرحّلة في الجذر، ولم تُعدّل مصادر Otlobli أو Damanak أو منطق الدفع والمحفظة. حزم Android الإنتاجية الموقعة أدناه ما تزال البناء `1`، وBuild 7 الخاص بالدخول عبر المزودين فقط مكتمل ومثبت، وBuild 8 المكتمل هو أول نسخة TestFlight مفعّل فيها RevenueCat.
- الدخول أصبح حصراً عبر Apple وGoogle؛ حُذفت حقول البريد وكلمة المرور وإنشاء الحساب والاستعادة من الشاشة ومن عقد `AuthRepository` و`AppController`، وحُذف مكوّن كلمة المرور ونصوصه، كما عُطل مزود البريد داخل Supabase. Google على iOS يستخدم Google Sign-In الأصلي ثم يرسل `idToken` و`accessToken` مباشرة إلى Supabase، مثل Android، ولا يمر بمسار الويب الذي انتهى سابقاً إلى صفحة فارغة. الشاشة روجعت في العربية RTL والإنجليزية والوضعين الفاتح والداكن. Apple وGoogle مفعّلان الآن في Supabase ونجحا فعلياً على iPhone الموقّع.
...
- حذف السجل والنتيجة متفائل وفوري ثم تُكتب قاعدة البيانات في الخلفية؛ يظهر «تراجع» مباشرة ويعيد العنصر عند الضغط. حذف كل النصوص من الإعدادات صار فعالاً ويعرض نجاحاً أو فشلاً ويستعيد السجل عند فشل التخزين. التصدير ينشئ ملف TXT حقيقياً يضم كل أقسام النص المحفوظ ويفتح المشاركة، وتشرح الإعدادات أن الصوت مؤقت ومحذوف بعد المعالجة. «النص الحرفي للتسجيل» أصبح مرجعاً تلقائياً مطوياً ومشروحاً، وليس خيار تلخيص ثانياً.
...
- نجح `dart format` و`flutter analyze` بلا أخطاء، وكل الاختبارات غير الذهبية وعددها 30، وكل الصور الذهبية الإحدى عشرة، وبناء Android debug محلياً. يثبت الاختبار الجديد أن Google يعيد مستخدماً موثقاً مباشرة من المسار الأصلي، وتثبت اختبارات الأخطاء تحويل أكواد Supabase إلى أسباب واضحة. تشمل الصور شاشة الدخول الفاتحة/الداكنة ودخول Apple/Google بالعربية RTL، إضافة إلى الإعدادات والمسجل بالعربية عند 390x844. نجح أيضاً اختبار Android الموجّه السابق `:app:connectedDebugAndroidTest` لمشاركة Opus الحقيقية على Note 8.
- commit `eccc611` اجتاز run [32922121853](https://github.com/m7madv/otlobli/actions/runs/32922121853) كاملاً في Android و11 golden وiOS غير الموقّع. run [32922142795](https://github.com/m7madv/otlobli/actions/runs/32922142795) أرشف التطبيق وShare Extension وحقق الملفات والصلاحيات والتوقيع، ثم قبلت Apple الرفع وأنهت المعالجة. Build `0.1.0 (8)` حالته `Complete` ومعرّفه `71d308f7-b859-46ec-9d56-7622e58ffe79` ومربوط تلقائياً بمجموعة `VoiceBrief Internal` ذات المختبر الواحد. IPA المحلية حجمها `34,411,364` وبصمتها `459B17DDAC178B40C598574B70A0769141B35C78A6890DEDA4F3019A9BDB72CA` تحت `C:\Users\MOHAMMAD\Projects\VoiceBriefAuthRepair\artifacts\voicebrief-ios-0.1.0-build8-appstore\ios\signed\`؛ لم يُنفذ بعد شراء/استعادة sandbox أو قبول build 8 على جهاز حقيقي.
...
- Google Cloud مكتمل لـWeb وAndroid وiOS في المشروع `voicebrief-prod-mhm1981x`، وOAuth صار `External / In production`. كان سبب الفشل بعد موافقة Google هو رفض Supabase الصريح `Passed nonce and nonce in id_token should either both exist or not.`؛ فُعّل `Skip nonce checks` المخصص لحالة iOS، ثم سجل الخادم نجاح Google `Login / INFO` في `2026-08-26 01:06:16` بتوقيت الرياض. كان Apple معطلاً ومعرّفاته القديمة تخص Damanak؛ صُححت إلى `app.voicebrief.mobile`، وأنشئ مفتاح VoiceBrief الرسمي `N4FK6753YL` وسر صالح حتى `2027-02-21T22:07:34Z`، ثم سجل الخادم نجاح Apple `Login / INFO` عند `01:08:42`. ملف `.p8` محفوظ خارج Git في `C:\Users\MOHAMMAD\.voicebrief\apple\`.
- App Store Connect: مجموعة `VoiceBrief Pro` رقم `22335194` ومنتجا `voicebrief_pro_monthly` و`voicebrief_pro_annual` موجودة بالعربية والإنجليزية، بسعري قطر `29.99 QAR` شهرياً و`229.00 QAR` سنوياً، وحالتهما `Prepare for Submission`. Google Play: تطبيق VoiceBrief رقم `4973918177043935523` تحت حساب المطوّر العامل `8441225038702199576`، والمنتجان الشهري والسنوي موجودان. رُفعت AAB `0.1.0 (7)` بحجم `58,909,813` وبصمة `B314E144FB3F19769D3668F03EAFCC168A1E877401124511071CE2EC099919A3` إلى مسودة اختبار داخلي. مُنح حساب الخدمة وصول VoiceBrief فقط مع صلاحيات المالية والطلبات/الاشتراكات وإدارة التوفر/المنتجات، وأصبح فحص المنتج يعيد `200`، لكن تحويل الأسعار والكتابة ما زالا يعيدان `The caller does not have permission`. اختارت محاولة أخيرة بجلسة المالك كل المناطق الـ177 وحوّلت الشهري بنجاح وأظهرت قطر `29.00 QAR`، ثم رفض Google الحفظ نفسه برسالة `تعذَّر حفظ التغييرات.`؛ لذلك لم تُفعّل الخطتان والعائق حالة تحقيق الربح/الحفظ للحساب لا بيانات الخطة. أضيف سكربت آمن قابل لإعادة التشغيل في `voicebrief_flutter/scripts/google_play_subscriptions.mjs`، ويلزم تكراره بعد انتشار الصلاحية ثم فتح تذكرة Play Support إن بقي فشل جلسة المالك. حساب Play الآخر المرتبط بـ`mhm1981x@gmail.com` رقم `7616754513484681287` محظور من النشر بسبب إثبات الهوية والهاتف ولا يحتوي VoiceBrief. ربط RevenueCat للمتجرين والمنتجات والاستحقاق والعرض ومفاتيح SDK والـwebhook مكتمل؛ المتبقي حدث webhook تجريبي وتأكيد البريد واختبارات sandbox.
- رُفع إصلاح الدخول إلى GitHub عند `7858e85`، ثم صُحح فحص رقم البناء عند `522e032`. التشغيل [32897857499](https://github.com/m7madv/otlobli/actions/runs/32897857499) نجح في بناء iOS الحي وتوقيع التطبيق والإضافة والتحقق من App Group وApple Sign-In ونسخة `0.1.0 (6)` ثم التحقق والرفع إلى Apple، والتشغيل [32899139810](https://github.com/m7madv/otlobli/actions/runs/32899139810) أخضر في Android وgolden وiOS غير الموقّع. IPA المحلية الموقعة في `C:\Users\MOHAMMAD\Projects\VoiceBriefAuthRepair\artifacts\voicebrief-ios-0.1.0-build6-appstore\ios\signed\`، حجمها `34,385,318` وبصمتها `B41B3BF695399B4482942053C145AA85988ECEFE0982AA6CE53D40D7BFFA7642`. TestFlight يعرض المجموعة `VoiceBrief Internal` بمختبر واحد وبناءين، ويسجل تثبيت `0.1.0 (6)` على `iPhone 16 Pro Max` بنظام `iOS 27.0`. هذا يثبت التسليم والتثبيت فقط؛ ما يزال قبول Google والبريد والمشاركة والتسجيل والقص على الجهاز نفسه مطلوباً. نقطة الرجوع السابقة `voicebrief-prebuild-20260824`، وApple Design Skill مثبت عند `d0bac1e765a27a696839e62962e36330ce72f0b7`.
- Build 7 عند commit المصدر `1ddfd5d`، وآخر commit للتوثيق وأداة اشتراكات Google هو `787840c`. التشغيل [32901316058](https://github.com/m7madv/otlobli/actions/runs/32901316058) وقّع التطبيق والإضافة وفحص الملفات التعريفية والصلاحيات ورفع `0.1.0 (7)`، والتشغيل [32901316077](https://github.com/m7madv/otlobli/actions/runs/32901316077) نجح في Android و11 golden وiOS غير الموقّع. IPA الموقعة `C:\Users\MOHAMMAD\Projects\VoiceBriefAuthRepair\artifacts\voicebrief-ios-0.1.0-build7-appstore\ios\signed\VoiceBrief-0.1.0-build7-AppStore.ipa`، حجمها `34,368,348` وبصمتها `15950D4EC8FDD54649126BC1473D37849D46B569F7E4DB2BE40E7AC65FD48FCD`. Apple أنهت المعالجة، والمجموعة `VoiceBrief Internal` تضم 3 أبنية ومختبراً واحداً، وتعرض تثبيت Build 7 على `iPhone 16 Pro Max / iOS 27.0`. قبول Apple وGoogle الفعلي نجح على الجهاز نفسه بعد إصلاح الخادم.

## مشروع ضمانك المستقل — إصلاح استعادة Apple في Build 31 (2026-09-02)

- متابعة 2026-09-05: تتبع رفض المستخدم الجديد إلى `SANDBOX_NOT_AVAILABLE` (رفض سماح اختبار، HTTP 403 وغير قابل لإعادة المحاولة تلقائياً). السماح الصريح محدود بـ24 ساعة ومربوط بالحساب والمتجر ويُحذف معهما؛ لا يكفي حذف التطبيق أو تكرار الاستعادة. صُححت الرسالة في المصدر المحلي وأضيف اختبار إعادة تسليم الإيصال بعد إصلاح السماح. نجح Flutter `246/246` وDeno `85/85` و`flutter analyze`. تفاصيل النطاق والقيود في `damanak_flutter/docs/BILLING_REVIEW_2026-09-05.md`؛ التشخيص الحي وإصلاح سياسة السماح ما زالا معلقين على الوصول إلى مشروع Supabase الصحيح. لم تُنشر الرقعة أو تُرفع حزمة جديدة؛ TestFlight بقي Build `31`.

- الفرع الصحيح `codex/damanak-simple-pos` ومصدر الإصدار الحالي `4.5.1+31` عند الالتزام `667299caf5481ab4c38c7b3d872e426e878c53e3`. Build `4.5.1 (31)` هو الأحدث في iPhone TestFlight وحالته `VALID` بالمعرّف `3d3e11d1-bf04-4022-9cc7-28c185ea81b7` وداخل `Damanak Internal` ذات المختبر الواحد. إصدار Apple العام `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة، بينما بقي Google Alpha على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً.
- Build `30` يفصل الوصول المجاني عن فوترة المتجر: 20 ضماناً تتجدد مع كل شهر تقويمي، وحساب واحد مرتبط بتثبيت محمي واحد، وفرع وعضو واحدان، ومن دون دورة شهرية/سنوية أو إيصال Apple/Google. الربط الحالي UUID تثبيت مع جلسة موثقة وليس App Attest/DeviceCheck أو Play Integrity؛ لذلك لا يُوصف بإثبات جهاز مادي.
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` منشورة على الإنتاج. توجه كل مداخل إنشاء المتجر القديمة والجديدة إلى القاعدة نفسها، وتبقي مرآة توافق فقط لـBuildات `23–29` بينما تكون الصلاحية الفعلية في جداول `private`. نظفت تجارب legacy وحالة Apple Sandbox الوحيدة إلى tombstones، وحصرت RPC الطرفي في `app_store`، وفشلت مغلقاً عند صف manual/activation_code غير متسق. تطابقت صفوف وبصمة عدادات الشراء الأربعة قبل النشر وبعده، ولم توجد بيانات Apple Production أو Google؛ وجدول refresh limiter الجديد فارغ. صارت صفوف الاشتراك التسعة القديمة `trial/canceled` نظيفة، ومنحها المجانية التسع `pending_device` حتى أول ربط تثبيت من Build `30`، وبقي tombstone Apple Sandbox واحداً `revoked` غير قابل للتحديث.
- أُعيد بناء تنظيم Flutter حول آلة حالة واحدة: `BillingScope` يعزل الحساب والمتجر ونسخة السياق، و`CatalogSnapshot` يحفظ عروض المتجر، و`BillingOperation` يسمح بعملية شراء/استعادة/مصالحة واحدة، و`SubscriptionPolicy` وحدها تقرر البدء أو الترقية أو تغيير الدورة أو المنع. تُرفض نتائج الكتالوج والأحداث ومعرّفات العمليات القديمة، ولا يستطيع حدث من حساب محذوف تحديث الحساب الجديد.
- الواجهة و`AppController` يستخدمان السياسة المركزية نفسها بفشل مغلق. الباقة الحالية لا تُشترى ثانية، والباقة الأدنى معلوماتية بلا مسار دفع، والمزود المنافس والحالة الناقصة يمنعان الدفع. خدمة المتجر تبقي الحراسة النهائية وترفض Google بدلاً من إنشاء `ReplacementMode.deferred`.
- قبل كل شراء يقرأ `AppController` صورة الاشتراك المرجعية من قاعدة البيانات، ثم يحدّثها من المزود إن كانت متجرية؛ لذلك لا تستطيع صورة محلية تجريبية/قديمة فتح دفع أدنى أو مزود ثانٍ. تُسلسل نتيجة الاستعادة الصفرية مع stream، ولا يكتب refresh أو workspace قديمان فوق إيصال أحدث، ولا يلغي خطأ stream تحققاً خادمياً جارياً، ولا يُحفظ الحدث في dedupe قبل نجاح تطبيق الاستحقاق محلياً.
- يبقي Build `30` فحص `Transaction.currentEntitlements` قبل كل شراء Apple. إذا لم يوجد ربط خادمي لكن Apple ID يملك منتج ضمانك، يمنع شراءً جديداً ويطلب الاستعادة الصريحة. وإذا وجد ربط خادمي، فلا يفتح StoreKit إلا عند تطابق `originalTransactionId` والمنتج الحاليين مع Apple ID المفتوح؛ غياب الاستحقاق أو اختلاف الحساب/السلسلة/المنتج يفشل مغلقاً بلا JWS أو استعادة أو نقل ربط صامت.
- يصلح Build `31` حالة «توسع السنوي» التي كان StoreKit يرفض شراءها بسبب `Transaction.unfinished` قديمة لا تظهر في `currentEntitlements`: الاستعادة الصريحة تجمع معاملات ضمانك غير المنتهية على دفعات عادلة بحد أقصى 8، تحصرها في سلسلة `originalTransactionId` الحالية أو orphan صريح ذي UUID، ترسل JWS إلى الخادم أولاً، ولا تستدعي `finish` إلا بعد تحقق وتطبيق الاستحقاق. تبقى الجلسة مفتوحة للأحداث المتأخرة، ويغلب فشل أي حدث على نجاح سابق، ويعاد إغلاق المعاملة محلياً بلا تكرار تحقق الخادم عند فشل `finish`. لا تُعرض رسالة نجاح كاملة عند فشل الاستعادة الرسمية أو بقاء معاملات إضافية.
- الحصص الشهرية حدود وليست أرصدة متراكمة: عند الترقية من `100` إلى `600` يبقى استخدام الشهر محسوباً ويصبح المتبقي `600 - usedWarranties`. مثال: بعد استخدام 30 من «بداية» يصبح المتاح 570 في «نمو»، لا 670 أو 700. تغيير الشهري/السنوي داخل الباقة لا يعيد الحصة.
- منع الخفض كامل داخل ضمانك، لكنه ليس قابلاً للفرض على صفحة إدارة اشتراكات Apple ما دامت المنتجات الأدنى داخل المجموعة نفسها. إعداد Apple صحيح: «توسع» Level 1 و«نمو» Level 2 و«بداية» Level 3، والشهري والسنوي في المستوى نفسه. إذا نفذت Apple انتقالاً موثقاً خارج التطبيق يبقى الخادم ملزماً بمصالحة حقيقة المزود، ولا يجوز إبقاء مزايا أعلى مقابل سعر أدنى.
- ثُبت أن منتجي «توسع» الشهري والسنوي موجودان وصحيحان في App Store Connect وبالمستوى الأعلى داخل مجموعة الاشتراك؛ كان التعطيل من حالة معاملة/استعادة سابقة لا من معرّفات الخطة أو سعر محلي. تعرض Flutter الكتالوج الجزئي القابل للاستخدام، وتسمي الخطة والدورة المفقودتين بلا كشف المعرّف التقني، وتتيح إعادة المحاولة من دون تعطيل المنتج المتاح.
- Build `26` ينظم الشراء والاستعادة كعملية واحدة محدودة: يمنع التداخل أثناء تحقق إيصال متأخر، يلغي مراقب العودة قبل التحقق الخادمي، ينتظر استعادة Apple البطيئة ضمن سقف، ويزيل خطأ الفوترة الأحمر عند نجاح الإيصال نفسه من دون مسح خطأ أعمال أحدث. معاملات Apple المملوكة/غير المكتملة توجه إلى الاستعادة بدلاً من دفع ثانٍ.
- الاستعادة الصريحة وحدها ترسل `recoveryRequested=true`. اشتراك Apple القديم بعد حذف حساب ضمانك لا ينتقل إلا بعد تحقق JWS والجذور المثبتة، والتأكد أن الحساب/المتجر القديمين محذوفان وأن الهدف متجر وحيد بلا entitlement؛ ثم يُحدث `appAccountToken` بعد commit. يبدأ v28 بفاحصي Apple الرسميين المتصل وعند `signedDate`، ولا يشغّل فاحص التوافق الصارم إلا إذا أعاد كلاهما الحالة `1`. Google يدعم ربط Build `23` الحسابي وسلسلة predecessor/out-of-app ذرياً بالشروط نفسها. المسارات الخلفية تبقى صارمة ولا تنقل legacy تلقائياً.
- أثبتت تجربة TestFlight الحية شراء Apple ناجحاً على Build `26` عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حذف المستخدم حساب ضمانك بعد نحو 8 ثوانٍ مع بقاء الاشتراك لدى Apple. بين `08:36:15` و`08:38:40` بتوقيت الرياض وصلت سبع محاولات استعادة v27 للهوية والمتجر نفسيهما؛ في كل محاولة أعاد الفاحصان الرسميان `VERIFICATION_FAILURE=1`، ثم رفض فاحص التوافق قبل أي كتابة، ولم يوجد أي رد تحقق `200`. محاولة «الشراء» لم تصنع معاملة جديدة لأن Apple تعتبر الاشتراك مملوكاً. بقي المتجر الهدف على `starter/canceled/source=trial` بلا entitlement أو مزود فوترة؛ لذلك لم تكن المشكلة غياب دفع أو منتج «توسع» ولا يلزم دفع جديد.
- تأكيد Google بعد تحقق الخادم أصبح دفاعاً مزدوجاً: مهمة خادمية غير حاجبة تعيد المحاولة، والجهاز يستدعي Android Billing مباشرة ويفحص `BillingResultWrapper.responseCode`. الرد غير `OK` لا يحذف المعاملة المحلية، لذلك يمكن استعادتها/إكمالها لاحقاً ولا يظهر نجاح كاذب.
- حذف حساب ضمانك لا يلغي فوترة Apple أو Google. حوار الحذف يحذر من استمرار التجديد، ويوجه الاشتراك المرتبط إلى إدارة المتجر قبل الحذف؛ الاستعادة الآمنة تعالج حالة إعادة التسجيل بعد حذف الحساب لكنها لا تلغي الاشتراك نيابة عن المستخدم.
- السبب المؤكد لفشل v27 هو بيئة الإنتاج `Deno/2.1.4` مع `SupabaseEdgeRuntime/1.74.3`: واجهات Node `X509Certificate.raw/verify/ca/publicKey` غير منفذة فيها، بينما كانت الاختبارات المحلية تعمل على Deno أحدث. أعاد تشغيل الاختبار نفسه على Deno `2.1.4` إنتاج الفشل قبل الرقعة. استبدلت v28 هذا الاعتماد بمحلل DER الصارم نفسه وتحقق تشفيري عبر `jsrsasign@11.1.5`، مع رفض SHA-1 للـleaf والـintermediate والإبقاء على الجذور الخارجية المثبتة وكل حواجز المطابقة والربط الذري.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations مطبقة حتى `20260901200000`. `verify-store-purchase` v30 `ACTIVE` مع `verify_jwt=false` وبصمة الحزمة `313eb3a460cb4d361595aeafc4c767a04290ba4084140d10ca8b5b742ecc02a8`، و`refresh-store-entitlements` v9 `ACTIVE` مع `verify_jwt=false` وبصمة `1548ef6902c5dff6f1ae5f8d79c6c4cac9975062a083b03e7a9f665beca1eb54`. يثبت v30 JWS الجهاز أولاً ويربط `transactionId` و`originalTransactionId` والمنتج والحزمة والبيئة، ثم يختار أحدث معاملة داخل السلسلة الموثقة نفسها فقط؛ لا يمكن لسلسلة B أن تُستخدم لإصلاح A. أعاد runtime بلا جلسة `401 AUTH_REQUIRED`، وبقيت جدولة `damanak-entitlement-refresh` نشطة كل 5 دقائق وشملت v9 المنطق نفسه.
- نجحت استعادة Apple orphan فعلياً بعد v28 عند `2026-09-01 09:22:26` بتوقيت الرياض وأعادت HTTP `200`، ثم أعاد التحديث المجدول التحقق عند `09:25:03`. صارت هذه حالة تاريخية بعد reset المستخدم ثم migration الجديدة: لا يوجد entitlement إنتاجي أو Google، وحالة Apple Sandbox الوحيدة tombstone `revoked`، ولا توجد نافذة مراجعة Apple مفتوحة. بقي سجل التدقيق وlineage، وأُنشئت تسع منح مجانية `pending_device` من دون ادعاء تفعيلها قبل دخول Build `30` وربط التثبيت.
- نجحت اختبارات الإنتاج السابقة داخل `BEGIN/ROLLBACK`، ثم نجح `free_access_billing_repair_live.sql` بعد النشر وأعاد `free access, quota, compatibility, terminal state, and production isolation passed; transaction rolled back`. ملف `supabase/schema.sql` المحدث حجمه `475,132` بايت وSHA-256 `97A391B3FC3F5119940233960DBDF54E62DE977D1D58DBBACFE742E80FD7EB12`، واختبار PGlite `1/1` ناجح.
- التحقق المحلي لمرشح `4.5.1+31`: `flutter analyze` نظيف، وFlutter `245/245`، وDeno `85/85`، واختبار الاشتراك المركز `129/129`، و`deno fmt --check` على 27 ملفاً و`deno check` و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK حجمه `180,893,512` بايت وSHA-256 `C61C577268A5A864F7358D9E2B615CDB3E4324777718474A62DF5B327FB20928`.
- راجعت واجهتا الاشتراك والإعداد وفق Apple Design في `damanak_flutter/docs/DESIGN_AUDIT.md`: تعرض الخطة المجانية بلا دورة متجر، وتعرض مزايا الباقات المدفوعة كاملة، وتمنع الحالية والأدنى من فتح الدفع. نجح RTL واختبار `320×568` مع تكبير `200%`، وأُغلق overflow قائمة الدولة ومحدد نوع الإعداد؛ لم يُنفذ قبول VoiceOver أو iPhone حقيقي أو جهاز ضعيف للواجهة الجديدة.
- بقيت نتيجتا الأمن المفتوحتان كما هما: إثبات الجهاز الحقيقي يحتاج App Attest/DeviceCheck وPlay Integrity وقرار الويب، وروابط الضمان تحتاج قرار مدة وسياسة v1 وإبطال. لم تُكسر الروابط الحالية ولم يُدّع إغلاق النقطتين.
- App Store Connect: أكد الفحص القرائي النهائي [33626964328](https://github.com/m7madv/otlobli/actions/runs/33626964328) أن Build `4.5.1 (31)` `VALID` بالمعرّف `3d3e11d1-bf04-4022-9cc7-28c185ea81b7` وداخل `Damanak Internal` التي صارت تضم 25 بناءً ومختبراً واحداً مع وصول all-builds. بقي إصدار `4.5.0` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707` `READY_FOR_SALE`، ولا توجد مسودة مراجعة أو إصدار iOS قابل للتعديل، و`mutationRequested=false`.
- نجح التحقق والرفع الموقّع في [33559362847](https://github.com/m7madv/otlobli/actions/runs/33559362847) من الالتزام `9ed1b59a460d856bb3fdb880998bb0d4003934ba`. Delivery UUID/Build ID هو `47da8d54-cadc-4f96-82cb-9e81a01c3664`. الـIPA المحلية `damanak_flutter/output/github-run-33559362847/damanak-ios-4.5.1-build-30-signed/ضمانك.ipa` حجمها `23,983,180` بايت وSHA-256 `4A8E69C835C7FF9B1E419DB51E3F550BA2F94B0671BD9297B024F4E23E3CF4E8`.
- نجح تشغيل التوقيع الجاف لـBuild `31` في [33624973532](https://github.com/m7madv/otlobli/actions/runs/33624973532)، ثم نجح التحقق والرفع الفعلي في [33625661083](https://github.com/m7madv/otlobli/actions/runs/33625661083) من الالتزام `667299caf5481ab4c38c7b3d872e426e878c53e3`. Delivery UUID/Build ID هو `3d3e11d1-bf04-4022-9cc7-28c185ea81b7`. الـIPA المحلية `damanak_flutter/output/github-run-33625661083/damanak-ios-4.5.1-build-31-signed/ضمانك.ipa` حجمها `23,991,319` بايت وSHA-256 `66C1B348091031ED7355048ECC8F50D5174FA1619B2B32B6A4CF033DA03061B8`. أكد الفحص القرائي النهائي [33626964328](https://github.com/m7madv/otlobli/actions/runs/33626964328) اكتمال المعالجة بحالة `VALID` ومن دون أي mutation.
- Google Play بقي على Build `23` في Alpha والمسار `4699097266255892286`. قائمة `All Apps - Closed Testers` دائمة وتضم بالضبط 12 مختبراً؛ لا تنشئ قائمة أخرى ولا تعدّلها. Note 8 يبقى على Build `22` المثبت من Play حتى قبول Build `23`.
- ثُبت تثبيت Build `26` وشراء Apple حقيقي على iPhone من TestFlight، ثم نجحت الاستعادة بعد حذف الحساب قبل reset الاختبار. Build `31` صار `VALID` في TestFlight لكنه لم يُثبت أو يُقبل بعد على جهاز حقيقي، ولم يُثبت بعد أنه أغلق معاملة «توسع السنوي» على جهاز المستخدم. سجل الشراء `sandbox` يبقى لدى Apple حتى يمسحه المستخدم من حساب الاختبار؛ لذلك قد تعيد الاستعادة تفعيله. لا يوجد قبول جهاز للخطة المجانية والبنية الجديدة أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو الجهاز الضعيف.

## الإصدار المنشور السابق: ضمانك 4.3.2+20 (2026-08-30)

- `damanak_flutter/` هو المصدر الحالي للإصدار `4.3.2+20`. التجربة «الضمان أولاً»، وحصص بداية/نمو/توسع هي `100/600/3000` ضمان شهرياً وحدود الفريق `2/5/15`.
- شاشة الاشتراك تعرض `بداية / نمو / توسع` معاً في محدد دائم، ثم بطاقة الباقة المختارة؛ لم تعد «توسع» مدفونة أسفل ثلاث بطاقات طويلة. تشرح بطاقة الخطة صراحة اسم الباقة التجريبية وحدود الضمانات وحسابات الفريق.
- أصل ظهور 600 ضمان و5 أعضاء للحساب الجديد هو أن `create_store_with_trial` كان ينشئ تجربة `growth`. migration الجديدة `20260830130000_damanak_starter_trial_consistency.sql` تجعل المتاجر الجديدة على `starter` لمدة 14 يوماً وتصحح التجارب الحالية الآمنة فقط عند عضوين و100 ضمان أو أقل. لم تُنشر بعد لأن جلستي Supabase الحالية وCLI لا تملكان المشروع الفعلي `exxayzlklvgeyqhvtzgi`؛ مؤسسة Damanak الظاهرة في Edge فارغة.
- تسجيل الخروج صار ينتقل فوراً إلى شاشة الدخول بعد إزالة الجلسة المحلية، ولا يترك شاشة الحساب باهتة بانتظار إبطال الشبكة. يكتمل الطلب الشبكي في الخلفية.
...
- التحقق الحالي للمصدر `4.3.2+20`: `flutter analyze` بلا مشاكل، و`55` اختباراً ناجحاً، وبناء Flutter Web وAndroid debug ناجحان. روجعت الشاشة بصرياً عند `390×844` واختبرت عند `320×568` مع تكبير `200%`.
- رفع التشغيل [33306208729](https://github.com/m7madv/otlobli/actions/runs/33306208729) iOS Build `20` موقّعاً إلى App Store Connect بعد نجاح التحقق والرفع. أكد الفحص [33306891813](https://github.com/m7madv/otlobli/actions/runs/33306891813) أنه `VALID` وداخل `Damanak Internal` المتاحة لمختبر واحد. IPA المحلية `damanak_flutter/output/github-run-33306208729/ضمانك.ipa` حجمها `23,305,466` وبصمتها `61525EC23D0761E0D18663FE0CC73546665943A3B2A2076ED495457B63F5224A`.
- نشر التشغيل [33305694146](https://github.com/m7madv/otlobli/actions/runs/33305694146) Android `versionCode 20` إلى الاختبار الداخلي بحالة `completed`. AAB المحلية `damanak_flutter/output/github-run-33305694146/damanak-android-internal-build-20-run-12/app/outputs/bundle/release/app-release.aab` حجمها `56,321,026` وبصمتها `E12929EE44A5344C2DA9B6AEE5983AD2C723CD690B3FE88D6FEE7D39F9ADB775`.
- فحص Apple الحي بعد Build `20` أكد المنتجات الستة `READY_TO_SUBMIT`، وتوافر قطر، وأسعارها، ووجود إصدارات الاشتراك `6/6` في مسودة `READY_FOR_REVIEW`. لم يُنفذ شراء حقيقي أو قبول جهاز؛ يلزم تثبيت Build `20` من TestFlight وتجربة الجلب والشراء والاستعادة وتسجيل الخروج بحساب جديد بعد نشر migration.
...
## موقع زبّطها لخدمات الأعطال في قطر — 2026-08-26

- أُنشئ مشروع مستقل داخل `qatar-home-fix/` باسم «زبّطها / ZABTHA»، من دون تعديل تطبيق Otlobli أو منطق الدفع والمحفظة. الاسم معتمد تحريرياً لهذا الموقع لكنه غير مسجل كعلامة بحسب المعلومات المتاحة.
- الموقع عربي أولاً ومتعدد الصفحات: الرئيسية، دليل الخدمات، 8 صفحات خدمة فريدة، المناطق، من نحن، التواصل، الخصوصية والشروط. حُذفت صفحة دليل الأسعار من المصدر والروابط وخريطة الموقع؛ يعيد `/prices` الآن 404، بينما بقيت عوامل السعر المرتبطة بكل خدمة داخل صفحة الخدمة نفسها.
- الخدمات المنشورة: سباكة، كهرباء، تركيب تلفزيون، تركيب أثاث، فك ونقل خفيف، صيانة عامة، تكييف وأجهزة منزلية.
- أُعيد تصميم الواجهة للمرة الرابعة بهوية رسمية أحادية اللون: أبيض نقي، أسود `#0A0A0A`، درجات رمادية محايدة، حدود رفيعة، زوايا محدودة، وصور صيانة واقعية بتدرج رمادي. أزيل الوضع الداكن التلقائي وكل الأزرق والظلال والقوالب متعددة الألوان.
- تستخدم الواجهة خط النظام العربي بدلاً من أي خط ويب؛ لم تعد حزمة المتصفح تنشر ملفات Noto. بقيت الصفحة الرئيسية ذات بطل مقسوم وشبكة خدمات غير متناظرة ومسار طلب ومناطق ونموذج وأسئلة، واستُبدل قسم دليل الأسعار بتعريف مباشر لنطاق زبّطها ومهماتها.
- نُشر الموقع بوصول عام على `https://zabta-qatar.mhm1981dx.chatgpt.site`، والنسخة الحية هي Sites version 14 من المصدر `c9d0f3930f991b3fe70bf40428873dbdecea1754`. بقي المسار `zabta-qatar` كما هو حتى لا تتغير العناوين التي أُرسلت إلى محركات البحث، بينما العنوان والاسم الظاهران هما «زبّطها».
- حزمة النشر: `outputs/zabtha-v14.tar.gz`، وSHA-256 المحلي هو `61B16AB03B79C1FE38EAAD060DEBC0DE07AA71FFD5DE04416BB02F2B72C68BFB`، وحجمها 1,236,696 بايت. سجلت الاستضافة المحتوى بالمعرف `sha256:75c2ba0938b786bdb005b5511820b77e7b90d720ef371533d5521ad1a819e7b2`.
- الفهرسة التقنية جاهزة: روابط `canonical` عامة، و`robots.txt`، و`sitemap.xml`، وبيانات `WebSite` و`Organization` و`Service` و`BreadcrumbList` و`ItemList` المنظمة، وبيانات مشاركة اجتماعية كاملة. حُذفت بيانات `FAQPage` بعد إيقاف Google لنتيجتها الغنية، وبقيت الأسئلة نصاً ظاهراً للمستخدم والزاحف.
- أُثبتت ملكية بادئة الموقع في Google Search Console داخل حساب `mhm1981x@gmail.com` بواسطة علامة HTML منشورة. أُرسلت `sitemap.xml` وقبلها Google بنجاح واكتشف 15 عنواناً. كان العنوان الرئيسي غير معروف بعد في فحص URL، وتعذر طلب الفهرسة اليدوي لأن الحساب تجاوز حصته اليومية؛ لم تُحل أو تُتجاوز reCAPTCHA، ويكفي الآن أن تعالج Google الخريطة دورياً أو يعاد الطلب في يوم لاحق.
- أُعيدت كتابة الصفحة الرئيسية والأسئلة وتعريف الخدمة لتجيب مباشرة عن ماهية زبّطها ونطاقها وطريقة اختيار مقدم خدمة مستقل. نُشر رقم الطلب وواتساب `+974 3061 5280`، وتقول الواجهة بدقة إن استقبال الطلبات متاح على مدار الساعة وإن الموعد يتأكد حسب توفر الفني. زبّطها مسؤولة عن التنسيق والمتابعة، ويبقى مقدم الخدمة المستقل مسؤولاً مهنياً عن التنفيذ؛ لا توجد ادعاءات بطاقم داخلي أو أعمال سابقة أو ضمان غير مثبت. تحليل المنافسين وخطة البحث في `qatar-home-fix/docs/SEARCH_STRATEGY.md`؛ لم يُضف `llms.txt` لأن إرشادات Google الحالية تقول إنه غير مطلوب لميزات البحث التوليدي.
- الحركة الرئيسية 10 ثوانٍ مرسومة على Canvas فوق صورة إصلاح واقعية بمعدل 12 إطاراً/ثانية و6 عند خفض الجودة وحد أقصى لكثافة البكسل يساوي 1.5. تتوقف خارج الشاشة وفي الخلفية وعند تقليل الحركة، مع زر إيقاف يدوي وحالة ثابتة بديلة.
- استُبدلت روابط `next/link` الداخلية بروابط HTML أصلية لأن حزمة التنقل في Vinext كانت تفشل في الإنتاج. جرى اختبار الانتقال من الرئيسية إلى `/services` بعد النشر ونجح بلا أخطاء أو تحذيرات جديدة.
- نجح `npm run lint` و`npm run build` للإصدار 14. أعادت الصفحة الحية عنوان «صيانة منزلية في قطر للأعطال الصغيرة | زبّطها»، وبيانات المؤسسة المنظمة تتضمن الاسم والاسم اللاتيني ورقم الهاتف. لم يُكرر الفحص البصري المتجاوب بعد تعديل الهوية والنصوص.
- نُشرت قناة YouTube مستقلة على حساب `mhm1981x@gmail.com` باسم `ZABTHA Qatar` ومعرّف `@zabthaqatarqa` ورابط `https://www.youtube.com/@zabthaqatarqa`. يتضمن وصفها العربي الخدمات ونموذج التنسيق ورابط الموقع، ولم يُنشر بريد الدخول. رفض YouTube الاسم العربي «زبّطها» آلياً، لذلك بقي الاسم اللاتيني بينما يبدأ الوصف بالاسم العربي. يربط الموقع القناة في التذييل و`Organization.sameAs`.
- سُجل الموقع في Bing Webmaster Tools بالحساب نفسه عبر استيراد ملكية Google Search Console. استورد Bing خريطة `sitemap.xml` ويعالجها من دون أخطاء أو تحذيرات ظاهرة، وبدأ فحصاً تقنياً باسم `ZABTHA full SEO GEO scan` بحد 50 صفحة. قد تستغرق تقارير Bing حتى 48 ساعة لتظهر.
- فُعل `IndexNow` بمفتاح عام ثابت داخل `public/7eb7e9a61ed84aae808ca9bf918f8d20.txt` وسكربت `npm run indexnow`. تحقق ملف المفتاح الحي بحالة HTTP 200، ثم قُبل إرسال جميع عناوين الخريطة وعددها 15 بحالة HTTP 202.
- نُشرت أربعة حضورات تحريرية عامة ومستقلة تربط الموقع مباشرة: مقال Medium في `https://medium.com/@zabtha-qatar/%D8%B2%D8%A8%D9%91%D8%B7%D9%87%D8%A7-%D9%85%D9%86%D8%B5%D8%A9-%D8%AA%D9%86%D8%B3%D9%8A%D9%82-%D8%AE%D8%AF%D9%85%D8%A7%D8%AA-%D8%A7%D9%84%D8%A3%D8%B9%D8%B7%D8%A7%D9%84-%D8%A7%D9%84%D9%85%D9%86%D8%B2%D9%84%D9%8A%D8%A9-%D9%81%D9%8A-%D9%82%D8%B7%D8%B1-1e27a4cc4457`، ومقال Blogger في `https://zabtha-qatar.blogspot.com/2026/08/blog-post.html`، ومقال WordPress في `https://zabthaqatar.wordpress.com/2026/08/26/%d8%ae%d8%af%d9%85%d8%a7%d8%aa-%d8%a7%d9%84%d9%85%d9%87%d8%a7%d9%85-%d8%a7%d9%84%d9%85%d9%86%d8%b2%d9%84%d9%8a%d8%a9-%d8%a7%d9%84%d8%b5%d8%ba%d9%8a%d8%b1%d8%a9-%d9%81%d9%8a-%d9%82%d8%b7%d8%b1-%d9%85/`، ومقال Substack في `https://zabthaqatar.substack.com/p/122`. النصوص أصلية وليست نسخاً متطابقة، وتصف زبّطها بصدق كمنصة تنسيق مع مستقلين. حُفظ اسم الناشر العام `زبّطها | ZABTHA Qatar` ولم يُنشر بريد الدخول.
- موقع WordPress على `https://zabthaqatar.wordpress.com` أُطلق للعامة، وخيار منع الفهرسة غير مفعّل، وخيار منع مشاركة المحتوى مع شركاء الطرف الثالث غير مفعّل. الاسم والوصف والرابط العام للموقع الرئيسي محفوظة في الملف العام.
- نُشر Substack على `https://zabthaqatar.substack.com` بملف عام `https://substack.com/@zabthaqatar` ومعرّف `@zabthaqatar`. المقال الرابع منشور على الويب فقط من دون إرسال بريدي، ويحتوي زرًا عامًا قابلًا للنقر إلى موقع زبّطها؛ تحقق الرابط المباشر كزائر. لم يُحفظ رابط الموقع داخل حقل Website للملف لأن سياسة أمان المتصفح رفضت الإجراء؛ مصدر الرابط الخارجي المثبت هو المقال المنشور.
- محاولة Tumblr وصلت إلى خطوة تاريخ الميلاد الإلزامية بعد ربط Google، ولم تُستكمل لأن تاريخاً صحيحاً لم يقدمه المستخدم ولم يُخترع تاريخ. لا تدّعِ وجود صفحة Tumblr عامة أو منشور هناك.
- بقية الأدلة التجارية المحلية لم تُنشر بعد. الملف التجاري على Google غير مؤهل حالياً وفق سياسة Google لأن النموذج تنسيق عملاء محتملين مع مقدمي خدمة مستقلين؛ لم يُنشأ ملف مخالف. Qatar Living يعيد صفحة الدخول بعد تعطل Google المضمّن، وConnect Qatar يتطلب حساباً بكلمة مرور وreCAPTCHA، وArabianTalks يتطلب بريداً تجارياً بنطاق خاص وقد يرفض Gmail. لا يوجد نطاق مخصص أو بريد أعمال أو اسم قانوني موثق حتى الآن.
- استبعد المستخدم TikTok صراحةً؛ لم يُنشأ عليه حساب ولا يجوز إضافته إلى الموقع أو خطة النشر.
- يلزم فحص النطاق والعلامة التجارية في قطر قبل الإطلاق التجاري النهائي.
- لم يُنفذ قبول على هاتف ضعيف حقيقي؛ اختبارات المتصفح وقيود الأداء ليست بديلاً عن اختبار جهاز فعلي.
- لا توجد قاعدة بيانات أو مدفوعات أو مزامنة Android/iOS لهذا المشروع المستقل؛ لذلك لم تُنفذ مزامنة أصلية.

## حضور زبّطها على LinkedIn — 2026-08-27

- أُنشئت صفحة LinkedIn العامة `زبّطها | ZABTHA Qatar` على الحساب المنتهي بـ`D`: `https://www.linkedin.com/company/zabtha-qatar/`، ومعرّف المؤسسة الداخلي `144751156`.
- نُشر الشعار الأحادي والغلاف الأبيض والأسود، وحُفظت الفئة `خدمات المستهلكين` والحجم الحقيقي `0–1` والموقع `الدوحة، قطر` ورابط الموقع العام وزر زيارته ورسائل الصفحة.
- حُفظ وصف عربي وإنجليزي يشرح نموذج تنسيق المهمات مع مقدمي خدمات مستقلين من دون ادعاء مقاول عام أو طاقم أو أعمال سابقة، وبقي بريد الدخول غير منشور.
- نُشر منشور الإطلاق العام: `https://www.linkedin.com/feed/update/urn:li:share:7498721561953247233?actorCompanyId=144751156`.
- رفض محرر LinkedIn حفظ حقل التخصصات الاختياري بسبب تعارض تحرير داخلي؛ لا تدّعِ أنه محفوظ. كلمات الخدمات موجودة في الوصف العام ومنشور الإطلاق، ولم تتأثر بقية الصفحة.

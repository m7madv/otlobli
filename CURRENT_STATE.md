...
The installed `86.193/1055` build was rechecked over USB on the iPhone 16 Pro Max. The user-visible failure is now precise: a clean install gives one interactive SHEIN entry; after leaving to Otlobli's chooser and entering SHEIN again, the fully rendered first page can stop accepting every tap. Killing Otlobli from the App Switcher does not recover it; deleting the app container and reinstalling does. A captured Home/background cycle kept app PID `1113`, host WebContent `1115/pageProxy7`, and SHEIN WebContent `1117/pageProxy40`; there was no native close, new page proxy, or `webViewWebContentProcessDidTerminate`. An earlier `1099 -> 1111` WebContent change was explicitly a same-`pageProxy43` cross-site provisional process swap, not a new WKWebView. These observations reject another blind foreground reload/rebuild, but do not yet distinguish a native hit-routing fault from a live DOM/overlay fault or persistent WebKit website-data change.

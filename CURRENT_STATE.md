# v86.244/1112 — إرسال App Review المصرّح للبناء الموجود (2026-08-30)

اعمل فقط داخل
`C:\Users\MOHAMMAD\Projects\otlobli-v86-212-testflight-auth` على الفرع
`codex/otlobli-v86-212-testflight-auth`. بقي رقم الإصدار `86.244` وارتفع رقم
البناء إلى `1112` لأن `1111` موجود لدى Apple ولا يمكن استبداله.

رفض Apple القديم يخص `86.230 (1095)`: الصور كانت ملفي
`*-login.png` يعرضان تسجيل الدخول فقط، ومسار السلة كان يعطّل المتابعة من دون
شرح موحد للمراجع. أزيلت الصورتان واستُبدلتا بست لقطات فعلية من واجهة Otlobli
للمتاجر والسلة والدفع: ثلاث لقطات `1242x2688` لـ6.5-inch iPhone وثلاث
`2048x2732` لـ13-inch iPad، ببيانات عرض غير شخصية. حارس
`verify:app-store-assets` يفحص PNG والأبعاد والعدد والأسماء وبصمتي صور الدخول
المرفوضتين، ويرفض أي صورة غير مسجلة أو غياب علامة `PREPARE_ONLY`.

منطق أهلية الدفع صار دالة نقية تختبر السلة الفارغة، التخصيص، التوفر، والحد
الأدنى. تعرض السلة السبب العربي الأول والمبلغ المتبقي الدقيق داخل
`role=status/aria-live`، ويبقى زر الإجراء قابلًا للضغط كي يعيد الملاحظة بدل
تعطيل صامت. يعاد التحقق داخل `confirmOrder` قبل إنشاء الطلب، ويستخدم الطلب
المشترك مجموعه وحده مع حده المعروض. نجحت حالتا واجهة معزولتان: `4300 SYP`
تعرض «أضف 700 ل.س» وتبقى في السلة، و`6400 SYP` تفتح «بيانات الاستلام» وتعرض
«الدفع الآن عبر شام كاش». لم يُنشأ طلب إنتاجي ولم تُحوّل أموال.

سكربت App Store يعيد استعمال سجل وحيد بحالة `REJECTED` أو
`PREPARE_FOR_SUBMISSION` مع حفظ metadata، ويرفع كل البدائل حتى `COMPLETE`
قبل حذف الصور القديمة. علامة `store-assets/app-store/PREPARE_ONLY` موجودة
ومفروضة في حارس البناء، لذلك التحضير لا يستطيع الإرسال العام. مدة طلب الدفع
العادي مؤكدة من الخادم بساعتين، بينما top-up ودفع حل المشكلة خمس دقائق؛ تعرض
الواجهة الموعد الذي يعيده الخادم ولا يوجد تعارض وظيفي.

نجح `npm run build` بكل اختبارات auth/security/release وSHEIN/Temu، ثم
`npx cap sync android` و`npx cap sync ios`. نجحت الميزانيات بلا رفع:
startup `674,276/720,000`، JS gzip `301,491/370,000`، CSS
`69,989/70,000`، نصوص المتاجر `320,230/470,000`، Temu Gecko
`172,513/180,000`، ومصدر المتاجر `582,416/600,000`. نجح Android R8
والتوقيع؛ `aapt` يؤكد `com.otlobli.app` و`86.244 (1112)`، وAPK ينجح v2/v3
بشهادة RSA-4096 الموثقة وAAB يعيد `jar verified`.

- APK: `artifacts/release-86.244/Otlobli-86.244-1112-release.apk` —
  `4,113,735` بايت — SHA-256
  `B8FD9B8BA63D5F074DAC52EC3437587EC67D75AD31B692FDF22964CBFB9AC00D`.
- AAB: `artifacts/release-86.244/Otlobli-86.244-1112-release.aab` —
...
خطوة App Review كانت متخطاة صراحة في ذلك التشغيل. أكد المالك لاحقًا نجاح
اختبار iPhone 16 Pro Max الكامل: SHEIN كزائر إلى أول منتج، عدم بقاء صفحة
الدخول، عدم خروج Google/Facebook إلى Safari، خمس دورات background/resume،
واختبار force-quit/cold-launch مستقل. وأكد أيضًا نجاح السلة والدفع حتى رمز
ShamCash على iPhone وiPad من دون ضغط «لقد دفعت».

استُبدلت علامة `PREPARE_ONLY` بسجل `SUBMISSION_AUTHORIZED.json` مرتبط حصريًا
بالإصدار `86.244 (1112)`. يتحقق الحارس من اكتمال مصفوفة القبول، ويتحقق سكربت
الإرسال من تطابق الإصدار والبناء قبل أي إرسال. أُذن الآن بإرسال هذا البناء
الموجود إلى App Review؛ نتيجة Apple النهائية قيد التنفيذ.

محاولة App Review الأولى رقم `33304324644` نفذت البناء والحواجز والتوقيع من
دون رفع TestFlight جديد، وأبقت التوزيع الداخلي سليمًا. رفعت الصور الست حتى
`COMPLETE` وحذفت صورتي الدخول القديمتين وربطت البناء `1112`، ثم توقفت بأمان
قبل الإرسال: الإصدار ما زال عنصرًا في طلب Apple القديم ذي المشاكل غير
المحلولة، ولا يجوز إضافته إلى طلب جديد. عُدّل السكربت ليجد الطلب القديم، يعلّم
عنصره المرفوض `resolved=true`، ثم يعيد إرسال الطلب نفسه وفق عقد Apple؛ إعادة
المحاولة الثانية `33304628281` أثبتت أن تصفية الطلبات حسب `IOS` لا تعرض الطلب
القديم؛ حقل platform اختياري في عقد Apple الحديث. أزيلت التصفية، وأضيف استرداد
مقيد لمعرّف الطلب الوحيد الذي تعيده Apple في خطأ ملكية العنصر، مع إعادة قراءة
العنصر والتحقق من مطابقته قبل أي PATCH. إعادة المحاولة قيد التنفيذ.

التشغيل `33304875681` لم يصل إلى توقيع أو App Review: توقف حارس المصادقة
مبكرًا لأن مرسل WhatsApp المشترك عاد غير متصل لحظة الفحص. لم يُتجاوز الحارس،
ولم تُحذف الجلسة أو يُغيّر الرقم أو يُرفع بناء. يلزم طلب OTP حقيقي جديد من أحد
التطبيقين لإيقاظ الجلسة والتأكد من وصوله، ثم إعادة تشغيل مسار الإرسال نفسه.
بعد إفادة المالك بوصول OTP، توقف التشغيل `33305605991` في الحارس نفسه لأن
عملية Otlobli بقيت `whatsappConnected=false/whatsappSenderReady=false`. هذا
يرجح أن الرمز خرج من عملية التطبيق الثاني على الرقم المشترك، لا من مرسل
Otlobli الذي يفحصه الإصدار. يلزم طلب الرمز من تطبيق Otlobli نفسه أو إعادة وصل
جلسة Otlobli `0` عبر نقطة الإدارة المحلية المحمية قبل إعادة المحاولة.

بعد تأكيد المالك وصول OTP حقيقي جديد من أحد التطبيقين، أضيف إلى مسار GitHub المسجل فرع
إرسال سريع للبناء الموجود: عند اختيار `distribute-existing` مع App Review
يفحص هوية المشروع وتفويض القبول ثم يعيد إرسال `86.244 (1112)` مباشرة. يتخطى
هذا الفرع أرشفة iOS والتوقيع ورفع TestFlight، ولا يغيّر أي ملف تطبيقي أو بناء
Apple. لا يفحص أو يغيّر Oracle أو جلسة WhatsApp؛ حارس المصادقة الحي باقٍ في
مسارات بناء ورفع النسخ الجديدة.

نفّذ التشغيل السريع `33306098541` هذا المسار من الالتزام `a0aecd8`، ونجحت
هوية `86.244 (1112)` وأصول App Store وتفويض القبول. أعاد فحص Otlobli الحي
فقط `whatsappConnected=false` و`whatsappSenderReady=false`، فتوقف خلال ثوانٍ
قبل تجهيز مفتاح Apple أو استدعاء App Store Connect. لا يوجد إرسال مراجعة أو
بناء أو رفع جديد. صحح المالك الدليل بعد ذلك صراحة: تسجيل الدخول جُرّب على
النسخة `86.244 (1112)` نفسها أكثر من مرة ووصل رمز WhatsApp ونجح الدخول. لذلك
أزيل فحص `/health` اللحظي من فرع إعادة تقديم البناء الموجود فقط؛ قبول الجهاز
المسجل هو الدليل، ولا يتدخل مسار App Review في Oracle أو جلسة WhatsApp. بقي
حارس المصادقة كاملًا في مسارات بناء/رفع TestFlight الجديدة.

التشغيل `33306887739` وصل إلى Apple من دون بناء أو رفع: تحقق من `86.244
(1112)` وربط البناء والصور الست، ثم كشف أن طلب الرفض القديم
`e5e27b8a-b628-4116-b135-361b91266929` يملك سجل App Store قديمًا مختلفًا، لا
سجل `86.244` الحالي. وفق عقد Apple، لا يقبل طلب بحالة `UNRESOLVED_ISSUES`
عنصرًا جديدًا قبل معالجة المرفوض. عُدّل السكربت ليتحقق من معرّفي الإصدار
والطلب والحالتين بدقة، ويزيل عنصر الإصدار القديم المرفوض من الطلب فقط عبر
`removed=true` من دون حذف إصدار أو بناء، ثم يعيد إضافة `86.244` إلى المسودة
الحالية ويرسلها. يبقى مسار الاسترداد السابق إذا كان العنصر المالك هو الإصدار
الحالي نفسه.

أثبت التشغيل `33307022048` أن طلب Apple القديم مرحّل: معرّف الإصدار القديم
موجود في علاقة `appStoreVersionForReview` لا داخل `items`. أضيف مسار مقيد لهذا
الشكل: لا يُلغى الطلب القديم إلا إذا كان `UNRESOLVED_ISSUES`، وعلاقته تطابق
الإصدار القديم الذي ذكره خطأ Apple، ولا يحتوي أي عناصر نشطة أخرى، ولا يطابق
سجل `86.244` الحالي. ينتظر انتقاله إلى `COMPLETE` ثم يضيف الإصدار الحالي؛ لا
يحذف إصدارًا أو بناءً.

أوقف التشغيل `33307160043` الإلغاء قبل أي تغيير لأن علاقة الطلب المرحّل لم
تطابق معرّف الحاجز كما افترض المسار المقيد. أضيف إخراج تشخيصي للمعرّفات وعدد
العناصر النشطة فقط؛ لا يطبع بيانات اعتماد، ولا ينفذ إلغاءً عند عدم التطابق.

أعاد `33307207063` القيم الآمنة: `relation=NONE`، والحاجز إصدار
`890129230`، وسجل `86.244` الحالي `a03a0acc-2555-44aa-accd-78429a3e6a39`،
وفي الطلب عنصر نشط واحد. بقي الطلب بلا تغيير. وُسّع التشخيص ليعرض نوع علاقة
هذا العنصر وحالته ومعرّفه فقط قبل اتخاذ أي قرار.

أثبت `33307283332` أن العنصر الوحيد `REJECTED` ومعرّفه base64url يفك حرفيًا
إلى `e5e27b8a-b628-4116-b135-361b91266929|6|890129230`. أضيف كاشف مقيد
يتطلب إعادة ترميز مطابقة، ومعرّف الطلب نفسه، والنوع `6`، ومعرّف الحاجز نفسه؛
عندها فقط يعامل العنصر كإصدار App Store القديم ويزيل عنصر المراجعة. لا يُلغى
الطلب كاملًا ولا يُحذف إصدار أو بناء.
...
قبول الجهاز المطلوب لـApp Review مسجل من إفادة المالك، وليس مستنتجًا من CI.
لا يزال قبول Android ضعيف لهذا البناء غير منفذ ولا يغيّر تفويض iOS الحالي.
...
الحالي لا يعيد استعمال سجل بحالة `REJECTED` بأمان. قبول iPhone الحقيقي وخمس
دورات الخلفية/العودة والتشغيل البارد ما زالت مطلوبة.
...
دُفع الالتزام `0a78e5764f39cbc30241c2bfd622d0d39cd022a7`. نجح مسار GitHub
[33075528666](https://github.com/m7madv/otlobli/actions/runs/33075528666)
خلال `8m57s`، ورفع `otlobli-v86.244-build-1110-testflight.ipa`. قبلته Apple
برقم التسليم `bd950715-b83a-49c6-966b-ec0cf04cdf28`، ثم تحقق المسار أن البناء
`VALID` وحالته `IN_BETA_TESTING` ومتاح لمجموعة `Otlobli Internal`، وأن المختبر
المتوقع حالته `INSTALLED`. حجم IPA `10,551,551` بايت وبصمتها SHA-256
`8FAB591376D0A6DAD28141BF6480664744B9C693B7F6F65315B64928AE7340BD`.
أثر GitHub رقم `9647984843` حجمه `25,260,454` بايت وبصمة ZIP
`91E053F44FB846F319DCD2F731911A7A8FE2CE61A95FD4EA388E2BCCB8BC8796`.
كانت خطوة App Review `skipped` صراحةً؛ لم يُرسل الإصدار العام.

قُرئت رسالة Apple من الواجهة بعد تسجيل الدخول. الرفض يخص فقط
`86.230 (1095)`، Submission ID
`e5e27b8a-b628-4116-b135-361b91266929`، على iPhone 17 Pro Max وiPad Air
11-inch (M3) بنظامي iOS/iPadOS 26.6. الأسباب الدقيقة: Guideline `2.3.3`
لأن صور 6.5-inch iPhone و13-inch iPad تعرض شاشة الدخول فقط، وGuideline
`2.1(a)` لأن المراجع لم يستطع متابعة الدفع بعد إضافة عناصر إلى السلة. حقل
`Version` في سجل الإصدار المرفوض ظاهر ومفعّل، لذلك المسار غير المدمر هو
إعادة استعمال السجل: تغيير `86.230` إلى `86.244`، فصل build `1095` وربط
`1110`، واستبدال صور الدخول بصور استخدام حقيقية، ثم إعادة الإرسال فقط بعد
اختبار دفع آمن على تثبيت نظيف iPhone/iPad. لم يُغيّر أي حقل ولم تُرسل إجابة
للمراجع ولم يُحذف السجل.

مراجعة فرق الكود لا تسمح بادعاء أن `1110` أصلح `2.1(a)`: إصلاح
`a53fbf9` يفصل ملكية سلة Temu عن SHEIN، وتصميم السلة اللاحق أوضح، لكن شروط
تعطيل «المتابعة للدفع» ومسار `setScreen('checkout')` الأساسيان بقيا كما كانا
تقريبًا في `1095`. قد يكون المراجع واجه حد الطلب (`5000 SYP` أو `$40`) أو
منتجًا ناقص التخصيص/التوفر أو خلط السلتين القديم، لكن لا توجد لقطة أو سجلات
تثبت واحدًا منها. قبل ربط `1110` بالمراجعة يلزم تثبيت نظيف على iPhone وiPad،
حساب مكتمل، ومنتج عادي متوفر في كل متجر بمجموع يتجاوز الشرطين بوضوح، ثم
التحقق من السلة الصحيحة، تمكين زر المتابعة، إنشاء طلب واحد «بانتظار الدفع»،
وظهور رمز ShamCash ومبلغ ووقت انتهاء صالحين من دون تحويل مال أو ضغط
«لقد دفعت». تعريف مدة الانتهاء في المستودع ما زال ساعتين بينما الواجهة/التوثيق
يشيران إلى 5 دقائق؛ يجب حسم هذا التعارض قبل App Review.

...
- أكد App Store Connect status run `33315830796` أن `0.1.0 (15)` حالته `VALID` وغير منتهٍ، لكن البناء لم يكن مرتبطًا تلقائيًا بأي مجموعة اختبار. عُدل workflow ليضيفه حصريًا إلى المجموعة الداخلية الموجودة `VoiceBrief Internal` عبر API الرسمية ويتحقق من العلاقة؛ يلزم نجاح rerun قبل القول إن النسخة وصلت إلى المختبرين. تبقى تجربة iPhone الحقيقية لظهور اسم الموعد في واجهة Apple، والنغمات، والقائمة، والإلغاء مطلوبة ولا يوجد ادعاء قبول جهاز بعد.

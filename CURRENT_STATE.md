...
- المصدر وإجراءات GitHub موحدة على `4.5.0+23`. يعالج هذا البناء فشل تعرف التطبيق على شراء Apple بعد نجاح الدفع، ويربط إيصال المتجر بالمتجر داخلياً بعملية ذرية، ويجدد الاستحقاقات دورياً، ويمنع منح تجربة جديدة بتبديل الحساب على التثبيت نفسه. لا يُعامل نجاح الدفع محلياً كاستحقاق قبل تحقق الخادم.
- نُشرت migrations من `20260831030000` إلى `20260831130000` على Supabase الصحيح `exxayzlklvgeyqhvtzgi`. دوال الإنتاج النشطة: `verify-store-purchase` v20، `refresh-store-entitlements` v4، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v15، و`warranty-card` v11. تعمل جدولة `damanak-entitlement-refresh` كل 5 دقائق ونجحت باستجابة `200` بعد تدوير السر في Functions وVault من دون كشفه.
- ملف `supabase/schema.sql` تفريغ حي لمخططي `public/private`، حجمه `333,072` بايت وبصمته `2B91E8FD226CBDAB62CEBF73A4781DBC59E75C88722DB628CF41FE2BB9383627`. سياسة الخصوصية المنشورة تفصح عن الذكاء الاصطناعي والمرفقات ومعرّف التثبيت والشراء والتكاملات والاحتفاظ بعد الحذف.
- نجح `flutter analyze` و`87/87` اختبار Flutter و`19/19` اختبار Deno مع `--allow-env`، ونجح Android debug. ملف APK المحلي `build/app/outputs/flutter-apk/app-debug.apk` حجمه `180,737,784` بايت وبصمته `71DA30AE36ED8C46F948032F177C08D3BF295767DB8C6834B06B599128F6DCA9`.
- App Store Connect: اكتملت بيانات إصدار `4.5.0` العربية والوصف والكلمات والفئة والتصنيف والتسعير والتوفر في دول الخليج الست وسياسة الخصوصية وبيانات المراجعة. اتفاقية التطبيقات المدفوعة والضرائب والحساب البنكي فعالة، والمنتجات الستة في المجموعة نفسها بحالة `Ready for Review`. بقي رفع Build `23` وربطه، والتحقق من حفظ هاتف وبريد المراجع، ورفع الصور ثم إرسال المسودة.
- أُنتجت خمس صور iPhone بدقة `1284×2778` وخمس صور iPad بدقة `2048×2732` داخل `app_store_assets/ios/`. أُصلحت أثناء المراجعة مشاكل الحروف العربية المكسورة والخطوط الصفراء والدعوة الفارغة وقص عنوان الاشتراك؛ اجتاز المولد `5/5`. إجراء `Damanak Apple Setup` الموجود على الفرع الافتراضي يشغّل رافع الصور عند `mode=apply` حتى يمكن رفعها قبل دمج الفرع.
- Google Play: الأيقونة وصورة العرض ولقطتا الشاشة وبطاقة المتجر وتصريحات الجمهور `18+` وأمان البيانات مكتملة. مسار `Alpha` رقم `4699097266255892286` مرتبط بقائمة **دائمة وقابلة لإعادة الاستخدام لأي تطبيق** باسم `All Apps - Closed Testers` وتضم `12` مختبراً؛ لا تُنشئ قائمة جديدة ولا تستبدلها. Build `22` قيد المراجعة، وسيُرفع Build `23` ثم يربط بالمسار نفسه. لا يبدأ عداد `14` يوماً إلا بعد قبول Google وانضمام المختبرين فعلياً من `https://play.google.com/apps/testing/com.damanak.damanak`.
- حساب مراجعة Google داخل متجر `Damanak Note 8 Test` يملك وصول `scale` حتى `2036-08-31` بلا شراء أو تجربة، ولم تُحفظ كلمة مروره في المشروع. ما يزال اختبار شراء/استعادة حقيقي على iPhone وAndroid وقبول VoiceOver/TalkBack والجهاز الضعيف منفصلاً؛ لا يُستنتج من نجاح البناء أو API.

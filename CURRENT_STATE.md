# v86.195 — live iOS Web Inspector diagnostic, not a recovery (2026-08-15)

The clean-install v86.194 result reproduced the exact fault in a brand-new app container. USB verified that the data container changed from `0D7DD653-...` to `A7FE8449-...` and that the installed binary remained `86.194/1056`. Entry one worked; after Otlobli exit and SHEIN re-entry the site became functionally frozen again. A 29-second capture on the clean second entry recorded one app PID, one browser ID, one attached/front/interactive native surface and one WKWebView, 24 distinct DOM tap attempts, 112 native hit-test records, 475 DOM diagnostic records, three completed document navigations, and no WebContent termination. Fast taps could route to another document, but each destination was inert or only partly populated.

All three newly loaded documents emitted the exact same JavaScript error hash `3ce0b2a11d15fff2` immediately after `sheinSaudiReady`. Their clean-container fingerprint was stable at 6 cookies, 52 localStorage entries, 1 sessionStorage entry, 0 CacheStorage caches, and 6 IndexedDB databases. The inherited frozen container had different cookie/localStorage counts and hashes but the same one-entry sessionStorage fingerprint and the same error hash. This proves the fault can be recreated from a clean install and rejects “old version debris” as the explanation. Native touch interception, detached rendering, a fixed overlay, and WebContent death are also rejected. The unresolved boundary is now SHEIN runtime/hydration or a network/risk failure created during the first session, not the back button or native hit routing.

A local Playwright/WebKit reproduction with the same Otlobli runtime loaded SHEIN's server-rendered shell while SHEIN repeatedly returned HTTP 403 for its recommendation feed. That automated browser is itself risk-detectable, so this is a useful hypothesis—not proof of the phone's response status. `86.195/1057`, marker `2026.08.15-v86.195-ios-live-web-inspector-diagnostic`, adds no behavioral recovery. It only sets `WKWebView.isInspectable=true` when the explicit diagnostic flag is active and enriches sanitized error logs with category/name/source-path hash/line/column/target tag. Normal builds keep the WebView non-inspectable. The next test is an in-place v86.195 update over the preserved frozen container, then attach WebKit Inspector before navigation to read the exact failing script and network responses. Android and iPhone 6 remain untouched.

GitHub run `31851860388` built the diagnostic IPA successfully from commit `4dcd222`. Artifact inspection confirmed bundle `com.otlobli.app`, version/build `86.195/1057`, arm64, iOS 15 minimum, device families `[1,2]`, the v86.195 diagnostic marker, no source maps, no embedded provisioning profile, and no app-level signature. Deliverable size is 6,582,113 bytes with SHA-256 `EB53DFE1EA8FDB0C93B71C4DB5B78E3E6A8E8C32632AFB8D3D58641EB30722AA`.

# v86.194 — passive iOS root-cause trace, not a recovery (2026-08-15)

The installed `86.193/1055` build was rechecked over USB on the iPhone 16 Pro Max. The user-visible failure is now precise: a clean install gives one interactive SHEIN entry; after leaving to Otlobli's chooser and entering SHEIN again, the fully rendered first page can become functionally frozen. Repeated fast/random taps can occasionally enter a section, but that destination is also inert or only partially hydrated. Killing Otlobli from the App Switcher does not recover it; deleting the app container and reinstalling does. A captured Home/background cycle kept app PID `1113`, host WebContent `1115/pageProxy7`, and SHEIN WebContent `1117/pageProxy40`; there was no native close, new page proxy, or `webViewWebContentProcessDidTerminate`. An earlier `1099 -> 1111` WebContent change was explicitly a same-`pageProxy43` cross-site provisional process swap, not a new WKWebView.

The first v86.194 device capture was deliberately useful but not clean: the diagnostic IPA was updated over the already-frozen v86.193 container. USB verified the installed binary as `86.194/1056` while its data container remained the inherited one. During a 59-second trace, all physical taps hit the visible, interactive `OtlobliSheinDiagnosticSurfaceView` and its attached `WKWebView`; trusted DOM `touchstart`, `touchend`, and `click` events reached capture and bubble with `defaultPrevented=false`, no fixed top layer, and no WebContent termination. On attempt 32 an actual anchor click changed the route and loaded a new document in the same WKWebView. The destination rendered its header/filter shell but not a working product body, and emitted one JavaScript error fingerprint `3ce0b2a11d15fff2`. Its persistent fingerprint was 5 cookies, 38 localStorage entries, 1 sessionStorage entry, 0 CacheStorage caches, and 6 IndexedDB databases (names/values never logged). This rules out native hit interception, a detached bitmap, and a generic overlay for the inherited-frozen case. It narrows the fault to SHEIN's page runtime/hydration or persistent WebKit state; a clean-install trace of the same binary is required before identifying the causal store or key.

`86.194/1056`, marker `2026.08.15-v86.194-ios-root-cause-diagnostic`, is a dedicated passive diagnostic build. It leaves the v86.193 browser behavior unchanged and adds one unified-log stream with category `SheinRootCause`: native surface hit tests and object/view hierarchy identity; sanitized DOM touch capture/bubble/final state and top/fixed elements; explicit `forceStoreVpnRecheck` keep/close decisions; open/show/hide/close/navigation/WebContent events; and count/hash-only fingerprints for cookies, local/session storage, CacheStorage, Service Workers, and IndexedDB. No raw storage key/value, cookie, URL query, label, or report payload is written to OSLog. The probe has no interval, MutationObserver, network request, event cancellation, replay, reload, cache clear, website-data clear, or automatic recovery.

Local TypeScript, production and diagnostic web builds, release hardening, SHEIN/Temu/surface guards, low-end budgets, and Capacitor iOS sync pass. GitHub/Xcode run `31849799496` also passes from commit `fb16b88`. The inspected unsigned IPA is `6,581,447` bytes, SHA-256 `448DBAB412F0B7F08DF5ECA535593FBCBC3F7EF07C2D9B225E5DBD9FE722257F`, `com.otlobli.app`, ARM64, iOS 15+, iPhone/iPad `[1,2]`, with no app-level `_CodeSignature`, embedded provisioning profile, or source maps; the expected pre-signed Facebook dependency frameworks retain their own signatures. The diagnostic build is enabled only by `VITE_SHEIN_IOS_ROOT_CAUSE_DIAGNOSTICS=true`. The clean-install comparison is still pending, so the responsible persistent component and final fix are not claimed. Do not modify Android or the iPhone 6 during this classification run.

# v86.196 — clean SHEIN runtime diagnostic from exact v86.193 (2026-08-20)

This branch is a diagnostic instrument, not a freeze fix or a customer release. `codex/ios-v86-196-clean-runtime-diagnostic` was created directly from clean commit `a6e0ca943c4d9a2722b5962a4193d3e34d2da248` (`86.193/1055`). The protected `codex/ios-v86-194-root-cause-diagnostic` worktree remains untouched at `9a91e9f6680d1dce48ae5a04bfe6544d8d26b954`. Version/build is `86.196/1058`; `86.196` and `1058` were verified unused before the bump.

The custom iOS SHEIN WKWebView is inspectable on iOS 16.4+ and emits Base64-chunked unified logs under subsystem `com.otlobli.app`, category `SheinCleanRuntime`. Every record carries a process PID, per-process run ID, native sequence, browser ID, and bounded evidence. Passive document-start instrumentation captures exact bounded JavaScript errors/rejection stacks with query-free source paths, resource-load/CSP failures, bounded Performance Resource and Navigation evidence, read-only cookie/storage fingerprints and counts, Cache Storage, IndexedDB, Service Worker/controller, storage estimate, document lifecycle, and online state. Native evidence covers app lifecycle, all main navigation callbacks/policies/failures, URL changes, open/show/hide/close/create/destroy, WKWebView/data-store/process-pool identities, hierarchy/interaction flags, memory warnings, and WebContent termination.

The contaminated tap component is absent: the dormant baseline file `src/services/sheinTapDiagnostics.ts` was removed, and neither `SHEIN_TAP_DIAGNOSTIC_CONTEXT_JS`, `window.__otlobliTapDiagnosticContext`, nor `SHEIN_TAP_DIAGNOSTIC_SCRIPT` is present in generated bundles. The new probe has no touch/pointer/click listener, `preventDefault`, synthetic click, fetch/XHR/console patch, DOM scan, MutationObserver, interval, reload, storage write, or `SHEIN_REQUIRED_COUNTRY` reference. Existing v86.193 navigation/product behavior remains unchanged. `forceStoreVpnRecheck()`, browser open/close, WKWebView construction/destruction/recovery, default website data store, process pool, cookies/cache/IndexedDB/Service Workers, region/session enforcement, text selection, blockers, capture, VPN, loading policy, and readiness rules were not changed.

TypeScript, release hardening, SHEIN freeze/architecture guard, Temu guard, store-surface guard, production build, performance budgets, and iOS/Android Capacitor sync pass. Final budgets: startup JS `639,165/720,000`, total JS gzip `266,126/370,000`, CSS `69,819/70,000`, fonts `81,364/100,000`, shipped store scripts `238,685/470,000`, and store source `557,374/600,000` bytes. Source commit `c9baa32c9fdff31e70bcdb111ad6bafbc9e47245` and GitHub/macOS Xcode run `32372570479` pass. The inspected unsigned Universal IPA is `6,574,344` bytes with SHA-256 `137CBF5EDA5BDC6A19C4AF0353940115CD6DA991D57E5C9CFE88D016DC490787`; it is `com.otlobli.app`, `86.196/1058`, thin ARM64 iPhoneOS, iOS 15+, iPhone/iPad `[1,2]`, with no app-root signature, provisioning profile, or source maps. Binary/web inspection confirms `setInspectable:`, `SheinCleanRuntime`, `OtlobliSheinRuntime`, and `otlobliCleanRuntimeDiagnostic`, while the contaminated tap-context markers are absent. Desktop copy: `C:\Users\MOHAMMAD\Desktop\Otlobli-v86.196-iPhone-iPad-Clean-Runtime-Diagnostic-UNSIGNED.ipa`. No physical-device run has been performed. The exact two-process capture protocol and decoder are in `docs/SHEIN_V86_196_RUNTIME_DIAGNOSTIC.md`; do not call a cause proven without the first-success/kill/second-freeze evidence set.

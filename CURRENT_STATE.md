...
Passive v86.202 diagnostics remain. `[OTLOBLI_PREFETCH_FIX]` proves lookup/compile/attachment identity and ordering without logging private values, and the CDP recorder/analyzer now classifies every matching Script/XHR/Fetch/Other request, completion/failure, blocked reason, initiator, response-body metadata, ChunkLoadError, runtime root, and interaction reaction. Local guards/build/sync pass; macOS WebKit compiled the raw-only JSON; GitHub/Xcode run `32412740745` built commit `c914b52c2dbc539a196d4d74385b350fffd20577`; artifact `9422864423`. Inspected IPA: `6,601,350` bytes, SHA-256 `73884D2E045DA72108FA2D000BA3070AAC4452CF755CF398DCED09137623B259`, `com.otlobli.app`, ARM64, iOS 15+, iPhone/iPad, unsigned/unprovisioned, zero source maps/relay placeholders. Path: `C:\Users\MOHAMMAD\Desktop\otlobli-ios-v86.203-shein-prefetch-cache-fix\otlobli-v86.203-ipad-iphone-universal-unsigned.ipa`.

This is not device-accepted yet. A clean-install preventive run must pass ten swipe-kill/cold-launch cycles with CDP/unified logs, followed by a separate in-place upgrade over a proven poisoned v86.202 cache. Do not report v86.203 ChunkLoadError count, cache state, migration need, or universal fix until those tests run. Full implementation, evidence boundary, and protocols: `docs/SHEIN_V86_203_PREFETCH_CACHE_FIX.md`.

# v86.202 — captured SHEIN cache/chunk root cause (2026-08-20)

The device capture proved the cause of the reproduced cold-launch freeze. In a working process, required chunks `68498` and `26652` loaded as Script 200 with real JavaScript bodies. SHEIN's own `prefetchJs` then re-requested the same canonical URLs as XHR because this WKWebView reports `rel=prefetch` unsupported; the stale CDN objects returned 304 without bodies, and SHEIN treats 304 as success. The device's WebKit network-cache records for both URLs then contained `text/plain` metadata with no body blob. On the next cold process, the initial Script requests received 304/empty, raised genuine `ChunkLoadError` before hydration, and left the visible home at inert root `#app` instead of working `#shein-branch`.

The frozen document was visible/focused with all seven event-loop heartbeats healthy. One attached/visible/interaction-enabled WKWebView and surface existed, with no pre-freeze close, duplicate, termination, BFCache restoration, Service Worker, or input interception. Forty-seven trusted events reached the frozen DOM and all 45 delayed reaction snapshots remained unchanged because SHEIN's runtime never hydrated. Full report: `docs/SHEIN_V86_202_ROOT_CAUSE_CAPTURE_REPORT.md`.

No app fix was added. The narrow next candidate is a SHEIN-only native content rule that blocks only XHR/raw prefetches to hashed `/pwa_dist/assets/*.js` URLs while leaving real Script loads untouched. It must be implemented in a new authorized version and validated across ten cold launches plus category/PDP/cart/login flows. Do not use reload, browser recreation, store switching, or website-data clearing as the root fix. The conditional Otlobli `clearCache()` path remains unproven for older runs because this diagnostic did not log its invocation.
...
No fix, reload, browser recreation, website-data clearing, input cancellation, synthetic event, network/console/history patch, or store lifecycle change is included. Local build/sync and all architecture/performance/hardening guards pass. GitHub/Xcode run `32402979859` built code commit `a56c2e3d342eee285bde1659064ef6204ca1b8b5`; artifact ID `9419317843`. Inspected IPA: `6,592,564` bytes, SHA-256 `8F93DD8390DBF454070AFA709B631C69FCB6D19BB188C6E75F11D78C17489548`, `com.otlobli.app`, `86.202/1064`, ARM64, iOS 15+, iPhone/iPad, unsigned/unprovisioned, no source maps, app-level signature, or relay placeholder. Desktop path: `C:\Users\MOHAMMAD\Desktop\otlobli-ios-v86.202-root-cause-timeline-diagnostic\otlobli-v86.202-ipad-iphone-universal-unsigned.ipa`. Physical evidence was captured. Protocol: `docs/SHEIN_V86_202_ROOT_CAUSE_TIMELINE_DIAGNOSTIC.md`; result: `docs/SHEIN_V86_202_ROOT_CAUSE_CAPTURE_REPORT.md`.

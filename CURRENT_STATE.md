...
## مشروع VoiceBrief المستقل (2026-09-01)

- اكتملت تهيئة App Store Connect للبناء `0.1.0 (18)` من دون إرسال نهائي: حُفظ تقييم العمر العالمي `4+`، وضُبط التطبيق مجانيًا ومتوافرًا في `175` بلدًا/منطقة مع البلدان المستقبلية، وحُفظت ملاحظات المراجعة ومسار اختبار التسجيل والمشاركة والمواعيد والاشتراك. تحتوي مسودة المراجعة الآن أربعة عناصر جاهزة: إصدار iOS نفسه، ومجموعة `VoiceBrief Pro`، والاشتراكان `voicebrief_pro_monthly` و`voicebrief_pro_annual`؛ كلا الاشتراكين في المستوى `1` لأن المزايا نفسها والاختلاف في المدة فقط. أصبح زر `Submit for Review` فعّالًا، لكنه لم يُضغط لأن الامتثال الفرنسي والبناء البديل ما زالا شرطين حقيقيين قبل الإرسال.
...
- حُفظت حقوق النشر باسم `2026 MOHAMMAD ALZOUABI`، وحُفظ إقرار Content Rights على أن التطبيق يصل إلى محتوى طرف ثالث ولدى المالك الحقوق اللازمة. حُفظت حقول App Review باسم `MOHAMMAD ALZOUABI` وحساب Google للمراجع `mhm1981d@gmail.com` والهاتف `+974 70067040`، وصُححت التعليمات لتطلب زر «المتابعة باستخدام Google» بدل تسجيل Apple؛ أدخل المالك كلمة المرور مباشرة ولم تُقرأ أو تُطبع. أبقى المالك فرنسا، واختير في استبيان Apple التشفير المعياري غير الخاص؛ ظهر طلب إلزامي لرفع `French encryption declaration approval form`. البناء 18 يعلن `ITSAppUsesNonExemptEncryption=false` رغم TLS المعياري المضمّن عبر Flutter/Dart BoringSSL، لذلك يلزم اعتماد ANSSI ورمز Apple وبناء بديل قبل الإرسال النهائي. صارت المسودة `4` عناصر بعد إضافة إصدار iOS، ولم يُرسل التطبيق للمراجعة بعد.
- أُعد ملحق ANSSI تقني فرنسي من فحص IPA البناء 18 نفسه في `output/pdf/VoiceBrief-ANSSI-technical-annex.pdf`: ثلاث صفحات، `82,233` بايت، SHA-256 ‏`045E6138E5B319D5E66FA606709F14283D159D000827E2B2C18C261873CE4D11`. يوثق Flutter `3.41.6` وDart `3.11.4` وBoringSSL المضمّن وTLS 1.2/1.3 والخوارزميات المعيارية، وقد أُعيد فتحه واستخراج نصه ومراجعة صفحاته بصريًا. هو مرفق تحضيري وليس تصريحًا مودعًا أو اعتماد ANSSI؛ بقيت صفة المصرّح والجنسية والعنوان والتوقيع قبل الإرسال الرسمي.
- نُزّل نموذج ANSSI الرسمي الأصلي إلى `output/pdf/ANSSI-crypto-declaration-form-v2.pdf`: حجمه `2,308,710` بايت وSHA-256 ‏`149CF1324CC144C5891B085ED0ACE5E8B90888BBA61DB7A4FAF5822EE7EBF8E7`. هو نموذج `XFA` ديناميكي من صفحة ANSSI الرسمية، ويعرض غلاف «يتطلب Adobe Reader» في Chrome/Poppler؛ يجب فتحه وحفظه إلكترونيًا في Adobe Reader ثم طباعة نسخة للتوقيع والمسح. لم تُملأ أو تُختلق أي بيانات قانونية.
- المرشح المحلي الحالي هو `0.1.0+18` على الفرع `codex/voicebrief-ios`. فوق إصلاح ثبات المظهر والرصيد في build 17، يضيف إعادة محاولة آمنة للرفع: إذا وصل الملف وانقطع الرد، يتحقق `create-audio-upload` من المسار والحجم و`Content-Type` ثم يعيد `uploadedAlready:true` ويكمل Flutter/Share Extension إلى المعالجة من دون استبدال جسم قد يكون قيد الاستخدام. طلب Responses يرسل `store:false`، وWebhook RevenueCat يقرأ JSON بحد فعلي لا يعتمد على `Content-Length`.
...
- أكد App Store status run `33339093610` أن `0.1.0 (18)` حالته `VALID` وغير منتهٍ وموجود في `VoiceBrief Internal`، وتاريخ انتهائه `2026-11-28T14:08:06-08:00` والحد الأدنى iOS 14. تقرير الحالة artifact رقم `9739954624` وبصمته API ‏`sha256:d5935e52f41efa97d6fe7237b57078b35d89b2dc051c7540609f9018ce895bd1`؛ ملف التقرير `413` بايت وبصمته SHA-256 `CF5F77E72588F207CCA7C1AB9763635B7AE248C9EFF6F1CE5B19037682B3181`. هذا توفر داخلي في TestFlight فقط، وليس إرسالًا عامًا إلى مراجعة Apple، ويبقى قبول iPhone الحقيقي للمظهر والحصة والمنبّه والصوت المخصص مطلوبًا.

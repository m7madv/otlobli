...
Marker: `2026.08.09-v86.77-shein-ios-freeze-diagnostics`; native version `86.77 / 937`. `npm run build`, the expanded freeze guard, patch reversibility, and the low-end budget pass: largest JS `1,197,993 / 1,200,000`, gzip `354,513 / 370,000`, SHEIN source `543,347 / 550,000`; Android/iOS are synchronized. The unsigned iPhone build is pending. This flag must be turned off before any normal release; no iPhone acceptance is claimed. When a freeze occurs, tap `LOG` before switching stores, restarting, or applying any recovery, then paste the copied report.

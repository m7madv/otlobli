...
public class TemuEmbeddedBrowserPlugin extends Plugin {
    ...
    private static final class PendingAddRequest {
        final String requestId;
        final GeckoResult<Object> result;
        Runnable timeout;

        PendingAddRequest(String requestId, GeckoResult<Object> result) {
    ...
    private static final long ADD_ACK_TIMEOUT_MS = 3500L;
    ...
    private final Object addAckLock = new Object();
    private long addRequestSequence;
    private PendingAddRequest pendingAddRequest;
    ...
    public void goHome(PluginCall call) {
        getActivity().runOnUiThread(() -> {
            ...
            // opens the store chooser instead.
            if (session != null && session.isOpen()) session.loadUri(HOME_URL);
            call.resolve();
        });
    }
    ...
    public void acknowledgeAdd(PluginCall call) {
        getActivity().runOnUiThread(() -> {
            String requestedId = call.getString("requestId", "");
            PendingAddRequest request = takePendingAddRequest(requestedId.isEmpty() ? null : requestedId);
            if (request == null) {
                call.reject("No pending Temu product capture");
                return;
            }
            try {
                ...
                android.util.Log.d("OtlobliTemu", "capture acknowledged " + request.requestId);
                call.resolve();
            } catch (Exception error) {
                request.result.completeExceptionally(error);
                call.reject("Unable to acknowledge Temu product capture", error);
            }
        });
    }
    ...
    private void showStoreLayer() {
        ...
        // Gecko and React already stay mounted on two stable surfaces. Fading
        // the upper surface from transparent exposed a white intermediate
        // frame around Otlobli's fixed navigation on every reveal.
    ...
    private GeckoResult<Object> handleExtensionMessage(Object message, WebExtension.MessageSender sender) {
        ...
        if (addRequest != null) {
            try {
                ...
                addRequest.result.completeExceptionally(error);
                takePendingAddRequest(addRequest.requestId);
    ...
    private PendingAddRequest beginPendingAddRequest() {
        GeckoResult<Object> result = new GeckoResult<>(mainHandler);
        PendingAddRequest request;
        PendingAddRequest superseded;
        synchronized (addAckLock) {
            String requestId = Long.toString(System.nanoTime(), 36) + "-" + Long.toString(++addRequestSequence, 36);
            request = new PendingAddRequest(requestId, result);
            superseded = pendingAddRequest;
            if (superseded != null && superseded.timeout != null) mainHandler.removeCallbacks(superseded.timeout);
            pendingAddRequest = request;
        }
        Runnable timeout = () -> {
            PendingAddRequest timedOut = takePendingAddRequest(request.requestId);
            if (timedOut != null) {
                android.util.Log.w("OtlobliTemu", "capture acknowledgement timed out " + timedOut.requestId);
                timedOut.result.completeExceptionally(new IllegalStateException("Otlobli cart acknowledgement timed out"));
            }
        };
        synchronized (addAckLock) {
        ...
        if (superseded != null) {
            superseded.result.completeExceptionally(new IllegalStateException("A newer Temu product capture replaced this request"));
        }
        mainHandler.postDelayed(timeout, ADD_ACK_TIMEOUT_MS);
        android.util.Log.d("OtlobliTemu", "capture pending " + request.requestId);
        return request;
    }

    @Nullable
    private PendingAddRequest takePendingAddRequest(@Nullable String expectedRequestId) {
        synchronized (addAckLock) {
            if (pendingAddRequest == null ||
                (expectedRequestId != null && !expectedRequestId.equals(pendingAddRequest.requestId))) return null;
            PendingAddRequest request = pendingAddRequest;
    ...
    protected void handleOnDestroy() {
        PendingAddRequest abandonedAdd = takePendingAddRequest(null);
        if (abandonedAdd != null) {
            abandonedAdd.result.completeExceptionally(new IllegalStateException("Temu browser was destroyed before cart acknowledgement"));
        }

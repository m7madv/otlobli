...
public class TemuEmbeddedBrowserPlugin extends Plugin {
    ...
    private GeckoResult<Object> pendingAddResult;
    private Runnable pendingAddTimeout;
    ...
    public void acknowledgeAdd(PluginCall call) {
        getActivity().runOnUiThread(() -> {
            GeckoResult<Object> result = takePendingAddResult(null);
            if (result == null) {
    ...
    private GeckoResult<Object> handleExtensionMessage(Object message, WebExtension.MessageSender sender) {
        ...
        GeckoResult<Object> addResult = type.equals("addToCart") ? beginPendingAddResult() : null;
    ...
    private GeckoResult<Object> beginPendingAddResult() {
        ...
        Runnable timeout = () -> {
            GeckoResult<Object> timedOut = takePendingAddResult(result);
            if (timedOut != null) {
                timedOut.completeExceptionally(new IllegalStateException("Otlobli cart acknowledgement timed out"));
            }
        };
        GeckoResult<Object> superseded;
        synchronized (addAckLock) {
            superseded = pendingAddResult;
            if (pendingAddTimeout != null) mainHandler.removeCallbacks(pendingAddTimeout);
            pendingAddResult = result;
            pendingAddTimeout = timeout;
        }
        if (superseded != null) {
            superseded.completeExceptionally(new IllegalStateException("A newer Temu product capture replaced this request"));
        }
    ...
    @Nullable
    private GeckoResult<Object> takePendingAddResult(@Nullable GeckoResult<Object> expected) {
        synchronized (addAckLock) {
            if (pendingAddResult == null || (expected != null && pendingAddResult != expected)) return null;
            GeckoResult<Object> result = pendingAddResult;
            if (pendingAddTimeout != null) mainHandler.removeCallbacks(pendingAddTimeout);
    ...
    protected void handleOnDestroy() {
        GeckoResult<Object> abandonedAdd = takePendingAddResult(null);
        if (abandonedAdd != null) {
            abandonedAdd.completeExceptionally(new IllegalStateException("Temu browser was destroyed before cart acknowledgement"));
        }

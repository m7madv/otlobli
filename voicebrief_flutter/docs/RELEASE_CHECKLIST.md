...
- [ ] RevenueCat project `VoiceBrief` (`59213dad`), entitlement `pro`, and offering `default` are created. Store connections/imports, monthly/annual package mappings, public SDK keys, webhook secret, and sandbox purchases remain. Apple configuration is prepared for `app.voicebrief.mobile` but is not saved until local-file upload is enabled and the Issuer ID is entered. A verified Google service-account JSON is prepared privately outside Git with a restricted ACL but has not been uploaded; the account email also remains unconfirmed.

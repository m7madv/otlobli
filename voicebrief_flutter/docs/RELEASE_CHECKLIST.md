...
## Appointment/reminder candidate — 2026-08-30
...
- Source version is `0.1.0+13`. Free results now retain dates; deterministic recovery covers today/tomorrow/day-after-tomorrow Arabic variants plus the existing explicit day/month phrases. Each result date offers `Set reminder` and `Add to calendar`; ambiguous spoken hours require date/time confirmation. iOS schedules a local notification, while Android delegates to the system alarm app.
- Local verification passes format across 73 files, analysis, 34 non-golden tests, eleven reviewed goldens, seven Deno date tests, Deno check, and an Android debug build. The APK is `177,609,821` bytes with SHA-256 `86EA9BF33CA6FC39C337A83EBE7879781EE21AE1080AC74CEBFAFDF85DA4ECE7`. The deployed version 11 live smoke returned HTTP 200 in `17,153ms`, kept tomorrow separate from `2026-09-05`, and cleaned temporary Auth, Storage, and entitlement state.
...
- App Store status run [33310667046](https://github.com/m7madv/otlobli/actions/runs/33310667046) confirms `0.1.0 (13)` is `VALID`, unexpired, and attached to `VoiceBrief Internal`. Status artifact `9732090968` has ZIP SHA-256 `760F65D86C11F37CEC16DFFB3E443E88914085FC6839DDC66D5AB57C81963A82`. Physical iPhone appointment/reminder acceptance remains pending.
...
- [ ] Signed physical-iPhone build 13 appointment card, ambiguous-time confirmation, local reminder delivery, calendar action, cold/warm/terminated notification routing, session refresh/sign-out, App Group cleanup, and playback/seek/trim acceptance.

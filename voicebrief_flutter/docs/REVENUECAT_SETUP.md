...
- RevenueCat still shows the account email as unconfirmed. A standard Google service-account JSON for the existing Play verifier was prepared privately at `C:\Users\MOHAMMAD\.voicebrief\google\revenuecat-play-service-account.json`; its active key was verified against Google's published certificate, its SHA-256 is `60BF02E2D1AD0C9AE2B2321A4EC207B7EAD0134C38D52AC5D467E15D56497107`, and its Windows ACL allows only the current user and SYSTEM. It is outside Git and has not been uploaded. The Google Play form is open with package `app.voicebrief.mobile` populated, but the connection is not saved. Production public SDK keys, product imports, package-to-product mapping, and webhook authorization secret are not configured yet.
...
4. Finish the open Google Play app connection by uploading the prepared private service-account JSON, then save it. Do not commit or print either store credential.
...
The live webhook URL is `https://jyehqpdbayslhzebdycj.supabase.co/functions/v1/revenuecat-webhook`, and the function is deployed. On 2026-08-26 the RevenueCat project, entitlement, default offering, and App Store connection were created after owner sign-in; the Google connection, products, public SDK keys, and webhook secret remain incomplete. Configure the random webhook secret only when the same value can be placed in both RevenueCat and Supabase without entering source control.

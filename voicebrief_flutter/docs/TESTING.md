...
- Widget: primary action semantics, authentication paths, home/usage/empty state, immediate History dismissal with Undo, explanatory transcript disclosure, paywall selection/prices, empty saved-text action feedback, and dark large-text error.
- Golden: light/dark authentication and home, processing, result, paywall, 1.6× text, and Arabic RTL Settings/Recorder views at 390×844.
...
- Unit/widget suite: 27 passed, including dBFS normalization, fast-attack/slow-release meter response, immutable waveform repainting, extensionless MP3 signature detection, optimistic deletion and clear-all recovery before delayed storage completion, History Undo, empty saved-text feedback, Arabic mirroring, the centered narrow Arabic History empty state, and the default `10/10` free allowance.
- Golden suite: ten passed after intentional regeneration and visual inspection; Material Icons, deterministic Roboto glyphs, and an Arabic-capable Windows test font are loaded by the harness.
...
- Latest local source verification: `dart format` required no changes, `flutter analyze` passed, all 27 non-golden tests passed, all ten golden tests passed, and `flutter build apk --debug` completed. The earlier scoped `:app:connectedDebugAndroidTest` still passed its real shared Opus intent test on `SM_N950F`; the unscoped aggregate Gradle task is not used because it also tries to instrument plugin library modules with incompatible test-app minimum SDK declarations.
- GitHub Actions run `32834391844` passed from commit `da76b88`: Linux completed format, analyze, all 27 non-golden tests, and the Android debug APK; Windows completed all ten golden tests; macOS completed `pod install`, a live-config release iOS build with `USE_MOCK_SERVICES=false`, bundle-ID/Share Extension checks, version/build parity checks, IPA packaging, and artifact upload. Both built plists and the independently inspected IPA report `0.1.0 (2)` for the app and extension.

...
- GitHub Actions run `32829955468` passed from commit `6279ad4`: Linux completed format, analyze, all 25 non-golden tests, and the Android debug APK; Windows completed all eight platform-stable golden tests; macOS completed `pod install`, the unsigned release iOS build, bundle-ID/Share Extension checks, version/build parity checks, IPA packaging, and artifact upload. Both built plists and the downloaded IPA report `0.1.0 (1)` for the app and extension, eliminating the empty Share Extension `CFBundleVersion` that caused the installer placeholder failure.

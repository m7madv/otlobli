...
## VoiceBrief build 16 default/custom-sound candidate — 2026-08-30

- Build 16 removes the three rejected generated WAVs and their Xcode resource references. AlarmKit receives `.default` unless the user imports an audio or video file; iOS extracts the first 29 seconds, converts it off the UI thread to mono PCM CAF in `Library/Sounds`, and supplies the validated file with `.named`. The preferred sound is reused for later reminders.
- `/alarms` now has an Arabic-only title, a persistent sound card, a centered bounded empty state, scalable time rows, and a dedicated dark Arabic golden. Import has separate audio/video actions, clear 29-second copy, progress, failure feedback, preview for imported sound, and at least 44-point controls.
- Local verification passed format across 75 files, analysis with no issues, 37 non-golden tests, all 12 goldens, and `flutter build apk --debug`. The `177,642,309`-byte APK has SHA-256 `5C285160AB09649C7409DF25AA28E2923EF768A8F822A28BF12E709D71FB50F0`.
- CI run `33319637732` passed Deno, Android, 12 Windows goldens, and Xcode 26.6 no-codesign. TestFlight run `33319637699` archived, signed, verified, validated, and uploaded the `34,612,519`-byte IPA with SHA-256 `C63AE9AD859F24A325095A954411D72A4501A96B44155098647AB432AD0BE32E`; delivery UUID/build ID is `1d2cc0f0-f375-4665-bd20-8b88f89dd431`. Status run `33320659130` reports `VALID` and attached to `VoiceBrief Internal`. Physical-iPhone acceptance for audio import, video extraction, named-sound ringing, system-default ringing, title display, silent/Focus behavior, list refresh, and cancellation remains pending.
...
iOS/macOS remaining: build 16 AlarmKit permission and near-future alarm delivery/UI/default or imported sound under silent and Focus, audio/video import, in-app list and cancellation, ambiguous-time confirmation, cold/warm/terminated notification tap, session refresh/sign-out, permission denial/re-enable, microphone interruption/call/Bluetooth, sandbox monthly/annual/restore, and VoiceOver/Dynamic Type. Build 13's appointment card and calendar action passed by user report, while its local-notification alarm substitute failed product acceptance. Build 12's WhatsApp processing/manual close/ready notification/result flow passed; builds 9 and 10 physically rejected the containing-app/direct-document launch expectation.

## Honest environment record

This Windows host has Android tooling and the `Pixel_7_API_35_Test` emulator. It has no local Xcode/CocoaPods/iOS Simulator/signing. GitHub Xcode 26.6 compiled, signed, verified, validated, and uploaded build 16, and App Store Connect reports it `VALID` in `VoiceBrief Internal`. The physical iPhone rejected builds 9/10 for WhatsApp direct opening, accepted build 12's notification-based flow, and accepted build 13's appointment/calendar path while rejecting its local-notification alarm substitute. Build 16 physical default/imported-sound acceptance remains pending.

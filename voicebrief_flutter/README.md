...
- Production uploads require a short-lived signed token bound to one authenticated user, job, exact path, MIME type, and byte size. Active and daily reservations are capped; direct authenticated insertion into `audio-temp` is denied. Reached audio is deleted locally and remotely after processing. The backend source also schedules two-pass cleanup of abandoned reservations through the public Storage API after expiry and bounded safety windows; the first pass retains a tombstone so a later pass can repeat deletion before retiring the pointer. Vault configuration and live deployment remain explicit operational steps.

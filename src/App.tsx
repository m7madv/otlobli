  ...
  useEffect(() => {
    const handleMessageFromWebview = (event: { id?: string; detail?: Record<string, unknown> }) => {
      ...
      if (TEMU_PERSONAL_SITE_MODE && Capacitor.getPlatform() === 'android' && selectedStoreRef.current === 'temu') {
        ...
        // Temu page, scroll and selections visible; the extension confirms the
        // successful native handoff on top of the product while Cart updates
        // in the React shell behind it.
        temuAddInFlightRef.current = false
      }
    }
  ...
  const renderScreen = () => {
    ...
    return (
      ...
      <MobileShell active="home" onNavigate={(target) => {
        if (TEMU_PERSONAL_SITE_MODE && Capacitor.getPlatform() === 'android' && selectedStoreRef.current === 'temu') {
          if (target === 'home') {
            void TemuEmbeddedBrowser.goHome().catch(() => undefined)
            return
          }
          screenRef.current = target
          flushSync(() => setScreen(target))

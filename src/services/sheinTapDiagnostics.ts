...
export const SHEIN_TAP_DIAGNOSTIC_SCRIPT = `!function(){
  ...
  function node(n){if(!n||n.nodeType!==1)return null;var s={},h='',label='';try{s=getComputedStyle(n);h=n.href||n.getAttribute('href')||'';label=n.getAttribute('aria-label')||n.getAttribute('title')||n.getAttribute('alt')||''}catch(e){}return{tag:cut(n.tagName,20),id:cut(n.id,80),cls:cls(n),role:cut(n.getAttribute&&n.getAttribute('role'),40),href:cut(h,300),label:cut(label,100),pe:cut(s.pointerEvents,20),display:cut(s.display,20),visibility:cut(s.visibility,20),opacity:cut(s.opacity,16),z:cut(s.zIndex,20),position:cut(s.position,20),rect:rect(n)}}
  ...
  function observe(e,phase){var q=point(e),p=productAt(e,q[0],q[1]),now=Date.now();if(e.type==='touchstart'&&phase==='capture'){clearTimeout(afterTimer);active={id:++seq,at:now,x:q[0],y:q[1],href:cut(location.href,600),product:p}}else if((!active||now-active.at>1800)&&e.type==='click'&&phase==='capture'){active={id:++seq,at:now,x:q[0],y:q[1],href:cut(location.href,600),product:p}}if(!active)return;if(!p&&Math.abs(q[0]-active.x)+Math.abs(q[1]-active.y)>40)return;if(phase==='bubble'&&bubbled)bubbled.add(e);var data={attempt:active.id,event:e.type,phase:phase,isTrusted:!!e.isTrusted,cancelable:!!e.cancelable,defaultPrevented:!!e.defaultPrevented,x:q[0],y:q[1],scrollY:Math.round(scrollY||0),productDetected:!!(p||active.product),hrefBefore:active.href,hrefChanged:location.href!==active.href};if(phase==='capture'){data.snapshot=snapshot(e,p||active.product,q[0],q[1]);var ev=e,attempt=active.id,before=active.href;setTimeout(function(){send('event-final',{attempt:attempt,event:ev.type,bubbleSeen:bubbled?bubbled.has(ev):null,cancelable:!!ev.cancelable,defaultPrevented:!!ev.defaultPrevented,hrefBefore:before,hrefChanged:location.href!==before})},0);if(e.type==='touchend'||e.type==='touchcancel'){clearTimeout(afterTimer);var a=active;afterTimer=setTimeout(function(){send('tap-after',{attempt:a.id,hrefBefore:a.href,hrefAfter:cut(location.href,600),hrefChanged:location.href!==a.href,region:region()})},720)}}send('event',data)}
  ['touchstart','touchend','touchcancel','click'].forEach(function(t){document.addEventListener(t,function(e){observe(e,'capture')},{capture:true,passive:true});document.addEventListener(t,function(e){observe(e,'bubble')},{capture:false,passive:true})});

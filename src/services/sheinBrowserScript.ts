...
function otlobliInstallIosProductTapFallback(){var u=navigator.userAgent||'',s,t;if(!(/iP(?:hone|od|ad)/i.test(u)||navigator.platform==='MacIntel'&&(navigator.maxTouchPoints||0)>1))return;function i(v){var m=String(v||'').match(/-p-(\\d+)/i);return m&&m[1]||''}function a(n){for(var x=0;n&&x<12;x++,n=n.parentElement)if(n.tagName==='A'&&i(n.getAttribute('href')||''))return n;return null}function e(n){if(!n||!n.getAttribute)return'';var k=['data-goods-id','data-goods_id','data-product-id','data-product_id','data-id','fsp-key'];for(var x=0;x<k.length;x++){var v=String(n.getAttribute(k[x])||'');if(/^\\d{5,}$/.test(v))return v}return''}function c(n){var l=a(n);if(l)return l;for(var x=0;n&&x<12;x++,n=n.parentElement){if(!n.querySelector||/^(?:IMG|PICTURE|SOURCE|SVG|PATH)$/i.test(n.tagName||''))continue;var k=String(n.className||''),q=n.querySelector('a[href*="-p-"]'),r=n.getAttribute&&n.getAttribute('role'),v=e(n);if((q||v)&&(r==='listitem'||r==='link'||/product-card|product-item|goods-card|goods-item|waterfall-item|flash-sale__product|multi-product-card/i.test(k)))return n}return null}function p(v){var n=v.changedTouches&&v.changedTouches[0];return n&&[n.clientX,n.clientY]}function h(n){try{var l=a(n),q=l||n&&n.querySelector&&n.querySelector('a[href*="-p-"]'),v=q&&(q.href||q.getAttribute&&q.getAttribute('href'))||'',d=i(v);if(v&&d)return[v,d];d=e(n);if(!d)return null;var m=String(location.pathname||'').match(/^\\/[^/]+/),b=m&&m[0]||'/ar';return[location.origin+b+'/product-p-'+d+'.html',d]}catch(x){return null}}function f(x){try{window.__otlobliFreezeProbe&&window.__otlobliFreezeProbe(x)}catch(v){}}function g(x,o){try{window.__otlobliTapDiagnostic&&window.__otlobliTapDiagnostic(x,o||{})}catch(v){}}document.addEventListener('touchstart',function(v){clearTimeout(t);var n=c(v.target),r=p(v),o=h(n);s=n&&r&&o?[n,location.href,Date.now(),r[0],r[1],o[0],o[1]]:null;if(n){f('product-tap-start'+(o?'-target':''));g(o?'armed':'ignored-no-product-target',{productHref:o&&o[0]||'',productId:o&&o[1]||'',pageHref:location.href})}},{capture:true,passive:true});document.addEventListener('touchend',function(v){var n=s,r=p(v);s=null;if(!n||!r||c(v.target)!==n[0]||Date.now()-n[2]>650||Math.abs(r[0]-n[3])+Math.abs(r[1]-n[4])>16)return;f('product-tap-fallback');g('scheduled',{pageHref:n[1],productHref:n[5],productId:n[6]});clearTimeout(t);t=setTimeout(function(){var m=i(location.href);if(m===n[6]){g('skipped-product-route-ready',{before:n[1],after:location.href,productId:n[6]});return}window.__otlobliProductTapAttemptAt=Date.now();window.__otlobliProductTapAttemptUrl=n[5];f('product-tap-route-fallback');g(location.href===n[1]?'location-assign':'override-non-product-route',{from:location.href,to:n[5],productId:n[6]});location.assign(n[5])},500)},{capture:true,passive:true})}

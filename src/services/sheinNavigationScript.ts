...
function otlobliInstallIosProductTapFallback(){var u=navigator.userAgent||'',s,t;if(!(/iP(?:hone|od|ad)/i.test(u)||navigator.platform==='MacIntel'&&(navigator.maxTouchPoints||0)>1))return;function i(v){var m=String(v||'').match(/-p-(\\d+)/i);return m&&m[1]||''}function a(n){for(var x=0;n&&x<12;x++,n=n.parentElement)if(n.tagName==='A'&&i(n.getAttribute('href')||''))return n;return null}function q(n){if(!n||!n.querySelectorAll)return null;var z=n.querySelectorAll('a[href*="-p-"]'),l=null,d='';for(var x=0;x<z.length&&x<16;x++){var v=z[x].href||z[x].getAttribute&&z[x].getAttribute('href')||'',p=i(v);if(!p)continue;if(d&&p!==d)return null;if(!d){d=p;l=z[x]}}return l}function e(n){if(!n||!n.getAttribute)return'';var k=['data-goods-id','data-goods_id','data-product-id','data-product_id','data-id','fsp-key'];for(var x=0;x<k.length;x++){var v=String(n.getAttribute(k[x])||'');if(/^\\d{5,}$/.test(v))return v}return''}function c(n){var l=a(n);if(l)return l;for(var x=0;n&&x<12;x++,n=n.parentElement){if(/^(?:IMG|PICTURE|SOURCE|SVG|PATH)$/i.test(n.tagName||''))continue;var z=q(n);if(z)return n;var k=String(n.className||''),r=n.getAttribute&&n.getAttribute('role'),v=e(n);if(v&&(r==='listitem'||r==='link'||/product-card|product-item|goods-card|goods-item|waterfall-item|flash-sale__product|multi-product-card/i.test(k)))return n}return null}function p(v){var n=v.changedTouches&&v.changedTouches[0];return n&&[n.clientX,n.clientY]}function h(n){try{var l=a(n),z=l||q(n),v=z&&(z.href||z.getAttribute&&z.getAttribute('href'))||'',d=i(v);if(v&&d)return[v,d];d=e(n);if(!d)return null;var m=String(location.pathname||'').match(/^\\/[^/]+/),b=m&&m[0]||'/ar';return[location.origin+b+'/product-p-'+d+'.html',d]}catch(x){return null}}function f(x){try{window.__otlobliFreezeProbe&&window.__otlobliFreezeProbe(x)}catch(v){}}function g(x,o){try{window.__otlobliTapDiagnostic&&window.__otlobliTapDiagnostic(x,o||{})}catch(v){}}document.addEventListener('touchstart',function(v){clearTimeout(t);var n=c(v.target),r=p(v),o=h(n);s=n&&r&&o?[n,location.href,Date.now(),r[0],r[1],o[0],o[1]]:null;if(n){f('product-tap-start'+(o?'-target':''));g(o?'armed':'ignored-no-product-target',{productHref:o&&o[0]||'',productId:o&&o[1]||'',pageHref:location.href})}},{capture:true,passive:true});document.addEventListener('touchend',function(v){var n=s,r=p(v);s=null;if(!n||!r||c(v.target)!==n[0]||Date.now()-n[2]>650||Math.abs(r[0]-n[3])+Math.abs(r[1]-n[4])>16)return;f('product-tap-fallback');g('scheduled',{pageHref:n[1],productHref:n[5],productId:n[6]});window.__otlobliProductTapAttemptAt=Date.now();window.__otlobliProductTapAttemptUrl=n[5];clearTimeout(t);t=setTimeout(function(){var m=i(location.href),w=window.__otlobliRecoverSheinChunkOnStalledTap;if(w&&w(n[5])){g('recover-recorded-chunk',{before:n[1],after:location.href,productId:n[6]});return}if(m===n[6]){g('skipped-product-route-ready',{before:n[1],after:location.href,productId:n[6]});return}f('product-tap-route-fallback');g(location.href===n[1]?'location-assign':'override-non-product-route',{from:location.href,to:n[5],productId:n[6]});location.assign(n[5])},500)},{capture:true,passive:true})}
otlobliInstallIosProductTapFallback();
`

const OTLOBLI_SHEIN_CHUNK_FAILURE_BRIDGE_JS = `function otlobliInstallSheinChunkFailureBridge(){if(!/shein/i.test(location.hostname)||window.__otlobliSheinChunkFailureBridge)return;window.__otlobliSheinChunkFailureBridge=1;var sent=0;function s(url,m){if(sent)return false;sent=Date.now();try{var d={type:'sheinChunkLoadFailure',url:url||location.href,message:String(m||'').slice(0,180)},h=window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.messageHandler;if(window.mobileApp&&window.mobileApp.postMessage)window.mobileApp.postMessage({detail:d});else if(h)h.postMessage({detail:d});return true}catch(e){return false}}function p(v){var m='';try{m=String(v&&((v.message||v.reason)||(v.error&&v.error.message))||v||'')}catch(e){}if(!/ChunkLoadError|Loading chunk\\s+\\d+\\s+failed/i.test(m))return;window.__otlobliSheinChunkFailureAt=Date.now();if(/-p-\\d+/i.test(location.pathname)||Date.now()-(window.__otlobliProductTapAttemptAt||0)<15000)s(window.__otlobliProductTapAttemptUrl,m)}window.__otlobliRecoverSheinChunkOnStalledTap=function(url){var a=window.__otlobliProductTapAttemptAt||0,n=Date.now();if(sent)return !!(a&&sent>=a&&n-a<15000);var f=window.__otlobliSheinChunkFailureAt||0;return a&&f>=a&&n-f<15000&&n-a<15000&&s(url,'stalled product tap after ChunkLoadError')};addEventListener('error',function(e){p(e&&((e.error&&e.error.message)||e.message))},true);addEventListener('unhandledrejection',function(e){p(e&&e.reason)})}otlobliInstallSheinChunkFailureBridge();`

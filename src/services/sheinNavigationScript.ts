...
const OTLOBLI_SHEIN_CHUNK_FAILURE_BRIDGE_JS = `function otlobliInstallSheinChunkFailureBridge(){if(!/shein/i.test(location.hostname)||window.__otlobliSheinChunkFailureBridge)return;window.__otlobliSheinChunkFailureBridge=1;var sent=0;function s(url,m){if(sent)return false;sent=1;try{var d={type:'sheinChunkLoadFailure',url:url||location.href,message:String(m||'').slice(0,180)},h=window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.messageHandler;if(window.mobileApp&&window.mobileApp.postMessage)window.mobileApp.postMessage({detail:d});else if(h)h.postMessage({detail:d});return true}catch(e){return false}}function p(v){var m='';try{m=String(v&&((v.message||v.reason)||(v.error&&v.error.message))||v||'')}catch(e){}if(!/ChunkLoadError|Loading chunk\\s+\\d+\\s+failed/i.test(m))return;window.__otlobliSheinChunkFailureAt=Date.now();if(/-p-\\d+/i.test(location.pathname)||Date.now()-(window.__otlobliProductTapAttemptAt||0)<15000)s(window.__otlobliProductTapAttemptUrl,m)}window.__otlobliRecoverSheinChunkOnStalledTap=function(url){if(sent)return true;return Date.now()-(window.__otlobliSheinChunkFailureAt||0)<600000&&s(url,'stalled product tap after ChunkLoadError')};addEventListener('error',function(e){p(e&&((e.error&&e.error.message)||e.message))},true);addEventListener('unhandledrejection',function(e){p(e&&e.reason)})}otlobliInstallSheinChunkFailureBridge();`

# Active handoff — v86.195 live iOS Web Inspector classification (2026-08-15)

Clean-install v86.194 reproduced the fault in confirmed new container `A7FE8449-...`: first SHEIN entry worked, internal exit/re-entry froze again. The 29-second frozen capture kept one app PID/browser/surface/WKWebView, recorded 24 distinct DOM attempts plus 112 native hit-test and 475 DOM rows, completed three full document navigations under rapid taps, and had no WebContent termination. Each document remained inert/partly populated and emitted error hash `3ce0b2a11d15fff2` immediately after `sheinSaudiReady`. The clean fingerprint was cookies 6, localStorage 52, sessionStorage 1, CacheStorage 0, IndexedDB 6. The old and clean containers differ in cookies/localStorage but share the error hash and one-entry sessionStorage fingerprint. This rules out inherited version debris and native hit/overlay/render detachment. Do not add a reload/back-button workaround.

Current code is `86.195/1057`, marker `2026.08.15-v86.195-ios-live-web-inspector-diagnostic`. It changes diagnostic observability only: on iOS 16.4+ the dedicated SHEIN WKWebView sets `isInspectable` to the existing diagnostic flag, and the error probe emits category/name/source-path hash/line/column/target tag while native keeps raw error text hashed. Customer builds remain non-inspectable; there is no reload, clear, recovery, network request, observer, or touch mutation. Update it over the frozen v86.194 container, enable Safari Web Inspector on the iPhone, attach `ios_webkit_debug_proxy`, and capture Console + Network before causing another route. A local WebKit run with the same runtime saw repeated SHEIN recommendation-feed HTTP 403 responses behind a server-rendered shell, but treat that only as a hypothesis because automation itself can trigger SHEIN risk controls.

Diagnostic build is ready from GitHub run `31851860388`, commit `4dcd222`. The inspected unsigned IPA is 6,582,113 bytes, SHA-256 `EB53DFE1EA8FDB0C93B71C4DB5B78E3E6A8E8C32632AFB8D3D58641EB30722AA`, bundle `com.otlobli.app`, arm64, iOS 15+, iPhone+iPad, with no signature/provision/source maps. Install as an update without deleting v86.194 so the frozen website state remains available for inspection.

Do not implement a reload, back-button rebuild, website-data deletion, or another lifecycle workaround. On the connected iPhone 16 Pro Max, installed `86.193/1055` renders SHEIN but can lose all taps on same-store re-entry. A real Home/background capture kept the same app PID, host/store WebContent PIDs, and page proxies and emitted no close or WebContent termination. App-Switcher force quit does not recover the frozen state, while deleting the app container does. This narrows the investigation to native hit routing, live DOM event/overlay state, or persistent `WKWebsiteDataStore` state, but it is not yet a proven root cause.

Current candidate is iOS-only `86.194/1056`, marker `2026.08.15-v86.194-ios-root-cause-diagnostic`. Enable it with `VITE_SHEIN_IOS_ROOT_CAUSE_DIAGNOSTICS=true` (the unsigned-iOS workflow has `ios_root_cause_diagnostics`). Capture unified logs containing `[OtlobliSheinDiag]` / category `SheinRootCause`. The native plugin logs open/show/hide/close, view and WKWebView identities, hierarchy/front hit, physical `hitTest`, navigation, WebContent termination, and sanitized host foreground decisions. Document-start probes log passive DOM touch capture/bubble/final state and count/hash-only cookie/local/session/CacheStorage/ServiceWorker/IndexedDB fingerprints. They never log raw values or cancel/replay a touch and contain no polling, DOM observer, network call, reload, clear, or recovery.

The first physical v86.194 capture inherited the frozen v86.193 app container because the user updated instead of deleting first. USB confirmed `86.194/1056`. This state is not “zero taps”: fast/random taps can occasionally route to a section, but that destination is also inert/partly hydrated. In the 59-second trace, native `hitTest` and trusted DOM touch/click capture+bubble were healthy (`defaultPrevented=false`, no fixed top layer, same attached surface/WKWebView, no termination). Attempt 32 navigated to a new document in the same WKWebView; the destination drew header/filters with a blank/non-working product body and emitted JS error hash `3ce0b2a11d15fff2`. The inherited fingerprint was cookies 5, localStorage 38, sessionStorage 1, CacheStorage 0, IndexedDB 6. Thus native interception, detached rendering, and a generic overlay are ruled out for this case. The remaining lane is SHEIN runtime/hydration versus persistent WebKit state. Next capture must delete the app, reinstall the exact same v86.194, start syslog before first SHEIN entry, then compare first entry, internal exit, and second entry. Do not remotely uninstall, and do not touch Android or iPhone 6.

Local TypeScript, both web build modes, all release/freeze/Temu/surface/performance gates, iOS sync, and GitHub/Xcode run `31849799496` pass at commit `fb16b88`. Inspected IPA: `6,581,447` bytes, SHA-256 `448DBAB412F0B7F08DF5ECA535593FBCBC3F7EF07C2D9B225E5DBD9FE722257F`, `com.otlobli.app`, ARM64, iOS 15+, iPhone/iPad `[1,2]`, no app-level signature/provisioning/maps. Required device sequence is one clean install only: launch → enter SHEIN → prove one working tap → use Otlobli's store exit → enter SHEIN again → make three taps on the frozen page → stop. Keep the app on the frozen page while USB logs are collected. Do not touch Android or the iPhone 6, and do not call the problem fixed before correlating native-hit, DOM-tap, and persistent-state lines.

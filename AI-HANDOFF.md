...
LATEST MANUAL REQUEST (2026-09-19): owner wants free organic discovery/promotion for the published iPhone app, not emulator work. Public Apple page independently fetched: app 6805194629, VoiceBrief: Audio Summaries, publisher mohammad alzouabi, version 0.1.2, 11 UI languages. Existing root was privacy-only. Six static bilingual marketing pages now deployed to the EXISTING `voicebrief-legal` Vercel project, production READY deployment `dpl_FufzDD2AZgWoXQTXdoAALnuj3XPg`, alias https://voicebrief-legal.vercel.app . See `voicebrief_flutter/docs/MARKETING_DISCOVERY.md` for exact hash, rollout/rollback, copy, tests, and limitations. No new app build, backend/store metadata/subscription changes, or emulator work.

Source: `voicebrief_flutter/scripts/build_marketing_site.mjs` generates root/ar, en/ar WhatsApp guides, en/ar press kit, sitemap, facts and strict CSP JSON-LD hashes; preserves original legal/forms/JS. `legal_site/marketing.css` retains existing brand. Eight selected screenshots and icon are copied byte-for-byte, not invented/redesigned. Rebuild before deploying any marketing-source changes. Tests: `node --test voicebrief_flutter/scripts/marketing_site_test.mjs`; loopback-only `preview_marketing_site.mjs`. Nine checks and six HTML validations pass; 24 browser layout cases at 320/390/768/1440, FAQ keyboard/images/language and 8 legal-page cases passed locally. No physical device or post-deploy page fetch; Vercel READY/alias is deployment evidence. Screenshots in output/playwright/voicebrief-marketing-20260919.

IndexNow at 2026-09-19T20:50:43Z returned 202 for 6 URLs via https://www.bing.com/indexnow (official metadata verified). Receipt means URLs received/key validation pending, NOT indexed. Public ownership file exists at /indexnow-key.txt; don't rotate or spam retries. No new automation. Google Search Console verification/sitemap and Product Hunt/AlternativeTo account-gated submissions remain PENDING; asked owner which account is available. Never claim an external listing/post was published. SaaSHub rejects free subdomains; don't submit there under current URL or buy a domain. Future promotion must be truthful and maker-disclosed, with no fake votes/reviews/downloads, mass unsolicited posts, or guaranteed AI recommendations. Previous emulator blockers below still apply if that task is resumed.

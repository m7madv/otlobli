...
Marker `2026.08.09-v86.78-shein-ios-freeze-race-guard`; `86.78 / 938`. Build, expanded guard, patch reverse-apply check and performance budget pass (`1,198,034 / 1,200,000` raw JS; `543,347 / 550,000` SHEIN source); Android/iOS synchronized. iPhone workflow/IPA is pending. This trace-enabled candidate must set `SHEIN_IOS_FREEZE_DIAGNOSTICS=false` before a normal production release. Existing quick-add, toast and lazy-size work remains pending behind this iPhone priority.

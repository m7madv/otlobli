...
- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+26`. Build `23` بقي داخل App Review وGoogle Alpha، وBuildا `24` و`25` بقيا في TestFlight فقط. لا تنسب تغييرات Build `26` إلى بناء أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
...
- Apple: Build `23` submission ذو 8 عناصر ما تزال حالته `Waiting for Review`. Build `25` ما يزال `VALID` بالمعرّف `8e864062-28e0-4250-abd2-dac25f8df1b9` وداخل `Damanak Internal` ذات مختبر واحد. Build `26` لم يُرفع بعد عند هذا التحديث، ولا يجوز إضافته إلى App Review ضمن هذه الدفعة.

...
## مشروع مستقل (2026-08-31) — ضمانك 4.5.0+23 قيد الإصدار

- المصدر وإجراءات GitHub على `4.5.0+23`. هذا هو مرشح الإصدار؛ لا تُعد استخدام Build `22`. أهم الإصلاحات هي تحقق الدفع بعد نجاح Apple، ربط إيصال المتجر ذرياً، تحديث الاستحقاقات المنتهية، وحماية التجربة المجانية على مستوى الحساب والتثبيت. لا تدّعِ قبول شراء جهاز حقيقي قبل تنفيذه.
- Supabase الصحيح هو `exxayzlklvgeyqhvtzgi`. migrations حتى `20260831130000` مطبقة ومتطابقة. دوال الإنتاج النشطة: `verify-store-purchase` v20، `refresh-store-entitlements` v4، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v15، و`warranty-card` v11. cron تحديث الاستحقاقات كل 5 دقائق ناجح و`ENTITLEMENT_REFRESH_SECRET` مدوّر في Functions وVault؛ لا تكشفه أو تعِد نشر الخادم على `talabieh` أو VoiceBrief.
- `supabase/schema.sql` تفريغ حي حجمه `333,072` بايت وبصمته `2B91E8FD226CBDAB62CEBF73A4781DBC59E75C88722DB628CF41FE2BB9383627`. سياسة الخصوصية الحية تغطي Gemini/OpenAI والمرفقات ومعرّف التثبيت ومعاملات المتجر والتكاملات والحذف.
- التحقق المحلي: `flutter analyze` نظيف، Flutter `87/87`، Deno `19/19` مع `--allow-env`، وAndroid debug ناجح. APK debug حجمه `180,737,784` وبصمته `71DA30AE36ED8C46F948032F177C08D3BF295767DB8C6834B06B599128F6DCA9`.
- Apple: إصدار `4.5.0` استكمل الوصف والكلمات والفئة والتصنيف والتسعير والتوفر في دول الخليج الست والخصوصية وبيانات المراجعة. الاتفاقية المدفوعة والبنك والضرائب فعالة، والاشتراكات الستة `Ready for Review`. بقي رفع Build `23` وربطه، التحقق من حفظ هاتف وبريد المراجع، رفع 5 صور iPhone و5 صور iPad، ثم إضافة الإصدار إلى مسودة الاشتراكات وإرسالها.
- الصور المعتمدة تحت `app_store_assets/ios/iphone-1284x2778/` و`app_store_assets/ios/ipad-2048x2732/`. المولد `tool/app_store_marketing_screenshots_test.dart` اجتاز `5/5`. سكربت `scripts/app_store_screenshots.mjs` يرفع المجموعتين، ويستدعيه `Damanak Apple Setup` عند `mode=apply` لأن هذا الإجراء موجود أصلاً على الفرع الافتراضي؛ لا تستخدم ملفات المحاكي المؤقتة.
- Google Play: بطاقة المتجر والجمهور `18+` وأمان البيانات والأصول مكتملة. مسار `Alpha` (`4699097266255892286`) مرتبط بالقائمة الدائمة `All Apps - Closed Testers` ذات `12` مختبراً، وهي جاهزة **لأي تطبيق**؛ لا تنشئ قائمة أو مختبرين جدد. Build `22` قيد المراجعة، ويجب رفع Build `23` وربطه بالمسار نفسه. يبدأ عداد `14` يوماً بعد قبول Google وانضمام المختبرين فعلياً عبر `https://play.google.com/apps/testing/com.damanak.damanak`.
- حساب مراجعة Google داخل `Damanak Note 8 Test` على `scale` حتى `2036-08-31` بلا تجربة أو شراء، ولا توجد كلمة مروره في Git. حساب الخدمة يملك الصلاحيات المحدودة اللازمة لتطبيق Damanak؛ لا توسّعها. ما يزال شراء/استعادة Sandbox على iPhone وAndroid وقبول الجهاز الضعيف وقارئ الشاشة مطلوباً بصدق.

...
## مشروع مستقل (2026-09-01) — ضمانك Build 24 على TestFlight وBuild 23 قيد المراجعة

- الفرع الصحيح `codex/damanak-simple-pos`. Build `23` بقي داخل App Review وGoogle Alpha؛ المصدر `4.5.0+24` رُفع إلى iPhone TestFlight فقط بعد قرار المستخدم. لا تنسب رقع Flutter الجديدة إلى Build `23`، ولا تضف Build `24` إلى App Review أو Google بلا قرار منفصل. `scripts/app_store_setup.mjs` يبقى على Build `23` ما دام submission الجاري يستخدمه.
- راجعت دفعة Build `24` دورة الاشتراك كاملة: الأسعار من المتجر فقط، منع مزود ثانٍ، مهلات وحالات purchase/restore، أحداث Apple المبكرة والمتأخرة وربط `appAccountToken` بالمتجر، ومصالحة Google عند الفتح والاستئناف. Google يفحص `billingResult` الحقيقي، ولا يستخدم legacy بلا `obfuscatedProfileId` كتوكن استبدال، ويختار شراء/استعادة المتجر المفتوح فقط في الحساب متعدد المتاجر.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`. migrations حتى `20260831180000` مطبقة. الدوال النشطة: `verify-store-purchase` v23، `refresh-store-entitlements` v7، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v16، و`warranty-card` v14. migration `1800` تمنع sibling/replay لسلسلة Google بنمط predecessor CAS؛ لا تكشف أسرار Functions/Vault ولا تنشر على `talabieh` أو VoiceBrief.
- `supabase/schema.sql` حجمه `369,377` وبصمته `EA30919FF7784D1EB6A79BA66C575E4EC10F9EF5B5D0D46DD84DDD46406CCF74`. نجحت اختبارات SQL الحية `subscription_lifecycle_live.sql` و`account_deletion_successor_live.sql` و`warranty_tenant_integrity_live.sql` داخل `BEGIN/ROLLBACK`.
- جدولة `damanak-entitlement-refresh` نشطة كل 5 دقائق. آخر فحص عند `2026-08-31 20:35 UTC` نجح HTTP `200`. لا يوجد Google RTDN؛ شراء خارجي جديد يعتمد على فتح التطبيق للمصالحة، فلا تدّعِ إغلاق هذه النقطة.
- تدقيق الأمن الأصلي بقي 5 نتائج بلا عالية أو حرجة. بقي توثيق التجربة بالجهاز عبر App Attest/DeviceCheck وPlay Integrity وقرار الويب، وسياسة انتهاء/إبطال روابط الضمان؛ راجع `damanak_flutter/docs/SECURITY_AUDIT_2026-08-31.md`.
- التحقق المحلي: `flutter analyze` نظيف، Flutter `150/150`، Deno `56/56`، التنسيق و`git diff --check` وweb release وAndroid debug ناجحة. APK Build `24` حجمه `207,919,707` وبصمته `745AB8B44F68C587BBE57294D7D012642C9E059D2C93760C22AF7070AD729B8F` ويثبت `versionCode 24`.
- Apple: الصور العشر رُفعت في run `33341350096`، وBuild `23` الموقّع رُفع في run `33341350366` وبصمة IPA `9C024E7444F339DDEABEB33FA69EF634368B5FAFF820EC9FF5B7D3E57C1E8815`. رفضت Apple الإصدار آلياً تحت `3.0.0` و`3.1.2` بسبب غياب رابط EULA من App Description ولطلب تأكيد سعري `توسع شهري` (`49.99 USD`) و`توسع سنوي` (`499.99 USD`). حُفظ رابط EULA القياسي في الوصف، وأُرسل تأكيد أن السعرين مقصودان وأن أسعار قطر `199.99/1999.99 QAR` تأتي من المتجر، ثم أُعيد submission `69104e50-7a9a-4102-a717-ca341d442897` نفسه عند `2026-08-31 20:34` بتوقيت الرياض. العناصر الثمانية كلها الآن `Waiting for Review` على Build `23` نفسه؛ لا تلغِ الطلب أو تغيّر الأسعار أو رقم البناء أثناء المراجعة.
- iPhone TestFlight: commit المصدر `7fe6bf074e02acfa56b905d8e93431473ef2babd`، وrun [33443358450](https://github.com/m7madv/otlobli/actions/runs/33443358450) نجح كاملاً في بناء وتوقيع وفحص ورفع `4.5.0 (24)`. أنهت Apple المعالجة؛ build ID هو `6b986de0-74bb-4649-8619-846aca1eb1b0`، وحالته `Ready to Submit` ومربوط تلقائياً بـ`Damanak Internal` ذات مختبر واحد. IPA حجمها `23,923,863` وبصمتها `EA55D5F1192886EE9019F0048E3476C1D5416E047B4F1F1823D234FC56BCF768`. هذا اختبار داخلي فقط ولم يغيّر submission Build `23`، ولم يُثبت البناء بعد على iPhone حقيقي.
- Google Play: run `33341350386` رفع Build `23` إلى `Alpha`، وبصمة AAB `5FB3FE00B91FFBC49FE143AF31A77938A208686B410668CCCCA00422DEFCF008`، وهو قيد مراجعة Google. القائمة الدائمة `All Apps - Closed Testers` تضم بالضبط `12` مختبراً ومخصصة لإعادة الاستخدام **مع أي تطبيق**؛ لا تنشئ قائمة ولا تعدّلها. يبدأ عداد `14` يوماً بعد القبول والانضمام عبر `https://play.google.com/apps/testing/com.damanak.damanak`.
- حساب مراجعة Google داخل `Damanak Note 8 Test` على `scale` حتى `2036-08-31`. يبقى Note 8 على Build `22` المثبت من Play حتى قبول Build `23`، ولا يُثبّت فوقه APK مختلف التوقيع. شراء/استعادة حقيقيان على iPhone وAndroid، وVoiceOver/TalkBack، والجهاز الضعيف ما تزال اختبارات فعلية مطلوبة؛ legacy Google المبهم يفشل عمداً قبل تغيير الخطة ويُرسل للخادم فقط ضمن استعادة صريحة ذات مرشح واحد.

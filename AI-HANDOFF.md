...
- Build `26` مثبت فعلياً من TestFlight، وثبت شراء Apple حقيقي قبل حذف الحساب. v28 تستفيد منها النسخة المثبتة فوراً من دون حزمة جديدة، لكن الاستعادة ما تزال بانتظار محاولة حقيقية واحدة من حساب/متجر نهائي واحد. بعد المحاولة افحص `function_logs` و`function_edge_logs`: المطلوب `APPLE_RECOVERY_RUNTIME_COMPATIBILITY_SUCCEEDED` وHTTP `200` ثم `source=store/provider=app_store` وentitlement في قاعدة البيانات. لا تمنح entitlement يدوياً ولا تدّع النجاح قبل هذه الأدلة. لا يوجد شراء/استعادة Android أو قبول VoiceOver/TalkBack أو جهاز ضعيف.

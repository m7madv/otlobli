...
نتيجة آخر فحوص المصدر: `flutter analyze` نظيف و`flutter test --concurrency=1` نجح `299/299` منها 53 لغة/تخطيط، وإعادة 100 صورة نجحت. تغيّر تخطيط بطاقات المطالبات لمنع التفاف الكلمات الروسية داخل عرض ضيق. الويب السابق ناجح وإعادة البناء الأخيرة جارية. مرشح 4.6.0+32 لم تُثبت له حزمة Apple بعد؛ الخطوة التالية حفظ المصدر وتشغيل سير التوقيع ثم إعداد مسودة 4.6.0 وصورها دون تغيير النسخة الحية.

الأحدث: مرشح `4.6.0+32` محلي غير مرفوع، في المستودع الأصلي على `codex/damanak-simple-pos` وليس worktree `12a5`. Apple inspect run `34037302990` أكد المنشور `4.5.0 READY_FOR_SALE` وBuild31 `VALID`. كتالوجات اللغات العشر كاملة 1205/1205. نتيجة كاملة سابقة 289/289، منها 43 اختبار لغة مع عرض 320 وتكبير 150%؛ التحليل والويب نجحا وتُعاد الفحوص بعد التحرير الأخير. لا تدّع اعتماداً لغوياً شاملاً أو جهازياً.

مصدر التحرير `tools/localization/overrides.json` و`editorial/*.json`؛ شغّل `node build-catalog.mjs --strict` من هذا المجلد ثم `flutter gen-l10n` من التطبيق. ARB الناتجة مشحونة داخل التطبيق، ولا ترجمة شبكية لبيانات العملاء. الترجمة المحلية مسودة تتطلب مراجعة السياق؛ لا تعاود الطلبات الآلية للخدمة العامة التي رفضتها. لا ترفع `.venv` أو `model-cache` أو كاش التأليف. موارد iOS وXcode والأوصاف المحلية جاهزة مصدرياً؛ التحقق الموقّع على macOS لم يُنفذ بعد.

مولّد الصور يقبل `DAMANAK_SCREENSHOT_LOCALE` ويخرج 5 صور لكل حجم إلى `app_store_assets/ios/localized/<locale>/`، 100 إجمالاً. إعادة التوليد الأخيرة تشمل إصلاح ظلال أداة الاختبار ونصوص المطالبات والدعوات وأسماء منتجات تجريبية محلية. `verify_localizations.mjs` و`verify_store_localizations.mjs` بوابتان بنيويتان وليستا اعتماداً لغوياً. `damanak-store-localizations.yml` يجهز مسودة إصدار محدد فقط؛ لا يرسل مراجعة أو يغيّر الأسعار/التوفر. المستخدم فوّض الإصدار اللغوي الجديد؛ لا تشغّل إعداد Apple القديم المثبت على Build23. اعتماد gh المالك `m7madv` ضمن بيئة العملية فقط؛ لا تبدّل الحساب العام `mhm1981x-ship-it`. حافظ على AGENTS.md والمشاريع الأخرى. عائق Sandbox/Supabase في `BILLING_REVIEW_2026-09-05.md` لم يُحل.

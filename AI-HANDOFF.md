...
## مشروع مستقل (2026-09-01) — ضمانك مرشح Build 30 والخطة المجانية الشهرية

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.1+30`. Build `4.5.1 (29)` يبقى أحدث iPhone TestFlight حتى ينجح رفع Build `30`. إصدار Apple `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة؛ Google Alpha بقي على Build `23`. ارفع Build `30` إلى TestFlight فقط؛ لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً.
...
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` لم تُنشر بعد. هي تلغي التجارب القديمة وتنظف Apple Sandbox فقط، وتحصر RPC الطرفي في `app_store`، وتحفظ tombstones والإنتاج وGoogle وسجل التدقيق وعدادات rate-limit. أُزيل مسح limiter غير المقيد والتنظيف الواسع لصفوف manual/activation_code؛ الحالة غير المتسقة خارج النطاق تفشل migration بدلاً من تعديلها. نجح snapshot قبل/بعد داخل المعاملة نفسها لكل بيانات الفوترة الخارجة عن النطاق وGoogle lineage/receipt والـlimiter، ونجحت مصالحة `scale→growth` عبر RPC المزود الفعلي. الترتيب الإلزامي: Build `30` متاح في TestFlight، ثم بصمة فوترة الإنتاج، ثم `db push` ونشر `verify-store-purchase` و`refresh-store-entitlements`، ثم البصمة والاختبار الحي والمخطط.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901012000` منشورة. `verify-store-purchase` v28 `ACTIVE` مع `verify_jwt=false` وSHA-256 `ecee6ad20c0efd25c452d581552aafe903be8316cc43c767a0b551bd83dd7262`، و`refresh-store-entitlements` v7. لا يصل فاحص التوافق إلى القبول إلا بعد `status1/status1` وبعد تحقق ES256 وP-256 وDER والأسماء والتواقيع وOID والتواريخ والجذر المثبت؛ ثم تبقى مطابقة App Store API وحواجز الحساب المحذوف والربط الذري. `FAILURE=7` وهوية التطبيق/البيئة/طول السلسلة تبقى fail-closed. استجاب runtime الحي بلا جلسة بـ`401`، ونجحت اختبارات الإنتاج الخمسة السابقة داخل `BEGIN/ROLLBACK`.
- نجحت Apple orphan restore سابقاً عند `2026-09-01 09:22:26` وأعادت HTTP `200`. بناءً على طلب المستخدم نُفذ reset حي ذري لاحقاً للاشتراك `sandbox` الوحيد: كل الاشتراكات الآن `starter` (5 trialing و4 canceled)، و`source=store` صفر، والاستحقاقات الحالية وrefresh candidates صفر، مع tombstone تاريخي واحد `scale/monthly/revoked`. حُذفت grant المستهدفة وبقيت نافذة مراجعة Build 23 و`grants_used=2` دون تغيير. أكد فحص قراءة فقط بعد دورة الجدولة بقاء الأعداد صفراً. لا توجد بيانات فوترة `production`. لا تحذف tombstone أو trial claims أو audit logs.
- `supabase/schema.sql` حجمه `395,324` وبصمته `5A1E083994801006E3C620E6D91C611E2097D3B216FA1D3E8123F97DAD2F8F3C`. اختبار PGlite `1/1` يطبق migration ويثبت تطابق تعريف RPC مع المخطط؛ لا تستبدل الملف بلقطة أقدم.
- التحقق المحلي لمرشح `4.5.1+30`: `flutter analyze --no-pub` نظيف، وFlutter `225/225`، وDeno `83/83`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ أكد `aapt` أن APK يحمل `4.5.1 (30)`. حجمه `180,876,532` وبصمته `C4CA7B5F6EABF80C74FE5121067E4D0AC11B85809F0FDC36BCA9FD4526D99479`.
...
- Apple: أكد inspect القرائي [33533818718](https://github.com/m7madv/otlobli/actions/runs/33533818718) أن `4.5.0` صار `READY_FOR_SALE` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707`، ولا توجد مسودة مراجعة أو نسخة iOS قابلة للتعديل، والاشتراكات الستة معتمدة. أكد inspect [33537059054](https://github.com/m7madv/otlobli/actions/runs/33537059054) أن Build `4.5.1 (29)` صار الأحدث و`VALID` بالمعرّف `e029d611-6d1a-44b7-a93b-be8478d6e7d0` وداخل `Damanak Internal` ذات مختبر واحد، مع `mutationRequested=false`.
- Build `29`: نجح فحص macOS الموقّع بلا رفع في [33534828721](https://github.com/m7madv/otlobli/actions/runs/33534828721)، ثم نجح التحقق والرفع في [33535634162](https://github.com/m7madv/otlobli/actions/runs/33535634162) من الالتزام `316e31771974e5b069a3f5e98b0e2df58ee302cf`. Delivery UUID هو `e029d611-6d1a-44b7-a93b-be8478d6e7d0`. الـIPA المحلية `damanak_flutter/output/github-run-33535634162/damanak-ios-4.5.1-build-29-signed/ضمانك.ipa` حجمها `23,972,034` وبصمتها `297C151419D1CC4AA09BE776C1673303CF1A00782F9B840555CEA34F2BD88EA6`. محاولة `4.5.0 (29)` السابقة في [33532659092](https://github.com/m7madv/otlobli/actions/runs/33532659092) رفضتها Apple بالرمزين `90062/90186` لأن قطار `4.5.0` أُغلق؛ لذلك نُقل رقم التسويق فقط إلى `4.5.1` مع إبقاء build number `29`.
...
- Build `26` مثبت فعلياً وثبت فيه شراء Apple ثم استعادة orphan قبل reset. Build `4.5.1 (29)` مرفوع و`VALID` لكنه لم يُثبت أو يُقبل بعد على جهاز حقيقي. سجل شراء Apple `sandbox` لم يُمح من جهة Apple؛ قد تعيده الاستعادة، ولشراء نظيف يجب مسح Purchase History لحساب الاختبار. لا يوجد قبول جهاز للبنية الجديدة أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو جهاز ضعيف.

...
GitHub/Xcode run `32412740745` built commit `c914b52`; artifact ID `9422864423`. The inspected unsigned universal ARM64 IPA is at `C:\Users\MOHAMMAD\Desktop\otlobli-ios-v86.203-shein-prefetch-cache-fix\otlobli-v86.203-ipad-iphone-universal-unsigned.ipa`, size `6,601,350`, SHA-256 `73884D2E045DA72108FA2D000BA3070AAC4452CF755CF398DCED09137623B259`. WebKit compiled the exact raw-only JSON on macOS and all local guards/build/sync/archive checks pass.

Do not call the freeze fixed. Run clean Test A first: delete the already-poisoned v86.202 app once, install v86.203, record CDP/unified logs, exercise full store flows, then perform ten App Switcher kill/cold-launch cycles without reinstalling or clearing again. Afterward run separate Test B as an in-place v86.202→v86.203 upgrade over proven poisoned cache. If A passes but B fails, prevention is confirmed but existing users need a later one-time HTTP memory/disk-cache-only migration; do not add it to v86.203. Exact analyzer/protocol/results fields are in `docs/SHEIN_V86_203_PREFETCH_CACHE_FIX.md`.

# Active handoff — v86.202 proved SHEIN chunk-cache poisoning (2026-08-20)

Stop investigating generic touch, scheduling, native hierarchy, BFCache, browser duplication, or the contaminated `SHEIN_REQUIRED_COUNTRY` hash as the captured root cause. The v86.202 device/CDP/cache evidence proves this sequence: working Script 200 bodies for chunks `68498`/`26652` → SHEIN official `prefetchJs` fallback re-requests the same canonical JS URLs as XHR → stale CDN 304 responses contain no body and are accepted as successful prefetch → WebKit cache records become/remain `text/plain` with no blob → next cold process's actual Script requests receive 304/empty → immediate real `ChunkLoadError` → SHEIN never hydrates and stays inert at `#app`.

The frozen page was visible/focused, its seven event-loop mechanisms were healthy, and its single WKWebView/surface was attached and interaction-enabled. Forty-seven trusted events reached product elements; 45 reaction snapshots produced zero URL/history/root changes. The load was a fresh `navigate`, `pageshow.persisted=false`, with no Service Worker, WebContent termination, close, duplicate, or overlay. Read `docs/SHEIN_V86_202_ROOT_CAUSE_CAPTURE_REPORT.md` before making another candidate.

No fix is present in v86.202. The first narrow fix experiment should use a native `WKContentRuleList` to block only SHEIN XHR/raw prefetches for hashed `sheinm.ltwebstatic.com/pwa_dist/assets/*.js`, leaving actual Script loads unchanged. Validate with CDP, ten cold launches, and all store flows. Do not clear website data, reload, rebuild the WebView, switch stores, patch XHR in JavaScript, or change lifecycle/session behavior. `forceStoreVpnRecheck()` and root-Back remain separate real defects, not this root cause. Whether Otlobli's conditional `clearCache()` ran in older sessions remains unproven.

# Historical handoff — v86.202 passive root-cause timeline diagnostic (2026-08-20)
...
The probe is passive: no page DOM mutation, preventDefault/propagation stop, synthetic input, fetch/XHR/console/history patch, navigation/reload, WebView lifecycle change, or website-data write/clear. Only a detached Text node is changed to test MutationObserver delivery. Local gates/build/iOS sync and GitHub/Xcode run `32402979859` pass from code commit `a56c2e3d342eee285bde1659064ef6204ca1b8b5`; artifact `9419317843`. Inspected IPA path `C:\Users\MOHAMMAD\Desktop\otlobli-ios-v86.202-root-cause-timeline-diagnostic\otlobli-v86.202-ipad-iphone-universal-unsigned.ipa`, size `6,592,564`, SHA-256 `8F93DD8390DBF454070AFA709B631C69FCB6D19BB188C6E75F11D78C17489548`; it is ARM64/iOS 15+/iPhone+iPad/unsigned with no provisioning, app signature, source maps, or relay placeholder. Device capture is complete and documented in the root-cause report above.

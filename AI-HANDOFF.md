...
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` لم تُنشر بعد. هي تلغي التجارب القديمة وتنظف Apple Sandbox فقط، وتحفظ tombstones والإنتاج وGoogle وسجل التدقيق وعدادات rate-limit. أُزيل مانع كان يمسح جدولي limiter بلا قيد، وأضيف اختبار عقد يمنع عودته. الترتيب الإلزامي: Build `30` متاح في TestFlight، ثم بصمة فوترة الإنتاج، ثم `db push` ونشر `verify-store-purchase`، ثم البصمة والاختبار الحي والمخطط.

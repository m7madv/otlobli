...
- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.1+29`. Build `28` أحدث iPhone TestFlight إلى أن يكتمل رفع Build `29`. إصدار Apple `4.5.0` صار `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة؛ Google Alpha بقي على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً ضمن رفع TestFlight.
...
- Apple: أكد inspect القرائي [33533818718](https://github.com/m7madv/otlobli/actions/runs/33533818718) أن `4.5.0` صار `READY_FOR_SALE` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707`، ولا توجد مسودة مراجعة أو نسخة iOS قابلة للتعديل. الاشتراكات الستة معتمدة، وBuild `4.5.0 (28)` ما يزال الأحدث و`VALID` بالمعرّف `a9038423-e4fc-4499-8c04-bba4c0f1d317` وداخل `Damanak Internal` ذات مختبر واحد.
- Build `29`: نجح فحص macOS الموقّع بلا رفع في [33531820327](https://github.com/m7madv/otlobli/actions/runs/33531820327). محاولة رفع `4.5.0 (29)` في [33532659092](https://github.com/m7madv/otlobli/actions/runs/33532659092) بنت ووقعت وصدرت IPA ثم رفضتها Apple بالرمزين `90062/90186` لأن قطار `4.5.0` أُغلق بعد الاعتماد؛ رقم التسويق الحالي هو `4.5.1` مع إبقاء build number `29` لإعادة الرفع إلى TestFlight فقط.

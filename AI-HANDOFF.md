...
- العمل الحالي `0.1.2+21`، وليس إعادة إرسال 20. الواجهة 11 لغة و266 رسالة لكل لغة، مع حفظ الاختيار واختيار النظام بحسب الأولوية، ودعم RTL للعربية والأردية. ملفات ARB هي الأصل؛ أعد `flutter gen-l10n` بعد تعديلها. لا تمس الاشتراكات والمظهر أو تعديلات المستخدم غير المتتبعة.
- صور المتجر 88 PNG أصلية من Flutter مع بيانات توضيحية، أربع لكل لغة وجهاز في `voicebrief_flutter/store_assets/localized/`. السكربت `scripts/prepare_store_localizations.mjs` يتحقق من المقاسات والبصمات ويجدد manifest. راجع `docs/LOCALIZATION.md` داخل Flutter للأوامر والقيود.
- الخادم لم يعد يقصر مخرجات التفريغ على العربية والإنجليزية، والعميل والمشاركة لا يفرضان لغة الصوت بناءً على لغة الجهاز. نشر process-audio وحده إلى `jyehqpdbayslhzebdycj` قيد التحقق؛ CLI الحالي يعاني `Unsupported Config Type ""` وبدأت محاولة باستخدام إصدار 2.83.0 وملف voicebrief. لا تغير الأسرار أو مشاريع Supabase الأخرى.
- اختبارات اللغات 15 وتوليد الصور 88 واختبارات Deno للصوت/التواريخ 13 نجحت؛ الفحوص الكاملة وCI ورفع صور المسودة قيد التنفيذ. لا قبول iPhone/iPad فعلي أو مراجعة لغوية بشرية.
- `voicebrief-localize-store.yml`: الدفع يشغّل inspect فقط، وprepare الصريح يجهز مسودة أبل 0.1.2. لا يقدم للمراجعة أو يختار بناء أو ينشر. يجب تسجيل نتيجة الربط الفعلية وأرقام التشغيل لاحقًا في هذه الملفات الثلاثة، دون ادعاء أن اللغات متاحة للعملاء قبل إصدار التحديث.
- تحديث: process-audio17 ACTIVE نُشر على المشروع الصحيح؛ ملف profile أُعيد كما كان. جميع الفحوص المحلية نجحت (70+12) وAPKdebug ببصمة مسجلة في CURRENT_STATE. CI34036838778 كشف overflow1.5px في AuthMark على Linux؛ أضيف Flexible ويتطلب إعادة CI. أبل أكدت0.1.1 READY_FOR_SALE.

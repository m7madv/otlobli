...
- App Store Connect محفوظ حاليًا كمسودة فقط. إعداد الإصدار `0.1.0 (18)` وصور iPhone/iPad وملاحظات المراجعة مكتملة؛ التطبيق مجاني ومتوافر في `175` بلدًا/منطقة، وتقييمه العالمي `4+`. مسودة Review Submission تحتوي مجموعة `VoiceBrief Pro` والاشتراك الشهري والسنوي، وكلا المنتجين في المستوى `1` وبحالة Ready for Review. إصدار التطبيق لم يُضف إلى المسودة، ولم يُضغط Publish لخصوصية التطبيق أو Submit for Review.
- App Privacy معدّ من manifests الحزمة الفعلية على `15` نوع بيانات، كلها بلا tracking. السياسة المنشورة حُدثت بإفصاح Google Sign-In وربط RevenueCat ونُشرت في Vercel deployment ‏`dpl_EMsF8EH79BfRKPTSxNr8wRDDxRNn` بحالة `READY`. بقيت موافقة النشر النهائية داخل App Store Connect.
- اعتمد المالك `2026 MOHAMMAD ALZOUABI` وحُفظ في الإصدار، وأكد حقوق المحتوى؛ App Information يعرض الآن أن التطبيق يملك الحقوق اللازمة لمحتوى الطرف الثالث. جهّز المتصفح اسم/بريد الحساب `mhm1981d@gmail.com`، والاسم `MOHAMMAD ALZOUABI`، والرقم الخام `70067040`، ثم سُلّمت الصفحة للمالك كي يدخل كلمة المرور بنفسه؛ لا تقرأها أو تطبعها. يلزم تأكيد رمز دولة الهاتف. محاولة ربط الإصدار بالمسودة عادت بها إلى `3` عناصر فقط مع رسالة أن إصدار iOS مطلوب، فلا تدّع إضافة الإصدار. التوزيع يشمل فرنسا، بينما build 18 يضم TLS معياريًا من Flutter/Dart عبر BoringSSL رغم `ITSAppUsesNonExemptEncryption=false`؛ يلزم تصريح فرنسا/رمز الامتثال وإعادة بناء عند طلبه، أو موافقة المالك الصريحة على استبعاد فرنسا مؤقتًا.

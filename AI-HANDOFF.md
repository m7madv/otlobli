...
## مشروع مستقل (2026-08-27) — ضمانك 4.3.0+18

- المصدر الحالي هو `damanak_flutter/` بالإصدار `4.3.0+18`. حافظ على تجربة «الضمان أولاً» وعلى شاشة الاشتراك المبسطة؛ خطة نمو هي الموصى بها والاستعادة وإدارة الاشتراك واضحتان.
- حصص بداية/نمو/توسع هي `100/600/3000` ضمان شهرياً، وحدود الأعضاء `2/5/15`. لا تعرض إلا المزايا المنفذة، والضمانات السابقة تبقى للعرض والمتابعة بعد انتهاء الاشتراك.
- لا تغيّر معرّفات منتجات Apple/Google أو تضف أسعاراً ثابتة؛ السعر والعملة يجب أن يأتيا من المتجر. حافظ على تجميع نتائج Apple الجزئية والمهلة الكلية ومسارات StoreKit 2 ثم StoreKit 1 والطلبات المجمعة؛ كان الإرجاع المبكر في Build `17` جزءاً من عطل `storekit_no_response`.
- فحص Apple النهائي [33074176969](https://github.com/m7madv/otlobli/actions/runs/33074176969) يثبت Build `18` بحالة `VALID` وعضويته في `Damanak Internal`، وبيانات إصدارات Apple `4.4.1` الصحيحة للمنتجات `6/6`، وتوفر `UPFRONT` وقطر والسعر `6/6`. تجاهل `MISSING_METADATA` القديم على أصل المنتج عند وجود `catalogMetadataReady=true`؛ مصدر الحقيقة هو `subscriptionVersions` وترجماتها.
- محاولة ربط إصدارات الاشتراكات بمسودة المراجعة أضافت `0` عناصر وفشلت بـ`STATE_ERROR.ENTITY_STATE_INVALID`؛ لم تُرسل مراجعة عامة. لا تكرر الإرسال ولا تربط إصدار App Store من دون قرار إطلاق عام من المالك؛ هذا غير مطلوب لاختبار TestFlight.
- نُشر الترحيلان `20260827120000_damanak_plan_value_quotas.sql` و`20260827150000_damanak_database_hardening.sql` وتطابقا محلياً وبعيداً. نُشرت صفحات `legal` واختُبرت.
- تحقق التسليم: `flutter analyze` بلا مشاكل، `52` اختباراً ناجحاً، `6/6` لقطات ناجحة، وبناء Flutter Web وAndroid debug ناجحان. iOS run [33071848783](https://github.com/m7madv/otlobli/actions/runs/33071848783) رفع Build `18`؛ IPA المحلية حجمها `23,303,261` وبصمتها `E697D870320F768458DF626C06F3EB6545A4488D6D1CD851B13CB7D5C0AED4E8`. Android run [33071309057](https://github.com/m7madv/otlobli/actions/runs/33071309057) نشر Build `18` داخلياً؛ AAB حجمها `56,313,036` وبصمتها `8FDC30EE19EE850649171E90A619BF40ACEC2D9E028B32D296A48D71607A7E08`.
- الخطوة التالية حصراً: تحديث iPhone من TestFlight إلى Build `18`، إغلاق Build `17` بالقوة، ثم فحص ظهور المنتجات `6/6` والاستعادة من شاشة الاشتراك. لا تنفذ شراءً حقيقياً من دون موافقة مستقلة ولا تسجل قبولاً قبل نتيجة الجهاز. Note 8 ما يزال على نسخة Play `16`؛ حدّثه من مسار Play الداخلي ولا تستخدم تثبيتاً جانبياً. محاولة تسعير Google الأخيرة مرفوضة بصلاحيات حساب الخدمة وقد تبقى الأسعار القديمة.

...
## مشروع مستقل (2026-09-01) — VoiceBrief 0.1.0 build 18
...
- App Store Connect محفوظ حاليًا كمسودة فقط. إعداد الإصدار `0.1.0 (18)` وصور iPhone/iPad وملاحظات المراجعة مكتملة؛ التطبيق مجاني ومتوافر في `175` بلدًا/منطقة، وتقييمه العالمي `4+`. مسودة Review Submission تحتوي الآن أربعة عناصر: إصدار iOS، ومجموعة `VoiceBrief Pro`، والاشتراك الشهري والسنوي؛ كلا المنتجين في المستوى `1` وبحالة Ready for Review. زر `Submit for Review` فعّال، لكنه لم يُضغط بسبب مانع الامتثال الفرنسي والبناء البديل.
...
- حقوق النشر `2026 MOHAMMAD ALZOUABI` وContent Rights محفوظان. حُفظت بيانات App Review باسم `MOHAMMAD ALZOUABI` وحساب Google للمراجع `mhm1981d@gmail.com` والهاتف `+974 70067040`، وصُححت الملاحظات لتشرح دخول Google بدل أن تطلب Apple؛ أدخل المالك كلمة المرور مباشرة ولم تُقرأ أو تُطبع. أبقى المالك فرنسا، واختير التشفير المعياري غير الخاص في استبيان Apple، فظهر طلب `French encryption declaration approval form`. البناء 18 يعلن `ITSAppUsesNonExemptEncryption=false`؛ يلزم اعتماد ANSSI، رفعه إلى Apple، انتظار رمز الامتثال، وإنشاء بناء جديد بالقيم الصحيحة ثم استبدال البناء 18 قبل الإرسال. المسودة الآن `4` عناصر وتحتوي إصدار iOS، لكنها لم تُرسل.
- الملحق التقني الفرنسي الجاهز لـANSSI هو `output/pdf/VoiceBrief-ANSSI-technical-annex.pdf`، ثلاث صفحات، `82,233` بايت، SHA-256 ‏`045E6138E5B319D5E66FA606709F14283D159D000827E2B2C18C261873CE4D11`. يوثق من IPA الفعلي Flutter `3.41.6` وDart `3.11.4` وBoringSSL وTLS 1.2/1.3 والخوارزميات المعيارية، وتحقق نصيًا وبصريًا. لا تسمّه اعتمادًا؛ يلزم استكمال نموذج ANSSI الرسمي بصفة المصرّح والجنسية والعنوان، توقيع المالك، ثم إرساله إلى `controle@ssi.gouv.fr` وانتظار `Attestation de déclaration`.
- نموذج ANSSI الرسمي الأصلي موجود في `output/pdf/ANSSI-crypto-declaration-form-v2.pdf`، حجمه `2,308,710` بايت وSHA-256 ‏`149CF1324CC144C5891B085ED0ACE5E8B90888BBA61DB7A4FAF5822EE7EBF8E7`. هو `XFA` ديناميكي ويتطلب Adobe Reader؛ لا تحوله إلى PDF ثابت بديل لأن ANSSI تطلب النموذج الإلكتروني المحفوظ إضافة إلى النسخة الموقعة الممسوحة. لم يُملأ بعد.
- المرشح المحلي الحالي `0.1.0+18`. يحافظ على ثبات المظهر وآخر رصيد مؤكد من build 17، ويضيف إعادة محاولة آمنة للرفع: يعيد الخادم `uploadedAlready:true` فقط بعد إثبات أن الجسم الموجود يطابق الحجم و`Content-Type`. ترسل Responses API ‏`store:false`، ويقرأ RevenueCat webhook جسم JSON فعلياً بحد أقصى مضبوط.

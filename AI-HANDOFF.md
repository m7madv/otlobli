...
## مشروع مستقل (2026-08-31) — VoiceBrief 0.1.0 build 18
...
- بعد تأكيد المستخدم الصريح، رُبطت قائمة `All Apps - Closed Testers` ذات 12 عضوًا بمسار Google المغلق `Alpha`، واختيرت جميع المناطق: `176` بلدًا/منطقة و«بقية بلدان العالم». نُشر بريد التواصل العام `mhm1981x@gmail.com` والموقع القانوني HTTPS، وحُفظ IARC بفئة «كل أنواع التطبيق الأخرى» مع المحتوى المولّد عبر الإنترنت والمشتريات الرقمية؛ التقييمات `Everyone` و`PEGI 3` و`IARC 3+` و`ClassInd 14+`. Data Safety محفوظ كمسودة كاملة وصادقة، لكن الحفظ النهائي والجمهور `18+` محجوبان حتى تفاصيل المراجع. Alpha ما زال غير نشط وبلا إصدار، والمنضمون الفعليون 0 ولم تبدأ مدة 14 يومًا؛ لا تدّع بدء الاختبار المغلق أو النشر العام.
...
- وافق المالك صراحة على البريد العام `mhm1981x@gmail.com` وIARC وشروطه والجمهور `18+` والنص القانوني. حُفظ IARC وحُفظ Data Safety كمسودة: جمع فقط بلا مشاركة، تشفير أثناء النقل، OAuth، حذف الحساب والبيانات عبر `/delete-account`، والأنواع الموثقة هي الاسم/البريد/معرّف المستخدم/معلومات اللغة والمنطقة/صورة المزود/سجل الشراء/التسجيلات/الملفات/المحتوى الناتج/إجراءات الاستخدام/التشخيص. لا يوجد email/password أو guest/review mode في build 18؛ يجب أن يكون حساب Google المراجع مخصصًا وقابلًا لإعادة الاستخدام بلا 2FA/OTP/passkey، وأن يسجل الدخول مرة لإنشاء UUID ثم يُمنح RevenueCat entitlement `pro` طويل المدة ويُتحقق من ظهور 300 دقيقة. عيّن المستخدم `mhm1981d@gmail.com` لحساب المراجعة، لكن لا تحفظ البيان ولا تؤكد الوصول الكامل حتى يدخل المالك كلمة المرور بنفسه في Play Console ويثبت الحساب/الاستحقاق حيًا.

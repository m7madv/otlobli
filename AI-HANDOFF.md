...
## مشروع مستقل (2026-09-01) — ضمانك مصدر Build 26 وإصلاح الاستعادة
...
- الاستعادة الصريحة فقط ترسل `recoveryRequested=true`. Apple orphan يتطلب JWS صحيحاً تتحقق منه مكتبة Apple الرسمية مع الجذور المثبتة، وحذف الهوية القديمة، ومتجراً هدفاً وحيداً بلا entitlement؛ بعدها يحدث `appAccountToken` إلى UUID المتجر في مهمة خلفية. Google يدعم Build `23` account-only وlinked/out-of-app مع RPC ذرية وأقفال token hashes؛ أي هوية قديمة حية أو ربط قائم يمنع النقل.
...
- أثبتت السجلات الحية أن شراء TestFlight نجح وربط الاشتراك بالحساب القديم عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حُذف الحساب بعد نحو 8 ثوانٍ. طلب الاستعادة نفسه على الحساب الجديد وصل إلى Apple ونجح في جلب السلسلة، لكنه فشل قبل الكتابة داخل فاحص JWS المتصل؛ بقي الهدف بلا entitlement، لذلك لا تطلب من المستخدم شراءاً ثانياً.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901012000` منشورة. `verify-store-purchase` v26 نشطة مع `verify_jwt=false` وSHA-256 `2e99b9097d3f18014720fd2f0d9b73a1ed2eaa5ae7f39fe6af9745d9917d65e4`، و`refresh-store-entitlements` v7. v26 تعيد فحص المهلة والحالات `1/2/6` بفاحص Apple الرسمي عند `signedDate`، لكنها تبقي `FAILURE=7` وهوية التطبيق/البيئة/طول السلسلة fail-closed؛ المطابقة الدقيقة وحواجز الحساب المحذوف والربط الذري لم تضعف. نجحت اختبارات الإنتاج الخمسة السابقة داخل `BEGIN/ROLLBACK`.
...
- التحقق المحلي: `flutter analyze` نظيف، Flutter `181/181`، Deno `71/71`، migration `1/1`، `deno check` للدالة و`deno fmt --check` على 23 ملفاً، و`git diff --check` وweb release وAndroid debug ناجحة. إصلاح v26 خادمي ولم يغير Flutter؛ APK Build `26` بقي حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لا ترفعه إلى Google بلا قرار منفصل.
...
- Build `26` مثبت فعلياً من TestFlight، وثبت شراء Apple حقيقي قبل حذف الحساب. ما تزال الاستعادة بعد v26 المنشورة بانتظار إعادة محاولة المستخدم، ولا يوجد شراء/استعادة Android أو قبول VoiceOver/TalkBack أو جهاز ضعيف؛ لا تدّع نجاح البنود غير المنفذة.

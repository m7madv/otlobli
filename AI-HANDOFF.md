...
## ضمانك — أحدث متابعة اللغات 2026-09-06

الأحدث: أُرسل4.6.0(32) إلى Apple بطلب المستخدم الصريح في2026-09-06 الساعة18:13 بتوقيت الرياض. طلب `e80028ff-2ee6-48d6-b414-6172d31b2c3a` بحالة Waiting for Review وعنصر واحد؛ النشر التلقائي بعد الموافقة محفوظ، دون مراحل أو مسح التقييمات. لا تلغِ الطلب ولا تعِد البناء/الرفع. `34040072308` أكد32 VALID ومعرّفه `7848f16a-97f9-4315-93bd-daea14024fc2` ومضافاً لمجموعة Damanak Internal. اكتملت en-GB في metadata run `34040227259` والصور `34040273515`: 11 محلية متجر و110 صور COMPLETE وبصماتها مطابقة، و10 لغات تطبيق. لا حاجة لBuild33 لهذه الأصول. فحص Sandbox مع دخول مالك المشروع ما يزال غير منجز ولا يُعد إرسال المراجعة إثباتاً لنجاح الدفع.

استخدم المستودع الأصلي `C:\Users\MOHAMMAD\Projects\SHEIN IN SIRYA` على `codex/damanak-simple-pos` لا worktree12a5. اقرأ `damanak_flutter/docs/LOCALIZATION_RELEASE_2026-09-06.md`. التطبيق4.6.0+32 من8f1327b، وCI الموقّع من24e6f9b؛ التغييرات اللاحقة تشغيل/توثيق فقط. 10×1205 نصوص مكتملة بنيوياً؛ لا ادعاء اعتماد لغوي شامل. Flutter299/299، اختبار اللغات53/53، analyze والويب ناجحان.

إصدار Apple4.6.0 هو `af5f3581-d237-476d-b8c4-a42c73f7cc19` وقد رُبط بالبناء32 وأُرسل للمراجعة. run `34039342937` نجح؛ IPA24965730 بايت وبصمته `E4728DFE2289CEC91FD9449AB057A514593F682BE1815B543F2F2DEF6CBBA1A7`. المنشور4.5.0 لم يتغير بعد. ملاحظات المراجعة المصححة توضّح اللغات العشر والدخول عبر Apple/Google والحصة المجانية20 شهرياً، دون ادعاء تجربة Starter. حساب Supabase الحالي لا يرى مشروع ضمانك؛ الفحص القرائي الأخير أظهر مشروعين آخرين فقط. لا تدّع إصلاح الدفع أو قبول جهاز/ترجمة لغوية شاملة.

لأعمال المتجر استخدم workflow341412875 `damanak-apple-setup.yml` مع asset_kind=metadata أوscreenshots وإصدار/لغة صريحين، لا setup القديم. gh المالكm7madv ضمن بيئة العملية فقط؛ لا تبدّل الحساب العام. فشل CI الأول بسبب حزمة أدوات التأليف المستقلة وصُحح، وفشل metadata الجزئي بسبب محلية تولدها Apple تلقائياً وصُحح وأعيد بنجاح؛ التفاصيل في التقرير.

مصادر التحرير tools/localization/overrides.json وeditorial/*.json؛ مولد الكتالوج من مجلد التأليف ثم flutter gen-l10n من التطبيق. لا تُشحن .venv/model-cache/translation-cache ولا تعاود المزود العام الذي رفض الأتمتة. احتفظ ببيانات المستخدمين كما هي. لا تغيّر بلدان الخليج أو منتجات الاشتراك أو Sandbox أو الحسابات بهذه المهمة؛ قيد Sandbox السابق في BILLING_REVIEW_2026-09-05.md لم يُحل. الموارد القانونية الخارجية وأسماء منتجات المتجر الأصلية لم تُترجم بهذه الدفعة. احفظ AGENTS.md وتغييرات المشاريع الأخرى.

Read `CURRENT_STATE.md`, then `AGENTS.md`, before editing.

## تحديث RevenueCat لـVoiceBrief — 2026-08-26

- اكتمل إعداد RevenueCat الأساسي لمشروع `VoiceBrief` (`59213dad`): Apple app `app85cd86a950` وGoogle Play app `appa60d40b8c1` محفوظان للمعرّف `app.voicebrief.mobile`. لا تعِد إنشاءهما. ملفات P8 وGoogle JSON خارج Git؛ ملف Google عند `C:\Users\MOHAMMAD\.voicebrief\google\revenuecat-play-service-account.json` وبصمته `60BF02E2D1AD0C9AE2B2321A4EC207B7EAD0134C38D52AC5D467E15D56497107` وACL محصور بالمستخدم الحالي وSYSTEM.
- منتجات RevenueCat الأربعة منشأة ومربوطة بالاستحقاق `pro`: Apple شهري `prodd7310149f3` وسنوي `prod917a173c57`، وGoogle شهري `prod4fffb4ba55` وسنوي `prode9c5469afa`. العرض `default` يربط الشهرية بـ`$rc_monthly` والسنوية بـ`$rc_annual` على المتجرين؛ lifetime بقي Test Store-only. مفتاحا SDK العامان محفوظان في GitHub Secrets، وBuild `0.1.0+8` هو أول بناء مُجهز لهما.
- webhook RevenueCat مكتمل: سر عشوائي 32 بايت محفوظ فقط في Supabase باسم `REVENUECAT_WEBHOOK_SECRET`، وقيمة `Bearer` المطابقة محفوظة في webhook `whintgr41c91a4676` (`VoiceBrief Supabase`) لكل أحداث الإنتاج وsandbox. تطابقت البصمة، والطلب غير المفوض يعيد `401`. لا تعرض أو تدوّر السر دون موافقة جديدة؛ المتبقي إرسال حدث تجريبي، تأكيد بريد الحساب، ثم اختبار شراء واستعادة sandbox. Google Play نفسه ما يزال يمنع حفظ/تفعيل base plans، ولذلك لن يعمل شراء Android قبل زوال عائق Google حتى مع اكتمال ربط RevenueCat.
- فرع `codex/voicebrief-ios` عند `eccc611`. run `32922121853` أخضر في Android و11 golden وiOS غير الموقّع، وrun `32922142795` وقّع التطبيق وShare Extension وفحصهما ورفع build 8 بنجاح. Apple أنهت المعالجة؛ TestFlight build ID هو `71d308f7-b859-46ec-9d56-7622e58ffe79` وحالته `Complete` ومربوط تلقائياً بـ`VoiceBrief Internal`. IPA حجمها `34,411,364` وSHA-256 `459B17DDAC178B40C598574B70A0769141B35C78A6890DEDA4F3019A9BDB72CA`. لا تدّعِ قبول شراء أو استعادة قبل اختبار sandbox على جهاز حقيقي.
...
- `deleteResult` و`restoreResult` متفائلان. History يزيل العنصر ويظهر Undo فوراً، وResult ينتقل مباشرة بعد التأكيد. `clearHistory` أيضاً متفائل لكنه يعيد السجل إذا فشل Drift. إعدادات التصدير تنشئ TXT يضم الملخص والنقاط والمهام والمواعيد والردود والنص الحرفي، ولا تضم الصوت. لا تربط الحركة بزمن Drift. «النص الحرفي للتسجيل» تلقائي ومطوي ومشروح؛ لا تعِد مفتاحاً يخلط بينه وبين الملخص.
...
- الدخول حصراً عبر Apple وGoogle؛ حُذفت واجهة البريد وكلمة المرور وإنشاء الحساب والاستعادة، كما حُذفت الدوال من `AuthRepository` و`AppController` ومكوّن `AppPasswordField` ونصوصه، وعُطل Email في Supabase. Android وiOS يطلبان `email/profile` عبر Google Sign-In الأصلي ويرسلان `idToken` و`accessToken` إلى Supabase؛ لا تُعد مسار iOS الخارجي `signInWithOAuth` لأنه ثبت حياً أنه يرتد إلى `http://localhost/` ويعرض صفحة فارغة. Google OAuth صار In production و`Skip nonce checks` مفعّل لحل رفض iOS المثبت في Auth Logs. Apple مفعّل للمعرّف `app.voicebrief.mobile` بمفتاح `N4FK6753YL` وسر ينتهي `2027-02-21T22:07:34Z`؛ دوّره قبل الانتهاء ولا تنقل `.p8` إلى Git.
- آخر تحقق: `dart format` نظيف، `flutter analyze` نجح، 30 اختباراً غير golden نجحت، 11 golden نجحت، وبناء Android debug نجح محلياً. اختبار Google الجديد يثبت قبول المستخدم الموثق من المسار الأصلي، واختبار الأخطاء يثبت تحويل أكواد Supabase إلى رسائل قابلة للتنفيذ. تشمل الصور دخول Apple/Google بالعربية RTL والفاتح/الداكن، وإعدادات ومسجل Arabic RTL عند 390x844. اختبار `:app:connectedDebugAndroidTest` السابق ما يزال ناجحاً لمشاركة Opus على `SM_N950F`.
...
- فرع `codex/voicebrief-ios` عند `787840c` (المصدر التشغيلي `1ddfd5d`، وأداة اشتراكات Google المضافة في `787840c`). run `32901316058` رفع Build 7 الموقّع وrun `32901316077` أخضر. IPA حجمها `34,368,348` وSHA-256 `15950D4EC8FDD54649126BC1473D37849D46B569F7E4DB2BE40E7AC65FD48FCD`. Build 7 مثبت على `iPhone 16 Pro Max / iOS 27.0`. سجل Supabase بعد الإصلاح يؤكد Google `Login / INFO` عند `01:06:16` وApple عند `01:08:42` في 2026-08-26 بتوقيت الرياض؛ هذا قبول جهاز حقيقي لكلا المزودين. المتبقي للدخول هو Play App Signing SHA واختبار Android الموقع من Play فقط.
- اشتراكات Apple موجودة في المجموعة `VoiceBrief Pro` (`22335194`): الشهري `voicebrief_pro_monthly`/`6805232846` بسعر قطر `29.99 QAR`، والسنوي `voicebrief_pro_annual`/`6805233746` بسعر `229.00 QAR`، مع ترجمة عربية وإنجليزية وتوفر إقليمي. حالتهما `Prepare for Submission` ويحتاجان إرفاقهما بإصدار جديد وصور مراجعة.
- Google Play VoiceBrief رقم `4973918177043935523` موجود تحت حساب `SAMIR KHALED ALZOUBI` (`8441225038702199576`). رُفعت AAB `0.1.0 (7)` إلى مسودة اختبار داخلي، والمنتجان الشهري والسنوي موجودان. لا تعِد إنشاءهما. مُنح `damanak-play-verifier@damanak-production.iam.gserviceaccount.com` وصول VoiceBrief فقط بصلاحيات المالية والطلبات/الاشتراكات وإدارة التوفر/المنتجات؛ القراءة صارت `200` فوراً، لكن تحويل الأسعار والكتابة ما زالا `The caller does not have permission`. محاولة أخيرة من جلسة المالك اختارت 177 منطقة وحوّلت الشهري بنجاح إلى `29.00 QAR` في قطر، ثم رفض الحفظ نفسه برسالة `تعذَّر حفظ التغييرات.`؛ هذا يثبت أن القيم صحيحة وأن العائق حالة تحقيق الربح/الحفظ في حساب Google. شغّل `voicebrief_flutter/scripts/google_play_subscriptions.mjs --activate` بعد انتشار الصلاحية؛ إن استمر فشل جلسة المالك فلابد من تذكرة Play Support. السكربت يرفض الإعداد الجزئي ويستهدف `29/229 QAR` ثم يتحقق من `ACTIVE`. حساب `mhm1981x@gmail.com` الآخر رقمه `7616754513484681287` محظور من النشر بسبب إثبات الهوية والهاتف ولا يحتوي التطبيق. RevenueCat مكتمل حتى ربط المنتجات والاستحقاق والعرض والمفاتيح والـwebhook؛ المتبقي حدث تجريبي وتأكيد البريد واختبارات sandbox.
- نقطة الرجوع `voicebrief-prebuild-20260824`، وApple Design Skill مثبت عند `d0bac1e765a27a696839e62962e36330ce72f0b7`.

## مشروع مستقل (2026-09-02) — ضمانك Build 31 وإصلاح معاملات Apple غير المنتهية

- تحقق متابعة 2026-09-05: نجح web release محلياً بلا نشر، ولم تُبنَ حزمة هاتف جديدة؛ لا تعتبر تعديل النص جزءاً من Build `31`.

- متابعة 2026-09-05 محلية غير منشورة: شكوى «تعذر التحقق من عملية نسخة الاختبار» تطابق `SANDBOX_NOT_AVAILABLE`، الناتج عن حارس سماح Sandbox وليس فشل توقيع JWS بالضرورة. افحص السماح المنتهي/المفقود بعد حذف الحساب قبل العودة لإصلاح StoreKit. السماح مقيد بـ24 ساعة و`ON DELETE CASCADE`؛ نوافذ المراجعة أغلقتها migration السابقة. صُححت رسالة Flutter لتوجيه الدعم بدلاً من انتظار وهمي؛ اختبار إعادة الإيصال بعد تصحيح السماح ناجح. Flutter `246/246` وDeno `85/85` والتحليل نظيف. أضيف `supabase/tests/sandbox_access_diagnostics.sql` قرائي ولم يُنفذ حياً. لا تفتح Sandbox لكل المستخدمين ولا تحذف الاشتراكات؛ أكمل التحقيق مع صلاحية المالك. التقرير `docs/BILLING_REVIEW_2026-09-05.md` يوضح أن الحل التشغيلي لم يكتمل، وأن Build `31` لم يتغير.

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.1+31` عند `667299caf5481ab4c38c7b3d872e426e878c53e3`. Build `4.5.1 (31)` هو أحدث iPhone TestFlight وحالته `VALID` بالمعرّف `3d3e11d1-bf04-4022-9cc7-28c185ea81b7` وداخل `Damanak Internal` ذات المختبر الواحد. إصدار Apple `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة؛ Google Alpha بقي على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً.
- الخطة المجانية الجديدة ليست اشتراك متجر: 20 ضماناً لكل شهر تقويمي، وفرع وعضو واحدان، وحساب واحد على تثبيت محمي واحد. `private.free_*` و`current_store_access` هما مصدر الصلاحية، و`subscriptions.free_access_mirror` مرآة توافق لـBuildات `23–29` فقط. UUID التثبيت والجلسة لا يساويان attestation؛ تبقى App Attest/DeviceCheck وPlay Integrity مفتوحة.
- migration `20260901200000_damanak_free_access_and_billing_repair.sql` منشورة. ألغت التجارب القديمة ونظفت Apple Sandbox فقط، وحصرت RPC الطرفي في `app_store`، وحفظت tombstones والإنتاج وGoogle وسجل التدقيق وعدادات rate-limit. لم توجد بيانات Apple Production أو Google، وتطابقت صفوف وبصمة عدادات الشراء الأربعة قبل/بعد، وجدول refresh limiter الجديد فارغ. سجل Apple Sandbox الوحيد صار `revoked` غير قابل للتحديث، وصفوف الاشتراك التسعة صارت نظيفة، ومنحها المجانية التسع `pending_device` حتى أول ربط تثبيت من Build `30`. نجح `free_access_billing_repair_live.sql` بعد النشر داخل `BEGIN/ROLLBACK`.
- بنية Flutter المركزية في `lib/features/subscriptions/domain/subscription_flow.dart`: `BillingScope` للحساب/المتجر/epoch، و`CatalogSnapshot` للأسعار، و`BillingOperation` لعملية واحدة، و`SubscriptionFlowMachine` لرفض النتائج القديمة، و`SubscriptionPolicy` لقرار `start/upgrade/cycleChange/blocked`. لا تعيد حقول الحالة المتوازية القديمة أو منطق انتقال ثانياً في الشاشة.
- `subscription_screen.dart` و`AppController` يستخدمان `SubscriptionPolicy` نفسها بفشل مغلق. الباقة الأدنى والحالية بلا شراء، والمزود المنافس والحالة الناقصة ممنوعان. `PlatformStoreBillingService` يعيد الحراسة، وGoogle يرمي `GOOGLE_SUBSCRIPTION_DOWNGRADE_NOT_ALLOWED` بدل `ReplacementMode.deferred`.
- كل شراء ينفذ preflight مرجعياً عبر `loadCurrentSubscription` من قاعدة البيانات، ثم `refreshStoreSubscription` إن كانت الصورة متجرية. لا تُعد الحالة المحلية وحدها إذناً لفتح الدفع. حراس serial/generation يمنعون background refresh وworkspace snapshot وrestore fallback القديمة من محو إيصال أحدث؛ stream error لا ينهي verify جارياً؛ وأحداث terminal تحرر الاستعادة النشطة والمعلقة.
- Build `30` يبقي فحص `Transaction.currentEntitlements` قبل كل شراء Apple عبر قناة iOS محدودة البيانات. بلا اشتراك خادمي، أي entitlement حالي لضمانك يمنع الدفع ويطلب Restore صريحاً. مع اشتراك خادمي يجب أن يملك Apple ID الحالي نفس `originalTransactionId` ونفس `productId`؛ الحساب الفارغ أو المختلف أو تغيّر المنتج يفشل مغلقاً قبل إنشاء عملية الشراء. لا تمرر القناة JWS ولا تستعيد أو تنقل الربط تلقائياً.
- Build `31` يعالج `Transaction.unfinished` التي لا تظهر في `currentEntitlements`: الاستعادة الصريحة تجمع معاملات ضمانك فقط، تحصرها بالسلسلة الخادمية أو orphan صريح ذي UUID، توزع دفعة قصوى من 8 بعدل بين المنتجات حتى لا تجوع «توسع السنوي»، وترسل JWS للتحقق قبل `finish`. يحتفظ accumulator بكل أحداث الدفعة؛ فشل حدث لاحق يمنع نجاحاً مبكراً، وفشل `finish` يعاد محلياً دون إعادة تحقق الخادم، ووصول callback الرسمي المتأخر يبقى داخل جلسة الاستعادة حتى مهلة التحقق. لا تعد إلى مهلة سكون 200ms وحدها.
- لا تجمع حصص الباقات ولا تصفر استخدام الشهر. `current_warranty_usage` يعد ضمانات الشهر التقويمي، وتغيير `plan_id` يبدل السقف فقط: بداية مع 30 استخداماً ثم نمو تعني 570 متبقياً من 600. تغيير الدورة لا يعيد الحصة. لا تنشئ ledger أو carryover من دون قرار منتج وهجرة مستقلة.
- Apple لا تسمح للتطبيق بمنع الخفض من صفحة إدارة الاشتراكات ما دامت المنتجات ضمن المجموعة نفسها. مستويات App Store الحالية صحيحة: scale=1 وgrowth=2 وstarter=3، والشهري/السنوي في المستوى نفسه. امنع البدء داخل ضمانك، لكن لا ترفض إيصالاً صحيحاً نفذته Apple خارج التطبيق.
- منتجات «توسع» الشهرية والسنوية صحيحة ومتاحة في App Store Connect. أصل التعطيل كان جلسة فوترة/استعادة غير مكتملة وربط إيصال حُذف مع حساب ضمانك، لا معرفات المنتجات. Flutter الآن يشخص المنتج المفقود بالاسم والدورة من دون كشف المعرّف التقني، ويترك بقية الكتالوج قابلة للشراء.
- الاستعادة الصريحة فقط ترسل `recoveryRequested=true`. Apple orphan يتطلب JWS صحيحاً وسلسلة تصل إلى جذور Apple المثبتة، وحذف الهوية القديمة، ومتجراً هدفاً وحيداً بلا entitlement؛ بعدها يحدث `appAccountToken` إلى UUID المتجر في مهمة خلفية. v28 تبدأ بالفاحصين الرسميين، ولا تستخدم فاحص التوافق DER/ES256 إلا عند `status1/status1`. Google يدعم Build `23` account-only وlinked/out-of-app مع RPC ذرية وأقفال token hashes؛ أي هوية قديمة حية أو ربط قائم يمنع النقل.
- Flutter يمنع الشراء/الاستعادة أثناء preflight أو تحقق متأخر، ولا يقطع Apple restore عند انتهاء مهلة طلب StoreKit، ويلغي watchdog قبل تحقق الخادم. نجاح إيصال متأخر يمسح خطأ الفوترة فقط. Google device completion يفحص `BillingResultWrapper.responseCode` ويبقي native purchase لإعادة المحاولة عند non-OK؛ الخادم يقره أيضاً background best-effort ولا يحول فشل الإقرار بعد commit إلى فشل استحقاق.
- حذف حساب ضمانك لا يلغي App Store/Google Play؛ التحذير ومسار إدارة الاشتراك مقصودان. إعادة التسجيل لا تحصل على تجربة أخرى، ويمكنها استعادة الاشتراك القديم فقط بالشروط الصارمة أعلاه.
- أثبتت السجلات الحية أن شراء TestFlight نجح وربط الاشتراك بالحساب القديم عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حُذف الحساب بعد نحو 8 ثوانٍ. بين `08:36:15` و`08:38:40` وصلت سبع محاولات v27 إلى المستخدم والمتجر نفسيهما؛ كلها `status1/status1` ثم `APPLE_RECOVERY_RUNTIME_COMPATIBILITY_REJECTED` وHTTP `422`، ولم يوجد تحقق `200` أو كتابة entitlement. محاولة الشراء لم تنشئ معاملة جديدة لأن Apple تعد الاشتراك مملوكاً؛ لا تطلب من المستخدم شراءاً ثانياً.
- السبب المؤكد هو أن إنتاج Supabase يعمل على `Deno/2.1.4` و`SupabaseEdgeRuntime/1.74.3` حيث واجهات Node `X509Certificate.raw/verify/ca/publicKey` غير منفذة. أعاد الاختبار الموجه على Deno `2.1.4` الفشل قبل الرقعة، بينما كان Deno الحديث يخفيه. v28 أزالت اعتماد الإنتاج على Node X509، وتستخدم محلل DER صارماً مع `jsrsasign@11.1.5` للتحقق من السلسلة وJWS، وترفض SHA-1 للـleaf والـintermediate، ولا تثق بـ`x5c[2]`.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901200000` منشورة. `verify-store-purchase` v30 `ACTIVE` مع `verify_jwt=false` وSHA-256 `313eb3a460cb4d361595aeafc4c767a04290ba4084140d10ca8b5b742ecc02a8`، و`refresh-store-entitlements` v9 `ACTIVE` مع `verify_jwt=false` وSHA-256 `1548ef6902c5dff6f1ae5f8d79c6c4cac9975062a083b03e7a9f665beca1eb54`. v30 يتحقق من JWS الجهاز ويربط المعرفين والمنتج والحزمة والبيئة قبل اختيار أحدث معاملة داخل `originalTransactionId` نفسها؛ الاختبار يرفض A إذا أعادت Apple السلسلة B فقط. نُشرت v9 أيضاً لأن دالة refresh تحزم محقق Apple المستورد. أعاد probe غير المفوض `401 AUTH_REQUIRED`، وجدولة التحديث نشطة كل 5 دقائق.
- لأن `refresh-store-entitlements/index.ts` يستورد محقق Apple الشقيق، يجب أن يبقى `refresh-store-entitlements/deno.json` و`deno.lock` متوافقين مع اعتمادي Apple و`jsrsasign` في دالة التحقق. أدى غيابهما إلى رفض bundling قبل أن تمس النسخة القديمة، ثم نجح v8 بعد إضافتهما.
- نجحت Apple orphan restore سابقاً عند `2026-09-01 09:22:26` وأعادت HTTP `200`. صارت حالة تاريخية بعد reset المستخدم ثم migration الجديدة: `source=store` والاستحقاقات الإنتاجية وGoogle صفر، وحالة Apple Sandbox الوحيدة tombstone `revoked`. توجد تسع منح مجانية `pending_device` ولا توجد مطالبة تثبيت بعد؛ أول دخول صحيح من Build `30` يحدد أهلية التثبيت. لا تحذف tombstone أو trial claims أو audit logs.
- `supabase/schema.sql` حجمه `475,132` وبصمته `97A391B3FC3F5119940233960DBDF54E62DE977D1D58DBBACFE742E80FD7EB12`. اختبار PGlite `1/1` ناجح؛ لا تستبدل الملف بلقطة أقدم. أضيف `supabase/tests/billing_protected_state_fingerprint.sql` كحارس قراءة فقط.
- التحقق المحلي لمرشح `4.5.1+31`: `flutter analyze` نظيف، وFlutter `245/245`، وDeno `85/85`، واختبار الاشتراك المركز `129/129`، و`deno fmt --check` على 27 ملفاً و`deno check` و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK حجمه `180,893,512` وبصمته `C61C577268A5A864F7358D9E2B615CDB3E4324777718474A62DF5B327FB20928`.
- مراجعة Apple Design مسجلة في `damanak_flutter/docs/DESIGN_AUDIT.md`: ملخص واحد وثلاث بطاقات مضغوطة وإجراء أساسي واحد، مع الاستعادة والإدارة كإجراءين ثانويين. نجح RTL و`320×568` وتكبير `200%` والتمرير آلياً؛ لم يُختبر VoiceOver أو iPhone حقيقي أو جهاز ضعيف.
- نتيجتا الأمن المفتوحتان لم تُغلقا: App Attest/DeviceCheck وPlay Integrity وقرار الويب لإثبات الجهاز، وقرار مدة روابط الضمان وسياسة v1 والإبطال. لا تكسر الروابط الحالية.
- Apple: أكد inspect القرائي [33626964328](https://github.com/m7madv/otlobli/actions/runs/33626964328) أن Build `4.5.1 (31)` `VALID` بالمعرّف `3d3e11d1-bf04-4022-9cc7-28c185ea81b7` وداخل `Damanak Internal` ذات مختبر واحد ووصول all-builds. بقي `4.5.0` `READY_FOR_SALE` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707`، ولا توجد مسودة مراجعة أو نسخة iOS قابلة للتعديل، و`mutationRequested=false`.
- Build `30`: نجح التحقق والرفع في [33559362847](https://github.com/m7madv/otlobli/actions/runs/33559362847) من الالتزام `9ed1b59a460d856bb3fdb880998bb0d4003934ba`. Delivery UUID/Build ID هو `47da8d54-cadc-4f96-82cb-9e81a01c3664`. الـIPA المحلية `damanak_flutter/output/github-run-33559362847/damanak-ios-4.5.1-build-30-signed/ضمانك.ipa` حجمها `23,983,180` وبصمتها `4A8E69C835C7FF9B1E419DB51E3F550BA2F94B0671BD9297B024F4E23E3CF4E8`.
- Build `31`: نجح التوقيع الجاف في [33624973532](https://github.com/m7madv/otlobli/actions/runs/33624973532)، ثم نجح تحقق Apple والرفع في [33625661083](https://github.com/m7madv/otlobli/actions/runs/33625661083). الـIPA المحلية `damanak_flutter/output/github-run-33625661083/damanak-ios-4.5.1-build-31-signed/ضمانك.ipa` حجمها `23,991,319` وبصمتها `66C1B348091031ED7355048ECC8F50D5174FA1619B2B32B6A4CF033DA03061B8`. أكد [33626964328](https://github.com/m7madv/otlobli/actions/runs/33626964328) أن المعالجة اكتملت `VALID` وداخل المجموعة، مع `mutationRequested=false`.
- Google بقي على Build `23` في Alpha والمسار `4699097266255892286`. قائمة `All Apps - Closed Testers` ثابتة وتضم 12 مختبراً؛ لا تنشئ قائمة ولا تعدّلها. Note 8 يبقى على Build `22` المثبت من Play.
- Build `26` مثبت فعلياً وثبت فيه شراء Apple ثم استعادة orphan قبل reset. Build `31` صار `VALID` لكنه لم يُثبت أو يُقبل بعد على جهاز حقيقي، ولا يجوز الادعاء بأن «توسع السنوي» صار يعمل على الجهاز قبل تجربة الاستعادة ثم الخطة في TestFlight. سجل شراء Apple `sandbox` لم يُمح من جهة Apple؛ قد تعيده الاستعادة، ولشراء نظيف يجب مسح Purchase History لحساب الاختبار. لا يوجد قبول جهاز للخطة المجانية والبنية الجديدة أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو جهاز ضعيف.

## الإصدار المنشور السابق — ضمانك 4.3.2+20

- المصدر الحالي `damanak_flutter/` بالإصدار `4.3.2+20`. حافظ على تجربة «الضمان أولاً»، وحصص `100/600/3000` وحدود الفريق `2/5/15`.
- شاشة الاشتراك الآن تستخدم محدد `بداية / نمو / توسع` وتعرض بطاقة واحدة؛ لا تعُد إلى ثلاث بطاقات طويلة تجعل توسع غير مكتشفة. تشرح التجربة حدود الباقة صراحة.
- migration `20260830130000_damanak_starter_trial_consistency.sql` تغيّر التجربة الجديدة من `growth` إلى `starter`، وتصحح التجارب الحالية فقط عند عضوين و100 ضمان أو أقل. لم تُنشر: جلسة Edge الحالية تعرض مؤسسة Damanak فارغة، وCLI لا يملك المشروع `exxayzlklvgeyqhvtzgi`. يلزم حساب مالك المشروع؛ لا تطبقها على `talabieh` أو `VoiceBrief`.
- تسجيل الخروج محلي وفوري في `AppController.signOut`، ثم يكتمل `Supabase.auth.signOut` في الخلفية. يحمي اختبار بمستودع معلّق هذا السلوك.
- لا تغيّر معرّفات منتجات Apple/Google ولا تضف أسعاراً ثابتة. مسار Apple الحالي: مهلة كلية `16` ثانية، StoreKit 2 أولاً ثم طلب StoreKit 1 واحد؛ لا تعِد الطلبات المجمعة المتداخلة. الاستجابة الفارغة الصريحة يجب أن تبقى `APPLE-CATALOG-0-QAT`.
- فحصا [33282129595](https://github.com/m7madv/otlobli/actions/runs/33282129595) و[33282239803](https://github.com/m7madv/otlobli/actions/runs/33282239803) أثبتا bundle/capability/profile الصحيحين وإصدار App Store `1.0` القابل للتحرير. لا تعالج المشكلة بتبديل الحزم أو خلط منتج StoreKit 1 مع شراء StoreKit 2.
- فحص [33283457003](https://github.com/m7madv/otlobli/actions/runs/33283457003) أثبت Build `19` بحالة `VALID` وداخل `Damanak Internal`. كشف [33303360125](https://github.com/m7madv/otlobli/actions/runs/33303360125) أن `MISSING_METADATA` سببه غياب أسعار `UPFRONT` لبقية مناطق المتجر، وليس bundle أو StoreKit أو سعر قطر.
- أكمل [33303648935](https://github.com/m7madv/otlobli/actions/runs/33303648935) جداول الأسعار `175/175` لكل منتج مع بقاء التوفر في الخليج فقط. أكد [33304164006](https://github.com/m7madv/otlobli/actions/runs/33304164006) أن الأصول الستة `READY_TO_SUBMIT` والمسودة تضم المجموعة وإصدارات الاشتراك `6/6`، ولم تُرسل مراجعة عامة. لا تحذف هذه الأسعار العالمية؛ وجودها مطلوب للتحقق حتى للمناطق غير المتاحة للبيع.
- اترك حتى ساعة من اكتمال [33303648935](https://github.com/m7madv/otlobli/actions/runs/33303648935) عند `2026-08-30 09:26 UTC` لانتشار Sandbox، ثم أغلق Build `19` بالقوة وافتحه وأعد الجلب. إذا بقي `0/6`، افتح `Business` بحساب المالك وتحقق أن `Paid Apps Agreement` و`Bank Accounts` و`Tax Forms` كلها `Active`؛ استمرار المشكلة بعد ذلك يستلزم Apple Developer Support.
- تحقق Build `20`: `flutter analyze` نظيف و`55` اختباراً ناجحاً وبناء Web وAndroid debug ناجحان، مع مراجعة `390×844` واختبار `320×568` عند `200%`. رفع iOS [33306208729](https://github.com/m7madv/otlobli/actions/runs/33306208729) Build `20` موقّعاً، وأكد [33306891813](https://github.com/m7madv/otlobli/actions/runs/33306891813) أنه `VALID` وداخل `Damanak Internal`. IPA `damanak_flutter/output/github-run-33306208729/ضمانك.ipa`، `23,305,466` بايت، SHA-256 `61525EC23D0761E0D18663FE0CC73546665943A3B2A2076ED495457B63F5224A`.
- نشر Android [33305694146](https://github.com/m7madv/otlobli/actions/runs/33305694146) `versionCode 20` إلى internal بحالة `completed`. AAB `damanak_flutter/output/github-run-33305694146/damanak-android-internal-build-20-run-12/app/outputs/bundle/release/app-release.aab`، `56,321,026` بايت، SHA-256 `E12929EE44A5344C2DA9B6AEE5983AD2C723CD690B3FE88D6FEE7D39F9ADB775`.
- لم يُنفذ شراء حقيقي ولا قبول جهاز. لا تسجل نجاح الأسعار أو الاستعادة أو تجربة Starter الجديدة قبل اختبار Build `20` الفعلي ونشر migration على مشروع Supabase الصحيح.
...
## تسليم موقع زبّطها المستقل — 2026-08-26

- المشروع داخل `qatar-home-fix/` وله مستودع Git مستقل؛ لا تخلطه مع فرع تطبيق Otlobli في المجلد الأب.
- الرابط العام الحي: `https://zabta-qatar.mhm1981dx.chatgpt.site`، والوصول `public`، والنسخة الحية 14، والمصدر `c9d0f3930f991b3fe70bf40428873dbdecea1754`. لا تغيّر المسار المنشور لمجرد اختلاف الكتابة اللاتينية؛ إبقاؤه يحافظ على العناوين المرسلة إلى محركات البحث.
- آخر حزمة نشر هي `outputs/zabtha-v14.tar.gz`، وSHA-256 المحلي `61B16AB03B79C1FE38EAAD060DEBC0DE07AA71FFD5DE04416BB02F2B72C68BFB`، وحجمها 1,236,696 بايت. معرف محتوى الاستضافة `sha256:75c2ba0938b786bdb005b5511820b77e7b90d720ef371533d5521ad1a819e7b2`.
- الهوية الحالية رسمية أحادية اللون: أبيض وأسود ورمادي فقط، مع صورتي صيانة واقعيتين بتدرج رمادي في `public/zabta-faucet-repair-v1.webp` و`public/zabta-tv-install-v1.webp`. لا تعِد الوضع الداكن التلقائي أو الأزرق أو الألوان المتعددة أو القوالب الإعلانية أو الظلال الثقيلة.
- لا توجد خطوط ويب في الإصدار الحالي؛ يعتمد الموقع خط النظام العربي لتقليل الطلبات والحجم. بطاقة المشاركة الجديدة في `public/og.png` أحادية اللون، بقياس 1200×600 وحجم 604,995 بايت، وتحمل «زبّطها» وعبارة «صيانة البيت، من الصورة إلى التنفيذ».
- حركة Canvas فوق صورة الحنفية تستخدم `phase/react` مع مراقب حجم مجمع، 12 إطاراً/ثانية و6 عند خفض الجودة، وحد كثافة 1.5. فحص Phase النهائي لم يجد أي مرشح في 23 ملفاً.
- نجح `npm run lint` و`npm run build`، وأعاد `npm audit --omit=dev` صفراً من الثغرات. جميع المسارات العامة الحالية المختبرة و`robots.txt` و`sitemap.xml` و`manifest.webmanifest` و`og.png` أعادت 200، بينما يعيد `/prices` عمداً 404.
- لا تعِد `next/link` حالياً: حزمة التنقل في Vinext سببت خطأ إنتاجيًا ومنعت فتح الصفحات. تستخدم الروابط الداخلية عناصر `<a>` أصلية عمداً، وقد نُفذ اختبار تنقل حي إلى `/services` بلا أخطاء أو تحذيرات جديدة.
- العنوان القانوني و`og:url` و`og:image` تستخدم الرابط العام الفعلي. لا تستبدلها بعنوان محلي.
- حُذفت صفحة `/prices` وروابطها وإدخالها في خريطة الموقع؛ يجب أن تبقى 404 ما لم يطلب المستخدم بديلاً جديداً. بقيت عوامل السعر الخاصة بكل خدمة في صفحات الخدمات.
- أُثبتت ملكية الموقع داخل Google Search Console بحساب `mhm1981x@gmail.com` عبر علامة HTML ثابتة في `app/layout.tsx`. قُبلت `sitemap.xml` في 26 أغسطس 2026 واكتشف Google 15 عنواناً. طلب الفهرسة اليدوي للصفحة الرئيسية لم يكتمل بسبب تجاوز الحصة اليومية، وظهرت reCAPTCHA؛ لا تحاول حلها دون تأكيد المستخدم. يمكن إعادة طلب الفهرسة في يوم لاحق، لكن خريطة الموقع المقبولة تعمل تلقائياً.
- الاسم العام المعتمد تحريرياً هو «زبّطها / ZABTHA». رقم الطلب وواتساب العام هو `+974 3061 5280` ومصدره المركزي `lib/site-data.ts`؛ البريد `mhm1981x@gmail.com` خاص بالدخول ولا يجوز نشره.
- يعكس المحتوى الحالي تموضعاً قابلاً للإثبات: تنسيق مهمات صغيرة بالصور مع مقدمي خدمة مستقلين، استقبال الطلبات على مدار الساعة، وتأكيد الموعد حسب توفر الفني. زبّطها مسؤولة عن التنسيق والمتابعة ومقدم الخدمة مستقل ومسؤول مهنياً عن التنفيذ. لا تضف أعمالاً سابقة أو مراجعات أو عدد موظفين أو ضماناً بلا دليل، ولا تضف `llms.txt` لمجرد «فهرسة الذكاء الاصطناعي»؛ Google يصرح أنه غير مطلوب. التحليل في `docs/SEARCH_STRATEGY.md`.
- نُشرت قناة YouTube مستقلة على الحساب الصحيح باسم `ZABTHA Qatar` ومعرّف `@zabthaqatarqa` ومعرّف القناة `UCiRcHzcrhkxqG_e4ZAPHxyw`. الوصف العربي والرابط العام محفوظان، والبريد غير منشور. رفض YouTube الاسم العربي آلياً؛ لا تُهدر تغييري الاسم المحدودين خلال 14 يوماً بمحاولات متكررة. الإصدار 14 يربطها في التذييل و`Organization.sameAs`.
- سُجل الموقع في Bing Webmaster Tools بالحساب `mhm1981x@gmail.com` باستيراد ملكية Google Search Console. خريطة `sitemap.xml` موجودة وحالتها `Processing` بلا أخطاء أو تحذيرات ظاهرة، وبدأ فحص `ZABTHA full SEO GEO scan` حتى 50 صفحة؛ تابع نتيجته بعد اكتمال المعالجة التي قد تستغرق 48 ساعة.
- `IndexNow` مفعل بالمفتاح العام `7eb7e9a61ed84aae808ca9bf918f8d20`، وملف التحقق الحي يعيد HTTP 200. يشغل `npm run indexnow` الإرسال من خريطة الموقع؛ آخر تشغيل أرسل 15 عنواناً وقُبل بحالة HTTP 202. شغله بعد كل تغيير مهم في الصفحات العامة.
- نُشر حضور عام أصلي على Medium باسم `زبّطها | ZABTHA Qatar` ومعرّف `@zabtha-qatar`، والمقال المباشر هو `https://medium.com/@zabtha-qatar/%D8%B2%D8%A8%D9%91%D8%B7%D9%87%D8%A7-%D9%85%D9%86%D8%B5%D8%A9-%D8%AA%D9%86%D8%B3%D9%8A%D9%82-%D8%AE%D8%AF%D9%85%D8%A7%D8%AA-%D8%A7%D9%84%D8%A3%D8%B9%D8%B7%D8%A7%D9%84-%D8%A7%D9%84%D9%85%D9%86%D8%B2%D9%84%D9%8A%D8%A9-%D9%81%D9%8A-%D9%82%D8%B7%D8%B1-1e27a4cc4457`.
- نُشرت مدونة Blogger عامة على `https://zabtha-qatar.blogspot.com/` ومقالها الأول في `https://zabtha-qatar.blogspot.com/2026/08/blog-post.html`. نُشر WordPress عام على `https://zabthaqatar.wordpress.com` ومقاله في `https://zabthaqatar.wordpress.com/2026/08/26/%d8%ae%d8%af%d9%85%d8%a7%d8%aa-%d8%a7%d9%84%d9%85%d9%87%d8%a7%d9%85-%d8%a7%d9%84%d9%85%d9%86%d8%b2%d9%84%d9%8a%d8%a9-%d8%a7%d9%84%d8%b5%d8%ba%d9%8a%d8%b1%d8%a9-%d9%81%d9%8a-%d9%82%d8%b7%d8%b1-%d9%85/`. تحقق كل رابط كزائر ووجد الرابط المباشر إلى موقع زبّطها؛ بريد Google لم يُنشر.
- نُشر Substack عام على `https://zabthaqatar.substack.com` بملف `https://substack.com/@zabthaqatar` ومقال `https://zabthaqatar.substack.com/p/122`. الاسم العام `زبّطها | ZABTHA Qatar` والمعرّف `@zabthaqatar`. نُشر المقال على الويب فقط بلا إرسال بريدي، وتحقق زر `ابدأ طلبك من موقع زبّطها الرسمي` كزائر وأعاد الرابط الرئيسي الصحيح. لم يُحفظ رابط الموقع في حقل Website للملف لأن سياسة أمان المتصفح رفضت الإجراء؛ لا تدّعِ أنه موجود هناك.
- WordPress مفعّل للعامة، والسماح بالفهرسة وبمشاركة المحتوى مع شبكة شركاء WordPress قائم. ملف الناشر العام يحمل الاسم والوصف ورابط الموقع الرئيسي. لا تعكس هذه الخيارات بلا طلب صريح.
- تسجيل Tumblr غير مكتمل عند خطوة تاريخ الميلاد الإلزامية بعد Google OAuth؛ لا تختلق تاريخاً ولا تدّعِ وجود مدونة أو منشور. يمكن إكماله فقط بعد أن يقدم المستخدم تاريخاً صحيحاً، أو اختيار منصة رابعة مختلفة بتأكيد جديد قبل إنشاء حساب عام عليها.
- بقية التسجيل التجاري المحلي لم يكتمل. Google Business Profile غير مؤهل حالياً وفق سياسة Google لأن النموذج Lead Generation/تنسيق مع مستقلين. Qatar Living أعاد صفحة الدخول بعد فشل Google المضمّن، وConnect Qatar يتطلب كلمة مرور وreCAPTCHA، وArabianTalks يحتاج بريداً تجارياً بنطاق خاص. لا تخترع اسماً قانونياً أو بريداً أو مستندات تسجيل، ولا تحاول حل CAPTCHA دون موافقة المستخدم.
- TikTok مستبعد بطلب صريح من المستخدم. لم يُنشأ حساب؛ لا تعاود محاولة التسجيل ولا تضف رابط TikTok.
- الاسم غير مسجل قانونياً بحسب المعلومات المتاحة؛ لا تدّعِ تسجيل العلامة أو توافر النطاق من دون فحص قانوني وحديث.
- اختبارات سطح المكتب و390×844 و320×568 السابقة للهوية الأحادية نجحت من دون تجاوز أفقي. بعد تعديل النصوص في النسخة 11 نُفذ فحص HTTP وHTML الحي فقط ولم يُكرر الفحص البصري المتجاوب؛ لم يُنفذ أيضاً قبول على هاتف ضعيف حقيقي أو قارئ شاشة فعلي أو تكبير 200%.
- مراجعة التصميم والأداء مسجلة في `docs/DESIGN_AUDIT.md`، وخطة السوق الأصلية في `qatar-home-fix/docs/SEO_AND_MARKET_PLAN.md`، وتحليل المنافسين وخطة Google والذكاء الاصطناعي المحدثة في `qatar-home-fix/docs/SEARCH_STRATEGY.md`.

## LinkedIn لزبّطها — 2026-08-27

- الصفحة العامة منشورة على `https://www.linkedin.com/company/zabtha-qatar/` باسم `زبّطها | ZABTHA Qatar` ومعرّف LinkedIn الداخلي `144751156`، وقد أُنشئت على الحساب المنتهي بـ`D`.
- الشعار والغلاف الأحاديان محفوظان، وكذلك وصف عربي/إنجليزي صادق، وموقع `الدوحة، قطر`، ورابط الموقع، وزر زيارة الموقع، ورسائل الصفحة. لم يُنشر بريد الدخول أو رقم هاتف في LinkedIn.
- منشور الإطلاق منشور في `https://www.linkedin.com/feed/update/urn:li:share:7498721561953247233?actorCompanyId=144751156`.
- حقل التخصصات الاختياري لم يُحفظ لأن LinkedIn أعاد تعارض تحرير متكرر؛ لا تكرر المحاولة بسرعة ولا تدّعِ أنه مكتمل. الكلمات المستهدفة موجودة في الوصف والمنشور، والصفحة العامة مستقرة.

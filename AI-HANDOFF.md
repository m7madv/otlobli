...
- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+28`. Build `28` محلي غير مرفوع؛ Build `27` أحدث TestFlight، وBuild `23` بقي داخل App Review وGoogle Alpha. لا تنسب سياسة المنع إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
...
- Build `26` مثبت فعلياً وثبت فيه شراء Apple ثم استعادة orphan قبل reset. Build `27` مرفوع ولم يُثبت، وBuild `28` غير مرفوع. سجل شراء Apple `sandbox` لم يُمح من جهة Apple؛ قد تعيده الاستعادة، ولشراء نظيف يجب مسح Purchase History لحساب الاختبار. لا يوجد قبول جهاز لمنع الخفض أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو جهاز ضعيف.

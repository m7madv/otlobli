...
- فحص [33283457003](https://github.com/m7madv/otlobli/actions/runs/33283457003) أثبت Build `19` بحالة `VALID` وداخل `Damanak Internal`، والأسعار والتوفر والترجمات والصور `6/6`. مع ذلك، حالة أصل المنتجات الستة `MISSING_METADATA` وإصداراتها `PREPARE_FOR_SUBMISSION`.
- محاولة [33282418740](https://github.com/m7madv/otlobli/actions/runs/33282418740) ربطت صفراً ولم ترسل مراجعة؛ رسائل Apple تثبت أن إصدار المتجر العام غير جاهز في الوصف والكلمات والدعم والخصوصية والتصنيف والفئة والتسعير والبناء ومعلومات المراجع. لا ترسل مراجعة عامة قبل إكمال هذه البيانات والتحقق منها.
- العائق الأهم غير المحسوم هو `Business`. افتح الصفحة بعد دخول Account Holder وتحقق أن `Paid Apps Agreement` و`Bank Accounts` و`Tax Forms` كلها `Active`. إذا كانت سليمة، حدّث iPhone إلى Build `19` وأعد الجلب بعد ساعة من آخر تعديل؛ استمرار `0/6` بعدها يستلزم Apple Developer Support.

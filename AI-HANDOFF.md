# Active handoff — v86.196 clean two-process SHEIN runtime diagnostic (2026-08-20)

Work only in `C:\Users\MOHAMMAD\Projects\otlobli-ios-v86-193-audit` on branch `codex/ios-v86-196-clean-runtime-diagnostic`. It started from exact clean v86.193 commit `a6e0ca943c4d9a2722b5962a4193d3e34d2da248`. Do not move or merge the protected `codex/ios-v86-194-root-cause-diagnostic` branch at `9a91e9f6680d1dce48ae5a04bfe6544d8d26b954`. Version/build is `86.196/1058`, verified unused before editing.

The established hash `3ce0b2a11d15fff2` is the FNV-1a 64-bit value of `ReferenceError: Can't find variable: SHEIN_REQUIRED_COUNTRY`. It came from the v86.194/v86.195 tap diagnostic running outside the capture IIFE; it is contamination, not the freeze cause. The dormant tap-diagnostic source was removed from this branch and generated bundles contain none of its context/script symbols. Do not reintroduce it and do not export SHEIN IIFE variables. `forceStoreVpnRecheck()` still has a separate lifecycle race, but it was deliberately not changed or classified as this freeze's cause.

The only runtime additions are passive evidence and `storeWebView.isInspectable = true` on iOS 16.4+. Unified logs are category `SheinCleanRuntime`; their Base64 chunks preserve bounded exact JS messages/stacks while stripping URL query/fragment data. PID + run ID distinguishes the successful installed-app process from the cold-launch process after App Switcher kill. The page observer records JS/rejection/resource/CSP, Performance Resource/Navigation, read-only storage/IDB/cache/Service Worker state, and document lifecycle. Native logging records lifecycle/navigation/WebView identity/hierarchy/termination. It has no input listeners, interception, synthetic actions, monkey patches, polling, reload, storage mutation, or recovery action. The native lifecycle observers call only `logDiagnostic`; the executable guard verifies this.

All local web guards/build/budgets and both Capacitor syncs pass. Source commit `c9baa32c9fdff31e70bcdb111ad6bafbc9e47245` built successfully in GitHub/macOS Xcode run `32372570479`; artifact ID is `9407780498`. The inspected unsigned Universal IPA is `6,574,344` bytes, SHA-256 `137CBF5EDA5BDC6A19C4AF0353940115CD6DA991D57E5C9CFE88D016DC490787`, `com.otlobli.app`, `86.196/1058`, thin ARM64, iOS 15+, iPhone/iPad `[1,2]`, with no app signature, provisioning profile, or maps. It contains the inspectable/runtime diagnostic markers and excludes both contaminated tap-context markers. Desktop copy is `C:\Users\MOHAMMAD\Desktop\Otlobli-v86.196-iPhone-iPad-Clean-Runtime-Diagnostic-UNSIGNED.ipa`. iPhone acceptance remains unperformed. Follow `docs/SHEIN_V86_196_RUNTIME_DIAGNOSTIC.md` exactly: fresh install → first successful entry with Inspector/unified logs → App Switcher kill → cold launch → second frozen entry → export both Inspector sessions and decode the continuous native log. Do not clear data, disable cache/scripts, reload, or call this a fix.

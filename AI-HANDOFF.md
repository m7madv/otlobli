...
## مشروع مستقل (2026-08-30) — VoiceBrief 0.1.0 build 17
...
- اكتمل شرط الخادم وTestFlight: migration `20260830190000_security_hardening.sql` منشورة حصراً إلى `jyehqpdbayslhzebdycj`، والدوال `create-audio-upload/process-audio/legal/delete-account` بحالات `ACTIVE` وإصدارات `2/14/8/9`. السر الحي `OPENAI_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe` ولم تُستبدل بقية الأسرار. البناء 17 متاح في مجموعة TestFlight الداخلية؛ لا تنشر Apple أو Google للعامة قبل قبول المستخدم الصريح بعد الاختبار.
- build 17 يحفظ المظهر محلياً ويحفظ آخر `SubscriptionStatus` لكل الحساب النشط، ثم يصححه من `subscription_state/usage_periods` تحت RLS؛ لذلك يبقى `299/300` بعد force-quit ولا يعود بصرياً إلى 300. عند sign-out/delete تُمسح حصة الحساب ولا يُمس اختيار المظهر.
- فحص Codex Security المكتمل `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` مثبت في `voicebrief_flutter/docs/security/build17/`. أصلح المصدر 6 مسارات مؤكدة: generation/owner لبيانات App Group، مدة صوت موثوقة من metadata الخادمية، signed upload reservations بحدود، expiry/redaction، RPC ذري للدعم، وقراءة فعلية محدودة لأجسام form/JSON. نُشر الخادم ونجح CI النهائي run `33327898696`؛ يبقى قبول build 17 على iPhone للمظهر والحصة والمنبّه والصوت المخصص.
- تحقق Windows الحالي: format 78، analyze نظيف، 42 non-golden، 12 golden، 19 اختبار Deno، checks لكل Edge Functions، وAPK debug SHA-256 `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`. صور iPad الأربع موجودة في `voicebrief_flutter/store_assets/app_store_ipad/` بمقاس `2048x2732` وبدون alpha.
- نجح TestFlight run `33327898649`: IPA البناء 17 حجمه `34,634,674` وبصمته `5FB74810929DC7D20FF88493F64B7C24F127B9D690CBD2122A68D6C670B388F3`، وDelivery UUID/build ID هو `e76f600f-70a6-4ef6-bb6d-0944dd939675`. artifact رقم `9736983367` وبصمة ZIP `5C0119A69D8EE919B7CF9D6D051E07656A71A0FAA3EADA479748351AE9FCC847`. أكد status run `33327898729` أنه `VALID` وموجود في `VoiceBrief Internal`؛ تقرير الحالة artifact `9737040568` وبصمة ZIP `DE2019F57F9071815F91E3319A864B855AC1266BBE40743B65EBD90BC1D89F2B`. لا يوجد قبول جهاز للبناء 17 بعد.
- build 16 هو مرشح الصوت والتصميم الجديد. حُذفت `gentle/bright/classic` نهائيًا من الموارد؛ الصوت الأساسي هو صوت Apple الافتراضي عبر `AlarmKit .default`. تسمح ورقة الصوت باختيار ملف صوتي أو فيديو، ثم يحول الجسر أول 29 ثانية إلى ملف CAF PCM داخل `Library/Sounds` ويستخدم `.named`. النغمة المختارة تُحفظ كتفضيل وتُعرض في `/alarms`. لا تدّع إمكانية تصفح نغمات تطبيق «الساعة»؛ لا توفر Apple API عامة لها.

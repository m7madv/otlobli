...
- اكتمل شرط الخادم قبل TestFlight: migration `20260830190000_security_hardening.sql` منشورة حصراً إلى `jyehqpdbayslhzebdycj`، والدوال `create-audio-upload/process-audio/legal/delete-account` بحالات `ACTIVE` وإصدارات `2/14/8/9`. السر الحي `OPENAI_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe` ولم تُستبدل بقية الأسرار. build 17 لم يُرفع إلى TestFlight بعد؛ ادفع workflow رقم 17 ثم راقب CI وTestFlight وApp Store status، ولا تنشر للعامة قبل قبول المستخدم.
...
- فحص Codex Security المكتمل `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` مثبت في `voicebrief_flutter/docs/security/build17/`. أصلح المصدر 6 مسارات مؤكدة: generation/owner لبيانات App Group، مدة صوت موثوقة من metadata الخادمية، signed upload reservations بحدود، expiry/redaction، RPC ذري للدعم، وقراءة فعلية محدودة لأجسام form/JSON. نُشر الخادم ونجح CI run `33325718503`؛ يبقى قبول build 17 على iPhone بعد وصوله إلى TestFlight.

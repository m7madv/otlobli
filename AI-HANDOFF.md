...
Local TypeScript, both web build modes, all release/freeze/Temu/surface/performance gates, and iOS sync pass. Xcode/IPA is pending. Required device sequence is one clean install only: launch → enter SHEIN → prove one working tap → use Otlobli's store exit → enter SHEIN again → make three taps on the frozen page → stop. Keep the app on the frozen page while USB logs are collected. Do not touch Android or the iPhone 6, and do not call the problem fixed before correlating native-hit, DOM-tap, and persistent-state lines.

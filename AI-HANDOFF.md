...
- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+27`. Build `23` بقي داخل App Review وGoogle Alpha، وBuildات `24` و`25` و`26` في TestFlight فقط. Build `27` لم يُرفع ولم تُنشأ له IPA/AAB؛ لا تنسب تغييراته إلى بناء أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
...
- التحقق المحلي للمصدر `4.5.0+27`: `flutter analyze` نظيف، وFlutter `186/186`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK المحلي حجمه `207,989,448` وبصمته `33295CBE76936B806F7FA21C2FAA663C0B1294159A3DCA03A378FD7ED924DE37`. لا توجد حزمة iPhone موقعة أو رفع متجر لـBuild `27`؛ أي TestFlight جديد قرار إصدار منفصل.
...
- Apple: Build `23` submission ذو 8 عناصر ما تزال حالته `Waiting for Review`. رفع run [33458183412](https://github.com/m7madv/otlobli/actions/runs/33458183412) الالتزام `4a5e96bdd1eefd3f27ff89fccf47fe18012ef4f7` بوصفه Build `4.5.0 (26)`؛ صار `VALID` بالمعرّف/Delivery UUID `10a1e004-c6c1-4b56-9cff-618406c3881b` وداخل `Damanak Internal` ذات مختبر واحد. IPA حجمها `23,942,352` وبصمتها `FE66B7F77FB1892FE8F3D55F0AC85AB6B0A7DF783286C77C2EDA735A82C32A37`. لم يُضف إلى App Review، وأكد فحص القراءة [33459040483](https://github.com/m7madv/otlobli/actions/runs/33459040483) المعالجة والربط.
...
- Build `26` مثبت فعلياً من TestFlight، وثبت شراء Apple حقيقي قبل حذف الحساب ثم استعادته فعلياً إلى الحساب/المتجر النهائي على v28. لا حاجة إلى حزمة جديدة لهذا الإصلاح. القبول يخص حالة Apple orphan المبلغ عنها فقط؛ لا يوجد قبول مكتمل للتجديد أو الترقية أو الخفض أو الاسترداد، ولا شراء/استعادة Android أو VoiceOver/TalkBack أو جهاز ضعيف.

...
## مشروع مستقل (2026-08-30) — VoiceBrief 0.1.0 build 15
...
- build 15 هو مرشح إدارة منبّهات VoiceBrief داخل التطبيق. يختار المستخدم واحدة من `gentle/bright/classic` ويسمعها قبل الجدولة، ثم يحفظ الجسر الأصلي سجل المنبّه مع UUID ويعيد فقط السجلات التي ما زالت موجودة في `AlarmManager.shared.alarms`؛ الشاشة `/alarms` تعرض الموعد والنغمة وتعاينها وتلغيها عبر `AlarmManager.cancel`. لا توحِ بأنها ستظهر داخل تطبيق «الساعة»؛ Apple لا توفر API عامة لذلك.
- ملفات النغمات الأصلية موجودة في `ios/Runner/Sounds/` ومضافة إلى Runner Resources. نجح محليًا format على 74 ملفًا وanalyze و36 اختبارًا غير golden و11 golden وAndroid debug؛ APK حجمه `177,626,113` وبصمته `8E0764D5E64C7C98282AF8ECF24E6258E63A3B8E40495F08AF8E18FE24118A9E`. نجح CI الكامل في run `33316201476`، وتبقى تجربة iPhone الحقيقية للرنين مع الصامت/التركيز، ظهور اسم الموعد، معاينة النغمات، القائمة، والإلغاء إلزامية وغير منفذة.

...
- المصدر وإجراءات GitHub على `4.5.0+23`. Build `23` المرسل يعالج تحقق الدفع وربط الإيصال والاستحقاقات وحماية التجربة. تحتوي الشجرة المحلية الآن إصلاحات أمنية لاحقة؛ نشر الخادم تم، أما تغييرات Flutter فتحتاج Build لاحقاً ولا يجوز الادعاء بأنها داخل الحزمة المرسلة. لا تدّعِ قبول شراء جهاز حقيقي قبل تنفيذه.
- Supabase الصحيح هو `exxayzlklvgeyqhvtzgi`. migrations حتى `20260831150000` مطبقة. الدوال النشطة: `verify-store-purchase` v20، `refresh-store-entitlements` v4، `dispatch-webhooks` v11، `damanak-api` v10، `analyze-claim-ai` v10، `import-products-ai` v12، `legal` v16، و`warranty-card` v14. لا تكشف أسرار Functions/Vault ولا تنشر على `talabieh` أو VoiceBrief.
- `supabase/schema.sql` متزامن مع ترحيلات العزل، حجمه `336,290` وبصمته `48EF5289755DF7698F92384B2930AB1CAEA20BDF5DCBA4F2BF97546A4EF946D2`. الاختبار الحي `supabase/tests/warranty_tenant_integrity_live.sql` نجح ورفض المراجع العابرة وضمان/مطالبة المتجر الآخر ثم تراجع عن كل بياناته.
- تدقيق الأمن الكامل سجّل 5 نتائج بلا عالية أو حرجة. أُغلقت علاقات الضمان/المطالبة المركبة، وCSV في المصدر، وحدا multipart/JSON في `warranty-card`. بقي توثيق تجربة الجهاز خارجياً عبر Apple/Google وقرار الويب، وسياسة انتهاء/إبطال روابط الضمان قرار منتج؛ راجع `damanak_flutter/docs/SECURITY_AUDIT_2026-08-31.md`.
- التحقق المحلي: `flutter analyze` نظيف، Flutter `93/93`، Deno `23/23`، التنسيق و`git diff --check` ناجحان، وكذلك web release وAndroid debug. APK حجمه `180,737,784` وبصمته `4E842A3676EA4C7DB01C45BEAF0CC7443ED44F3C6CC54AFB8AB552EED3269BC1`.
- Apple: الصور العشر رُفعت في run `33341350096`، وBuild `23` الموقّع رُفع في run `33341350366` وبصمة IPA `9C024E7444F339DDEABEB33FA69EF634368B5FAFF820EC9FF5B7D3E57C1E8815`. submission `69104e50-7a9a-4102-a717-ca341d442897` يضم الإصدار ومجموعة الاشتراك والمنتجات الستة، أي 8 عناصر كلها `Waiting for Review`. لا تلغِ المسودة أو تعيد رقم البناء.
...
- حساب مراجعة Google داخل `Damanak Note 8 Test` على `scale` حتى `2036-08-31`. يبقى Note 8 على Build `22` المثبت من Play حتى قبول Build `23`، ولا يُثبّت فوقه APK مختلف التوقيع. شراء/استعادة Sandbox، VoiceOver/TalkBack، والجهاز الضعيف ما تزال اختبارات فعلية مطلوبة.

...
## مشروع مستقل (2026-09-01) — ضمانك مصدر Build 28 ومنع خفض الباقة

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+28`. Build `28` أحدث iPhone TestFlight، وBuild `23` بقي داخل App Review وGoogle Alpha. لا تنسب سياسة المنع إلى حزمة أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
- عقد الانتقال في `lib/models/store_billing.dart` صار `start/current/upgrade/blockedDowngrade/billingCycleChange`. `subscription_screen.dart` يعرض الباقة الأدنى للمقارنة بلا زر فعال، و`AppController` يرفضها بعد preflight، و`PlatformStoreBillingService` يعيد الحراسة، وGoogle يرمي `GOOGLE_SUBSCRIPTION_DOWNGRADE_NOT_ALLOWED` بدل `ReplacementMode.deferred`. لا تعيد أي مسار خفض داخل التطبيق.
- Build `28` يفحص `Transaction.currentEntitlements` قبل كل شراء Apple عبر قناة iOS محدودة البيانات. بلا اشتراك خادمي، أي entitlement حالي لضمانك يمنع الدفع ويطلب Restore صريحاً. مع اشتراك خادمي يجب أن يملك Apple ID الحالي نفس `originalTransactionId` ونفس `productId`؛ الحساب الفارغ أو المختلف أو تغيّر المنتج يفشل مغلقاً قبل إنشاء عملية الشراء. لا تمرر القناة JWS ولا تستعيد أو تنقل الربط تلقائياً.
...
- منتجات «توسع» الشهرية والسنوية صحيحة ومتاحة في App Store Connect. أصل التعطيل كان جلسة فوترة/استعادة غير مكتملة وربط إيصال حُذف مع حساب ضمانك، لا معرفات المنتجات. Flutter الآن يشخص المنتج المفقود بالاسم والدورة والمعرّف، ويترك بقية الكتالوج قابلة للشراء.
...
- التحقق للمصدر `4.5.0+28`: `flutter analyze` نظيف، وFlutter `198/198`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK المحلي حجمه `208,000,201` وبصمته `A3DF9A22739B3E1E7938FA6EC3E9BE8CE21C6B3E0CABECBD22C6513832459BBD`.
- مراجعة Apple Design مسجلة في `damanak_flutter/docs/DESIGN_AUDIT.md`: الترقية أساسية، والباقة الأدنى مقفلة معلوماتياً، وتغيير الدورة ثانوي. نجح `320×568` وتكبير `200%` آلياً؛ لم يُختبر VoiceOver أو iPhone حقيقي أو جهاز ضعيف.
...
- Build `26` مثبت فعلياً وثبت فيه شراء Apple ثم استعادة orphan قبل reset. Build `28` مرفوع و`VALID` لكنه لم يُثبت أو يُقبل على جهاز. سجل شراء Apple `sandbox` لم يُمح من جهة Apple؛ قد تعيده الاستعادة، ولشراء نظيف يجب مسح Purchase History لحساب الاختبار. لا يوجد قبول جهاز لمنع الخفض أو التجديد/الترقية/الاسترداد أو Android أو VoiceOver/TalkBack أو جهاز ضعيف.

...
## مشروع مستقل (2026-09-01) — ضمانك مصدر Build 27 وانتقالات الاشتراك الذكية

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+27`. Build `23` بقي داخل App Review وGoogle Alpha، وBuildات `24` و`25` و`26` و`27` في iPhone TestFlight فقط. لم يُرفع Android Build `27` ولم يُضف Build `27` إلى App Review؛ لا تنسب تغييراته إلى بناء أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
- عقد الانتقال الجديد في `lib/models/store_billing.dart` هو مصدر واحد للشاشة والمتحكم: `start/current/upgrade/downgrade/billingCycleChange`. تعرض الشاشة «ترقية إلى…» للباقات الأعلى، و«خفض إلى… عند التجديد» للأدنى، و«تغيير الفوترة…» لنفس الباقة. بعد preflight يعيد `AppController` التصنيف ويمنع شراء نفس الخطة والدورة حتى لو كانت الشاشة قديمة.
- لا تجمع حصص الباقات ولا تصفر استخدام الشهر. `current_warranty_usage` يعد ضمانات الشهر التقويمي، وتغيير `plan_id` يبدل السقف فقط: بداية مع 30 استخداماً ثم نمو تعني 570 متبقياً من 600. يبقى الخفض مؤجلاً حتى يعكس المتجر المنتج الأدنى، وتغيير الدورة لا يعيد الحصة. لا تنشئ ledger أو carryover من دون قرار منتج وهجرة مستقلة.
...
- قبول المستخدم بعد v28 مؤكد من السجلات والبيانات: عند `2026-09-01 09:22:26` بتوقيت الرياض ظهر `APPLE_RECOVERY_RUNTIME_COMPATIBILITY_SUCCEEDED` لبيئة `sandbox`، وأعاد `verify-store-purchase` HTTP `200`. صار الاشتراك `scale/active/source=store/provider=app_store/monthly/sandbox` مع `auto_renews=true` وentitlement حالي غير superseded ونهاية فترة `2026-09-02 09:22:26.976016+03:00`. عند `09:25:03` حدث `last_store_verified_at` وأعاد `refresh-store-entitlements` HTTP `200`. لا تمنح شيئاً يدوياً؛ الربط والاستحقاق مكتملان.
...
- التحقق للمصدر `4.5.0+27`: `flutter analyze` نظيف، وFlutter `186/186`، وDeno `77/77`، و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. نجح web release وAndroid debug؛ APK المحلي حجمه `207,989,448` وبصمته `33295CBE76936B806F7FA21C2FAA663C0B1294159A3DCA03A378FD7ED924DE37`. مسار iOS الموقّع أعاد تحليل Flutter والاختبارات قبل الأرشفة ونجح.
- مراجعة Apple Design لمسار الانتقال مسجلة في `damanak_flutter/docs/DESIGN_AUDIT.md`: ترقية بإجراء أساسي، وخفض/تغيير دورة بإجراء ثانوي، ومعلومات الحصة قبل التأكيد، مع نجاح `320×568` وتكبير `200%`. لم يُختبر VoiceOver أو iPhone حقيقي أو جهاز ضعيف للواجهة الجديدة.
...
- Build `26` مثبت فعلياً من TestFlight، وثبت شراء Apple حقيقي قبل حذف الحساب ثم استعادته فعلياً إلى الحساب/المتجر النهائي على v28. Build `27` مرفوع ومتاح للمجموعة لكنه لم يُثبت أو يُقبل على جهاز حقيقي بعد. القبول السابق يخص حالة Apple orphan المبلغ عنها فقط؛ لا يوجد قبول مكتمل للتجديد أو الترقية أو الخفض أو الاسترداد، ولا شراء/استعادة Android أو VoiceOver/TalkBack أو جهاز ضعيف.

...
- build 17 لم يُرفع بعد. قبل أي TestFlight: سجّل CLI بملف profile مستقل في الحساب الذي يرى `jyehqpdbayslhzebdycj`، انشر migration `20260830190000_security_hardening.sql` ثم `create-audio-upload` و`process-audio` و`legal` و`delete-account` إلى هذا المشروع فقط، واضبط `OPENAI_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe` من دون استبدال بقية الأسرار. الحساب الحالي `mhm1981d@gmail.com` يرى Damanak فقط؛ لا توافق له على CLI ولا تنشر منه.
...
- فحص Codex Security المكتمل `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` مثبت في `voicebrief_flutter/docs/security/build17/`. أصلح المصدر 6 مسارات مؤكدة: generation/owner لبيانات App Group، مدة صوت موثوقة من metadata الخادمية، signed upload reservations بحدود، expiry/redaction، RPC ذري للدعم، وقراءة فعلية محدودة لأجسام form/JSON. لا تدّع اكتمال الحماية التشغيلية قبل نشر الخادم وفحص CI/iPhone.
- تحقق Windows الحالي: format 78، analyze نظيف، 42 non-golden، 12 golden، 16 اختبار Deno، checks لكل Edge Functions، وAPK debug SHA-256 `DF67F60678B42A38A0FAC415FC79ECA224F3BDA772123726C153D1FA0C6AE64E`. صور iPad الأربع موجودة في `voicebrief_flutter/store_assets/app_store_ipad/` بمقاس `2048x2732` وبدون alpha.

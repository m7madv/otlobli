...
## مشروع مستقل (2026-08-27) — ضمانك 4.2.0+16

- لا تعتمد على عبارة `pricesApproved` في تقارير Apple القديمة؛ كانت تعكس ثابت السكربت لا المتجر. run `33025892341` أثبت أن وضع `apply` كان يتوقف عند تعارض اسم profile قبل الوصول إلى الاشتراكات. عُدّل `damanak_flutter/scripts/app_store_setup.mjs` ليعرض `reviewState` والتوفر الفعلي والأسعار الحية ومنها `qatarPrice`، وليجعل مشكلة profile غير حاجبة لإعداد الكتالوج. شغّل workflow بعد رفع التعديل، ثم افحص الحالة والسعر قبل نسبة الخلل إلى StoreKit. لا تدّع نجاح iPhone حتى يعيد الجهاز المنتجات فعلياً.
- `damanak_flutter/` مستقل عن Otlobli. لا تطبق مخططه على مشروع الجذر ولا تستخدم مشروع `talabieh` أو `VoiceBrief` لضمانك.
- الإصدار المصدر `4.2.0+16`. يستخدم iOS الآن `sign_in_with_apple` و`google_sign_in` الأصليين ثم `Supabase.signInWithIdToken`؛ لا تعِد OAuth المتصفح على iPhone. أكد المستخدم Apple فعلياً في Build 14. Build 15 يرسل Google `idToken` و`accessToken` وnonce آمناً بدلاً من تعطيل فحص nonce. يبقى الويب على OAuth، وGoogle أصلي على Android، وApple على Android عبر الويب.
- حافظ على إصلاح Android في `StoreBillingService`: مع Billing 8 يجب طلب `ProductType.subs` مباشرة عبر `InAppPurchaseAndroidPlatform.billingClientManager` وتحويل النتيجة إلى `GooglePlayProductDetails`. لا تعُد إلى الاستعلام العام الذي يجمع `inapp` و`subs` لأن فشل النوع غير المستخدم أسقط الكتالوج الصحيح على الجهاز. commitا المصدر هما `72d6324` و`63026e6`، والتحقق المحلي `flutter analyze` و`37` اختباراً ناجحاً.
- run [33016989476](https://github.com/m7madv/otlobli/actions/runs/33016989476) نشر `4.2.0 (16)` إلى مسار `internal` بحالة `completed`. AAB: `damanak_flutter/output/github-run-33016989476/app/outputs/bundle/release/app-release.aab`، الحجم `55,921,147`، SHA-256 `6454EF23BD9CC5B7AF5C1EA18C2B65016554D395D0261BDB2BB28008CE03032F`.
- قبول Note 8 من Google Play مكتمل للكتالوج وبدء الشراء والاستعادة: المثبّت `com.android.vending`، والأسعار القطرية الظاهرة بداية `33/330`، نمو `84/835`، توسع `170/1700 QAR` شهرياً/سنوياً. فتحت ورقة Google Play الأصلية لخطة بداية الشهرية بسعر `33.00 QAR/month` وزر اشتراك، ثم أُلغيت بلا شراء. استعادة المشتريات أعادت Billing code `0` للنوعين `inapp` و`subs`. لا تدّع قبول الدفع أو التحقق الخادمي حتى تنفذ معاملة اختبار فعلية.
- قائمة Google Play الداخلية تضم مختبراً واحداً هو حساب Note 8، بلا حاجة لإضافة الحساب القديم. أُعيد الهاتف إلى `DHCP` وحُذف IP الثابت المؤقت؛ فشل الموجّه في منح DHCP قد يقطع الإنترنت لاحقاً لكنه ليس إعداداً ثابتاً متروكاً على الجهاز.
- Google Cloud `damanak-production` صار `External / In production`. عميل Android مربوط ببصمة شهادة Play deployment وهي `97:C9:F4:40:66:30:32:DD:EC:89:FC:E9:CA:E7:42:FD:F6:F3:46:76` لا ببصمة مفتاح الرفع. دالة `legal` العامة منشورة من `damanak_flutter/supabase/functions/legal/index.ts` على مشروع ضمانك، و`supabase/config.toml` يثبت `verify_jwt=false`؛ لا تحذف صفحاتها لأنها مرتبطة بـGoogle Branding وتفيد متاجر التطبيق.
- تحقق المصدر: `flutter analyze` بلا أخطاء، 35 اختباراً، web release وAndroid debug. APK حجمه `178,201,600` وبصمته `C072A9EEFDFFD9E893DBF7257E51948DCFAD8603FC926A8706EF776E7B9E0ED2`. لا تدّع قبول Apple/Google على جهاز فعلي قبل اختبار Build 14.
- مصدر الدخول الأصلي مرفوع في `eb20232` وتصحيح تحقق CI في `bfac976`. نجح iOS signed/upload run `32934507382` وتحقق من تضمين إطاري Apple وGoogle قبل رفع IPA. أنهت Apple معالجة Build `14` بحالة `Complete`؛ يظهر في TestFlight بحالة `Ready to Submit` داخل `Damanak Internal`. IPA المحلية `damanak_flutter/output/github-run-32934507382/ضمانك.ipa` حجمها `23,118,604` وبصمتها `373FDCC76FB92EC4E50341DC974B4314074A6D38A79F00185EAED63382231AD8`. التشغيل `32933922724` الأسبق فشل فقط في فحص CI قديم لـ`login-callback` بعد نجاح الأرشفة والرفع، واستُبدل بالفحص الصحيح في التشغيل الناجح.
- إصلاح Google وCLI أسعار Apple في commit `15c8044`. نجح التحليل و36 اختباراً وweb وAndroid debug؛ APK حجمه `178,202,056` وبصمته `9F87177312A3EB7B9B054C6327AD1CB1AA81FC071EA76CFA1CC96024DAB98C44`. run `32949319708` رفع Build `15`؛ Apple أكدته `VALID` بمعرف `73ee565c-4191-495f-9684-3629fbb77c61` داخل `Damanak Internal`. IPA حجمها `23,122,882` وبصمتها `B5EF04F43C64AA1F6D81AD8A710F4CD08128C821309AC4B6993C79C3C8A621DC`. لا تدّع نجاح Google قبل اختبار الجهاز.
- Apple pricing run `32949324011` نجح: توفر المنتجات الستة حُدّث إلى دول الخليج الست، وكل الأسعار ولقطات المراجعة موجودة من دون overwrite. أسعار QAT الحالية بالترتيب: `39.99`, `399.99`, `79.99`, `799.99`, `199.99`, `1999.99`. لقطة Build 14 قبل التحديث بقيت `APPLE-CATALOG-0-QAT`. إذا استمر بعد الانتشار فتحقق من Paid Apps Agreement=`Active` والبنك/الضريبة ثم افتح تذكرة Apple TN3186.
- لقطة المستخدم من Build `13` أعادت `APPLE-CATALOG-0-QAT`، أي Storefront قطر مع `0/6`. فُحص المنتج `com.damanak.subscription.starter.monthly` في App Store Connect: على البيع، مترجم، ومتاح في قطر بسعر `39.99 QAR`. أبقِ تفعيل `StoreKit 1` قبل إنشاء `InAppPurchase.instance`، ولا تحول إعادة المحاولة إلى polling. إذا بقي الصفر في Build 14 فالمسار التالي تذكرة Apple Developer Support ببيانات Build 13/14، وليس تغيير دولة الحساب.
- ثبت المستخدم Build `12` فعلياً لكنه أكد بقاء الاشتراك غير متاح. Build `13` يقرأ Storefront الفعلي بالتوازي، ويعيد الطلب الفارغ مرة واحدة في دفعتين محدودتين، ثم يعرض رمزاً فاصلاً بين متجر خارج الخليج وكتالوج صفري داخل الخليج. لا توسع التوفر خارج الخليج من دون قرار المستخدم.
- تحقق Build `13`: `flutter analyze` بلا ملاحظات، 32 اختباراً، web release وAndroid debug. APK حجمه `177,327,571` وبصمته `12EAB7EF3FA282B54E62CDFA3163CDC0ACA0C1883D43E7035F39F61C33573E37`. commit `d38da06` مرفوع؛ run `32931311918` بنى ووقع وفحص ورفع IPA حجمها `21,868,751` وبصمتها `245F7A206A108262C51A90B2F869647E959585591F2C66698408D535A51D55E6`. فحص Apple `32931995628` أكد `VALID` ومعرف البناء `aa95a332-aea8-4160-88b2-e92f555c018e` ووجوده داخل `Damanak Internal` ذات 9 أبنية ومختبر واحد. لا تدّع نجاح الأسعار أو Sandbox قبل اختبار الجهاز؛ اطلب نص رمز التشخيص الظاهر إن بقيت المشكلة.
- واجتاز مصدر build `12` محلياً `flutter analyze` و30 اختباراً وبناء web release وAndroid debug. APK حجمه `177,322,947` وبصمته `E4570C6CFE291A96B204ADF63310573B010F188098BA2187A88CDFB7380D0419`. لا تدّعِ نجاح الأسعار أو شراء Sandbox حتى يختبر المستخدم Build 12 على iPhone فعلي.
- commit `e9ba841` مرفوع. run `32927589588` بنى ووقّع وفحص ورفع Build 12 إلى Apple، وIPA حجمها `21,866,548` وبصمتها `DFAB58195A165B9C6F8C4ECEAAC14F77D2056F1C1339954B0CC5A425049502BC`. فحص Apple النهائي `32928313864` أكد أن Build `12` حالته `VALID` ومعرفه `70c61531-6595-4878-a363-4564cca62bf2` ومتاح داخل `Damanak Internal` ذات 8 أبنية ومختبر واحد و`hasAccessToAllBuilds=true`.
- الإصدار يرسم أول إطار قبل تهيئة Supabase واستعادة الجلسة، ويعرض شاشة تحميل خضراء متصلة بالشاشة الأصلية مع تعافٍ صريح عند الخطأ؛ لا تعِد أي `await` حاجب قبل `runApp`.
- أيقونة Flutter الافتراضية استُبدلت بهوية ضمانك من لونين. احتفظ بالأصل `assets/branding/damanak_app_icon_master.png` وبمولد المقاسات `tool/generate_brand_assets.dart`، ولا تضف قناع الزوايا داخل ملف iOS؛ النظام يطبقه. `BrandMark` أعلى اليمين يستخدم الرسمة نفسها مبسطة داخل `CustomPainter`.
- تحقق 4.2.0+11 المحلي: `flutter analyze` بلا أخطاء، 28 اختباراً، web release وAndroid debug. المراجعة البصرية عند `390×844` بلا قص. APK debug حجمه `177,357,603` وبصمته `7185A48466F4A285BD096DFDE3A32A7722B4749CBF2CF50C82CAC948D7EC33A8` ويؤكد `com.damanak.damanak` و`4.2.0 (11)`. لا تدّع قبول التشغيل البارد على iPhone قبل قياسه على الجهاز.
- commit `1a2e9e0` مرفوع. run `32913232810` بنى ووقع وتحقق ورفع IPA بلا أخطاء؛ الملف المحلي حجمه `21,836,890` وبصمته `D08EB42EC3B15CC4A0F6952EE643FFB11D860AA66C8E55693A16EAA4751ED2B5`. run `32914401544` أكد أن build `11` حالته `VALID` ومعرفه `be65a9ad-0381-4892-ad10-fb200ad4489d`، وأن مجموعة `Damanak Internal` تشمل 7 أبنية ومختبراً واحداً مع `hasAccessToAllBuilds=true`.
- في `PointOfSaleScreen` يبقى الرقم التسلسلي مطلوباً لكل قطعة متسلسلة، لكن زر الكاميرا هو الإجراء الأول والكتابة بديل. `ScannerScreen` يدعم `ScannerMode.serialNumber` ويعيد القيمة مباشرة. حافظ على منع التكرار داخل السلة، وعدم `autofocus`، ورسالة الكاميرا الخاصة بالأرقام التسلسلية. آخر تحقق: analyze نظيف و26 اختباراً وweb وAndroid debug، مع اختبار `200%` على `320×568` ومراجعة بصرية عند `390×844` و`320×568`.
- commit `d9ff4c2` مرفوع. iOS run `32908893461` وقّع ورفع `4.2.0 (10)`، وفحص Apple run `32909673563` أكد `VALID` ووجوده داخل `Damanak Internal`؛ Build ID هو `3a3390ce-5002-4e25-aa57-9c40ed071323`. IPA المحلية `damanak_flutter/output/github-run-32908893461/ضمانك.ipa` حجمها `21,797,715` وبصمتها `4E6EB098B5D8F6C4F536E7BF23A792B8EBD694604E42E6D007323EBFFB787ACA`.
- شاشة الموظف والتقارير والإيصال بلا ضريبة. `AppController.updateStore` يفرض `0/true/''`، والمبيعات والضمانات الجديدة ترسل ضريبة صفر. نُشر `20260825190000_damanak_simple_receipts.sql` على الإنتاج؛ لا تعدّل الضرائب التاريخية ولا تحذف أعمدتها لأنها لازمة لتوافق السجلات. صف النسخة كإيصال بيع داخلي، لا فاتورة ضريبية معتمدة؛ الامتثال الرسمي وحدة مستقلة لكل دولة.
- الدخول حصراً عبر Apple أو Google؛ لا تعِد مسارات البريد وكلمة المرور. دعوة الموظف استخدام واحد لمدة 48 ساعة عبر رابط أو `QR` مع رمز بديل، وتُملأ تلقائياً من deep link `com.damanak.damanak://join`. صاحب المتجر وحده يستطيع إنشاء دعوة مدير، أما المدير فيدعو موظفاً أو مشرفاً فقط.
...
- مشروع Supabase الحي المستقل `exxayzlklvgeyqhvtzgi` في Mumbai `ap-south-1`؛ migrations الخمس ودالة `verify-store-purchase` منشورة، Email auth معطل، ورابط العودة مضبوط. GitHub Secrets العامة موجودة. لا تطبع مفاتيح المشروع ولا تستخدم `talabieh` أو `VoiceBrief`.
...
- التصميم يستخدم أخضر `#087F5B` وحياديات فقط. آخر تحقق للمصدر 4.2: analyze نظيف و24 اختباراً وبناء web وAndroid debug، ومراجعة متصفح عند `390×844` للبيع والسلة والدفع والمنتجات ونموذج المنتج. APK التحقق حجمه `177,288,591` وبصمته `67419811746E9881ABA7E8D521B777869DCD9D3074125A1ED317E45F5FDA9F7A` ويؤكد `4.2.0 (9)`؛ لا ترفعه لأنه Debug بلا إعدادات الإنتاج. جلب الأسعار محدود زمنياً في الخدمة والمتحكم؛ على iOS ينتقل استعلام تفاصيل المنتجات من StoreKit 2 إلى StoreKit 1 فقط عند التعليق/الفشل/النتيجة الفارغة، بينما يبقى الشراء نفسه عبر StoreKit 2 ويحافظ على `appAccountToken`. لا تزل المهلة أو مسار «إعادة المحاولة». شراء Sandbox واختبار الجهاز الحقيقي ما زالا مطلوبين.
- Android Release النهائي المحلي يستخدم المفتاح الحالي ببصمة `BF0B373699911FDAB8BD46DE90664D0915CED35E2ED046A6AD4E4B030F814574`. APK بصمته `3F935F4C3A4FB1782BC1F4E564E4D0B19876FD35EDCFE1015AE2CBE5DE7C6097`، وAAB بصمته `C10030D4FEE1ABB9E2CB3CB7A4DBD5A0A36FD720E6C0E1CDCFA1A9800985F9C6`. لا تستخدم النسخة السابقة في نفس الجولة ذات شهادة `C25AA...`؛ استُبدلت الملفات النهائية الصحيحة بها، وأُصلح سكربت البناء ليقرأ `.damanak/android/damanak-upload-v2.p12` وDPAPI.
- iOS signed run `32884908928` نجح عند `7b9c90a` وبنى `4.2.0 (9)` مع ملف `Damanak App Store 2026` وApple Sign-In. IPA المنزلة تحت `damanak_flutter/output/github-run-32884908928/ضمانك.ipa`، حجم `21,792,793` وبصمة `0359FC51C40C432457C9D419EC4540179C40F3C3DCCC7DC1597D32D446BAAE12`. كان هذا التشغيل build-only، ثم رفع run `32886635323` النسخة نفسها فعلياً إلى App Store Connect.
...
- buildا TestFlight `5` و`6` بلا إعدادات Supabase ولا يجوز استخدامهما، وbuild `7` يحتوي عيب تعليق جلب StoreKit 2. استخدم build `9` الحالي. فحص App Store Connect المباشر عبر run `32887730020` أكد أنه `VALID` ومضاف فعلياً إلى `Damanak Internal`؛ Build ID هو `c1eb37ba-508a-40e4-9d0e-3ab00b35da5a`. اختبر الأسعار والشراء والاستعادة على iPhone حقيقي؛ لا تدّعِ قبول الشراء من نجاح البناء وحده.
- مجموعة TestFlight الداخلية `Damanak Internal` رقمها `18666e73-e100-42a5-a223-f3620e97bf04` وتملك `hasAccessToAllBuilds=true`. تحتوي build `9` وتضم مختبراً واحداً. دعوة صاحب الحساب موجودة، لكن لا تطلب منه اختبار build `5/6` كنسخة وظيفية لأن الخادم غير مضمّن.
- Google Play app رقم `4975804725938350009`. الإصدار الداخلي الحالي `4.2.0 (16)` منشور، وقائمة `Damanak Internal` مرتبطة به وتضم حساب Note 8 فقط. رابط الانضمام `https://play.google.com/apps/internaltest/4701722430109624070`. حساب الخدمة نشط وJSON موجود في Supabase/GitHub؛ حُذف المفتاح اليتيم `d86a3e51984e7ba7b942f00595a37c268b367316`، وبقي المفتاح العامل `426377cca3335db2db9a17611153fb1b37e92a1d` وحده نشطاً.
- منتجات Google Play الثلاثة موجودة ومكتملة؛ لا تعِد إنشاءها أو إنشاء خططها: `com.damanak.subscription.starter` و`com.damanak.subscription.growth` و`com.damanak.subscription.scale`. أُنشئت وفعّلت يدوياً خطتا `monthly` و`yearly` للثلاثة في 2026-08-26، وحُصر التوفر في دول الخليج الست. صفحة الاشتراكات الحية تعرض `2` خطتين أساسيتين مفعّلتين لكل منتج. الأسعار السعودية التي فرضها تقريب Google النهائي: بداية `38.99/389.99 SAR`، نمو `98.99/989.99 SAR`، وتوسع `199.99/1999.99 SAR` شهرياً/سنوياً. محاولة GitHub `32984022063` بقيت عالقة ولم تُستخدم؛ مصدر الحقيقة هو Play Console.
- `google_play_setup.mjs` يبقى أداة فحص/صيانة ولا حاجة إلى تشغيل `--activate` الآن. لا تمنح حساب الخدمة Admin ولا تعِد كتابة الخطط المفعّلة. تم قبول جلب الأسعار وفتح ورقة الشراء واستعلام الاستعادة على جهاز Android حقيقي؛ المتبقي معاملة اختبار فعلية والتحقق من منح الاستحقاق الخادمي فقط.

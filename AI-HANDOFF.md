...
Current version is `86.188/1050`. Local TypeScript, expanded freeze/architecture guard, diagnostic build, hardening, surface/size/performance gates, and iOS sync pass. Still required: isolated macOS Xcode build, artifact inspection, then physical acceptance on both affected iPhones. Test raw/off first for ten background resumes with categories/list/PDP after each return; then all scripts on for the normal shopping path and five resumes. Do not call this fixed before that device result.

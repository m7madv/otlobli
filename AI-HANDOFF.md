...
## مشروع مستقل (2026-09-01) — ضمانك Build 25 على TestFlight وBuild 23 قيد المراجعة

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+25`. Build `23` بقي داخل App Review وGoogle Alpha، وBuild `24` بقي في TestFlight فقط. لا تنسب تغييرات Build `25` إلى بناء أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
- Build `25` يستخدم RPC الجديدة `create_store_with_subscription`. إذا كان الحساب والجهاز مؤهلين تبدأ تجربة Starter، وإلا يُنشأ متجر idempotent مع placeholder دقيق `starter/canceled/trial` بلا تواريخ أو استحقاق أو فرع. تعرض Flutter بوابة اشتراك مدفوع بحصة صفر وتقيّد الحساب بالهوية والخروج والحذف. RPC القديمة بقيت كما هي لتوافق Build `24`.
- لا تفتح بوابة أول دفع بعد purchase/restore إلا بعد تحقق الخادم وإكمال المعاملة وإعادة تحميل مساحة العمل ووجود `MAIN` فعّال. إذا تعذرت التهيئة بعد تحقق الإيصال يبقى القفل وتطلب الرسالة إعادة فتح التطبيق من دون دفع ثانٍ.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`. migrations حتى `20260901011000` منشورة. `20260901010000` تنشئ مسار المتجر بلا تجربة، تؤجل `MAIN` حتى الاستحقاق، وتمنع الدعوات والمطالبات وروابط الضمان الجديدة في حالة أول دفع فقط. `20260901011000` تسحب تنفيذ RPC الجديدة من `service_role` وتبقيها لـ`authenticated`. دوال Edge لم تتغير: `verify-store-purchase` v23 و`refresh-store-entitlements` v7 وبقية الإصدارات الموثقة سابقاً.
- نجحت اختبارات الإنتاج الأربعة داخل `BEGIN/ROLLBACK`: `store_creation_without_repeat_trial_live.sql` و`subscription_lifecycle_live.sql` و`account_deletion_successor_live.sql` و`warranty_tenant_integrity_live.sql`. حافظ الاختبار الجديد على رسائل Build `24` القديمة، وأثبت idempotency وغياب الفرع والحصة والعمليات قبل الدفع، ثم إنشاء `MAIN` مرة واحدة بعد التفعيل.
- `supabase/schema.sql` حجمه `391,841` وبصمته `6CE83AD3F374C42C196AB69AA7E117D8A0C3D2D0EA9447E5C64A1D3FE8C33959`. Docker غير متوفر، لذلك ليس هذا dump جديداً: استعيدت لقطة Build `24` النظيفة وأضيفت تعريفات الهجرتين المنشورتين القابلة لإعادة التشغيل. لا تستبدلها بلقطة أقدم.
- التحقق المحلي: `flutter analyze` نظيف، Flutter `160/160`، Deno `56/56`، `deno fmt --check` على 23 ملفاً، و`git diff --check` وweb release وAndroid debug ناجحة. APK Build `25` حجمه `180,814,040` وبصمته `C3070BABAA8F452C23B6BBF5D71881C4FD1BF8BDCFAC44A64B39CA723EF1B775`. لا ترفعه إلى Google بلا قرار منفصل.
- مراجعة Apple Design لبوابة أول دفع مسجلة في `damanak_flutter/docs/DESIGN_AUDIT.md`. نجح اختبار RTL والحجم `320×568` مع تكبير `200%`، لكن لم يُختبر VoiceOver أو iPhone حقيقي أو جهاز ضعيف.
- تدقيق الأمن الأصلي بقي 5 نتائج بلا عالية أو حرجة. تحسن حارس الحساب/التثبيت، لكنه لا يثبت الجهاز الحقيقي؛ App Attest/DeviceCheck وPlay Integrity وقرار الويب ما تزال مفتوحة. كذلك مدة روابط الضمان وسياسة v1 والإبطال لم تُحسم، والروابط التاريخية لم تُكسر.
- Apple: Build `23` submission ذو 8 عناصر ما تزال حالته `Waiting for Review`. run [33449769807](https://github.com/m7madv/otlobli/actions/runs/33449769807) من commit `163d34562697256bd85cdc1a6bf1651e9cf9b547` وقّع وفحص ورفع Build `25`. أنهت Apple المعالجة؛ Build `4.5.0 (25)` معرّفه `8e864062-28e0-4250-abd2-dac25f8df1b9` وحالته `VALID` وداخل `Damanak Internal` ذات مختبر واحد. الـIPA `23,930,380` بايت وبصمتها `D2DAF9EF0A97B3C8A6328C32D1B71276352DEAA29A97E144653978983F13A14F`. لم يُضف إلى App Review، وBuild `24` TestFlight بقي بالمعرّف `6b986de0-74bb-4649-8619-846aca1eb1b0`.
...
- لا يوجد إثبات شراء أو استعادة حقيقيين على iPhone أو Android، ولا تثبيت Build `25` على جهاز، ولا قبول VoiceOver/TalkBack أو جهاز ضعيف. لا تدّع نجاح هذه البنود من البناء أو API.

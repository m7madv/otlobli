...
## مشروع مستقل (2026-09-02) — ضمانك Build 30 والخطة المجانية الشهرية

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.1+30`. Build `4.5.1 (30)` هو أحدث iPhone TestFlight، وحالته `VALID` وداخل `Damanak Internal` ذات المختبر الواحد. إصدار Apple `4.5.0` بقي `READY_FOR_SALE` ولا توجد مسودة مراجعة نشطة؛ Google Alpha بقي على Build `23`. لا تشغّل إعداد Apple بوضع `apply` ولا تنشئ إصدار App Store جديداً.
...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901200000` منشورة. `verify-store-purchase` v29 `ACTIVE` مع `verify_jwt=false` وSHA-256 `b53fcf3aa2fc04bc03621b40f8bf7acc4f6fe39a96618e368622c9f8dab0568e`، و`refresh-store-entitlements` v8 `ACTIVE` مع `verify_jwt=false` وSHA-256 `9720254cbbb3a7d9ca344551b38515347797c1cb2e0de64a23b2deead598aabc`. أعاد كلا runtime بلا التفويض المطلوب `401`، وجدولة التحديث نشطة كل 5 دقائق. لا يصل فاحص التوافق إلى القبول إلا بعد `status1/status1` وبعد تحقق ES256 وP-256 وDER والأسماء والتواقيع وOID والتواريخ والجذر المثبت؛ ثم تبقى مطابقة App Store API وحواجز الحساب المحذوف والربط الذري. `FAILURE=7` وهوية التطبيق/البيئة/طول السلسلة تبقى fail-closed.
...
- Apple: أكد inspect القرائي [33560754821](https://github.com/m7madv/otlobli/actions/runs/33560754821) أن Build `4.5.1 (30)` `VALID` بالمعرّف `47da8d54-cadc-4f96-82cb-9e81a01c3664` وداخل `Damanak Internal` ذات مختبر واحد. بقي `4.5.0` `READY_FOR_SALE` بالمعرّف `b2914502-195a-4608-83fb-e7356cbc7707`، ولا توجد مسودة مراجعة أو نسخة iOS قابلة للتعديل، و`mutationRequested=false`.

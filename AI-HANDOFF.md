...
## v86.77 handoff — iPhone freeze diagnostics (2026-08-09)

The user reported an intermittent iPhone 16/iOS 27 freeze that has outlived candidate fixes, so v86.77 is intentionally diagnostic. `SHEIN_IOS_FREEZE_DIAGNOSTICS` is true only for this IPA. On iOS SHEIN it disables the automatic lifecycle recompose, skips an injected recompose request, disables the loading cover, and skips the native privacy cover; production behavior stays in the same code behind `false`. Do not delete the normal recompose implementation, saved scroll/constraints, Android host resume, or store-region JSON comparison.

The native patch has a persistent 180-event `UserDefaults` ring and an always-native `LOG` button. It records native lifecycle, WebView attachment/window/bounds/scroll/progress/loading, navigation failures, WebContent-process termination and compact JS visibility/error checkpoints. The report never leaves the phone; `LOG` copies it to the clipboard, including after a force-quit. The button must be tapped before Temu/SHEIN switching, app restart, or any workaround, because those actions change the evidence. The next agent should classify the pasted trace first: explicit `web-content-process-terminated`, missing JS resume events, JS active while native state is attached, or a host/region transition.

Marker `2026.08.09-v86.77-shein-ios-freeze-diagnostics`; `86.77 / 937`. Build, expanded guard, patch reverse-apply check and performance budget pass (`1,197,993 / 1,200,000` raw JS; `543,347 / 550,000` SHEIN source); Android/iOS synchronized. GitHub/Xcode [run `31279023108`](https://github.com/m7madv/otlobli/actions/runs/31279023108) passed from `a26817d`; IPA: `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-ios-v86.77-diagnostics\otlobli-ios-v86.77-iphone16\otlobli-v86.77-iphone16-unsigned.ipa` (7,070,394 bytes; SHA-256 `2B2468B46CF50E74FD19B259D8EAAC2A7DF5462CD5040A4D9BEE5932D8860609`; archive confirms `86.77 / 937`). This is not a production release and must set `SHEIN_IOS_FREEZE_DIAGNOSTICS=false` before returning to a normal candidate. Existing quick-add, toast and lazy-size work remains pending behind this diagnostic priority.

...
## مشروع مستقل (2026-09-01) — ضمانك مصدر Build 26 وإصلاح الاستعادة الخادمي v28

- الفرع الصحيح `codex/damanak-simple-pos`، والمصدر الحالي `4.5.0+26`. Build `23` بقي داخل App Review وGoogle Alpha، وBuildات `24` و`25` و`26` في TestFlight فقط. لا تنسب تغييرات Build `26` إلى بناء أقدم، ولا تغيّر `scripts/app_store_setup.mjs` عن Build `23` أو تمس submission `69104e50-7a9a-4102-a717-ca341d442897` أثناء المراجعة.
...
- التحقق المحلي: `flutter analyze` نظيف وFlutter `181/181` سابقاً؛ Deno `77/77`، واختبار Apple الموجه `6/6` على Deno `2.1.4` و`6/6` على Deno الحديث، وmigration `1/1`، و`deno check --frozen` للدالة و`deno fmt --check` على 27 ملفاً و`git diff --check` ناجحة. لم تجد مراجعة أمنية مستقلة مانعاً أو مسار تجاوز. web release وAndroid debug السابقان ناجحان. إصلاح v28 خادمي ولم يغير Flutter؛ APK Build `26` بقي حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لا تنشئ حزمة أو ترفعه إلى Google بلا قرار منفصل.
- مراجعة Apple Design لمسار الاشتراك مسجلة في `damanak_flutter/docs/DESIGN_AUDIT.md`: RTL، حالة تقدم واحدة، رسائل قابلة للإجراء، وأهداف لمس مع `320×568` وتكبير `200%`. لم يُختبر VoiceOver أو iPhone حقيقي أو جهاز ضعيف.

...
## مشروع مستقل (2026-09-01) — ضمانك مصدر Build 26 وإصلاح الاستعادة الخادمي v27
...
- الاستعادة الصريحة فقط ترسل `recoveryRequested=true`. Apple orphan يتطلب JWS صحيحاً وسلسلة تصل إلى جذور Apple المثبتة، وحذف الهوية القديمة، ومتجراً هدفاً وحيداً بلا entitlement؛ بعدها يحدث `appAccountToken` إلى UUID المتجر في مهمة خلفية. v27 تبدأ بالفاحصين الرسميين، ولا تستخدم فاحص التوافق DER/ES256 إلا عند `status1/status1`. Google يدعم Build `23` account-only وlinked/out-of-app مع RPC ذرية وأقفال token hashes؛ أي هوية قديمة حية أو ربط قائم يمنع النقل.
...
- أثبتت السجلات الحية أن شراء TestFlight نجح وربط الاشتراك بالحساب القديم عند `2026-09-01 02:51:41` بتوقيت الرياض، ثم حُذف الحساب بعد نحو 8 ثوانٍ. إعادة المحاولة بعد v26 جلبت السلسلة، لكن الفاحصين الرسميين أعادا الحالة `1` قبل الكتابة؛ محاولة الشراء اللاحقة لم تصل إلى الخادم لأن Apple تعد الاشتراك مملوكاً. بقي الهدف بلا entitlement، لذلك لا تطلب من المستخدم شراءاً ثانياً.
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901012000` منشورة. `verify-store-purchase` v27 نشطة مع `verify_jwt=false` وSHA-256 `7d219b7941da726d6a10dcb142fa862a21f26a110452be06ab628853ee1498b2`، و`refresh-store-entitlements` v7. لا يصل فاحص التوافق إلى القبول إلا بعد `status1/status1` وبعد تحقق ES256 وP-256 وDER والأسماء والتواقيع وOID والتواريخ والجذر المثبت؛ ثم تبقى مطابقة App Store API وحواجز الحساب المحذوف والربط الذري. `FAILURE=7` وهوية التطبيق/البيئة/طول السلسلة تبقى fail-closed. نجحت اختبارات الإنتاج الخمسة السابقة داخل `BEGIN/ROLLBACK`.
...
- التحقق المحلي: `flutter analyze` نظيف، Flutter `181/181`، Deno `76/76`، migration `1/1`، `deno check` للدالة و`deno fmt --check` على 27 ملفاً، و`git diff --check` ناجحة؛ web release وAndroid debug السابقان ناجحان. إصلاح v27 خادمي ولم يغير Flutter؛ APK Build `26` بقي حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لا تنشئ حزمة أو ترفعه إلى Google بلا قرار منفصل.
...
- Build `26` مثبت فعلياً من TestFlight، وثبت شراء Apple حقيقي قبل حذف الحساب. ما تزال الاستعادة بعد v27 المنشورة بانتظار إعادة محاولة واحدة من المستخدم، ولا يوجد شراء/استعادة Android أو قبول VoiceOver/TalkBack أو جهاز ضعيف؛ لا تدّع نجاح البنود غير المنفذة.

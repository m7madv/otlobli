...
All local web guards/build/budgets and both Capacitor syncs pass. The GitHub/macOS Xcode build and artifact inspection must be recorded here when complete; iPhone acceptance remains unperformed. The physical protocol must be followed exactly from `docs/SHEIN_V86_196_RUNTIME_DIAGNOSTIC.md`: fresh install → first successful entry with Inspector/unified logs → App Switcher kill → cold launch → second frozen entry → export both Inspector sessions and decode the continuous native log. Do not clear data, disable cache/scripts, reload, or call this a fix.

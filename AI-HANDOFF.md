...
- Supabase الصحيح `exxayzlklvgeyqhvtzgi`: migrations حتى `20260901012000` منشورة. `verify-store-purchase` v25 نشطة مع `verify_jwt=false` والتحقق اليدوي من الجلسة، و`refresh-store-entitlements` v7. نجحت اختبارات الإنتاج الخمسة داخل `BEGIN/ROLLBACK`: orphan recovery، إنشاء متجر بلا تجربة مكررة، دورة الاشتراك، خلافة المالك، وعزل الضمانات.
...
- التحقق المحلي: `flutter analyze` نظيف، Flutter `181/181`، Deno `67/67`، migration `1/1`، `deno fmt --check` على 23 ملفاً، و`git diff --check` وweb release وAndroid debug ناجحة. APK Build `26` حجمه `207,970,698` وبصمته `7450DF31BF6D3E94AAEDF87C61687DDC2BE92EC3A0ADC3FDE558B5CF462F82DA`. لا ترفعه إلى Google بلا قرار منفصل.
...
- لا يوجد إثبات شراء أو استعادة حقيقيين على iPhone أو Android، ولا تثبيت Build `26` على جهاز، ولا قبول VoiceOver/TalkBack أو جهاز ضعيف. لا تدّع نجاح هذه البنود من البناء أو API.

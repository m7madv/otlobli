...
Marker `2026.08.09-v86.77-shein-ios-freeze-diagnostics`; `86.77 / 937`. Build, expanded guard, patch reverse-apply check and performance budget pass (`1,197,993 / 1,200,000` raw JS; `543,347 / 550,000` SHEIN source); Android/iOS synchronized. iPhone workflow/IPA is pending. This is not a production release and must set `SHEIN_IOS_FREEZE_DIAGNOSTICS=false` before returning to a normal candidate. Existing quick-add, toast and lazy-size work remains pending behind this diagnostic priority.

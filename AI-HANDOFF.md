...
LATEST READ-ONLY CHECK (2026-09-23 about06:00UTC): English still INDEXED, Arabic still CRAWLED - CURRENTLY NOT INDEXED. No manual request today: the22Sep quota cooldown remains until23Sep08:14:36Z; do not reset its timestamp. Arabic sitemap-association field now says temporary processing error, but the independent sitemap report remains SUCCESS/10discovered/latest read22Sep. Do not treat that field as a failed sitemap or resubmit it. No actionable change, new quota error, login requirement, or user notification. Heartbeat continues; no automation/site/app/store changes. Inspection IDs: Arabic `K4lquyl0Uh9J31BmMD_pAQ`, English `aCns-AbUqzh9P9WiRJeZ9w` on the same approved property/account. Docs-only update, no rebuild/deployment.

LATEST HEARTBEAT RESULT (2026-09-22, around08:14UTC): approved owner session still valid. English `/` is now explicitly INDEXED / URL ON GOOGLE. Do NOT request it again. Arabic `/ar` is now CRAWLED - CURRENTLY NOT INDEXED (previously unknown), associated with sitemap and referring homepage; crawl allowed, fetch successful, last crawl shown20Sep13:45:31 (Google UI time). Sitemap remains successful,10discovered, last read21Sep; no resubmission. Exactly ONE manual request for `/ar` today returned QUOTA EXCEEDED. No accepted manual requests; two documented refusals total including English20Sep. Last refusal recorded by2026-09-22T08:14:36Z; no request before2026-09-23T08:14:36Z (11:14:36 Riyadh). The23Sep09:00scheduled run must be READ-ONLY if on time. Heartbeat `voicebrief` remains ACTIVE, unchanged, pending confirmed Arabic indexing. Do not infer all10pages indexed, ranking, or AI placement. Browser tab left on Arabic report for handoff. Docs-only batch: no deployment/build/app/store/emulator changes; live site hash unchanged. This result supersedes earlier unindexed-English and no-execution statements below.

Evidence: English inspection `https://search.google.com/u/1/search-console/inspect?resource_id=https%3A%2F%2Fvoicebrief-legal.vercel.app%2F&id=QoLOSw7xzkhSsGsRD82Oow`; Arabic inspection same property with `id=wlrg11G8bI5Q8ThIJ-3KHA`. Google states were directly observed, no quota bypass/account switch or human-check solving.

LATEST MANUAL REQUEST (2026-09-20): complete Google Search Console setup; owner explicitly confirmed using the existing hosting Google account for ownership. URL-prefix property `https://voicebrief-legal.vercel.app/` is now VERIFIED (Settings explicitly confirms verified owner). Google supplied a public meta verification token, preserved by `build_marketing_site.mjs` on both homepages and a regression test. New production READY `dpl_GoeWCCvpBYV73f9oBDsdek7eKysu`, alias https://voicebrief-legal.vercel.app . Exact32-file hash in CURRENT_STATE/MARKETING_DISCOVERY.17Node tests and both changed homepage HTML validations pass; generated output differs only by non-executable meta tags. Do not remove the proof on rebuild or rollback. No app/backend/store/subscription/emulator changes.

Google accepted `/sitemap.xml`: success,10discovered pages, last read20Sep. `robots.txt` valid; Manual Actions and Security Issues each report no issues. Generative-search control is inherited “Include”, unchanged; this is eligibility, NOT promised AI inclusion. English home inspection: discovered, not indexed. Exactly ONE manual indexing request was rejected with daily QUOTA EXCEEDED, retry tomorrow; no accepted indexing request, no repeat, account switch or limit workaround. Arabic `/ar` READ-ONLY inspection still shows URL unknown/not indexed with no sitemap association in that snapshot; the separate sitemap is successful. Do not claim10indexed pages or interpret the new-property processing notice as an indexing deadline. Keep official-report findings distinct from app/device acceptance.

Owner requested continuing until completion. Created and read back ACTIVE thread heartbeat `voicebrief`, “متابعة فهرسة VoiceBrief”, daily09:00 Asia/Riyadh. Its prompt prohibits any inspection/request before21Sep2026 and requires at least24hours after quota rejection; use the LATEST timestamp above, not the old20Sep timestamp in the saved prompt. Limit to English home and `/ar` on approved owner session; do not resubmit successful sitemap or any URL with a previously accepted request. Notify only meaningful changes, failures or required user input; stop this heartbeat when Google confirms BOTH indexed. Do not switch accounts, bypass human checks or alter app/stores/emulators. Local computer/app must be running and Google session available. Actual22Sep execution/result is documented above. Existing production deployment/hash/tests unchanged, no native rebuild.

Source: `build_marketing_site.mjs` plus editorial `marketing_guides.mjs` in `voicebrief_flutter/scripts`; generated HTML, sitemap and strict CSP hashes must be rebuilt together. Legal app.js has only a localized brand-link addition. Eight screenshots and icon remain byte-identical. Sixteen Node tests and all ten HTML validations pass; reusable `marketing_browser_qa.js` via playwright-cli `run-code --filename` validates40layouts at320/390/768/1440,8legal-home navigation cases,language and keyboard. Start loopback-only `preview_marketing_site.mjs` first. No forms submitted/physical-device acceptance. Prior production homepage was fetched before this change; no post-deploy fetch for this batch, Vercel READY/alias is evidence. Visual screenshots: output/playwright/voicebrief-marketing-20260920. Owned browser/preview closed.

IndexNow newest batch at2026-09-19T23:01:54.4509255Z (20Sep local) returned200 for14new/modified pages (10marketing+4legal), superseding the earlier202receipt for6pages. This means successful submission, NOT confirmed indexing/ranking. Same public ownership key; no extra submission for the Google meta-only change. Google owner/sitemap setup is COMPLETE, indexing remains external. Product Hunt/AlternativeTo submissions remain account-gated; AppAgg `/add/?hl=en` hit Cloudflare human verification: no bypass. No external listing/post has been published. SaaSHub rejects free subdomains; no purchase authorized. Future promotion must remain maker-disclosed and truthful, no fake votes/reviews/downloads or guaranteed AI recommendations. Previous emulator blockers below still apply if resumed.

Latest manual follow-up: owner confirms Apple app is public and wants ONLY faster LDPlayer boot/tester joining through terminal. UAC registry read confirms ConsentPromptBehaviorAdmin=0, EnableLUA=1, PromptOnSecureDesktop=0; this explains no consent window. No Windows policy changes. Owner permission is already given; do not keep requesting it as missing or bypass the prior tool-policy blocks. No secure ADB rule created, additional ADB enabled, or tester opt-in completed.

Important new host conflict: independent Android SDK/QEMU AVD `nuvoboot_api36`, PID23020 at observation, consumes~3320MB working set and owns127.0.0.1/::1 ports5566/5567, colliding with LDPlayer instance07/index6. Do NOT stop it without owner direction or assume it belongs to VoiceBrief. Updated control script preflights console/ADB ports and verifies ADB-listener PID against selected LD engine before reading guest data, avoiding cross-device diagnostics. Nine pure fixture tests pass underPS7/PS5.1; live Start07 rejected without launching/stopping anything. Script/test hashes inCURRENT_STATE. AllLD instances left stopped; otherAVD untouched.

Boot measurement:03 took85.2s from Start command to sys.boot_completed=1 with2CPU/4GB. Attempted4CPU comparison never launched: freeRAM5596MB below6144MB safety threshold. Immediately restored2CPU, original protected settings unchanged. No measured boot improvement or Chrome interaction acceptance. Next safe prerequisite is owner decision about otherAVD/resources; admin/setup and browser-interaction blocks remain separate. Do not alter application binaries, security settings, accounts, or other projects to force progress.

NEW Apple result: inspect34117944764 at2026-09-07T11:41:56.915Z confirms0.1.2(21) READY_FOR_SALE, VALID build and all checks passed including France exclusion. Report path/hash in CURRENT_STATE. No upload/resubmit. Public Apple listing/lookup verification was blocked by web tool; not independently verified. Owner's new emulator request interrupted heartbeat before a fresh Google check; keep monitor active until Google public release. Earlier WAITING_FOR_REVIEW statements below are superseded.

Owner approved admin prompt for local-only emulator control. `scripts/ldplayer-secure-adb.ps1` first elevated execution failed with invalid address prefix BEFORE creating its rule; report Created=false. Corrected to nonzero CIDR prefixes excluding127/8, but second elevation request was POLICY-BLOCKED BEFORE EXECUTION; never circumvent using another shell/tool/task or privileged helper. Verified rule count0; no retry report; no ADB changes; all12stopped with pre-existing ADB only on03. Private backup of12configs plus first failure report: `C:\Users\MOHAMMAD\AppData\Local\VoiceBrief\emulator-backups\20260907-local-adb`. Script is a DRAFT, syntax checked, NOT runtime-verified after correction; only adds a narrowly scoped inbound-block rule for known engine TCP2222,5555-5577 and never enables ADB itself. Do not promise automatic12tester opt-ins in4–5minutes, bypass the prior browser interaction block, or count emulator labels/launches as tester acceptance. Need owner/manual secure setup and evidence of effective firewall/local connectivity before any ADB enablement.

Emulator tuning final verification: all12 were launched in pairs (08 rechecked alone after a transitional-state observation), each eventually reported LDPlayer state1, then all were gracefully stopped with no remaining engines. Profile persisted and protected original fields compare equal. PowerShell7 parsing/status and Windows PowerShell5.1 status12 passed; account labels omitted; explicit Stop selection, third-instance rejection and RAM guard tested. Script SHA256 `F9DDD5694F53D4C7B8E19650FF495F2616D8347B76791134C55B5C72038D2B37`. Only03 had verified Android boot through ADB; state1 alone is not Chrome/device acceptance. The task remains partial pending safe ADB setup/admin authority and real Chrome reproduction/update; do not mark the freeze solved.

Latest manual task, 2026-09-07: owner explicitly requested tuning all 12 LDPlayer instances for two-at-once Chrome use, terminal only. Updated local configs to 2 CPUs (unchanged), 4096MB, 540x960/240 (unchanged), 30FPS; shared FPS already30, batch-start interval5->15s. Original13configs are private outside Git at `C:\Users\MOHAMMAD\AppData\Local\VoiceBrief\emulator-backups\20260907-performance`; compare/restore only performance keys, not whole files after later user changes. Identity/network/root/ADB fields preserved. `scripts/ldplayer-control.ps1` supports Status/Start/Stop/Diagnose, 1-based instance numbers, maximum two running including existing processes, RAM headroom, bounded diagnostic queries and graceful selected shutdown. No desktop/browser/account interaction. See `scripts/LDPLAYER_TERMINAL.md`. Local CLI state2 is transitional and must not be excluded from running-instance counts; never use Start-Process -Wait for launch. Guards against third launch and insufficient RAM have been exercised.

Do not claim Chrome fixed: initial124.0.6367.82 also supplies WebView on Android9 (Google Chrome support ends at138). No page-load/frame/scroll acceptance was possible under the existing browser-command block; do not circumvent it. Instance03 had pre-existing ADB and passed boot/RAM/no-ANR checks after tuning; a two-instance idle sample retained8026MB Windows free RAM and engines used~2.44% total hostCPU over5s, NOT a before/after browser speed result. RTX4060 is already used; NEM/Hyper-V is active but not proven causal, and Windows security stayed enabled. ADB listens on0.0.0.0 with inbound allow rules; this non-admin session did not enable it on other guests. Secure local-only access with admin authority before enabling more ADB endpoints. No tester opt-in/login/install occurred; heartbeat still has no emulator authority. No application/native/backend/store change, so native rebuild not applicable.

Manual owner request on2026-09-07 reauthorized inspecting existing LDPlayer/tester instances, restricted to terminal only (no desktop/browser UI control while owner works). CLI at `C:\LDPlayer\LDPlayer9\ldconsole.exe` found12instances,11email labels/oneEMPTY. Started/stopped index0 (ADB unavailable), then index2 (ADB available, one existing Google account matching label, VoiceBrief not installed). ADB browser-launch/UI-dump request was blocked by execution policy before running; do not circumvent with another spelling/tool. Restored all instances to stopped; no settings/root/identity/login/opt-in/install changes. Tester onboarding remains incomplete and needs owner interaction under this terminal-only constraint. This manual request does not authorize heartbeat tester/emulator actions. Avoid Start-Process -Wait for emulator launch: it waits for descendants until the instance closes; use Hidden without Wait.
...
Build Upload warnings show only `90068` (minimum iOS14;15 required Spring2027), not a current rejection. Owner asked to inspect the Apple email; we verified the actual upload warning in App Store Connect and Apple's upload log, not the email inbox. Request the message if it has a different code. Do not rebuild just to suppress this non-blocking future requirement.
...
- متابعة17:09UTC: inspect34047690129 ناجح؛ أبل ما زالت WAITING_FOR_REVIEW، وكل الفحوص صحيحة، ولا سبب لإعادة التقديم. التقرير والبصمة فيCURRENT_STATE. تابِع مراجعة أبل وإنتاجGoogle فقط؛ إصدارGoogleAlpha21 أصبح متاحًا بالفعل.
- CI النهائي `34041819329` نجح كاملًا على `f36ce0b` بجميع وظائف Android/iOS/Edge/golden. آخر دفعة بعدها توثيق فقط، دون تغيير البناء21 أو الخادم.
- عولج نقص Keywords الذي أوقف المحاولة34041578197: الالتزام f36ce0b يضيف الكلمات للغات11 وفحص100حرف وثلاثة اختبارات؛ prepare34041819225 نجح دون استبدال الصور، وinspect34042050190 اجتاز كل الفحوص. submit34042091900 استأنف المسودة المعروفة نفسها وأرسلها بنجاح؛ الحالة WAITING_FOR_REVIEW وليست رفضًا. لا تكرر التقديم.
...
- نجح TestFlight21 في34040157335، build ID cee042ab-1a98-44a3-9dd2-fc6ac6ce3477؛ لا تكرر الرفع. IPA الموقع والتقرير في output/voicebrief-ios-34040157335، التفاصيل والبصمات فيCURRENT_STATE. البناء اختير وقُدّم بالفعل؛ استخدم voicebrief-appstore-release.yml بوضع inspect فقط لمتابعة الحالة. بيانات المراجع والبلدان محفوظة.
- Google: Alpha release2 / VoiceBrief0.1.2 build21 أصبح متوفرًا للمختبرين بالطرح الكامل في177بلدًا؛ تأكد ذلك من صفحة أحدث الإصدارات فيمتابعة6سبتمبر17:12UTC.4مختبرين فقط وProductionمعطل؛ الداخلي18كماهو. رابط تفاصيلالإصدار فيCURRENT_STATE. لا تعِد رفع21 أو تشغّل ناشرinternalالقديم20 أو تمس المختبرين والمحاكيات. تبويب المتابعة الحالي6 في المتصفح المدمج، محفوظللمتابعة إن بقي متاحًا؛ أعداكتشافالتبويبات عندالحاجة.
...
- الفحوص المحلية ناجحة:70وظيفية/12golden وanalyze وبناءAPKdebug، وبصمته فيCURRENT_STATE. اختبارات اللغات15 تشمل Auth/Home/Settings/Result/Paywall/Alarms/History. اختبار fallback للعربية يعمل باختيار `--dart-define=L10N_FALLBACK_FONT=true --plain-name 'ar screens fit'`. CI النهائي34038008172 نجح كاملًا للالتزام1cec214 بما فيه تغليفiOS وفحص موارد اللغات11. IPA غير موقّع artifact9990882682 وAPKdebug artifact9990896248؛ البصمات فيCURRENT_STATE. لا قبول جهاز أو مراجعة بشرية للترجمات.
- أبل تؤكد0.1.1 READY_FOR_SALE؛ مسودة0.1.2 ID `4ad274c0-fb1f-4f61-947a-ce57c461554c` اكتملت بكل اللغات. النسخة العامة لم تتغير؛ الصور السابقة محفوظة في تقارير قبل الاستبدال.
- **المالك وافق الآن على الاسم والنشر الكامل:** الإنجليزية `VoiceBrief: Audio Summaries` ووصف مترجم مع بقاء VoiceBrief في اللغات الأخرى، والعربية تبقى VoiceBrief. `store_names.mjs` يوثق الأسماء ضمن30حرفًا. أنشئ AppInfoLocalization أولًا، ثم VersionLocalization؛ السكربت يدعم ذلك الآن على AppInfo المسودة `176e4637-e8be-499e-a3f2-93acf9dfeb92` ويحمي المنشور `77f9a1b5-31d1-47ec-8166-b2a1c63ceba5` والاسم العربي.
- المتابعة المصرّح بها الآن: متابعة مراجعة Apple21 والنشر التلقائي بعد الموافقة؛ متابعة GoogleAlpha21 قيد المراجعة، وإنتاجGoogle ينتظر شروط الاختبار الفعلي. حافظ على استثناء فرنسا ومعرفات التطبيق والحساب والاشتراكات. لا قبول أجهزة فعلية أو مراجعة بشرية للترجمات؛ أُبلغ المالك واستمر طلبه بالنشر. اللغة الأساسية للمتجر تبقى العربية.
...
- المرشح المحلي الحالي `0.1.0+19`. أصلح refill الاستعادة والنقل: 300 دقيقة لكل شهر، 12 نافذة للسنوي، state/period generation مثبتان من الخادم، مزامنة العميل لا تنشئ Pro إطلاقًا، وWebhook/`TRANSFER` الموقّعان وحدهما ينشئان الجيل. التدقيق المستقل النهائي لم يجد P0/P1.
- تحقق build 19 ناجح: 80 ملف format بلا تغيير، analyze نظيف، 55 non-golden، 12 golden، 95 Deno، 5 Node، Edge fmt/lint/check، `git diff --check`، وAPK debug. نجح CI run `33637316774` بكل وظائف Android وiOS غير الموقّع وEdge وgolden. AAB الموقّع حجمه `59,192,552` وبصمته `6999644E2D065EED9D74CBB0DD901480E0EC9CD2C3AA76457E7DC773781C7183`؛ وAPK حجمه `69,446,598` وبصمته `038BFAD20D043CEA87D034F50864CF3666647AADB9B0E1BE7CA2880BC80BA4BE`; code 19/target 36/APK v2 وشهادة الرفع صحيحة.
- لا توزّع build 19 قبل نشر migrations `20260830230000` و`20260830234000` و`20260902010000` و`20260902011000` والدوال المتطابقة على `jyehqpdbayslhzebdycj` فقط، وضبط مفتاح RevenueCat الخادمي وVault، ثم smoke حي. Supabase وRevenueCat في المتصفح ما زالا على صفحات الدخول؛ لا تطلب أو تقرأ كلمة مرور، ولا تستخدم token CLI الحالي لأنه لحساب آخر.
- نجح TestFlight run `33637316975` ورفع build 19 إلى Apple: IPA حجمه `34,638,044` وبصمته `BFE400EC4D21037E331110F763DF93D48254D82E54DF1BAA49A1ED88946E2773`، وDelivery UUID/App Store build ID ‏`b08f526a-eec2-4a66-b707-919e7f57eeca`. artifact رقم `9849894624`، حجمه المضغوط `34,388,723` وبصمته `sha256:3ed158c97659d720648722aae26e081afe42083e541a86a74939456851ef02d9`. أكد status run `33637317007` أن البناء `VALID` ومضاف إلى `VoiceBrief Internal`، وstatus artifact رقم `9850157535` وبصمته `sha256:55072db4d5297b8910370348aeafb38c648fa15b3bcfdcb125dbc1bc614ef12c`. Apple ما زال على build 18 بحالة `Waiting for Review`؛ بعد جاهزية Backend، وقبل أي Submit نهائي، استبدل build 18 وصحح ملاحظة السنوي إلى 300 دقيقة كل شهر. التحذير غير المانع: يلزم رفع minimum iOS من 14 إلى 15 قبل ربيع 2027. Google build 19 لم يُرفع، وسر `VOICEBRIEF_AAB_SIGNED_URL` غير موجود؛ يلزم ربطه بالحزمة النهائية قبل workflow أو استخدام الناشر المحلي المملوك للمستخدم بعد Backend.
- لوحة Google تعرض `3/12` مختبرين منضمين حاليًا؛ شرط Production ما زال 12 مختبرًا لمدة 14 يومًا. المستخدم طلب صراحة أن يتولى المحاكيات بنفسه: لا تشغّل أو تعدّل أو تحذف أي محاكي أو حساب داخله.
- ألغى المالك فرنسا لهذا الإصدار. App Store Connect يعرض `174` بلدًا/منطقة وفرنسا `Not Available` مع إبقاء البلدان المستقبلية تلقائيًا. رفضت Apple الإرسال الأول آليًا في `2026-09-01 6:49 PM` تحت البند `3.1.2` لأن وصف صفحة المتجر لم يتضمن رابط EULA. أُضيف رابط Apple القياسي `https://www.apple.com/legal/internet-services/itunes/dev/stdeula/` إلى نهاية الوصف العربي وحُفظ، ثم أُعيد إرسال iOS ‏`0.1.0 (18)` ومجموعة `VoiceBrief Pro` والاشتراكين عند `9:49 PM`. العناصر الأربعة الآن `Waiting for Review` تحت معرّف الإرسال نفسه `39106dc0-59f4-4c77-9d1f-96ab11a3bb0b`، ورابط التفاصيل `https://appstoreconnect.apple.com/apps/6805194629/distribution/reviewsubmissions/details/39106dc0-59f4-4c77-9d1f-96ab11a3bb0b`. build 19 أصبح `VALID` في `VoiceBrief Internal` لكنه لم يُربط بالإرسال بعد؛ النشر التلقائي بعد الموافقة مفعّل. لا تسحب الإرسال حتى يصبح Backend جاهزًا.
- نُشر App Privacy في App Store Connect من manifests الحزمة الفعلية على `15` نوع بيانات، كلها بلا tracking. السياسة المنشورة حُدثت بإفصاح Google Sign-In وربط RevenueCat ونُشرت في Vercel deployment ‏`dpl_EMsF8EH79BfRKPTSxNr8wRDDxRNn` بحالة `READY`.
- حقوق النشر `2026 MOHAMMAD ALZOUABI` وContent Rights محفوظان. حُفظت بيانات App Review باسم `MOHAMMAD ALZOUABI` وحساب Google للمراجع `mhm1981d@gmail.com` والهاتف `+974 70067040`، وصُححت الملاحظات لتشرح دخول Google بدل أن تطلب Apple؛ أدخل المالك كلمة المرور مباشرة ولم تُقرأ أو تُطبع. استبعاد فرنسا ألغى مانع التصريح الفرنسي لهذا الإصدار، وقبل App Store Connect الإرسال بالبناء 18. لا تستبدل البناء ولا تعيد فرنسا أو مسار `ANSSI` إلا بطلب صريح أو إذا طلبت Apple تغييرًا.
- ملفات `ANSSI` الأربعة في `output/pdf/` تحضيرية محلية غير مستخدمة ولم تُرسل إلى أي جهة: النموذج الرسمي، والنسخة المعبأة غير الموقعة، والملحق التقني، ودليل المنتج. ليست اعتمادًا ولا مانعًا حاليًا، ولا يلزم توقيعها أو متابعة Adobe Reader. احتفظ بها من دون حذف لأنها مخرجات غير متتبعة، ولا تستأنف هذا المسار إلا إذا قرر المالك إعادة فرنسا لاحقًا.
- المرشح السابق `0.1.0+18` حافظ على ثبات المظهر وآخر رصيد مؤكد من build 17، وأضاف إعادة محاولة آمنة للرفع: يعيد الخادم `uploadedAlready:true` فقط بعد إثبات أن الجسم الموجود يطابق الحجم و`Content-Type`. ترسل Responses API ‏`store:false`، ويقرأ RevenueCat webhook جسم JSON فعلياً بحد أقصى مضبوط.
- تنظيف الصوت المحلي غير منشور بعد: `20260830230000_independent_audio_cleanup.sql` و`cleanup-expired-audio`. يعمل على مرحلتين؛ إزالة أولى تبقي tombstone، وبعد 15 دقيقة على الأقل إزالة ثانية قبل حذف المؤشر، مع دفعة 100 وlease قابل للاسترداد بعد 30 دقيقة وجدولة كل 15 دقيقة. إعادة محاولة التذكرة مسموحة فقط خلال أول ساعتين، ويفرض `CHECK` سقفًا ثابتًا للحجز عند أربع ساعات من `created_at`. لا توجد كتابة SQL إلى `storage.objects`. يلزم إنشاء `voicebrief_project_url` و`voicebrief_secret_key` في Vault ثم نشره واختباره حيًا على مشروع VoiceBrief الصحيح فقط.
- حذف الحساب المحلي غير منشور بعد: يحذف RevenueCat أولاً بمفتاح خادم `REVENUECAT_SECRET_API_KEY`، ويعامل `200/404` كنجاحين آمنين. عند فشل RevenueCat لا يبدأ حذف Supabase. migration `20260830234000_revenuecat_account_deletion.sql` تجعل أحداث webhook اللاحقة مجهولة UUID مع الاحتفاظ بـ`event_id` لمنع الإعادة. يلزم دخول المالك إلى RevenueCat وإنشاء مفتاح سري ثم ضبطه في Supabase؛ لا تكتب المفتاح في الملفات أو المحادثة.
- نُشرت سياسة build 18 وصفحة `/delete-account` ثنائية اللغة إنتاجيًا على مشروع Vercel `voicebrief-legal`. النشر النهائي `dpl_3apBv66qn7cKF2RsqUfPnC1y1yDg` حالته `READY`، وصفحات الخصوصية والحذف العربية والإنجليزية تعيد HTTP 200. الصفحة تشرح حذف الحساب، الطلب اليدوي الموثق، حذف سجل محلي كامل أو نتيجة منفردة دون حذف الحساب، ومدد تنظيف الصوت/النتائج؛ إلغاء اشتراك المتجر يبقى خطوة منفصلة. لم تُنشر أي دالة أو migration من build 18 ضمن هذا التغيير.
- تحقق build 18 المحلي ناجح: `flutter analyze --no-pub`، و45 اختبار Flutter غير golden، و12 golden، و42 اختبار Deno، و`deno fmt/lint/check`. حزمة Android الموقعة `VoiceBrief-0.1.0-build18-production-signed.aab` حجمها `59,195,892` وبصمتها `AEC8B94AA105FA083372FC7B65419A602AC2A3452DA8F98161CE59E53E262AAD`. APK الموقّع حجمه `69,446,598` وبصمته `BA2971E1C656EF7BB5D7E340F25A5455C9B313D4D35CD6A6DBE588858B9A06B0`، ورقم الحزمة `app.voicebrief.mobile` وtarget SDK 36 وشهادة الرفع SHA-256 `81AA4DA6E425794E880C8DDE0A7568F68CEE774A3C1889170221AD0446EE8FD9`.
- نجح CI للبناء 18 في run `33337662952`. نجح TestFlight run `33337662810` ورفع IPA حجمه `34,633,872` وبصمته `2025A7FA77D2C737A6B588982A4C2E053948E6D989B5CC499898351BFD755EA6`؛ Delivery UUID/build ID هو `59ccf8f9-cbc9-4ac1-9f4c-54492f8ffcaf`، وartifact رقم `9739706290` وبصمته API ‏`sha256:dcc508aa07823e9f8adbb477f3dd332e835a34cc58750450591b4d710627a739`. أكد status run `33339093610` أن `0.1.0 (18)` هو `VALID` وموجود في `VoiceBrief Internal`؛ artifact الحالة `9739954624` وبصمته API ‏`sha256:d5935e52f41efa97d6fe7237b57078b35d89b2dc051c7540609f9018ce895bd1`، وملف التقرير بصمته `CF5F77E72588F207CCA7C1AB9763635B7AE248C9EFF6F1CE5B19037682B3181`. أُرسل البناء نفسه إلى App Review وأصبح `Waiting for Review` ضمن الإرسال الموثق أعلاه؛ تذكر Apple أن المراجعة قد تستغرق حتى 48 ساعة.
- نُشر build 18 إلى Google Play ‏`internal` فقط عبر run `33341090128` من `26b99e8e8e6132e14f7b26a61e5361178667289b`. التقرير يثبت `committed:true` و`completed` للإصدار 18 وبصمة AAB الصحيحة؛ artifact رقم `9740576360` وبصمته API ‏`sha256:b98c07031726bcb38cf88b000dc7a7d01ab3a2874102788a706ab39f32f87647`، وملف التقرير `838` بايت وبصمته `6E42CB83EBBBA56450A800647544F5D823F83A7BAB22EAB3E56A95F58D324E36`. Play Console يعرض `VoiceBrief 0.1.0 build 18` «متاح للمختبِرين الداخليين»؛ هذا ليس نشر Production.
- ناشر Google المحروس موجود على `main` عند `d11c8ec` وعلى فرع VoiceBrief عند `26b99e8`. بيئة `voicebrief-google-play-internal` محمية ومقيدة بالفرع؛ مُنح حساب الخدمة لتطبيق VoiceBrief فقط إذن «إصدار تطبيقات للجماهير المستهدفة للاختبار» من دون إذن Production، وحُذف السر المؤقت `VOICEBRIEF_AAB_SIGNED_URL` بعد نجاح التشغيل.
- بعد تأكيد المستخدم الصريح، رُبطت قائمة `All Apps - Closed Testers` ذات 12 عضوًا بمسار Google المغلق `Alpha`، واختيرت جميع المناطق: `176` بلدًا/منطقة و«بقية بلدان العالم». نُشر بريد التواصل العام `mhm1981x@gmail.com` والموقع القانوني HTTPS، وحُفظ IARC بفئة «كل أنواع التطبيق الأخرى» مع المحتوى المولّد عبر الإنترنت والمشتريات الرقمية؛ التقييمات `Everyone` و`PEGI 3` و`IARC 3+` و`ClassInd 14+`. حُفظ الجمهور `18+` وData Safety النهائي، واختيرت حزمة build 18 الموجودة في مكتبة Play وحدها لإصدار Alpha باسم `VoiceBrief 0.1.0 build 18`. أُرسلت 14 تغيّرًا إلى Google وتعرض نظرة النشر «التغييرات قيد المراجعة». عند الإرسال كان المنضمون الفعليون 0 ولم تبدأ مدة 14 يومًا؛ لا تدّع توفر Alpha أو بدء 12/14 يومًا أو النشر العام حتى تؤكد Play ذلك.
- أُضيفت بصمة Google Play App Signing إلى مشروع Google Cloud الصحيح بعميل Android مستقل `VoiceBrief Android Play` (`872187920899-krms6k2de8ad87isa4aba7828a4u1u6n.apps.googleusercontent.com`) للحزمة `app.voicebrief.mobile` وSHA-1 ‏`05:E4:8C:69:53:7A:BB:EF:D2:CE:01:CB:C5:C4:59:68:07:19:1A:A5`; SHA-256 للشهادة `32:C1:07:D0:43:9F:B5:83:C0:E1:DB:12:8A:A0:81:5C:1B:98:E8:25:AB:48:D3:BB:4B:9D:92:8B:5C:29:88:35`. لا تعتبر تسجيل Google مقبولًا من Play قبل اختبار تثبيت المتجر بعد انتشار OAuth.
- مسار `internal` صار نشطًا بعد ربط `All Apps - Closed Testers` (12) به، وقبل `mhm1981d@gmail.com` الدعوة من `https://play.google.com/apps/internaltest/4700504097313794694`. لا تخلط ذلك بمسار Alpha: هذا لا يبدأ شرط 12/14 يومًا. الإرسال البعيد إلى Note 8 توقف بأمان عند إعادة تحقق Google؛ تبويب التحقق مُسلّم للمستخدم، ولا تطلب أو تكتب كلمة المرور. الجهاز المتصل ما زال يحمل build 1 بمفتاح الرفع، لذلك تحقق من توقيع/بيانات النسخة قبل أي إزالة؛ لا تحذفها دون موافقة صريحة.
- وافق المالك صراحة على البريد العام `mhm1981x@gmail.com` وIARC وشروطه والجمهور `18+` والنص القانوني، وعيّن `mhm1981d@gmail.com` لحساب المراجعة. يظهر في Play سجل محفوظ باسم `Google reviewer account` يحوي اسم المستخدم/كلمة المرور/التعليمات؛ لم تُقرأ كلمة المرور ولم تُخرج. لا يوجد email/password أو guest/review mode في build 18، لذلك يجب أن يبقى حساب Google قابلًا لإعادة الاستخدام بلا 2FA/OTP/passkey. لم يُثبت حيًا بعد أن الحساب سجّل الدخول إلى build 18 أو أن UUID المطابق يملك RevenueCat entitlement `pro` طويل المدة و300 دقيقة؛ تابع ذلك فور توفر جلسة المالك لأن فشل دخول المراجع أو نقص الوصول قد يسبب رفض Google، ولا تعدّل جداول الاشتراك يدويًا.
- اكتمل فحص فرق build 18 برقم `a6a51d74-8bdb-4d43-8c39-a4634125cc8d`. وجد نقطة واحدة منخفضة الخطورة في التجديد المنزلق للحجز، وأُغلقت بسقف الساعتين/الأربع ساعات في migration و`schema.sql`. التغطية جزئية فقط لأن سباق `PUT` طويل جدًا عبر مرورَي التنظيف يعتمد على سلوك Supabase الحي ولم يُثبت؛ اختبره بعد الدخول إلى المشروع الصحيح. فحص build 17 السابق `a3c7b83e-9d4b-4870-a335-4e3a784b1b50` محفوظ في `voicebrief_flutter/docs/security/build17/`.
- build 16 هو الأساس التاريخي للصوت والتصميم الذي يرثه build 18. حُذفت `gentle/bright/classic` نهائيًا من الموارد؛ الصوت الأساسي هو صوت Apple الافتراضي عبر `AlarmKit .default`. تسمح ورقة الصوت باختيار ملف صوتي أو فيديو، ثم يحول الجسر أول 29 ثانية إلى ملف CAF PCM داخل `Library/Sounds` ويستخدم `.named`. النغمة المختارة تُحفظ كتفضيل وتُعرض في `/alarms`. لا تدّع إمكانية تصفح نغمات تطبيق «الساعة»؛ لا توفر Apple API عامة لها.
- أُعيد تصميم `/alarms` بعنوان «المنبّهات»، وبطاقة واضحة للصوت، وحالة فراغ متوسطة، وبطاقات للمواعيد مع وقت قابل للتصغير وإلغاء مؤكد. نجح محليًا format على 75 ملفًا وanalyze و37 اختبارًا غير golden و12 golden وAndroid debug؛ APK حجمه `177,642,309` وبصمته `5C285160AB09649C7409DF25AA28E2923EF768A8F822A28BF12E709D71FB50F0`. نجح CI run `33319637732`.
- نجح TestFlight run `33319637699`: IPA build 16 حجمه `34,612,519` وبصمته `C63AE9AD859F24A325095A954411D72A4501A96B44155098647AB432AD0BE32E`، وDelivery UUID/build ID هو `1d2cc0f0-f375-4665-bd20-8b88f89dd431`، وartifact رقم `9734711162` وبصمة ZIP `1F09A8271A994C569D806A78831D9DAD2B792F59A94E7C19424EBEF2C0ECABA5`. أكد status run `33320659130` أنه `VALID` وموجود في `VoiceBrief Internal`؛ تقرير الحالة artifact `9734784217` وبصمة ZIP `D139033ED636C29593C225FD8C4AF8FC81DB2D6A6BE17489974E01B4C97713F1`. بقي اختبار iPhone الحقيقي للاستيراد والرنين والاسم والقائمة والإلغاء.
- نجح TestFlight run `33315830789` ورفع IPA build 15 ببصمة `44502A8D9754F5B71927866689DB08E18A549A23CE66E44FE66859F945160174` وDelivery UUID `08fc7b55-add6-4223-8540-39a7919a733b`. أكد status run `33317195493` أن البناء `VALID` ومرتبط فعليًا بـ`VoiceBrief Internal`، ولذلك هو متاح للمختبرين. تقرير الحالة artifact رقم `9733809725` وبصمة ZIP `7AF3A431885AC3C5F55B61543A7562C896AB7236B55FB047201BFF09032F6DF1`.
...
- `process-audio` منشور حصراً إلى `jyehqpdbayslhzebdycj`: `ACTIVE` version `14`، id `e4a26c2e-b9f2-43d3-9a6b-61467b8c8747`. منع التكرار يطابق التاريخ التقويمي حتى عند وجود وقت كامل، ويتعرف على عبارات الإعادة، مع اختبار يحافظ على حدثين مختلفين في اليوم نفسه. `OPENAI_TRANSCRIPTION_MODEL` الحي هو `gpt-4o-mini-transcribe`؛ لا تمس بقية الأسرار أو Damanak أو `talabieh`.
- شاشة النتيجة تعرض «ضبط منبّه» و«إضافة إلى التقويم» لكل موعد. أثبت iPhone الحقيقي أن تقويم البناء 13 يعمل، لكن المستخدم رفض الإشعار المحلي بوصفه منبّهًا. البناء 14 يستخدم `AlarmKit` عبر `voicebrief/reminders` على iOS 26.1 فما بعد، مع `NSAlarmKitUsageDescription` وإذن مستقل، ويُبقي إشعارًا محليًا فقط على iOS 14–26.0؛ Android يستخدم `AlarmClock.ACTION_SET_ALARM`. هذا منبّه نظامي تابع لـVoiceBrief وليس إدخالًا داخل تطبيق «الساعة». الوقت العربي المبهم يفرض منتقي التاريخ والوقت حتى لو أعاد الخادم ISO كاملًا.
- اختبار build 17 الحي عبر المسار الجديد بعينة المستخدم نفسها `29.208s` أعاد HTTP 200 خلال `16,141ms`، وفصل الغد `2026-08-31` عند الخامسة عن `2026-09-05` بلا وقت مخترع، وطلب التأكيد لكليهما، ثم حذف المستخدم وStorage والاستحقاق. اختبار تكرار الموعد أعاد حدثين فقط بعد إصلاح الإصدار 14.
...
- البناء 10 مرفوض جهازياً لمسار WhatsApp المباشر رغم نجاح التوقيع والرفع. التحقق المحلي للبناء 11 نجح: 71 ملف format، analyze، 31 non-golden، 11 golden، وAPK debug بصمة `CF8F63D8CBC22A9AC681A2F51FAED8F7C86CB0B6DA88D6856A84B24657D84E30`. CI run `33304112117` وTestFlight run `33304112113` وApp Store status run `33304112131` كلها ناجحة؛ البناء `0.1.0 (11)` هو `VALID` ومضاف إلى `VoiceBrief Internal`. Delivery UUID/build ID هو `4fdd86e6-da1e-41d6-820e-48290839548a`، وبصمة IPA `AACAFA2BADBCA2026B1AF8080B66D0F455E79724E825BF15A8F0F352F04C577D`، وartifact رقم `9730071886`. تقرير الحالة artifact رقم `9730121859` وبصمة ZIP `66FAF74DDB4D12EA5B9359B00D917498AC1D9DB00D89BF54E93A4BEDE9100660`. هذه أدلة البناء 11 التاريخية؛ تدفق الحفظ اليدوي استُبدل في البناء 12.
...
- التحقق المحلي للبناء 13: format على 73 ملفاً، analyze بلا مشكلات، 34 non-golden، 11 golden، و7 اختبارات Deno مع check. APK debug حجمه `177,609,821` وبصمته `86EA9BF33CA6FC39C337A83EBE7879781EE21AE1080AC74CEBFAFDF85DA4ECE7`. الاختبار الحي للإصدار الخادمي 11 أعاد HTTP 200 خلال `17,153ms` وموعدين منفصلين، ونظف المستخدم وStorage والاستحقاق.
- CI run `33310667058` نجح بكل وظائفه. TestFlight run `33310667057` نجح ورفع IPA حجمه `34,495,012` وبصمته `7AF8B0C14248BB40B1D5566362B56C271254385F383C20C0E116153970B6C4DF`؛ Delivery UUID/build ID `0625345c-86d1-495b-adba-430f117da4f7`، وartifact رقم `9732022681` وبصمة ZIP `DBC688437838411FFCC323F4E316CB2330A51AD049B778C9A7335882523BEDA7`.
- المستخدم قبل بطاقة الموعد والتقويم في build 13 ورفض المنبّه لأنه كان إشعارًا محليًا. build 14 يبدله بـ`AlarmKit` على iOS 26.1+. نجح CI run `33312938537`، ونجح TestFlight run `33312941887` في تجميع Swift بـXcode 26.6 والتوقيع والتحقق والرفع. IPA حجمه `34,505,689` وبصمته `DD407238013187CDC315FAA0627F73EAA674467B94D55FD06B9B78C1E4887AF6`؛ Delivery UUID/build ID هو `b5d2a430-b878-42e6-9436-2cedabb0647f`، وartifact رقم `9732779976` وبصمة ZIP `8252ABBA01FCE551BC0E6145E32528AF04BF0628129F020D49F8F1BC57FDF049`.
- App Store status run `33313777125` يؤكد أن `0.1.0 (14)` هو `VALID` وغير منتهٍ ومضاف إلى `VoiceBrief Internal`. تقرير الحالة artifact رقم `9732841813` وبصمة ZIP `01D9F57C262508F111A1987921AA1D246BD9D952EDEF525404948C6AE70D4A4D`. في اختبار الجهاز، وافق على إذن المنبّهات والمؤقتات، واضبط موعدًا قريبًا ثم تحقق من واجهة المنبّه وصوته حتى مع الصامت/التركيز ومن زر الإيقاف. لا تدّع قبول الجهاز قبل ذلك.

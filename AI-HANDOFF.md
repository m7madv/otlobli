...
## v86.76 handoff — iPhone resume recompose (2026-08-08)

The reported iPhone 16/iOS 27 freeze is a candidate regression in the v86.5.1 lifecycle burst. The known-good `v85.8.92` patch used one `appDidBecomeActive` reattach at `0.25s`; the current code had four forced calls and also repeated them from `willEnterForeground`, meaning up to eight forced detaches per resume. v86.76 restores the proven one-call path and normal 1-second cooldown. Never remove the actual detach/reattach, saved-scroll restore, constraints restore, injected message fallback, Android `otlobliOnHostResume()`, or App.tsx JSON comparison. The guard now explicitly asserts the single `0.25s` recompose and rejects an accidental lifecycle burst by absence.

Marker `2026.08.08-v86.76-iphone-resume-single-recompose`; `86.76 / 936`. Build, freeze guard and performance budget pass (`1,198,131 / 1,200,000` raw JS; `544,453 / 550,000` SHEIN source); Android/iOS synchronized. GitHub/Xcode [run `31277870400`](https://github.com/m7madv/otlobli/actions/runs/31277870400) passed from `8845a91`; IPA `C:\Users\MOHAMMAD\OneDrive\Desktop\otlobli-ios-v86.76\otlobli-ios-v86.76-iphone16\otlobli-v86.76-iphone16-unsigned.ipa` (7,064,759 bytes, SHA-256 `84B080C0893BF3BABA854A9866C91FF53F2D81630D7B412CF318B4BCFEA12B2B`) is verified as `86.76 / 936`. Require real iPhone 16 proof: five background/resume cycles plus force-quit/cold launch. Do not claim this eliminates the freeze before that test. The v86.75 quick-add safe-space work remains included. The user's later report about hiding the native add-success toast and expanding lazy size choices is still pending after this priority hotfix.

...
- Current local code candidate: v85.8.56 / `APP_VERSION = 2026.07.18-v85.8.56-temu-nav-bottom-offset-12-no-otp-test`.
- User report after v85.8.55: the Temu injected bottom nav is closer but still needs to move down a little more.
...
- Change: lowers the Temu nav container from `bottom:-8px` to `bottom:-12px` and bumps the injected nav style version so the WebView refreshes the inline style.
...
- Validation: targeted ESLint for script/config, injected-script parse plus `bottom:-12px` marker check, `git diff --check`, `npm run build`, GitHub build, and embedded v85.8.56 marker/offset checks passed. Real-device acceptance is still pending.
...
- Next step: install v85.8.56 on the real iPhone and compare only the bottom nav vertical alignment between Temu Home and Orders/Cart. The v85.8.52 no-gap behavior should remain unchanged.
